...
def main():
    ...
    parser.add_argument(
        "-n",
        "--genome-name",
        help="The genome_parameter for " "retrieving the pyensembl database",
        default=None,
    )
    parser.add_argument("-g", "--gtf", help="The gtf file for pyensembl", default=None)
    ...
    motif_re = (
        "\((?P<gene_name>[^\)]+)\)_\((?P<species>[^\)]+)\)_" "\((?P<rbd_score>[^\)]+)\)"
    )
    ...
    with open(args.meme) as meme_f:
        for line in meme_f:
            if line.startswith("MOTIF"):
                ...
                if m is None:
                    msg = (
                        "Could not parse gene name. Guessing the entire "
                ...
                try:
                    ...
                    msg = (
                        "Could not find Ensembl identifier for gene_name: "
                        "'{}'".format(gene_name)
                    )
                    logger.warning(msg)
