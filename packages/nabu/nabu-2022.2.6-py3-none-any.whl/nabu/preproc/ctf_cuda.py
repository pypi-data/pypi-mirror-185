...
class CudaCTFPhaseRetrieval(CTFPhaseRetrieval):
    ...
    def __init__(
        self, shape, geo_pars, delta_beta,
        padded_shape="auto", padding_mode="reflect", translation_vh=None, normalize_by_mean=False,
        lim1=1.0e-5, lim2=0.2, use_rfft=True, fftw_num_threads=None,
        logger=None, cuda_options=None,
    ):
        """
        ...
        super().__init__(
            shape, geo_pars, delta_beta,
            padded_shape=padded_shape, padding_mode=padding_mode,
            translation_vh=translation_vh, normalize_by_mean=normalize_by_mean,
            lim1=lim1, lim2=lim2, logger=logger, use_rfft=True
        )
    ...
    def _init_ctf_filter(self):
        ...
        self._d_filter_denom = garray.to_gpu(
            (1. / (2 * self.unreg_filter_denom * self.unreg_filter_denom + self.lim)).astype("f")
        )
    ...
    def _init_cuda_padding(self):
        ...
        coords_R = np.roll(
            np.pad(R, pad_width, mode=self.padding_mode),
            (-pad_width[0][0], -pad_width[1][0]),
            axis=(0, 1)
        )
        coords_C = np.roll(
            np.pad(C, pad_width, mode=self.padding_mode),
            (-pad_width[0][0], -pad_width[1][0]),
            axis=(0, 1)
        )
