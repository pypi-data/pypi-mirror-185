...
class FlatFieldArrays:
    ...
    @staticmethod
    def interp(pos, indices, weights, array):
        """
    ...
    def get_flat(self, pos, dtype=np.float32):
        flat = self.interp(pos, self.flats_idx, self.flats_weights, self.flats_arr)
        if flat.dtype != dtype:
    ...
    def normalize_single_radio(self, radio, radio_idx, dtype=np.float32):
        ...
        flat = self.get_flat(radio_pos, dtype=dtype)
        flat = flat - dark
