...
def main():
    ...
    if "phase" in proc.processing_steps:
        ...
        if (proc.dataset_info.energy is None or proc.dataset_info.energy < 1e-3) and proc.nabu_config["phase"][
            "method"
        ] is not None:
    ...
    if ("rotate_projections" in proc.processing_steps) and abs(
        proc.processing_options["rotate_projections"]["angle"]
    ) > 15:
        reconstructor_cls = GroupedReconstructor
