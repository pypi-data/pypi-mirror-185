...
class FullFieldReconstructor:
    ...
    def _get_reconstruction_range(self):
        ...
        self.n_angles = self.process_config.dataset_info.n_angles
        self.n_x, self.n_z = self.process_config.dataset_info.radio_dims
    ...
    def _compute_max_chunk_size(self):
        ...
        if self.backend == "cuda":
            self.gpu_max_chunk_size = estimate_max_chunk_size(
                ...
                pipeline_part="all",
                step=5,
                # max_mem_allocation_GB=17 #2**32 elements - see estimate_required_memory docstring note
            )
        ...
        self.chunk_size = min(self.max_chunk_size, self.n_z)
    ...
    def _compute_max_group_size(self):
        ...
        if self.backend == "cuda":
            ...
            self.chunk_size_sinos = min(min(self.gpu_max_chunk_size_sinos, self.cpu_max_chunk_size_sinos), self.delta_z)
        user_max_group_size = self.extra_options.get("chunk_size", None)
        if user_max_group_size is not None:
            self.max_group_size = min(self.max_group_size, user_max_group_size)
        self.group_size = min(self.max_group_size, self.n_angles)
    ...
    def _get_pipeline_mode(self):
        ...
        if self._margin_v > self.chunk_size // 3:
            ...
            if n_stages > 1:
                msg = str("Margin (%d) is too big for chunk size (%d)" % (self._margin_v, self.gpu_max_chunk_size))
                chunk_is_too_small = True
    ...
    def _compute_margin(self):
        ...
        if self.process_config.resume_from_step == "sinogram":
            ...
            margin_v = max(unsharp_margin[0], phase_margin[0], translations_margin[0])
            margin_h = max(unsharp_margin[1], phase_margin[1], translations_margin[1])
            if margin_v > 0:
    ...
    def _compute_phase_margin(self):
        ...
        if phase_options is None:
            ...
            margin_v = ceil((self.n_z - self.delta_z) / 2)
            margin_h = 0  # unused
        else:
            margin_v, margin_h = (0, 0)
    ...
    def merge_hdf5_reconstructions(
        ...
        user_rec_config = self.process_config.nabu_config["reconstruction"]
        if (
            self._margin_v > 0
