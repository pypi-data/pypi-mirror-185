...
def get_default_output_volume(
    ...
    if not output_type in valid_file_formats:
        raise ValueError(
            f"output_type is not a valid value ({output_type}). Valid values are {valid_file_formats}"
        )
    ...
    if isinstance(input_volume, (EDFVolume, TIFFVolume, JP2KVolume)):
        ...
        if output_type == "hdf5":
            ...
            metadata_file_parent_path, metadata_file_name = os.path.split(
                input_volume.metadata_url.file_path()
            )
            ...
            data_url = DataUrl(
                file_path=os.path.join(
                    data_file_parent_path, output_dir, data_file_name
                ),
            ...
            metadata_url = DataUrl(
                file_path=os.path.join(
                    metadata_file_parent_path, output_dir, metadata_file_name
                ),
            ...
            if output_type == "tiff":
                ...
                Constructor = JP2KVolume
            file_parent_path, file_name = os.path.split(
                input_volume.data_url.file_path()
            )
...
def cast_volume(
    ...
    if not isinstance(input_volume, VolumeBase):
        raise TypeError(
            f"input_volume is expected to be a {VolumeBase}. {type(input_volume)} provided"
        )

    if not isinstance(output_volume, VolumeBase):
        raise TypeError(
            f"output_volume is expected to be a {VolumeBase}. {type(output_volume)} provided"
        )

    if not isinstance(output_data_type, numpy.dtype):
        raise TypeError(
            f"output_data_type is expected to be a {numpy.dtype}. {type(output_data_type)} provided"
        )
...
def clamp_and_rescale_data(
    ...
    rescaled_data = rescale_data(
        data, new_min=new_min, new_max=new_max, data_min=data_min, data_max=data_max
    )
    # clamp data
...
def find_histogram(
    volume: VolumeBase, scan: Optional[TomoScanBase] = None
) -> Optional[DataUrl]:
    """
    ...
    if not isinstance(volume, VolumeBase):
        ...
        if volume.url is not None:
            ...
            data_path = volume.data_url.data_path().replace(
                HDF5Volume.DATA_DATASET_NAME, "histogram/results/data"
            )
