...
class TestCoarseToFineSinoCor:
    def test_coarse_to_fine(self):
        ...
        assert np.isscalar(cor_coarse), f"cor_position expected to be a scale, {type(cor_position)} returned"
        cor_fine = sino_cor.estimate_cor_fine()
        assert np.isscalar(cor_fine), f"cor_position expected to be a scale, {type(cor_position)} returned"
