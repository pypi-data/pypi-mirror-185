...
def crop_mesh_by_convex_hull(source:trimesh.Trimesh, cutters: List[trimesh.Trimesh], inside : Boolean = True ) -> trimesh.Trimesh:
...
# Returns a filtered pcd with only points which are inside the convex hull
...
def segment_pcd_by_connected_component(pcd:o3d.geometry.PointCloud, eps=0.03, minPoints=10,printProgress=False) -> List[o3d.geometry.PointCloud]:
    ...
    if len(pcd.points)<minPoints:
        raise ValueError ('len(pcd.points)<minPoints')
...
def array_to_colors(array: np.array, colors:np.array) -> np.array:
    ...
    #validate inputs
    if colors.shape[1] != 3:
        raise ValueError('colors.shape[1] != 3')        
    if colors.shape[0] != len(values):
        raise ValueError('colors.shape[1] != values.shape[1]')
    if np.amax(colors)>1:
        colors=np.c_[colors/255]  
...
def normalize_vectors(a, axis=-1, order=2) -> np.array:
...
def volume_mesh_BIM(depthmapFBIM:np.array, depthmapBimMin:np.array,depthmapBimMax:np.asarray,resolution)-> np.array:
    ...
    Args:
        depthmapFBIM is the distances between the grid per object and the mesh
        depthmapBimMin is the distances between the grid per object and the bottom of the bim
        depthmapBimMax is the distances between the grid per object and the top of the bim
        resolutionis the resolution of the grid
...
def volume_theoretical_BIM( depthmapBimMin:np.array,depthmapBimMax:np.asarray,resolution)-> np.array:
    """Calculate the theoretical volume per element :
...
def calculate_completion(volumeMeshBIM,volumeBIM):
    """Calculate the completion using the theoretical and practical volumes of the bim objects :

    Args:
        volumeMeshBIM: the volume between mesh and bim
        volumeBIM is the theoretical BIM

    Returns:
        array of completness (scale 0 to 1)"""
...
def color_BIMNode(completion, BIMNodes):
    ...
    """Colors the BIM mesh geometries in the computed LOA color    Args:
...
def get_mesh_intersections (geometry, grid:np.array)-> np.array:
...
def get_bim_intersections (geometries:List[o3d.geometry.TriangleMesh], grid:np.array)-> np.array:
    """Returns [N , X * Y] matrix of distances between a grid and a set of input geometries. \n
    ...
    b=[]
    n=0
    while n<len(geometries):
        gl = o3d.t.geometry.TriangleMesh.from_legacy(geometries[n])
        scene = o3d.t.geometry.RaycastingScene()
        id = scene.add_triangles(gl)
...
def get_mesh_inliers(sources:List[o3d.geometry.TriangleMesh], reference:o3d.geometry.TriangleMesh) -> List[int]:
    ...
    for i,s in enumerate(sources):
        ...
        #pcdPoints = s.points
        #query_points = o3d.core.Tensor(np.asarray(pcdPoints), dtype=o3d.core.Dtype.Float32)
        query_points = o3d.core.Tensor(np.asarray(s.vertices), dtype=o3d.core.Dtype.Float32)
        occupancy = scene.compute_occupancy(query_points)
...
def get_geometry_from_path(path : str, type:str = "None") -> o3d.geometry:
