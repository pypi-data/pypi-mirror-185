...
class Array1D():
    ...
    def position(self, normalize=False):
        ...
        if normalize:
            ...
            return [sensor.x for sensor in self.sensors] # modified
        else:
            return [sensor.x for sensor in self.sensors]
    # cmpcc only spacing take one relatve and add spacing  cmpcc + spcing
    # shot as it is
    ...
    def waterfall(self, ax=None, time_ax="y", amplitude_detrend=True,
        ...
        for i, position in enumerate(positions):
            traces[i, :] *= amplitude_scale             # can be canceled out $$$
            traces[i, :] += position
    ...
    def plot(self, ax=None, sensor_kwargs=None, source_kwargs=None):
        ...
        try:
            spacing_txt = f"Sensor spacing: {self.spacing} unit."
        except ValueError:
            spacing_txt = "Sensor spacing is not constant."
        source_txt = f"CMPCC trace @ {round(self.source.x, 2)} m."
