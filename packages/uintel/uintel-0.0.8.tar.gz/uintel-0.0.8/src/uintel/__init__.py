...
def reset(force=False, verbose=True):
    ...
    install._install_styles(force, verbose)

    # Step 3 - Install fonts
    install._install_fonts(force, verbose)

reset()

config = json.load(open(_config_path, "r"))

import aws, colours, esri, geometry, server, slack, sql, zonal_statistics
