...
import typing, copy, subprocess, logging, os

logging.basicConfig(format='%(asctime)s %(levelname)-8s %(message)s', level=logging.INFO, datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger(__name__)

DEFAULT_QUERY_PARAMETERS = {'where': '1=1', 'outFields': '*'}
...
def _check_endpoint_url(endpoint_url: str) -> typing.Tuple[str, dict]:
    ...
    if 'query?' in endpoint_url:
        ...
        query_parameters = copy.deepcopy(DEFAULT_QUERY_PARAMETERS)
...
if __name__ == "__main__":
    import sys
    if sys.argv[2] == "token":
        sys.stdout.write(_generate_token(sys.argv[3], sys.argv[4]))