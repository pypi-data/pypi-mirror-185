import random, slack_sdk, slack_sdk.errors, socket, textwrap, warnings, uintel as ui
...
_HAPPY_EMOJIS = [":tada:", ":cheering-bec:", ":smiley_mitch:", ":ecstatic_tom:", ":partying_face:", ":happy-patrick:", ":happy_tom:", ":dabtom:"]
_SAD_EMOJIS = [":sad_will:", ":sad_willandluci:", ":cry:", ":disappointed_relieved:", ":angywill:"]

TOKENS = ui.config["slack"].copy()

def post_message_to_slack(where_to_post, message_type, identifier=None, message=None, greet=True, silent_usernames=None, emojis=False, domain="UI"):
    """
    ...
    domain: String
        The slack domain you wish to post to. This default to the University of Canterbury's slack (aka Civil Systems). The two choices are: 'UC' and 'UI'.
    Returns
    -------
    ...
    try:
        token = TOKENS[domain.upper()]
    except KeyError:
        warnings.warn(f"You have provided an unsupported domain. I will assume you wish to post this to Urban Intelligence's slack. But next time, please either use one of the following: {list(TOKENS.keys())}.")
        token = TOKENS["UI"]
    except FileNotFoundError:
        raise FileNotFoundError("Please ensure you create tokens.json and enter in a dictionary of domain:token, and try again.")
    ...
    if not where_to_post.startswith("#"):
        where_to_post = _get_users_information_from_name(where_to_post, "id", client)

    # Generate the content used in the message
    header, header_lines = _generate_header(message_type, identifier, emojis) 
    blocks = _generate_blocks(message_type, message, header_lines, greet)
...
def _generate_header(message_type, identifier, emojis):
    """
...
def _generate_blocks(message_type, message, header_lines, greet):
    """
...
def post_files_to_slack(where_to_post, filenames, message, greet=True, domain="UC"):
    """
...
def _get_users_information_from_name(user_name, wanted_information, client):
    """
