...
def get_media_urls(
    tweet: twitter.models.Status, media_type: str, logger: logging.Logger
) -> List[str]:
    if not tweet.media:
    ...
    for index, media in enumerate(tweet.media):
        if not media.type:
            logger.warning(
                f"Media {media} at index {index} for {tweet} does not have type"
            )
            continue
        if media.type != media_type:
            logger.debug(
                f"Skip media {media} at index {index} for {tweet} of type {media.type}"
            )
            continue
        if not media.media_url:
            logger.warning(
                f"Media {media} at index {index} for {tweet} does not have media_url"
            )
            continue
