    ...
    with fileinput.FileInput(file_path, inplace=True) as f:
        for line in f:
            ...
            if old_line_number_begin == line_number:
                ...
                if old_line_number_begin < line_number and line_number <= old_line_number_end:
...
def move_line_block_in_file(file_path, old_line_number_begin, old_line_number_end, new_line_number):
    if (old_line_number_begin <= new_line_number and new_line_number <= (old_line_number_end + 1)):
        return
    else:
        new_line_content = del_line_block_in_file(file_path, old_line_number_begin, old_line_number_end)
        if (old_line_number_begin > new_line_number):
            add_line_in_file(file_path, new_line_number, new_line_content)
        else:
            add_line_in_file(file_path, new_line_number - (old_line_number_end - old_line_number_begin + 1), new_line_content)
...
def full_page_snapshot(url, dir_snapshot):
    ...
    try:
        main_scroll_height = driver.execute_script("return document.getElementById('MAIN').scrollHeight")
        main_scroll_top = driver.execute_script("return document.getElementById('MAIN').scrollTop")    
        page_height = driver.execute_script("return document.body.scrollHeight")
        flag_main_element = True
    except Exception as e:
        print(e)
        main_scroll_height = driver.execute_script('return document.body.parentNode.scrollHeight')
        main_scroll_top = driver.execute_script('return document.body.parentNode.scrollTop')
        page_height = driver.execute_script('return window.innerHeight')
    
    print("main_scroll_height is %d main_scroll_top is %d page_height is %d" % (main_scroll_height, main_scroll_top, page_height))
    while True:
        driver.execute_script("window.scrollBy(0, %d);" % page_height)
        time.sleep(1)
        if flag_main_element:
            new_main_scroll_height = driver.execute_script("return document.getElementById('MAIN').scrollHeight")
        else:
            new_main_scroll_height = driver.execute_script('return document.body.parentNode.scrollHeight')
    ...
    if flag_main_element:
        ...
        main_scroll_height = driver.execute_script("return document.documentElement.scrollHeight")
        page_height = driver.execute_script('return window.innerHeight')
    
    window_page_y_offset = driver.execute_script("return window.pageYOffset")
    print("page_index is %d window_page_y_offset is %d main_scroll_height is %d page_height is %d" % (page_index, window_page_y_offset, main_scroll_height, page_height))
    
    while ((window_page_y_offset + page_height < main_scroll_height) and (page_index < 100)):
        driver.execute_script("window.scrollBy(0, %d);" % page_height)
