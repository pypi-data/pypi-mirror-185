    ...
    for dataset_name in dataset_names:
        for classifier_name in classifier_names:
            ...
            if classifier_name == 'fcn':
                ...
                return fcn.fit(trainloader, valloader, input_shape, nb_classes, dataset_name, epochs)

            elif classifier_name == 'cnn':
                from test_tscai_1.models import cnn
                print('Everything is ok and now CNN is calling')
                return cnn.fit(trainloader, valloader, input_shape, nb_classes, dataset_name, epochs)

            elif classifier_name == 'mlp':
                from test_tscai_1.models import mlp
                print('Everything is ok and now MLP is calling')
                return mlp.fit(trainloader, valloader, input_shape, nb_classes, dataset_name, epochs)

            elif classifier_name == 'resnet':
                from test_tscai_1.models import resnet
                print('Everything is ok and now RESNET is calling')
                return resnet.fit(trainloader, valloader, input_shape, nb_classes, dataset_name, epochs)
                ...
                return inception.fit(trainloader, valloader, input_shape, nb_classes, dataset_name, epochs)