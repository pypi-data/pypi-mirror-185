...
def _create_probability_plots(data: list,
    ...
    while current_frm < end_frm:
        ...
        ax.set_ylabel(clf_name + ' probability', fontsize=style_attr['font size'])
        if current_frm > fps * 60:
            ax.xaxis.set_ticks(np.arange(0, current_frm, int(fps*60)))
            x_labels = [int(x / fps) for x in list(range(0, current_frm, int(fps * 60)))]
            ax.set_xticklabels(x_labels, fontsize=style_attr['font size'])
        elif current_frm > ((fps * 60) * 10):
            ax.xaxis.set_ticks(np.arange(0, current_frm, int(fps*120)))
            x_labels = [int(x / fps) for x in list(range(0, current_frm, int(fps * 120)))]
            ax.set_xticklabels(x_labels, fontsize=style_attr['font size'])
        else:
            ax.xaxis.set_ticks(np.arange(0, current_frm, int(fps*1)))
            x_labels = [int(x / fps) for x in list(range(0, current_frm, int(fps*1)))]
            ax.set_xticklabels(x_labels, fontsize=style_attr['font size'])
...
class TresholdPlotCreatorMultiprocess(object):
    ...
    def __init__(self,
        ...
        self.frame_setting, self.video_setting, self.cores, self.style_attr = frame_setting, video_setting, cores, style_attr
