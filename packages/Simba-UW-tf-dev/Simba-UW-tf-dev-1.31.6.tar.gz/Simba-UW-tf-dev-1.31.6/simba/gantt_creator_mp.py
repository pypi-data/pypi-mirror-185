...
def _image_creator(data: np.array,
    ...
    while current_frm < end_frm:
        ...
        x_length = (round(current_frm / fps)) + 1
        if x_length < 10:
            x_length = 10
...
class GanttCreatorMultiprocess(object):
    ...
    def __create_last_frm(self):
        fig, ax = plt.subplots()
        for i, event in enumerate(self.bouts_df.groupby("Event")):
        ...
        x_length = (round(len(self.data_df) / self.fps)) + 1
        ...
        ax.set_yticklabels(self.clf_names, rotation=self.y_rotation)
        ...
        buffer_.close()
        plt.close(fig)
        frame_save_path = os.path.join(self.out_parent_dir, '{}_{}.png'.format(self.video_name, 'final_frame'))
        cv2.imwrite(frame_save_path, frame)
        print('Final gantt frame for video {} saved at {}...'.format(self.video_name, frame_save_path))
