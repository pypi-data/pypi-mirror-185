...
class DistancePlotterSingleCore(object):
    ...
    def __init__(self,
        ...
        self.video_setting, self.frame_setting, self.files_found, self.style_attr, self.line_attr = video_setting, frame_setting, files_found, style_attr, line_attr
    ...
    def create_distance_plot(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            y_ticks = list(range(0, int(max_y) + 1))
            y_ticks = y_lbls = y_ticks[0::10]
            ...
            for i in range(distance_arr.shape[0]):
                ...
                x_ticks.append(i)
                x_labels.append(str(round((i / self.fps), 2)))

                if len(x_labels) > self.fps * 60:
                    x_labels_plot, x_ticks_plot = x_labels[0::250], x_ticks[0::250]
                elif len(x_labels) > ((self.fps * 60) * 10):
                    x_labels_plot, x_ticks_plot = x_labels[0::150], x_ticks[0::150]
                else:
                    x_labels_plot, x_ticks_plot = x_labels[0::75], x_ticks[0::75]

                plt.xlabel('time (s)')
                plt.ylabel('distance (cm)')
                plt.xticks(x_ticks_plot, x_labels_plot, rotation='vertical', fontsize=self.style_attr['font size'])
                plt.yticks(y_ticks, y_lbls, fontsize=self.style_attr['font size'])
...
# line_attr = {0: ['Center_1', 'Center_2', 'Grey'], 1: ['Ear_left_2', 'Ear_left_1', 'Red']}
#
# test = DistancePlotter(config_path=r'/Users/simon/Desktop/envs/troubleshooting/two_black_animals_14bp/project_folder/project_config.ini',
#                        frame_setting=False,
#                        video_setting=True,
