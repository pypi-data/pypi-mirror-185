    ...
    def create_plots(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            for index, row in data_df.iteritems():
                p_list.append(row)
                plt.plot(p_list, color=self.style_attr['color'], linewidth=self.style_attr['line width'])
                plt.plot(index, p_list[-1], "o", markersize=self.style_attr['circle size'], color=self.style_attr['color'])
                plt.ylim([0, highest_p])
                plt.ylabel(self.clf_name, fontsize=self.style_attr['font size'])
                ...
                plt.grid()
                plt.xticks(x_ticks_show, x_labels_show, rotation='vertical', fontsize=self.style_attr['font size'])
                plt.suptitle(self.clf_name, x=0.5, y=0.92, fontsize=self.style_attr['font size'] + 4)
                buffer_ = io.BytesIO()
