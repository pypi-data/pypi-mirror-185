...
class PathPlotterSingleCore(object):
    ...
    def __init__(self,
        ...
        self.video_setting, self.frame_setting, self.style_attr, self.files_found, self.animal_attr, self.clf_attr = video_setting, frame_setting, style_attr, files_found, animal_attr, clf_attr
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            for frm_cnt in range(len(self.data_df)):
                ...
                for animal_name, animal_data in self.deque_dict.items():
                    cv2.circle(img, (self.deque_dict[animal_name]['deque'][0]), 0, self.deque_dict[animal_name]['clr'], self.circle_size)
                    cv2.putText(img, animal_name, (self.deque_dict[animal_name]['deque'][0]), self.font, self.font_size, self.deque_dict[animal_name]['clr'], self.font_thickness)
                ...
                for animal_name, animal_data in self.deque_dict.items():
                    for i in range(len(self.deque_dict[animal_name]['deque'])-1):
                        ...
                        cv2.line(img, position_1, position_2, line_clr, self.line_width)
    ...
    def __get_styles(self):
        ...
        if self.style_attr is not None:
            self.circle_size = self.style_attr['circle size']
            self.font_size = self.style_attr['font size']
            self.bg_clr = self.color_dict[self.style_attr['bg color']]
            self.max_lines = int(self.style_attr['max lines'] * (int(self.video_info['fps'].values[0]) / 1000))
            self.font_thickness = self.style_attr['font thickness']
            self.line_width = self.style_attr['line width']
            if self.style_attr['width'] == 'As input':
                ...
                self.width, self.height = self.style_attr['width'], self.style_attr['height']
        else:
            space_scaler, radius_scaler, res_scaler, font_scaler = 25, 10, 1500, 0.8
            self.width = int(self.video_info['Resolution_width'].values[0])
            self.height = int(self.video_info['Resolution_height'].values[0])
            max_res = max(self.width, self.height)
            self.circle_size = int(radius_scaler / (res_scaler / max_res))
            self.font_size = int(font_scaler / (res_scaler / max_res))
            self.bg_clr = self.color_dict['White']
            self.max_lines = int(self.video_info['fps'].values[0] * 2)
    ...
    def __get_deque_lookups(self):
        ...
        for animal_cnt, animal in enumerate(self.multi_animal_id_lst):
            ...
            self.deque_dict[animal]['deque'] = deque(maxlen=self.max_lines)
            self.deque_dict[animal]['bp'] = self.animal_attr[animal_cnt][0]
            color = self.color_dict[self.animal_attr[animal_cnt][1]][::-1]
            self.deque_dict[animal]['clr'] = tuple(x / 255 for x in color)
