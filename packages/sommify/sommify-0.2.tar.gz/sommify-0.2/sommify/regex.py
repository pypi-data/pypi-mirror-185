...
START_IRREG = rf"(?:sliced |cut |\w+ into |(?:\w+ed )?in |with(?:out)? |from |- |on |to (?=[a-z])|for |weighing |mixed (?:into |with |to |in )|is )"
START_POST_MOD = rf" ?, ?"
START_ALT_ING = rf"(?: or | and | & )"
...
PROTEIN = one_of(
    [
        rf"""(?P<{key.replace(' ', '_')}>{one_of(values)})(?:$|.*(?P<{key.replace(' ', '_')}_portion>{
