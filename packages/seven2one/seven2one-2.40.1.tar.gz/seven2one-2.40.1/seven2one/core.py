...
class TechStack():
    ...
    logToServer: bool = True
        Send logs to central log store in TechStack.
    ...
    def __init__(
        ...
        if usePorts == False:
            ...
            authzEndpoint = f'https://{host}/authz'
        else: 
            ...
            authzEndpoint = f'http://{host}:8030'
        ...
        if logToServer:
            LogUtils._init_logging(logEndpoint, token, logLevel)
        ...
        #with Utils.create_client(self) as session:
        self.client = Utils.create_client(self)
        self.scheme = self.client.introspection
            #self.scheme = session.introspection
        self.host = host
        self.user = user
        ...
        self.Authorization = Authorization(authzEndpoint, self)
    ...
    def addItems(
        ...
        correlationId = correlationId or str(uuid4())
        with logger.contextualize(correlation_id=correlationId):
    ...
    def updateDataFrameItems(
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            self.updateItems(inventoryName, items, correlationId)
            return
