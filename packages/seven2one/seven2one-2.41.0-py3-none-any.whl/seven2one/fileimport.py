...
class FileImport():
    ...
    def importNewInventory(self, filePath:str, delimiter:str):
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                result = client.createInventory(inventoryName, propertyList, correlationId)
            ...
            for file in files:
                ...
                if len(items) > chunkSize:
                    ...
                    for i in range(0, len(items), chunkSize):
                        result = client.addItems(inventoryName, items[i : i + chunkSize], correlationId)
                        logger.info(f"{len(result)+lenResult} items of {len(items)} imported.")
                        sleep(pause)
                else:
                    result = client.addItems(inventoryName, items, correlationId)
                ...
                if len(validityItems) > 0:
                    if len(validityItems) > chunkSize:
                        ...
                        for i in range(0, len(validityItems), chunkSize):
                            result = client.addItems(inventoryName, validityItems[i : i + chunkSize], correlationId)
                            logger.info(f"{len(result) + lenResult} items of {len(validityItems)} imported.")
                            sleep(pause)
                    else:
                        result = client.addValidityItemsToParents(inventoryName, validityItems, correlationId)
                ...
                if len(newItems) > 0:
                    if len(newItems) > chunkSize:
                        ...
                        for i in range(0, len(newItems), chunkSize):
                            result = client.addItems(inventoryName, newItems[i : i + chunkSize], correlationId)
                            logger.info(f"{len(result) + lenResult} items of {len(newItems)} imported.")
                            sleep(pause)
                    else:
                        result = client.addItems(inventoryName, newItems, correlationId)
                        logger.info(f"{len(result)} items of file '{filePath}' imported.")
            ...
            for file in files:
                ...
                if len(timeSeriesItems) > chunkSize:
                    ...
                    for i in range(0, len(timeSeriesItems), chunkSize):
                        result = client.TimeSeries.addTimeSeriesItems(inventoryName, timeSeriesItems[i : i + chunkSize], correlationId)
                        logger.info(f"{len(result)+lenResult} items of {len(timeSeriesItems)} imported. Waiting {pause} second(s) before continuing...")
                        sleep(pause)
                else:
                    result = client.TimeSeries.addTimeSeriesItems(inventoryName, timeSeriesItems, correlationId)
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                for i in range(0, tsLength, chunkSize):
                    ...
                    client.TimeSeries.setTimeSeriesDataCollection(timeSeriesData=tsChunk, correlationId=correlationId)    
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                if len(items) > chunkSize:
                    lenResult = 0
                    for i in range(0, len(items), chunkSize):
                        result = client.TimeSeries.addTimeSeriesItemsToGroups(groupInventoryName, items[i : i + chunkSize], correlationId)
                        logger.info(f"{len(result)+lenResult} items of {len(items)} imported.")
                        sleep(pause)
                else:
                    result = client.TimeSeries.addTimeSeriesItemsToGroups(groupInventoryName, items, correlationId)
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                for i in range(0, len(items)):
                    try:
                        client.TimeSeries.addTimeSeriesItemsToGroups(groupInventoryName, [items[i]], correlationId)
                        sleep(pause)
                    except:
                    ...
                    for k in range(0, tsLength, chunkSize):     
                        ...
                        try:
                            client.TimeSeries.setTimeSeriesDataCollection(timeSeriesData=[tsDict], correlationId=correlationId)
                        except Exception as err:
            ...
            for file in files:
                ...
                if properties['isArray'][referenceField] == True:
                    ...
                    for key, value in mappedItems.items():
                        try:
                            client.updateArrayProperty(inventory, dfUpdateItems['sys_inventoryItemId'][key], referenceField, 'insert', value, correlationId)
                            if len(value) != 0:
                                itemsUpdated += 1
                    ...
                    client.updateItems(inventory, updateItems, correlationId)
    ...
    def updateItems(
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            client.updateDataFrameItems(inventoryName, df, correlationId)
            return
