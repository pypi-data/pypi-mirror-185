...
from .vtk import write_vtk, read_vtk
from .utils import S3_normalize, get_par_fs_lookup_table


class HarmonizationSphere(torch.utils.data.Dataset):
    def __init__(self, files, scanner_ids, ages, n_vertex,
                 bcp_mean, bcp_std, map_mean, map_std, john_mean, john_std):
        self.files =files 
        self.ages = ages
    ...
    def __getitem__(self, index):
        ...
        if scanner_id == 0:
            data_mean = self.bcp_mean[round(age/30/3)]
            data_std = self.bcp_std[round(age/30/3)]
        elif scanner_id == 1:
            data_mean = self.map_mean[round(age/30/3)]
            data_std = self.map_std[round(age/30/3)]
        elif scanner_id == 2:
            data_mean = self.john_mean[int(age/360)]
            data_std = self.john_std[int(age/360)]
        else:
            raise NotImplementedError()
            
        data = (data - data_mean)/data_std
         
        return data[:, np.newaxis].astype(np.float32), scanner_id.astype(np.int64), self.files[index], age
...
class SingleBrainSphere(torch.utils.data.Dataset):
    ...
    """  
    def __init__(self, files, n_vertex, val=False):
        self.n_vertex = n_vertex
        self.val = val
        self.files = files
    ...
    def __getitem__(self, index):
        ...
        if file.split('.')[-1] == 'vtk':
            ...
            tmp = np.load(file)
        sulc = S3_normalize(tmp[:, 0])[0:self.n_vertex]
        curv = S3_normalize(tmp[:, 1])[0:self.n_vertex]
...
class LongitudinalRegAndParcSpheres(torch.utils.data.Dataset):
    """ return two surfaces of the same subject for longitudinal reg and parc
    ...
    def __getitem__(self, index):
        ...
        age = [ float(x.split('/')[-2].split('_')[1]) for x in files ]
        ...
        if data.shape[0] < self.num_long_scans:
            ...
            parc_label = np.concatenate((parc_label, np.repeat(parc_label[[-1],:,:],
                                                               self.num_long_scans-parc_label.shape[0], axis=0)), axis=0)
