...
def convert(filename, newFileName=None, **kwargs):
    """
    filename    : str | segy file name with location
    ...
    **kwargs : attribute dict contains
    ...
    info = {
        'day': trhead.day_of_year,
        'year': trhead.year_data_recorded,
        'hh': trhead.hour_of_day,
        'mm': trhead.minute_of_hour,
        'ss': trhead.second_of_minute,
        'jobid': binhead.job_identification_number,
        'tsort' : binhead.trace_sorting_code,
        'unit': binhead.measurement_system
    }
    ...
    for key in list(TRACE_HEADER_FORMAT.keys()):
        th_dict.update({key: trfmt.format_th(key, strm[0].stats.segy.trace_header)})
    ...
    for i in range(n_traces):
        ...
        th_dict = dict()
        for key in list(TRACE_HEADER_FORMAT.keys()):
            th_dict.update({key: trfmt.format_th(key, strm[i].stats.segy.trace_header)})
