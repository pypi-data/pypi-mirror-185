...
def format_fh(k, info):
    ...
    if k == ak[0]:
        ...
        if m == 1:
            ...
            print('file contains an invalied TRACE SORT method, set a valied TRACE SORT method')
            return 'UNKNOWN'
        ...
        if m == 1:
            ...
            print('file contains an invalid unit, set a valied unit')
            return 'UNKNOWN'
...
class TraceFormat(object):
    def __init__(self, fh_dict, file):
        self.fh_dict = fh_dict
        self.fname = file

    def format_th(self, k, th_dict):
        ...
        if k == ak[0]:
            ...
            need parameter list                                             ##############################
            """
            m = int(self.fh_dict['amplitude_recovery_method'])
            if m == 1:
                value = 'NONE'
            elif m == 2:
            ...
            # END_OF_GROUP
            if int(self.fh_dict['number_of_data_traces_per_ensemble']) != int(th_dict['trace_number_within_the_ensemble']):
                return "0"
            else:
                return "1"
            ...
            if int(th_dict['gain_type_of_field_instruments']) == 1:
                v = f"{self.fh_dict['instrument_early_or_initial_gain']} DB"
                return v
            else:
                return "0 DB"
            ...
            # POLARITY
            v = int(self.fh_dict['impulse_signal_polarity'])
            if v == 1:
                return '-1'
            elif v == 2:
                return "1"
            ...
            # RECEIVER
            return f"VERTICAL_GEOPHONE {self.fh_dict['number_of_data_traces_per_ensemble']}"
