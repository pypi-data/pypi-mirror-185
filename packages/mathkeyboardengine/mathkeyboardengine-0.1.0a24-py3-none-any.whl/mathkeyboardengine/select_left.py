...
def select_left(k: KeyboardMemory) -> None:
  ...
  if (
    (isinstance(k.current, TreeNode) and k.current.parent_placeholder.nodes.index(k.current) + old_diff_with_current >= 0) or 
    (isinstance(k.current, Placeholder) and old_diff_with_current > 0)
  ):
