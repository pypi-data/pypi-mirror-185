...
class ResNetPreprocessor(Preprocessor):
    ...
    def __init__(self, mode: DatasetMode, config: PreprocessorConfig) -> None:
        ...
        if mode == DatasetMode.TRAINING:
            layers.append(transforms.RandomCrop(size=32, padding=config.padding))
        layers.append(transforms.Resize(config.size))
        if mode == DatasetMode.TRAINING:
...
class AutoResNet(AutoModel):
    ...
    def _build_model(self):
        ...
        with open(param_file, 'rb') as f:
            ...
            if _model.get_parameter('fc.weight').shape[0] != state_dict['fc.weight'].shape[0]:
