...
short_cut_df = pd_df.groupby(["USER_ID", "USER_LOGIN", "OBJECT_ID", "OBJECT_NAME", "OBJECT_TYPE"])[
    "TRANSACTION_ID"].agg("count").reset_index()
short_cut_df= mig_short_cut.bld_change_log_shortcut_df(conn=conn,short_cut_df=short_cut_df,change_log_df=pd_df)
mig_short_cut.run_short_cut_build(conn=conn, project_id=change_query["short_cut_proj_id"],
                                  ...
                                  object_id_col="OBJECT_ID",object_type_col="OBJECT_TYPE"
                                  ,object_name_col="NEW_OBJECT_NAME",object_desc_col="OBJECT_DESC")
