...
class bld_short_cuts:
    ...
    def bld_change_log_shortcut_df(self,conn,short_cut_df,change_log_df):
        #user_id,  object_id,object_name,object_type
        ...
        for index, obj in short_cut_df.iterrows():
            ...
            short_cut_df.at[index, "OBJECT_DESC"]=self._bld_desc_str(change_log_df.loc[change_log_df["OBJECT_ID"] == obj["OBJECT_ID"]])
        return short_cut_df
    ...
    def bld_short_cut(self,conn,project_id,folder_id,object_id,
        ...
        #opens an instance of the created short cut object
        short_cut_obj=self.glob.get_short_cut_obj(conn=conn, project_id=project_id,
                                                  object_id=short_cut_obj_rep.json()["id"],
                                                  type=18)
        self.glob._rename_object(object=short_cut_obj, new_name=new_name, desc_str=object_desc)

    def run_short_cut_build(self, conn,project_id,short_cut_df,folder_id,object_id_col,object_name_col,object_desc_col,object_type_col):
        ...
        for index, obj in short_cut_df.iterrows():
            ...
            if obj[4]!=str(18):
                self.bld_short_cut(conn=conn
                                   ...
                                   ,folder_id=folder_id
                                   ,object_id=obj[object_id_col]
                                   ,object_type=obj[object_type_col]
                                   ,new_name=obj[object_name_col]
                                   ,object_desc=self.str._get_first_x_chars(str_=obj[object_desc_col],i=249)
                                   )
            else:
