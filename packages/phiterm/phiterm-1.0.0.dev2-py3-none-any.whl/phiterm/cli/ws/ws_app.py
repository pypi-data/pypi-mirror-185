...
def up(
    shorthand: Optional[str] = typer.Argument(
        None,
        help="Shorthand filter. Format - ENV:CONFIG:GROUP:NAME:TYPE",
        metavar="[shorthand]",
    ),
    ...
    \b
    The Filter can also be provided as a single argument as - ENV:CONFIG:APP:NAME:TYPE
    \b
    Examples:
    ...
    if shorthand is not None:
        if not isinstance(shorthand, str):
            raise TypeError(
                f"Invalid shorthand. Expected: str, Received: {type(shorthand)}"
            )
        (
            target_env,
            target_config_str,
