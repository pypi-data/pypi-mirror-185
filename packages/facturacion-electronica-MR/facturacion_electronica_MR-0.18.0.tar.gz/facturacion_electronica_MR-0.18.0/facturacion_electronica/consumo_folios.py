...
<MntTotal>0</MntTotal><FoliosEmitidos>0</FoliosEmitidos><FoliosAnulados>0\
        ...
        for dteDoc in vals:
            for docData in dteDoc.get("documentos", []):
                if not docData.get('sii_xml_request'):
                    docData.update(docData.get("Encabezado", {}).get('Emisor', {}))
                    docu = Doc(
                                docData,
    ...
    def _rangosU(self, resumen, rangos, continuado=True):
        ...
        if 'Anulado' in resumen and resumen['Anulado'] == 'A':
            ...
            rangos['itemsAnulados'] = self.item_anulados.get(resumen['TpoDoc'], [])
            return rangos
        ...
        rangos['itemsUtilizados'] = self.item_utilizados.get(resumen['TpoDoc'], [])
        return rangos
...
% rec.name)
    ...
    def validar(self):
        ...
        listado = ['TipoDocumento', 'MntNeto', 'MntIva', 'TasaIVA',
                   'MntExento', 'MntTotal', 'FoliosEmitidos', 'itemsAnulados', 'FoliosAnulados',
                   'FoliosUtilizados', 'itemsUtilizados']
        TpoDocs = []
        if self.Documento:
            resumenes = self._get_resumenes()
            for r, value in resumenes.items():
                ...
                for i in listado:
                    if i in value:
                        ...
                        if 'itemsUtilizados' in Rangos:
                            ...
                            folios.append({'itemsUtilizados': utilizados})
                        if 'itemsAnulados' in Rangos:
                            anulados = []
                            for rango in Rangos['itemsAnulados']:
                                anulados.append({'RangoAnulados': rango})
                            folios.append({'itemsAnulados': anulados})
