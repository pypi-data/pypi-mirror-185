...
def set_from_keys(obj, vals={}, priorizar=set()):
    for p in priorizar:
        val = vals.get(p)
        if val and p in obj.__dict__:
            setattr(obj, p, val)
            del vals[p]
        else:
            _logger.warning(f"Atributo no encontrado en {type(obj)}: {p}")
    for k, v in vals.items():
        if k in obj.__dict__:
            setattr(obj, k, v)
        else:
            _logger.warning(f"Atributo no encontrado en {type(obj)}: {k}")
