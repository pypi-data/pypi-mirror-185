...
xsi:schemaLocation="http://www.sii.cl/SiiDte RespuestaEnvioDTE_v10.xsd" >
    ...
    def generate_xml_send(self):
        tots_dte = defaultdict(lambda : defaultdict(int))
        documentos = ''
        self.errores = []
        ...
        if self.errores:
            return
        SubTotDTE = '\n'.join([f"<SubTotDTE>\n<TpoDTE>{key}</TpoDTE>\n<NroDTE>{value['total']}</NroDTE>\n</SubTotDTE>" for key, value in tots_dte.items()])
        self.filename += ''.join([f"T{key}F" + ''.join([f"-{f}" for f in value["folios"]]) for key, value in tots_dte.items()]) + ".xml"
    ...
    def do_dte_send(self):
        ...
        if self.errores or not self.sii_xml_request:
            errors = self.errores if self.errores else 'No se creó xml'
        ...
        detalles = {r.NroDTE: {
                    ...
                    } for r in self.Documento if hasattr(self, '_api') or r.es_boleta()}
