...
def _load_schema_data() -> Dict[str, Any]:
    ...
    v3_schema_path = platonpm_spec_dir / "spec" / "v3.spec.json"
...
def validate_build_dependencies_are_present(manifest: Dict[str, Any]) -> None:
    if "buildDependencies" not in manifest:
        raise PlatonPMValidationError("Manifest doesn't have any build dependencies.")

    if not manifest["buildDependencies"]:
        raise PlatonPMValidationError("Manifest's build dependencies key is empty.")
...
def validate_raw_manifest_format(raw_manifest: str) -> None:
    """
    Raise a PlatonPMValidationError if a manifest ...
    ...
    if raw_manifest != compact_manifest:
        raise PlatonPMValidationError(
            ...
            "PlatonPM-Spec for document format. "
            "http://platonpm.github.io/platonpm-spec/package-spec.html#document-format "
        )
