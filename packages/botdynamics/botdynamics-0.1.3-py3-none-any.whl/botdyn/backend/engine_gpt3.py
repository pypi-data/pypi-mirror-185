    ...
    def forward(self, prompts: List[str], *args, **kwargs) -> List[str]:
        ...
        while not success and retry < self.max_retry:
            try:
                ...
                try:
                    if 'maximum context length' in sym:
                    ...
                    self.logger.warn(f"Failed to remedy the exceeding of the maximum token limitation!")
            retry += 1
        
        if not success:
            msg = f"Failed to query GPT-3 after {self.max_retry} retries. Errors: {errors}"
            # interpret error
            from botdyn.symbol import Symbol
            sym = Symbol(errors)
            sym.analyze(query="Explain the issue in this error message", exception=errors[-1])
            msg_reply = f"{msg}\n Analysis: {sym}"
