...
class TCoreAPI():
    def __init__(self,eventclass=None) -> None:
        ...
        self.quoteobj=None
    #连线
    def connect(self,appid="",servicekey="8076c9867a372d2a9a814ae710c256e2"):#AlgoMaster2
        if appid=="":
            path=""
            pids=psutil.pids()
            for pid in pids:
    ...
    #解订K线数据
    def unsubbar(self,btype:BarType,symbol:str):
        del self.quoteobj.eventobj.bardata[symbol+"-"+str(btype)]
    ...
    #获取历史greeks数据
    def getgreekshistory(self,datatype:GreeksType,symbol:str,starttime:str,endtime=datetime.strftime(datetime.now()-timedelta(hours=7),'%Y%m%d%H')):
        try:
            datetime.strptime(starttime,'%Y%m%d%H')
            datetime.strptime(endtime,'%Y%m%d%H')
        except:
            print("日期时间格式错误，请输入格式为‘yyyyMMddHH’")
        self.quoteobj.eventobj.greekshistory[symbol+"-"+str(datatype)+":"+starttime+"~"+str(endtime)]=[]
        te=self.quoteobj.subquote(datatype,symbol,int(starttime),int(endtime))
        qre=win32event.WaitForSingleObject(self.quoteobj.eventobj.greekshistoryevent, 20000)
        greekshistory=self.quoteobj.eventobj.greekshistory[symbol+"-"+str(datatype)+":"+starttime+"~"+str(endtime)]
        del self.quoteobj.eventobj.greekshistory[symbol+"-"+str(datatype)+":"+starttime+"~"+str(endtime)]
    ...
    def getATM(self,symbol:str,atmindex:int):
        ...
        qre=win32event.WaitForSingleObject(self.quoteobj.eventobj.atmevent, 2000)
    ...
    def getsymbol_allinfo(self,symbol:str):
        if self.quoteobj.eventobj.symbinfoready:
            return self.quoteobj.getsymbol_allinfo(symbol)
    #获取合约名称描述
