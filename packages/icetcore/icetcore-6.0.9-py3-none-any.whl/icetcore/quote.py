...
class BaseEvents():
                                ...
                                if bartype==BarType.DK:
                                    ...
                                    if interval==1:
                                        ...
                                        if te["DateTime"]<=bar[-1]['DateTime']:
                                            ...
                                            if ktemp["DateTime"].second%5!=0:
                                                print(ktemp["DateTime"])
                                            ktemp["Symbol"]=symbol
                                            ktemp["Open"]=te["Close"]
                                            ktemp["High"]=te["Close"]
                                            ktemp["Low"]=te["Close"]
        ...
        if DataType==1 and SymbolType!=8: #实时
            ...
            if "TC.F" in quote.Symbol:
                ...
                self.ongreeksreal(DataType,quote.Symbol,
                        ...
                        'Delta':iff(quote.GetGreeksValue("Delta"),10000000000), 
            ...
            for i in range(quote.GetValueData("Count")):  
                ...
                dogshis.append({"DateTime":datetime.strptime(str(quote.GetItemValue(i,0))+(" 0" if len(ttemp)==5 else " ")+ttemp,"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                    ...
                    'cvol':iff(quote.GetItemValue(i,39)),
                    'pvol':iff(quote.GetItemValue(i,40)),
                    'coi':iff(quote.GetItemValue(i,41)),
                    'poi':iff(quote.GetItemValue(i,42)),
                    ...
                    'Bid_UpdateDate':iff(quote.GetItemValue(i,71)),
                    'Bid1_UpdateDate':iff(quote.GetItemValue(i,72)),
                    'Bid2_UpdateDate':iff(quote.GetItemValue(i,73)),
                    'Bid3_UpdateDate':iff(quote.GetItemValue(i,74)),
                    'Bid4_UpdateDate':iff(quote.GetItemValue(i,75)),
                    'Bid_UpdateTime':iff(quote.GetItemValue(i,76)),
                    'Bid1_UpdateTime':iff(quote.GetItemValue(i,77)),
                    'Bid2_UpdateTime':iff(quote.GetItemValue(i,78)),
                    'Bid3_UpdateTime':iff(quote.GetItemValue(i,79)),
                    'Bid4_UpdateTime':iff(quote.GetItemValue(i,80)),
                    ...
                    'Ask_UpdateDate':iff(quote.GetItemValue(i,101)),
                    'Ask1_UpdateDate':iff(quote.GetItemValue(i,102)),
                    'Ask2_UpdateDate':iff(quote.GetItemValue(i,103)),
                    'Ask3_UpdateDate':iff(quote.GetItemValue(i,104)),
                    'Ask4_UpdateDate':iff(quote.GetItemValue(i,105)),
                    'Ask_UpdateTime':iff(quote.GetItemValue(i,106)),
                    'Ask1_UpdateTime':iff(quote.GetItemValue(i,107)),
                    'Ask2_UpdateTime':iff(quote.GetItemValue(i,108)),
                    'Ask3_UpdateTime':iff(quote.GetItemValue(i,109)),
                    'Ask4_UpdateTime':iff(quote.GetItemValue(i,110)),
                    'us':quote.GetItemString(i,111),
                    'us_d':iff(quote.GetItemValue(i,123)),
                    'us_t':iff(quote.GetItemValue(i,124),1000)+80000,
                    ...
                    'uf_d':iff(quote.GetItemValue(i,140)),
                    'uf_t':iff(quote.GetItemValue(i,141),1000)+80000,
                    ...
                    'usf_d':iff(quote.GetItemValue(i,157)),
                    'usf_t':iff(quote.GetItemValue(i,158),1000)+80000,
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    if "TC.F" in quote.Symbol:
                        ...
                        "Straddle":iff(item.GetOptValue("Straddle")),
    ...
    def OnSetTopic(self, strTopic, lParam, strParam, pvParam):
        #print("####################",strTopic, lParam, strParam, pvParam)
