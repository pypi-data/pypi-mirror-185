...
class Pandapower(mosaik_api.Simulator):
    ...
    def setup_done(self):
        ...
        entity_full_ids = list(
            '%s.%s' % (self.sid, eid) 
            for eid, edict in self.simulator.entity_map.items()
            if edict['etype'] in ('Load', 'Sgen')
        )
        related_entities = yield self.mosaik.get_related_entities(entity_full_ids)

        for full_id, related in related_entities.items():
            # We activate only those loads and sgens that are connected to other
            # simulator’s entities. For this, we exclude those related entities
            # whose full IDs start with our simulator ID. We include the '.' to
            # distiguish 'Grid-1' from 'Grid-10'.
            our_prefix = f'{self.sid}.' 
            if any(not other_id.startswith(self.sid) for other_id in related.keys()):
                ...
                edict = self.simulator.entity_map[eid]
                if edict['etype'] == 'Sgen':
                    self.simulator.net.sgen.in_service.at[edict['idx']] = True
                elif edict['etype'] == 'Load':
                    self.simulator.net.load.in_service.at[edict['idx']] = True
