...
def executer(
    ...
    with basic_pbar() as pbar:
        pbar.set_description(f"Executing {file.name}")
        task = pbar.add_task("Waiting ...", total=None)
        df = isql.run_sql_file(file)

        pbar.update(task, advance=1)
        pbar.set_description(f"Writing results to {dir}")
        ...
        if out is None:
            ...
            parquet_out = df.write_parquet(file=parquet_file)
