...
def executer(
    ...
    override_default_dir: str = typer.Option(None, "-od", "--override-default-dir", help="Override the default directory 📁. Defaults to `assets/data` within the package directory 📁"),
    ) -> None:
    ...
    elif not override_default_dir:
        outfile = assets_dir / "data" / Path(out).name
    else:
        outfile = assets_dir / "data" / f"result-{file.name}.txt"

        open(outfile, "w+").write(raw)
        sys.exit(0)
