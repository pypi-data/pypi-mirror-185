...
class ModelAnalysis:
    ...
    def __init__(
        ...
        output_dir: str = "",
        pred_col: str = conf.MA_PRED_COL,
        pred_proba_col: str = conf.MA_PRED_PROBA_COL,
    ):
    ...
    def analyze(self):
        ...
        self._analyze_metrics_one_dataset(conf.MA_TRAIN_DATASET_NAME)
        self._analyze_metrics_one_dataset(conf.MA_VAL_DATASET_NAME)
    ...
    def _show_feature_importance(self):
        train_feature = self._get_dataset(conf.MA_TRAIN_DATASET_NAME)[
    ...
    def _get_dataset(
        ...
        if dataset_name == conf.MA_TRAIN_DATASET_NAME:
        ...
        if dataset_name == conf.MA_VAL_DATASET_NAME:
        ...
        raise ValueError(
            ...
            f"('{conf.MA_TRAIN_DATASET_NAME}', '{conf.MA_VAL_DATASET_NAME}')"
        )
    ...
    def _analyze_on_feature(
        ...
        overall_scores = self._get_metric_dict(
            feature_name, conf.MA_OVERALL_NAME, df_with_pred
        )
    ...
    def _get_metric_dict(
        ...
        res = {
            ...
            conf.MA_SAMPLE_COUNT_NAME: len(df_with_pred),
        }
        for metric in self.metrics:
            if metric.need_pred_proba:
                res.update(
                    metric.compute_scores(labels, df_with_pred[conf.MA_PRED_PROBA_COL])
                )
            else:
                res.update(
                    metric.compute_scores(labels, df_with_pred[conf.MA_PRED_COL])
                )
        return res
    ...
    def _get_df_pred_csv_path(self, dataset_name):
        ...
        if not dirname.exists():
            dirname.mkdir()
        return Path(self.output_dir) / dataset_name / conf.MA_PREDICTION_FILENAME
    ...
    def _show_and_log_overall_scores(
        ...
        for key, val in overall_scores.items():
            if val == conf.MA_OVERALL_NAME or key == conf.MA_SAMPLE_COUNT_NAME:
                continue
