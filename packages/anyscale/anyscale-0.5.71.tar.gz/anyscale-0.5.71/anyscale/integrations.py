...
def wandb_send_run_info_hook(run: Any) -> None:
    ...
    if os.environ.get("ANYSCALE_HA_JOB_ID"):
        ...
        ray_job_id = _try_get_ray_job_id()
        cluster_id = os.environ.get("ANYSCALE_SESSION_ID")
        ray_job = _search_ray_job(ray_job_id, cluster_id, api_client)
        ...
        run.config.anyscale_logs = get_endpoint(f"/interactive-sessions/{ray_job.name}")
...
def _search_ray_job(
    ray_job_id: Optional[str], cluster_id: Optional[str], api_client: DefaultApi
):
    """
    Find Ray job that is associated with a cluster given the Ray job id (output of
    ray.get_runtime_context().get_job_id()).

    TODO(nikita): Update this method to use ray.runtime_context.get_runtime_context().get_job_submission_id()
    to get the job submission ID, so we don't need to page through all interactive sessions
    associated with a cluster. This will be available with Ray 2.3: https://github.com/ray-project/ray/issues/28089
    """
    if not (ray_job_id and cluster_id):
        return None

    def _search_ray_job_in_page(paging_token: Optional[str]):
        ray_job_resp = api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get(
        ...
        for ray_job in ray_job_resp.results:
            if ray_job_id == ray_job.ray_job_id:
                return ray_job, ray_job_resp.metadata.next_paging_token
        return None, ray_job_resp.metadata.next_paging_token

    ray_job, next_paging_token = _search_ray_job_in_page(None)
    while next_paging_token and not ray_job:
        ray_job, next_paging_token = _search_ray_job_in_page(next_paging_token)
