# -*- coding: utf-8 -*-
"""
Created on Sun Jan  8 18:29:27 2023
...
At-the-Money approximation. This is a very fast approximation for implied volatility. It is used to estimate a starting point for the search functions.
Newton-Raphson Search. This is a fast implied volatility search that can be used when there is a reliable estimate of Vega (i.e., European options)
Bisection Search. An implied volatility search (not quite as fast as a Newton search) that can be used where there is no reliable Vega estimate (i.e., American options).
"""
...
def _bisection_implied_vol(val_fn, option_type, fs, x, t, r, b, cp, precision=.00001, max_steps=100):
    ...
    if abs(cp - cp_mid) < precision:
        ...
        raise GBS_CalculationError(
            "Implied Vol did not converge. Best Guess={0}, Price diff={1}, Required Precision={2}".format(v_mid, diff, precision))
