...
class AsyncDocker:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        pawn.console.log(f"__exit__ :: before session: {self.client.session.closed} ")
        res = asyncio.run(self.close())
        pawn.console.log(f"__exit__ :: after session: {res} == session: {self.client.session.closed} ")
    ...
    def run_async_loop(self, function):
        loop = None
        pawn.console.log(f"run_async_loop(), {function}, {loop}")
...
async def run_dyn_container(config, *args, **kwargs):
    ...
    pawn.console.log(f"Running container_name={container_name}")
