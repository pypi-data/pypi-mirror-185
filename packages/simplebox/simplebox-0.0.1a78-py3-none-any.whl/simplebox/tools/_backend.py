...
_inner_type = dir(__builtins__)
_inner_type.extend(['str', 'int', 'float', 'bool', 'list', 'dict', 'tuple', 'complex', 'set', 'bytes'])
...
class _Backend(object):
    ...
    def __init__(self, func: Callable, args: Iterable = None, kwargs: Dict = None, log_path: str or Path = None):
        ...
        self.__fifo_file = f"simplebox-fifo-{self.__module_name}-{self.__func_name}-{ObjectsUtils.generate_random_str(6)}"
        self.__fifo()
        ...
        for arg in self.__all_args[0]:
            att_name = type(arg).__name__
            if att_name not in _inner_type:
                self.__import_statement.append(f"from {self.__module_name} import {att_name};")
        for arg in self.__all_args[1].values():
            att_name = type(arg).__name__
            if att_name not in _inner_type:
                self.__import_statement.append(f"from {self.__module_name} import {att_name};")
        ...
        self.__import_statement.append("import") \
            ...
            .append(f"{self.__module_name}.{self.__func_name}(*obj[0], **obj[1])") \
