...
class HTMLRenderer(Renderer):
    ...
    def generate(self, element: Union[YAMLRoot, BaseModel], context: HTMLContext) -> None:
        ...
        if context.head.slot:
            render_as = self.style_engine.slot_render_as(context.head.slot.name)
        else:
            render_as = None
        if context.target_depth == 0:
            ...
            logger.debug(f"Collection {context.head.slot.name} render_as={render_as}")
    ...
    def generate_document(self, element: Union[YAMLRoot, BaseModel], context: HTMLContext) -> None:
        ...
        if self.style_engine:
            title_slot = self.style_engine.title_slot(context.head_element_type.name)
            if title_slot:
                        ...
                        with a.div(class_="mermaid"):
                            a(
                                mermaid_renderer.render(
                                    element,
                                    context.schemaview,
                                    source_element_name=context.head_element_type.name,
                                )
                            )
        ...
        with a.div():
            ...
            if title_slot:
                ...
                if title:
                    with a.h2():
                        a(title)
            description_slot = self.style_engine.description_slot(context.head_element_type.name)
            if description_slot:
    ...
    def elements_to_unordered_list(
        ...
        a = context.airium
        with a.div():
            if not context.in_object_reference and context.head_element_type.name == "string":
    ...
    def anchor_id(self, context: HTMLContext, index: str) -> str:
        return f"{context.head_element_type.name}__{index}"
