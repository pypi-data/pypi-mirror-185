...
class Context:
    ...
    @property
    def head(self) -> Optional[PathComponent]:
    ...
    def in_collection(self) -> bool:
        """
        Is the current element in a collection?
    ...
    def index_extend(self, index: Any, target_element: Any = None) -> "Context":
        new_context = self._copy()
        new_context.head.index = index
        if target_element:
            new_context.target_path.append(target_element)
        return new_context

    @property
    def head_element_type(self) -> Union[ClassDefinition, EnumDefinition, TypeDefinition]:
        elt = self.schemaview.get_element(self.head.element_type)
