...
class DownloadKit(object):
    ...
    def add(self,
        ...
        if thread_id is not None:
            print('创建进程')
            thread = Process(target=self._run, args=(thread_id, ))
            self._threads[thread_id] = {'thread': thread, 'mission': self._missions_num}
            thread.start()
            # thread.run()
        else:
            self._waiting_list.put(self._missions_num)

        return mission

    def _run(self, ID: int):
        mission = self._missions[self._threads[ID]['mission']]
        while True:
            if not mission:
                if not self._waiting_list.empty():
                    mission = self._missions[self._waiting_list.get()]
                else:
                    break
