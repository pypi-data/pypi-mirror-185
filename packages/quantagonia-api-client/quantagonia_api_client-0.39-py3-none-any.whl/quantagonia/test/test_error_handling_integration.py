...
from quantagonia.spec_builder import QuboSolverType
...
class TestErrorHandling(unittest.TestCase):
    ...
    def test_job_interruption(self):
        ...
        base_specs = [
            QUBOSpecBuilder(type=QuboSolverType.THROW_DICE_SDP_ROOT)
        ]
        ...
        for spec in base_specs:
            for _, row in instances_df.iterrows():
                ...
                sense = "MAXIMIZE" if row.sense == "max" else "MINIMIZE"
                spec.set_sense(sense)
                if dual_subspec is not None:
                    dual_subspec['opt_sense'] = 'MINIMIZE' if row.sense == "max" else "MAXIMIZE"
                    spec.set_option("dual_solver", dual_subspec)
