...
from velait.velait_django.services.utils import create_random_str
from velait.velait_django.authentication.services import authorize_user
...
def user_login_view(request):
    request_args = urlencode({
        "response_type": "code",
        "state": create_random_str(64),
        "nonce": create_random_str(64),
...
def user_authentication_view(request):
    ...
    response.set_cookie(key='access_token', value=access_token, expires=expiration_time, httponly=True, secure=True)
    response.set_cookie(key='refresh_token', value=refresh_token, expires=expiration_time, httponly=True, secure=True)
    return response
