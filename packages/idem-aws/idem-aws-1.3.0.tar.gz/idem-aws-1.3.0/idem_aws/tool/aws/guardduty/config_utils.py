...
async def is_organization_configuration_updated(
    ...
    if auto_enable != before.get("auto_enable"):
        return True
    if data_sources.get("S3Logs").get("AutoEnable") != before.get("data_sources").get(
        "S3Logs"
    ).get("AutoEnable"):
        return True
    if data_sources.get("Kubernetes").get("AuditLogs").get("AutoEnable") != before.get(
        "data_sources"
    ).get("Kubernetes").get("AuditLogs").get("AutoEnable"):
        return True
