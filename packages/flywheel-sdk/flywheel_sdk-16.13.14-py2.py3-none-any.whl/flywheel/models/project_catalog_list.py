# coding: utf-8

"""
    Flywheel

    No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)  # noqa: E501

    OpenAPI spec version: 0.0.1
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


## NOTE: This file is auto generated by the swagger code generator program.
## Do not edit the file manually.

import pprint
import re  # noqa: F401

import six

from flywheel.models.project_sharing_settings_project_contact import ProjectSharingSettingsProjectContact  # noqa: F401,E501
from flywheel.models.project_sharing_settings_project_institution import ProjectSharingSettingsProjectInstitution  # noqa: F401,E501
from flywheel.models.project_sharing_settings_project_stats import ProjectSharingSettingsProjectStats  # noqa: F401,E501

# NOTE: This file is auto generated by the swagger code generator program.
# Do not edit the class manually.


class ProjectCatalogList(object):

    swagger_types = {
        'id': 'str',
        'label': 'str',
        'tags': 'list[str]',
        'summary': 'str',
        'therapeutic_areas': 'list[str]',
        'body_regions': 'list[str]',
        'organ_systems': 'list[str]',
        'project_institutions': 'list[ProjectSharingSettingsProjectInstitution]',
        'project_contacts': 'list[ProjectSharingSettingsProjectContact]',
        'stats': 'ProjectSharingSettingsProjectStats'
    }

    attribute_map = {
        'id': '_id',
        'label': 'label',
        'tags': 'tags',
        'summary': 'summary',
        'therapeutic_areas': 'therapeutic-areas',
        'body_regions': 'body-regions',
        'organ_systems': 'organ-systems',
        'project_institutions': 'project-institutions',
        'project_contacts': 'project-contacts',
        'stats': 'stats'
    }

    rattribute_map = {
        '_id': 'id',
        'label': 'label',
        'tags': 'tags',
        'summary': 'summary',
        'therapeutic-areas': 'therapeutic_areas',
        'body-regions': 'body_regions',
        'organ-systems': 'organ_systems',
        'project-institutions': 'project_institutions',
        'project-contacts': 'project_contacts',
        'stats': 'stats'
    }

    def __init__(self, id=None, label=None, tags=None, summary=None, therapeutic_areas=None, body_regions=None, organ_systems=None, project_institutions=None, project_contacts=None, stats=None):  # noqa: E501
        """ProjectCatalogList - a model defined in Swagger"""
        super(ProjectCatalogList, self).__init__()

        self._id = None
        self._label = None
        self._tags = None
        self._summary = None
        self._therapeutic_areas = None
        self._body_regions = None
        self._organ_systems = None
        self._project_institutions = None
        self._project_contacts = None
        self._stats = None
        self.discriminator = None
        self.alt_discriminator = None

        if id is not None:
            self.id = id
        if label is not None:
            self.label = label
        if tags is not None:
            self.tags = tags
        if summary is not None:
            self.summary = summary
        if therapeutic_areas is not None:
            self.therapeutic_areas = therapeutic_areas
        if body_regions is not None:
            self.body_regions = body_regions
        if organ_systems is not None:
            self.organ_systems = organ_systems
        if project_institutions is not None:
            self.project_institutions = project_institutions
        if project_contacts is not None:
            self.project_contacts = project_contacts
        if stats is not None:
            self.stats = stats

    @property
    def id(self):
        """Gets the id of this ProjectCatalogList.

        Unique database ID

        :return: The id of this ProjectCatalogList.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this ProjectCatalogList.

        Unique database ID

        :param id: The id of this ProjectCatalogList.  # noqa: E501
        :type: str
        """

        self._id = id

    @property
    def label(self):
        """Gets the label of this ProjectCatalogList.

        Application-specific label

        :return: The label of this ProjectCatalogList.
        :rtype: str
        """
        return self._label

    @label.setter
    def label(self, label):
        """Sets the label of this ProjectCatalogList.

        Application-specific label

        :param label: The label of this ProjectCatalogList.  # noqa: E501
        :type: str
        """

        self._label = label

    @property
    def tags(self):
        """Gets the tags of this ProjectCatalogList.

        Array of application-specific tags

        :return: The tags of this ProjectCatalogList.
        :rtype: list[str]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this ProjectCatalogList.

        Array of application-specific tags

        :param tags: The tags of this ProjectCatalogList.  # noqa: E501
        :type: list[str]
        """

        self._tags = tags

    @property
    def summary(self):
        """Gets the summary of this ProjectCatalogList.


        :return: The summary of this ProjectCatalogList.
        :rtype: str
        """
        return self._summary

    @summary.setter
    def summary(self, summary):
        """Sets the summary of this ProjectCatalogList.


        :param summary: The summary of this ProjectCatalogList.  # noqa: E501
        :type: str
        """

        self._summary = summary

    @property
    def therapeutic_areas(self):
        """Gets the therapeutic_areas of this ProjectCatalogList.


        :return: The therapeutic_areas of this ProjectCatalogList.
        :rtype: list[str]
        """
        return self._therapeutic_areas

    @therapeutic_areas.setter
    def therapeutic_areas(self, therapeutic_areas):
        """Sets the therapeutic_areas of this ProjectCatalogList.


        :param therapeutic_areas: The therapeutic_areas of this ProjectCatalogList.  # noqa: E501
        :type: list[str]
        """

        self._therapeutic_areas = therapeutic_areas

    @property
    def body_regions(self):
        """Gets the body_regions of this ProjectCatalogList.


        :return: The body_regions of this ProjectCatalogList.
        :rtype: list[str]
        """
        return self._body_regions

    @body_regions.setter
    def body_regions(self, body_regions):
        """Sets the body_regions of this ProjectCatalogList.


        :param body_regions: The body_regions of this ProjectCatalogList.  # noqa: E501
        :type: list[str]
        """

        self._body_regions = body_regions

    @property
    def organ_systems(self):
        """Gets the organ_systems of this ProjectCatalogList.


        :return: The organ_systems of this ProjectCatalogList.
        :rtype: list[str]
        """
        return self._organ_systems

    @organ_systems.setter
    def organ_systems(self, organ_systems):
        """Sets the organ_systems of this ProjectCatalogList.


        :param organ_systems: The organ_systems of this ProjectCatalogList.  # noqa: E501
        :type: list[str]
        """

        self._organ_systems = organ_systems

    @property
    def project_institutions(self):
        """Gets the project_institutions of this ProjectCatalogList.


        :return: The project_institutions of this ProjectCatalogList.
        :rtype: list[ProjectSharingSettingsProjectInstitution]
        """
        return self._project_institutions

    @project_institutions.setter
    def project_institutions(self, project_institutions):
        """Sets the project_institutions of this ProjectCatalogList.


        :param project_institutions: The project_institutions of this ProjectCatalogList.  # noqa: E501
        :type: list[ProjectSharingSettingsProjectInstitution]
        """

        self._project_institutions = project_institutions

    @property
    def project_contacts(self):
        """Gets the project_contacts of this ProjectCatalogList.


        :return: The project_contacts of this ProjectCatalogList.
        :rtype: list[ProjectSharingSettingsProjectContact]
        """
        return self._project_contacts

    @project_contacts.setter
    def project_contacts(self, project_contacts):
        """Sets the project_contacts of this ProjectCatalogList.


        :param project_contacts: The project_contacts of this ProjectCatalogList.  # noqa: E501
        :type: list[ProjectSharingSettingsProjectContact]
        """

        self._project_contacts = project_contacts

    @property
    def stats(self):
        """Gets the stats of this ProjectCatalogList.


        :return: The stats of this ProjectCatalogList.
        :rtype: ProjectSharingSettingsProjectStats
        """
        return self._stats

    @stats.setter
    def stats(self, stats):
        """Sets the stats of this ProjectCatalogList.


        :param stats: The stats of this ProjectCatalogList.  # noqa: E501
        :type: ProjectSharingSettingsProjectStats
        """

        self._stats = stats


    @staticmethod
    def positional_to_model(value):
        """Converts a positional argument to a model value"""
        return value

    def return_value(self):
        """Unwraps return value from model"""
        return self

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self.to_dict())

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, ProjectCatalogList):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other

    # Container emulation
    def __getitem__(self, key):
        """Returns the value of key"""
        key = self._map_key(key)
        return getattr(self, key)

    def __setitem__(self, key, value):
        """Sets the value of key"""
        key = self._map_key(key)
        setattr(self, key, value)

    def __contains__(self, key):
        """Checks if the given value is a key in this object"""
        key = self._map_key(key, raise_on_error=False)
        return key is not None

    def keys(self):
        """Returns the list of json properties in the object"""
        return self.__class__.rattribute_map.keys()

    def values(self):
        """Returns the list of values in the object"""
        for key in self.__class__.attribute_map.keys():
            yield getattr(self, key)

    def items(self):
        """Returns the list of json property to value mapping"""
        for key, prop in self.__class__.rattribute_map.items():
            yield key, getattr(self, prop)

    def get(self, key, default=None):
        """Get the value of the provided json property, or default"""
        key = self._map_key(key, raise_on_error=False)
        if key:
            return getattr(self, key, default)
        return default

    def _map_key(self, key, raise_on_error=True):
        result = self.__class__.rattribute_map.get(key)
        if result is None:
            if raise_on_error:
                raise AttributeError('Invalid attribute name: {}'.format(key))
            return None
        return '_' + result
