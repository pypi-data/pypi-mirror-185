...
def flatten(x, title=None):
    ...
    if isinstance(x, List):
        for i in x:
            resp.append(LabelOption(value='%s%s' % (title, i.title), schemaId=i.key))
            for j in i.children:
                resp += flatten(j, i.title)
    else:
        resp.append(LabelOption(value='%s%s' % (title, x.title), schemaId=x.key))
        for j in x.children:
            resp += flatten(j, x.title)
    return resp