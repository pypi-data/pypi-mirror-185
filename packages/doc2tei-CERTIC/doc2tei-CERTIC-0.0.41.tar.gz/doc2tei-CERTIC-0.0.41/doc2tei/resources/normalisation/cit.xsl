...
<xsl:template match="text:p[@text:style-name='TEI_quote_continuation']">
    <xsl:choose>
        <xsl:when test="preceding-sibling::*[2][local-name()='p' and starts-with(@text:style-name,'TEI_quote')]">
            <xsl:comment>(suppr.) quote continuation intégrée au cit</xsl:comment>
        </xsl:when>
        <xsl:otherwise>
            <xsl:comment>quote continuation conservée</xsl:comment>
            <xsl:copy-of select="."/>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
