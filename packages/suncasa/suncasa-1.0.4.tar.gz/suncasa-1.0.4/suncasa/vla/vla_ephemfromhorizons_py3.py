...
def readJPLephem(fmfile, version=''):
    ...
    headers = {
        ...
        'radii': {
            'pat': r'Target radii\s*:\s*([0-9.]+\s*x\s*[0-9.]+\s*x\s*[0-9.]+)\s*km.*Equator, meridian, pole|Target radii\s*:\s*([0-9.]+)\s*km\s*',
            'unit': 'km'},
    ...
    for origline in lines:
        ...
        # elif re.match(r'^\$\$SOE\s*$', line):  # Start of ephemeris
            ...
            for hk in headers:
                # print("hk=",hk)
                if py3:
                    haskey = retdict.__contains__(hk)
                else:
                    haskey = retdict.has_key(hk)
                if not haskey:
    ...
    try:
        ...
        print("Error!")
        if py3:
            haskey = retdict.__contains__('data')
        else:
            haskey = retdict.has_key('data')
        if haskey:
            if py3:
                haskeym = retdict['data'].__contains__('MJD')
            else:
                haskeym = retdict['data'].has_key('MJD')
            if haskeym:
                if py3:
                    haskeymd = retdict['data']['MJD'].__contains__('data')
                else:
                    haskeymd = retdict['data']['MJD'].has_key('data')
                if haskeymd:
    ...
    for hk in headers:
        ...
        if haskey:
            if py3:
                haskeyu = headers[hk].__contains__('unit')
            else:
                haskeyu = headers[hk].has_key('unit')
            if haskeyu:
                if hk == 'radii':
    ...
    if py3:
        haskeya = retdict['data'].__contains__('ang_sep')
    else:
        haskeya = retdict['data'].has_key('ang_sep')
    if haskeya:
    ...
    for dk in retdict['data']:
        if dk == 'obs_code':
            continue
        if py3:
            haskeyu = cols[dk].__contains__('unit')
        else:
            haskeyu = cols[dk].has_key('unit')
        if haskeyu:
    ...
    if py3:
        haskeyra = retdict['data'].__contains__('NP_RA')
        haskeydec = retdict['data'].__contains__('NP_DEC')
    else:
        haskeyra = retdict['data'].has_key('NP_RA')
        haskeydec = retdict['data'].has_key('NP_DEC')
    if len(retdict.get('radii', {'value': []})['value']) == 3 \
            and haskeyra and haskeydec:
    ...
    if py3:
        haskeyv = retdict.__contains__('VS_CREATE')
    else:
        haskeyv = retdict.has_key('VS_CREATE')
    if haskeyv:
...
def ephem_dict_to_table(fmdict, tablepath='', prefix=''):
    ...
    if clashing_cols:
        raise ValueError(
            'The input dictionary lists ' + ', '.join(clashing_cols) + ' as both keyword(s) and column(s)')
    ...
    info = {
        'readme': 'Derived by JPLephem_reader.py from a JPL-Horizons ephemeris (http://ssd.jpl.nasa.gov/horizons.cgi#top)',
...
def make_ephem(vis, ephemfile=None, observatory=None):
    '''
