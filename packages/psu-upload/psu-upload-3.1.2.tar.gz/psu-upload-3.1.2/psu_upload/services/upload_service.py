...
def using_s3():
    ...
    return (
        utility_service.get_setting("DEFAULT_FILE_STORAGE")
...
def upload_files(
    ...
    for ff in request.FILES.getlist(input_name):
        ...
        filename = (
            f"{sequenced_filename}{filename_seq}{dot_ext}"
            if sequenced_filename
            else None
        )
...
def upload_file(
    file_instance, specified_filename=None, parent_directory=None, tag=None
):
    """
    ...
    try:
        ...
        if using_s3():
            # At some point all files started showing up in S3 root directory.
            # I suspect the file name must now contain the full path
...
def build_s3_path(original_file_name, parent_directory, specified_filename):
    ...
    if specified_filename:
        file_path = os.path.join(
            file_path,
            f"{specified_filename}{given_extension.lower() if given_extension else ''}",
        )
    else:
...
def _read_file_content(file_instance, byte_limit, convert_to_string):
    ...
    try:
        if file_size > byte_limit:
            message_service.post_error(
                f"File size limit exceeded: {filename} ({file_size} bytes)"
            )
            return None
        ...
        if convert_to_string:
            try:
                ...
                try:
                    ...
                    message_service.post_error(
                        f"Unable to convert file to plain-text: {filename}"
                    )
                    log.debug(f"Conversion error: {ee}")

        return contents
    except Exception as ee:
        error_service.unexpected_error(f"Unable to read uploaded file: {filename}", ee)
        return None
