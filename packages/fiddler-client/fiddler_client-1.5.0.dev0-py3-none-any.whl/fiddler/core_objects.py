...
class DataTypeMap:
    """A mapping of pandas dtype names to internal Fiddler DataTypes"""

    dtypes = {
        'bool': DataType.BOOLEAN,
        'category': DataType.CATEGORY,
        'complex64': DataType.STRING,
        'complex128': DataType.STRING,
        'complex256': DataType.STRING,
        'object': DataType.STRING,
        'period[D]': DataType.STRING,
        'datetime64[ns]': DataType.STRING,
        'datetime64[ns, UTC]': DataType.STRING,
        'timedelta64[ns]': DataType.STRING,
        'float16': DataType.FLOAT,
        'float32': DataType.FLOAT,
        'float64': DataType.FLOAT,
        'float128': DataType.FLOAT,
    ...
    def violation_of_type(self, value):
        ...
        if self.data_type.value == DataType.INTEGER.value:
            ...
            if isinstance(value, float):
                if int(value) == value:
                    value = int(value)
            return not isinstance(value, int) and not pandas.api.types.is_int64_dtype(value)
...
class DatasetInfo:
    ...
    def datatype_from_pandas_dtype(pandas_col) -> DataType:
        dtype = pandas_col.infer_objects().dtype.name
        if dtype in DataTypeMap.dtypes:
            return DataTypeMap.dtypes[dtype]
        else:
