...
from .const import UD2OLIA_mappings, mapobject, upos2olia
from .const import OffsetBasedString, RFC5147String
from .const import EntityOccurrence, TermOccurrence
from .utils import delete_accents, delete_diacritics
from .utils import natural_sort

from rdflib import Graph
from rdflib.term import IdentifiedNode, Literal, URIRef
from rdflib.namespace import Namespace, NamespaceManager, RDF, RDFS, XSD, DC, DCTERMS
from .const import NIF, NIF_ONTOLOGY, OLIA, ITSRDF
            ...
            if not isinstance(uri, URIRef):
                if not isinstance(self, NifContext):
                    ...
                    if uri_format == OffsetBasedString:
                        ...
                        uri = (
                            uri + "#char=" + str(self.beginIndex) + "," + str(endIndex)
                        )
...
class NifContext(NifString):
    ...
    def __repr__(self):
        ...
        if self.firstSentence is not None:
            s += f'  firstSentence : "{self.firstSentence.anchorOf}"\n'
        if self.lastSentence is not None:
            s += f'  lastSentence : "{self.lastSentence.anchorOf}"\n'
    ...
    def load(self, graph: Graph = None, uri: URIRef = None):
        ...
        for s, p, o in graph.triples((uri, None, None)):
            if p == RDF.type:
                ...
                self.set_anchorOf(o)
            elif p == NIF.firstSentence:
                first_sentence = o
            elif p == NIF.lastSentence:
                last_sentence = o
            elif p == NIF.nextSentence:
                self.set_nextSentence(o)
            elif p == NIF.previousSentence:
    ...
    def __repr__(self):
        ...
        if self.nextSentence is not None:
            if len(self.nextSentence.anchorOf) > 100:
                s += f'  nextSentence : "{self.nextSentence.anchorOf[0:100]}..."\n'
            else:
                s += f'  nextSentence : "{self.nextSentence.anchorOf}"\n'
        if self.previousSentence is not None:
            if len(self.previousSentence.anchorOf) > 100:
                s += f'  previousSentence : "{self.previousSentence.anchorOf[0:100]}..."\n'
            else:
                s += f'  previousSentence : "{self.previousSentence.anchorOf}"\n'
...
class NifContextCollection(NifBase):
    ...
    def add_context(self, context: NifContext = None):
        if context is not None and context.uri not in [c.uri for c in self._hasContext]:
            self._hasContext.append(context)
...
# class NifPropertyBasedAnnotation(NifAnnotation):

# class NifTextSpanAnnotation(NifAnnotation):

# class NifEntityOccurrence(NifTextSpanAnnotation):
...
# class NifTermOccurrence(TextSpanAnnotation):
