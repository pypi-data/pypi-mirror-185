...
# Citation: 
# PEREIRA, V. (2021). Project: pyMultiojective, File: sms.py, GitHub repository: <https://github.com/Valdecy/pyMultiojective>
...
def reference_points(M, p):
    ...
    axis_list = []
    for i in range(0, M):
        axis_matrix      = np.zeros((ref_points.shape))
        axis_matrix[:,i] = ref_points[:, i]
        axis_list.append(axis_matrix)
    ref_points = np.concatenate(axis_list)
    ref_points = np.unique(ref_points, axis = 0)
    return ref_points

# Function: Normalize Objective Functions
def normalization(population, z_min, z_max, number_of_functions):
    M                 = number_of_functions
    population[:,-M:] = population[:,-M:] /(z_max - z_min + 0.00000000000000001)
    return population

# Function: Association
def association(srp, population, z_min, z_max, number_of_functions):
    M   = number_of_functions
    p   = copy.deepcopy(population)
    p   = normalization(p, z_min, z_max, M)
    g   = distance.cdist(p[:,-M:], srp, 'chebyshev')
    idx = []
...
sol = s_metric_selection_evolutionary_multiobjective_optimization_algorithm(references = 25, mutation_rate = 0.1, min_values = [-5,-5], max_values = [5,5], list_of_functions = [dent_f1, dent_f2], generations = 1500, mu = 1, eta = 1, k = 7, verbose = True)

plot_mooa_function(min_values = [-5, -5], max_values = [5, 5], list_of_functions = [dent_f1, dent_f2], step = [0.1, 0.1], solution = sol, view = 'browser', show_pf = True, show_pts = True, show_sol = True, pf_min = True, custom_pf = [])
