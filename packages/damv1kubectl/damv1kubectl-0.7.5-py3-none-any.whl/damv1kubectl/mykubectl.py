                ...
                if len(target_str) !=0:
                    for target in target_str:
                        ...
                        if re.search(re_target, _strcmd):
                            get_bStatus_complete_sshcmd, get_exception_message_handler, oput = self.execsshcmd(_strcmd) #  specific objects スペシャルオブジェクト
                else:
                    print('\r{0}              Scanning target regex '.format(time7.currentTime7()), end='')
                    ...
                    get_bStatus_complete_sshcmd, get_exception_message_handler, oput = self.execsshcmd(_strcmd) # all objects すべてオブジェクト
        except Exception as e:
