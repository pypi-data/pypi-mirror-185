...
def compare_tracks(
    ...
    if download_spotify_playlist:
        if isinstance(config, UtilsConfig):
            ...
            setattr(config, "CHECK_TRACKS_LOCAL_DIRS", [])
    ...
    if download_spotify_playlist:
        if isinstance(config, UtilsConfig):
            ...
            del config.__dict__["CHECK_TRACKS_SPOTIFY_PLAYLISTS"]
            del config.__dict__["CHECK_TRACKS_LOCAL_DIRS"]
