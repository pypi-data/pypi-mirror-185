...
def generate_rout_layer(job,step,srcLayer,disLayer,mode):
    data = {
...
def inch2nm(num:float):
    ...
    return count
# 数据转换
def data_processing(number:float, number_format_l:int, number_format_r:int):
    ...
    return (d1,d2)
# 槽孔数据
def slotLenth(job:str, step:str, layer:str, location:dict, unit:bool, angle:float, mirror:bool, number_format_l:int, number_format_r:int, pointx:float, pointy:float, delta_x:float, delta_y:float):
    ...
    for n in info:
        ...
        xs = data_processing(xs,number_format_l,number_format_r)
        ys = data_processing(ys,number_format_l,number_format_r)
        xe = data_processing(xe,number_format_l,number_format_r)
        ye = data_processing(ye,number_format_l,number_format_r)
        xs = 'X'+xs[0]+xs[1]
        ys = 'Y'+ys[0]+ys[1]
        xe = 'X'+xe[0]+xe[1]
        ye = 'Y'+ye[0]+ye[1]
    ...
    return digital
# 圆孔
def isPad(location:dict, unit:bool, angle:float, mirror:bool, number_format_l:int, number_format_r:int, pointx:float,pointy:float, delta_x:float, delta_y:float):
    ...
    x = data_processing(x,number_format_l,number_format_r)
    y = data_processing(y,number_format_l,number_format_r)
    x = 'X'+x[0]+x[1]
    y = 'Y'+y[0]+y[1]
    xy = x+y
    return xy
            ...
            if THolesize.count(hole) == 1:
                ...
                for t in range(e+1,len(data)):
                    ...
                    if hole == iholesize and t != e and slotlenth == lenth:
                        ...
                        Toc.append(to_dict)
                        ab.append(t)
                    elif hole == iholesize and t != e and slotlenth != lenth:
                        location.extend(locat)
                        location = [dict(t) for t in set([tuple(d.items()) for d in location])]
                        to_dict['iToolIdx'] =tool
                        to_dict['iHoleSize'] = hole
                        to_dict['iSlotLenth'] = lenth
                        to_dict['vLocations'] = location
                        Toc.append(to_dict)
                        ab.append(t)
    return Toc
