...
from ...features.ipfx.feature_extractor import SpikeFeatureExtractor
...
class SubthresholdFeatures: 
    ...
    def process_sag(self): 
        ...
        # check for hyerpolarization steps
        #---------------------------------
        # remove sweeps > 0 pA stimuli 
        
        self.hyperpol_sweepdata, self.hyperpol_sweeptimes, self.hyperpol_stimuli = drop_depolsweeps(self.sweepdata, self.sweeptimes, self.stimuli)

        # remove unstable sweeps
        #-------------------------
        # remove sweeps > rms cutoff 
        ...
        if len(self.stable_sweepdata) > 0: 
            ...
            for t, v, i in zip(self.stable_sweeptimes, self.stable_sweepdata, self.stable_stimuli): 
                ...
                # voltage_deflection
                #--------------------
                deflect_v, deflect_index = voltage_deflection(t,v,i, start = self.start, end = self.end, deflect_type = None)
                
                # sag feature extraction
                #-----------------------
                sag_value, sag_voltage_fromsteady = sag(t,v,i, start = self.start, end = self.end,\
                                                        peak_width=self.peak_width, baseline_interval=self.baseline_interval)
                ...
                self.filtered_amp_idx = rem_sag_amps(sag_voltages_fromsteady = sag_voltages_fromsteady, amp_cutoffs = self.sag_amp_threshold) 
                ...
                df_sag_all_sweeps = pd.DataFrame({'stable_sweep_number': self.stable_sweepidx[0][self.filtered_amp_idx],\
    ...
    def plot_sag(self, sweeps = None, deflect_v = False, show_all_stable = False, xlim = [0, 2.5],\
        ...
        if sweeps is not None: 
            ...
            raise ValueError(f'pass sweeps as a list | sweeps: {sweeps} ...')
    ...
    def _find_sweepdata(self): 
        ...
        try: 
            self.sweepdata = self.data.preprocessed_sweepdata # find preprocessed data 
        except:
            try: 
                self.sweepdata = self.data.sweepdata # if not, default to raw 
            except: 
                abf_id = self.data.metadata['abf_id']
    ...
    @property
    def _find_sweeptimes(self): 
        """ find sweep times """
 
        try: 
            self.sweeptimes = self.data.times # find preprocessed data 
        except:
            abf_id = self.data.metadata['abf_id']
    ...
    def _find_stimuli(self): 
        ...
        try: 
            self.stimuli = self.data.commandwaveform        
        except: 
...
class SubthresholdGroupFeatures(SubthresholdFeatures): 
    ...
    def group_export_sag_plots(self, figdir = 'figures/subthreshold/sag', file_extension = '.pdf', xlim = [0, 2.5], scale_bar = False): 
            ...
            for data in self.group_data: 
                ...
                if len(self.stable_sweepidx[0]) > 0: 
                    ...
                    self.plot_sag(sweeps = self.stable_sweepidx[0], deflect_v = True, xlim = xlim,\
                                    ylim_v = [None, None], ylim_i = [None, None], scale_bar = scale_bar, axis = False,\
