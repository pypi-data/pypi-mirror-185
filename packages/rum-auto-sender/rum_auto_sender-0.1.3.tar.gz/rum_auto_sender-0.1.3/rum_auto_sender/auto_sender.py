...
def is_content_same(content1, content2):
    pttn = r"[\s\r\n\t]"
    return re.sub(pttn, "", content1) == re.sub(pttn, "", content2)
...
class AutoSender:
    ...
    def save_to_file(self, title, content):
        ...
        filepath = os.path.join(self.local_dir_path, title + ".md")
        if os.path.exists(filepath):
            filepath = os.path.join(self.local_dir_path, title + "_" + str(int(time.time())) + ".md")
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
            logger.info("init file %s finished", filepath)
