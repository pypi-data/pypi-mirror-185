...
def start_sfgwas(role: str, demo: bool = False, protocol: str = "SFGWAS") -> None:
    ...
    doc_ref_dict: dict = get_doc_ref_dict()
    send_results: str = (
        doc_ref_dict["personal_parameters"][doc_ref_dict["participants"][int(role)]]
        .get("SEND_RESULTS", {})
        .get("value")
    )
    ...
    if doc_ref_dict["setup_configuration"] == "website":
        data_path = doc_ref_dict["personal_parameters"]["DATA_PATH"]["value"]
        if demo and not data_path:
            data_path = "sfkit_example_data/demo"
