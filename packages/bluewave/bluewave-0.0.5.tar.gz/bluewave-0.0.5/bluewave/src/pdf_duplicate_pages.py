...
def find_duplicate_pages(data_a, data_b):
    ...
    # If two files are same then read the file text once
    if (
        data_a["filename"] == data_b["filename"]
        and data_a["path_to_file"] == data_b["path_to_file"]
    ):
        page_nums_b, texts_b = page_nums_a, texts_a
    else:
    ...
    if corpus:
        ...
        # parallel computing pairwise distance
        m = pairwise_distances(x_, x_, metric="cosine", n_jobs=-1)
...
def generate_cross_pairs(duplicate_pages):
    for pair in duplicate_pages:
        if pair[0][0][1] != pair[0][1][1]:
            yield pair
...
def get_dup_page_results(duplicate_pages):
    ...
    for pair in generate_cross_pairs(duplicate_pages):
        tups, dist = pair
        page1, file_index1, text1 = tups[0]
        page2, file_index2, _ = tups[1]
        result = {
            "type": DUPLICATE_PAGE,
...
def remove_duplicate_pages(file_data, duplicate_pages):
    ...
    """
    pages_to_ignore = set()
    for pair, _ in duplicate_pages:
        betw_files = pair[0][1] != pair[1][1]
        if betw_files:
            for page, file_index, _ in pair:
                if (
                    page not in pages_to_ignore
                    and file_index == file_data["file_index"]
                ):
                    pages_to_ignore.add(page)
        elif (
            pair[1][0] not in pages_to_ignore and pair[0][1] == file_data["file_index"]
        ):
