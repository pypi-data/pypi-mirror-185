...
def compare_images(hash_info_a, hash_info_b):
    ...
    # Find common hashes
    common_hashes = set(info_a) & set(info_b)

    # Build list of common image info
    common_hash_info = [(h, info_a[h], info_b[h]) for h in common_hashes]
...
def get_similarity_scores(file_data, suspicious_pairs, methods_run):
    ...
    reorg_sus_pairs = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    similarity_scores = defaultdict(lambda: defaultdict(dict))
            ...
            for method_long, method_short in method_names:
                if method_short in methods_run:
                    try:
                        ...
                        sus_pairs = []
                    if method_long == "Identical image":
                        ...
                        a_clean = file_data[a]["full_" + method_short]
                        b_clean = file_data[b]["full_" + method_short]
...
def compare_pdf_files(
    ...
    file_data = []
    duplicate_files = len(set(filenames)) == 1
    f_data = None
    for file_index, file_name in enumerate(filenames):
        if not duplicate_files or file_index == 0:
            f_data = get_file_data(file_name, regen_cache, VERSION)
        else:
            f_data = f_data.copy()
        f_data["file_index"] = file_index
...
def execute_pages_method(a, b, suspicious_pairs):
    ...
    if duplicate_pages:
        suspicious_pairs.extend(get_dup_page_results(duplicate_pages))
        a_new = remove_duplicate_pages(a, duplicate_pages)
        if a["filename"] == b["filename"] and a["path_to_file"] == b["path_to_file"]:
            b_new = a_new.copy()
        else:
            b_new = remove_duplicate_pages(b, duplicate_pages)
        a_new["file_index"] = a["file_index"]
        b_new["file_index"] = b["file_index"]
    else:
        a_new = a
        b_new = b
    return a_new, b_new
...
def execute_images_method(a, a_new, b, b_new, suspicious_pairs, verbose):
    ...
    if any_images_are_sus:
        a_file_index_ = a["file_index"]
        b_file_index_ = b["file_index"]
        ...
        for img_hash, info_a, info_b in identical_images:
            ...
            sus_result = {
                ...
                "pages": [
                    {
                        "file_index": a_file_index_,
                        "page": sus_page_a,
                        "bbox": bbox_a,
                    },
                    {
                        "file_index": b_file_index_,
                        "page": sus_page_b,
                        "bbox": bbox_b,
                    },
                ],
