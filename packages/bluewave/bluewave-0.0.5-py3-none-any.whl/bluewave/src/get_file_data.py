...
def read_blocks_and_hashes(filename):
    ...
    with concurrent.futures.ProcessPoolExecutor(cpu_count()) as executor:
        ...
        for item in executor.map(get_page_block_and_hashes, *zip(*args_)):
...
def get_file_data(full_filename, regen_cache, version):
    ...
    if not regen_cache and os.path.exists(cached_filename):
        try:
