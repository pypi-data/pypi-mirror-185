...
def calculate_importance_score(suspicious_pairs):
    ...
    if suspicious_pairs:
        ...
        for sus_pair in suspicious_pairs:
            if "Duplicate page" in sus_pair["type"]:
                corpus.append(sus_pair["page_text"])
                dummies.append((0, 0, 1, 0))
            elif "Common text string" in sus_pair["type"]:
                corpus.append(sus_pair["block_text"])
                dummies.append((0, 1, 0, 0))
            elif "Common digit sequence" in sus_pair["type"]:
                corpus.append(sus_pair["block_text"])
                dummies.append((1, 0, 0, 0))
            elif "Identical image" in sus_pair["type"]:
                corpus.append("")
