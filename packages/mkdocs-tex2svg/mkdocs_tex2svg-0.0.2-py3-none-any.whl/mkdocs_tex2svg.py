...
BLOCK_RE_TEX2SVG = re.compile(
        r'^[\s]*```tex2svg[\s]+(?P<command>\w+)\s+(?P<filename>[^\s]+)\s*\n(?P<content>.*?)```\n$',
...
class MkdocsTex2SvgExtension(markdown.Extension):
    ...
    def extendMarkdown(self, md):
        ...
        md.preprocessors.register(MkdocsTex2SvgPreprocessor(md, self.config), 'graphviz_block', int(self.config['priority'][0]))
