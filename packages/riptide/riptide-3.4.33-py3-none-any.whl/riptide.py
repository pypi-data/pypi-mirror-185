...
def _find_best_fit(frac_range, argDict):
    ...
    if argDict['initial_bestfit'] == True:
        if argDict['silent'] == False:
            print('\nEstablishing initial fit to transcriptome...')
        best_fit = contextualize(model=argDict['model'], transcriptome=argDict['transcriptome'], silent=True)
        if argDict['silent'] == False:
            print('Parallelizing analysis of context-specific subnetwork flux...')
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=argDict['cpus']) as executor:
        ...
        for job in concurrent.futures.as_completed(maxfit_jobs):
            ...
            if result.concordance['r'] > best_fit.concordance['r']:
                best_fit = result
...
def maxfit(model, transcriptome = 'none', frac_min = 0.25, frac_max = 0.85, frac_step = 0.1, prune = True,
    ...
    argDict = {'model':model, 'transcriptome':transcriptome, 'silent':silent, 'cpus':cpus,
               'samples':samples, 'minimum':minimum, 'initial_bestfit':True,
...
def contextualize(model, transcriptome = 'none', samples = 500, silent = False, prune = True,
    ...
    * Other arguments from iterative implementation are carried over
    ...
    for x in range(0, transcriptome['replicates']):
        ...
        active_rxns = _constrain_and_analyze_model(constrained_model=riptide_model, coefficient_dict=min_coefficient_dict, fraction=fraction, sampling_depth=0, 
    ...
    flux_samples, fva_result, concordance = _constrain_and_analyze_model(constrained_model=riptide_model, coefficient_dict=max_coefficient_dict, fraction=fraction, 
...
def _constrain_and_analyze_model(constrained_model, coefficient_dict, fraction, sampling_depth, objective, tasks, minimum_threshold, skip_fva):
...
def _gapsplit(model, depth):
    fva = flux_variability_analysis(model, list(model.reactions), fraction_of_optimum=0.0, processes=1)
