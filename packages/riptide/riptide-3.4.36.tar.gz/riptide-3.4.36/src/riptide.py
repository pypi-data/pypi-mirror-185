...
def _find_best_fit(frac_range, argDict, prev_best=None):
    ...
    maxfit_report = {}
    best_fit = contextualize(model=argDict['model'], transcriptome=argDict['transcriptome'], fraction=frac_range[0], 
    ...
    for frac in frac_range[1:]:
        current_fit = contextualize(model=argDict['model'], transcriptome=argDict['transcriptome'], fraction=frac, 
                         silent=True, samples=argDict['samples'], 
                         minimum=argDict['minimum'], conservative=argDict['conservative'], objective=argDict['objective'], 
                         additive=argDict['additive'], important=argDict['important'], set_bounds=argDict['set_bounds'], 
                         tasks=argDict['tasks'], exclude=argDict['exclude'], gpr=argDict['gpr'], threshold=argDict['threshold'], 
                         open_exchanges=argDict['open_exchanges'], phase=argDict['phase'])
        maxfit_report[current_fit.fraction_of_optimum] = current_fit.concordance
        if current_fit.concordance['r'] > best_fit.concordance['r']: best_fit = current_fit
...
def maxfit(model, transcriptome = 'none', frac_min = 0.25, frac_max = 0.85, frac_step = 0.1, prune = True,
    ...
    argDict = {'model':riptide_model, 'transcriptome':transcriptome, 'silent':silent, 'cpus':cpus,
...
def contextualize(model, transcriptome = 'none', samples = 1000, silent = False, prune = True,
    ...
    set_bounds = True, tasks = [], exclude = [], gpr = False, threshold = 1e-5, open_exchanges = False, phase=1):
    ...
    * Other arguments from iterative implementation are carried over
    ...
    if open_exchanges == True:
        for rxn in riptide_model.exchanges: rxn.bounds = (-10., 1000.)
