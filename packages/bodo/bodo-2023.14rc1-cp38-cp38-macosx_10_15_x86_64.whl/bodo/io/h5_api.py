...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        qba__twln = []
        zkyh__ncjr = h5g_get_num_objs(obj_id)
        for rlm__mpike in range(zkyh__ncjr):
            jalo__lyvb = h5g_get_objname_by_idx(obj_id, rlm__mpike)
            qba__twln.append(jalo__lyvb)
        return qba__twln
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    kfs__dyw = parse_dtype(dtype)
    ojq__vhokv = np.int32(numba_to_c_type(kfs__dyw))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            irb__leun = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    vanza__bgvd = tuple_to_ptr(starts)
    nlev__gpmhr = tuple_to_ptr(counts)
    pvlzf__jrz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, vanza__bgvd,
        nlev__gpmhr, is_parallel, out_arr.ctypes, pvlzf__jrz)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    nlev__gpmhr = tuple_to_ptr(counts)
    pvlzf__jrz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, vanza__bgvd,
        nlev__gpmhr, is_parallel, out_arr.ctypes, pvlzf__jrz)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        tvli__crqb = types.Array(dtype, ndim, 'C')
        oyoda__nce = args[3]
        tvli__crqb = self.context.resolve_function_type(operator.getitem, [
            tvli__crqb, oyoda__nce], {}).return_type.copy(layout='C')
        return signature(tvli__crqb, *args)
...
def get_filter_read_indices(bool_arr):
    ocg__xbxsu = bool_arr.nonzero()[0]
    ajupy__fshd = bodo.libs.distributed_api.get_rank()
    pmte__hkss = bodo.libs.distributed_api.get_size()
    bik__inxwz = np.empty(pmte__hkss, np.int64)
    cwgi__tvg = len(bool_arr)
    bodo.libs.distributed_api.allgather(bik__inxwz, cwgi__tvg)
    txs__xlyx = bik__inxwz.cumsum()[ajupy__fshd] - cwgi__tvg
    ocg__xbxsu += txs__xlyx
    mmtug__nopfj = bodo.libs.distributed_api.dist_reduce(len(ocg__xbxsu),
        np.int32(sum_op))
    whenz__gkkke = bodo.libs.distributed_api.gatherv(ocg__xbxsu)
    if ajupy__fshd == 0:
        jhfj__vwgd = whenz__gkkke
    else:
        jhfj__vwgd = np.empty(mmtug__nopfj, ocg__xbxsu.dtype)
    bodo.libs.distributed_api.bcast(jhfj__vwgd)
    qak__cec = bodo.libs.distributed_api.get_start(mmtug__nopfj, pmte__hkss,
        ajupy__fshd)
    vjlk__qzj = bodo.libs.distributed_api.get_end(mmtug__nopfj, pmte__hkss,
        ajupy__fshd)
    return jhfj__vwgd[qak__cec:vjlk__qzj]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        fgcpr__geg = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], fgcpr__geg)
        return builder.bitcast(fgcpr__geg, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    nlev__gpmhr = tuple_to_ptr(counts)
    pvlzf__jrz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, vanza__bgvd,
        nlev__gpmhr, is_parallel, out_arr.ctypes, pvlzf__jrz, read_indices.
