...
def create_trig_func_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_func(arr):
            ...
            mss__jzoz = 'def impl(arr):\n'
            mss__jzoz += (
                f'  return bodo.libs.bodosql_array_kernels.{func_name}_util(arr)'
                )
            ntb__aykg = {}
            exec(mss__jzoz, {'bodo': bodo}, ntb__aykg)
            return ntb__aykg['impl']
    else:
        func_name = func_name.lower()

        def overload_func(arr0, arr1):
            args = [arr0, arr1]
            for kuc__glkub in range(2):
                if isinstance(args[kuc__glkub], types.optional):
                    return unopt_argument(
                        f'bodo.libs.bodosql_array_kernels.{func_name}', [
                        'arr0', 'arr1'], kuc__glkub)
            mss__jzoz = 'def impl(arr0, arr1):\n'
            mss__jzoz += (
...
def create_trig_util_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_trig_util(arr):
            ...
            lav__qmxgi = ''
            if func_name == 'ACOS':
                lav__qmxgi += 'res[i] = np.arccos(arg0)'
            elif func_name == 'ACOSH':
                lav__qmxgi += 'res[i] = np.arccosh(arg0)'
            elif func_name == 'ASIN':
                lav__qmxgi += 'res[i] = np.arcsin(arg0)'
            elif func_name == 'ASINH':
                lav__qmxgi += 'res[i] = np.arcsinh(arg0)'
            elif func_name == 'ATAN':
                lav__qmxgi += 'res[i] = np.arctan(arg0)'
            elif func_name == 'ATANH':
                lav__qmxgi += 'res[i] = np.arctanh(arg0)'
            elif func_name == 'COS':
                lav__qmxgi += 'res[i] = np.cos(arg0)'
            elif func_name == 'COSH':
                lav__qmxgi += 'res[i] = np.cosh(arg0)'
            elif func_name == 'SIN':
                lav__qmxgi += 'res[i] = np.sin(arg0)'
            elif func_name == 'SINH':
                lav__qmxgi += 'res[i] = np.sinh(arg0)'
            elif func_name == 'TAN':
                lav__qmxgi += 'res[i] = np.tan(arg0)'
            elif func_name == 'TANH':
                lav__qmxgi += 'res[i] = np.tanh(arg0)'
            elif func_name == 'RADIANS':
                lav__qmxgi += 'res[i] = np.radians(arg0)'
            elif func_name == 'DEGREES':
                lav__qmxgi += 'res[i] = np.degrees(arg0)'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            qujaf__ickqv = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(ykeci__erp, tkdfk__xej, cbp__msbo,
                lav__qmxgi, qujaf__ickqv)
    else:

        def overload_trig_util(arr0, arr1):
            verify_int_float_arg(arr0, func_name, 'arr0')
            verify_int_float_arg(arr1, func_name, 'arr1')
            ykeci__erp = ['arr0', 'arr1']
            tkdfk__xej = [arr0, arr1]
            cbp__msbo = [True, True]
            lav__qmxgi = ''
            if func_name == 'ATAN2':
                lav__qmxgi += 'res[i] = np.arctan2(arg0, arg1)\n'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            qujaf__ickqv = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(ykeci__erp, tkdfk__xej, cbp__msbo,
...
def _install_trig_overload(funcs_utils_names):
    for zxbas__xgo, hfnq__yci, func_name in funcs_utils_names:
        tvxew__oaslu = create_trig_func_overload(func_name)
        overload(zxbas__xgo)(tvxew__oaslu)
        gwb__juoxs = create_trig_util_overload(func_name)
        overload(hfnq__yci)(gwb__juoxs)
