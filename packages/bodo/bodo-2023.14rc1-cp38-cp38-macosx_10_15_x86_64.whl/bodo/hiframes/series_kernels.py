...
def _column_filter_impl(B, ind):
    exjkl__dpfj = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,), None)
    for hltjj__jckr in numba.parfors.parfor.internal_prange(len(exjkl__dpfj)):
        if ind[hltjj__jckr]:
            exjkl__dpfj[hltjj__jckr] = B[hltjj__jckr]
        else:
            bodo.libs.array_kernels.setna(exjkl__dpfj, hltjj__jckr)
    return exjkl__dpfj
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    zra__ljsaj = len(B)
    mktlx__xcsef = 0
    for hltjj__jckr in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, hltjj__jckr):
            mktlx__xcsef += 1
    srcww__wbq = zra__ljsaj - mktlx__xcsef
    rcw__lozo = bodo.libs.str_arr_ext.num_total_chars(B)
    exjkl__dpfj = bodo.libs.str_arr_ext.pre_alloc_string_array(srcww__wbq,
        rcw__lozo)
    bodo.libs.str_arr_ext.copy_non_null_offsets(exjkl__dpfj, B)
    bodo.libs.str_arr_ext.copy_data(exjkl__dpfj, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(exjkl__dpfj, -1)
    return exjkl__dpfj
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    kiicn__hsp = first_moment / count
    wvge__stzqv = (third_moment - 3 * second_moment * kiicn__hsp + 2 *
        count * kiicn__hsp ** 3)
    amt__qvm = second_moment - kiicn__hsp * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * wvge__stzqv / amt__qvm ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    kiicn__hsp = first_moment / count
    lfzjh__hjkf = (fourth_moment - 4 * third_moment * kiicn__hsp + 6 *
        second_moment * kiicn__hsp ** 2 - 3 * count * kiicn__hsp ** 4)
    houlj__ohey = second_moment - kiicn__hsp * first_moment
    ftei__qtum = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    hhea__xcslb = count * (count + 1) * (count - 1) * lfzjh__hjkf
    vqw__qxy = (count - 2) * (count - 3) * houlj__ohey ** 2
    s = (count - 1) * (hhea__xcslb / vqw__qxy - ftei__qtum)
    s = s / (count - 1)
    return s
