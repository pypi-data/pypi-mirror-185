...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        eqp__ealvu = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, eqp__ealvu)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            oqc__pkllf = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(oqc__pkllf.dtype)
            file_write(unicode_to_utf8(fname), oqc__pkllf.ctypes, 
                dtype_size * oqc__pkllf.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            oqc__pkllf = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(oqc__pkllf.dtype)
            wgmfs__nix = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                oqc__pkllf.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), oqc__pkllf.ctypes,
                start, count, wgmfs__nix)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            rlpwi__fmhtl = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                rlpwi__fmhtl = min(rlpwi__fmhtl, count * dtype_size)
            if rlpwi__fmhtl < 0:
                return -1
            return rlpwi__fmhtl
        return impl
