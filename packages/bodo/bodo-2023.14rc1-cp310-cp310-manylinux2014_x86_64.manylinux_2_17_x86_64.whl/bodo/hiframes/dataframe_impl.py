...
def generate_col_to_index_func_text(col_names: Tuple):
    if all(isinstance(a, str) for a in col_names) or all(isinstance(a,
        ...
        ckwv__kynw = f'bodo.utils.conversion.coerce_to_array({col_names})'
        return (
            f'bodo.hiframes.pd_index_ext.init_binary_str_index({ckwv__kynw})\n'
            )
...
def overload_dataframe_columns(df):
    jzy__mwbor = 'def impl(df):\n'
    if df.has_runtime_cols:
        jzy__mwbor += (
            '  return bodo.hiframes.pd_dataframe_ext.get_dataframe_column_names(df)\n'
            )
    else:
        atfrq__sdhrz = (bodo.hiframes.dataframe_impl.
            generate_col_to_index_func_text(df.columns))
        jzy__mwbor += f'  return {atfrq__sdhrz}'
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def overload_dataframe_values(df):
    ...
    zhw__hwsos = len(df.columns)
    wqya__fwm = set(i for i in range(zhw__hwsos) if isinstance(df.data[i],
        IntegerArrayType))
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(i, '.astype(float)' if i in wqya__fwm else '') for i in
        range(zhw__hwsos))
    jzy__mwbor = 'def f(df):\n'.format()
    jzy__mwbor += '    return np.stack(({},), 1)\n'.format(data_args)
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'np': np}, cnpf__vsgvm)
    wsvyw__ufimc = cnpf__vsgvm['f']
    return wsvyw__ufimc
...
def overload_dataframe_to_numpy(df, dtype=None, copy=False, na_value=_no_input
    ...
    iivwh__fylkp = {'dtype': dtype, 'na_value': na_value}
    nfd__qmznb = {'dtype': None, 'na_value': _no_input}
    check_unsupported_args('DataFrame.to_numpy', iivwh__fylkp, nfd__qmznb,
...
def overload_dataframe_size(df):
    if df.has_runtime_cols:
        ...
        def impl(df):
            ...
            iyayo__wdlc = bodo.hiframes.table.compute_num_runtime_columns(t)
            return iyayo__wdlc * len(t)
        return impl
...
def overload_dataframe_dtypes(df):
    ...
    trui__bosft = ',' if len(df.columns) == 1 else ''
    index = f'bodo.hiframes.pd_index_ext.init_heter_index({df.columns})'
    jzy__mwbor += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}{trui__bosft}), {index}, None)
"""
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
...
def overload_dataframe_astype(df, dtype, copy=True, errors='raise',
    ...
    iivwh__fylkp = {'errors': errors}
    nfd__qmznb = {'errors': 'raise'}
    check_unsupported_args('df.astype', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
...
"""
    ...
    if _bodo_object_typeref is not None:
        ...
        lfur__igl = _bodo_object_typeref.instance_type
        assert isinstance(lfur__igl, DataFrameType
            ), 'Bodo schema used in DataFrame.astype is only supported for DataFrame schemas'
        if df.is_table_format:
            for i, name in enumerate(df.columns):
                if name in lfur__igl.column_index:
                    idx = lfur__igl.column_index[name]
                    arr_typ = lfur__igl.data[idx]
                else:
                    arr_typ = df.data[i]
                gbysc__oijx.append(arr_typ)
        else:
            extra_globals = {}
            adakm__sdy = {}
            for i, name in enumerate(lfur__igl.columns):
                arr_typ = lfur__igl.data[i]
                extra_globals[f'_bodo_schema{i}'] = get_castable_arr_dtype(
                    arr_typ)
                adakm__sdy[name] = f'_bodo_schema{i}'
            data_args = ', '.join(
                f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {adakm__sdy[miuqz__rtl]}, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
                 if miuqz__rtl in adakm__sdy else
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
                 for i, miuqz__rtl in enumerate(df.columns))
    elif is_overload_constant_dict(dtype) or is_overload_constant_series(dtype
        ):
        tzjsv__zfz = get_overload_constant_dict(dtype
            ) if is_overload_constant_dict(dtype) else dict(
            get_overload_constant_series(dtype))
        if df.is_table_format:
            tzjsv__zfz = {name: dtype_to_array_type(parse_dtype(dtype)) for
                name, dtype in tzjsv__zfz.items()}
            for i, name in enumerate(df.columns):
                if name in tzjsv__zfz:
                    arr_typ = tzjsv__zfz[name]
                else:
                    arr_typ = df.data[i]
                gbysc__oijx.append(arr_typ)
        else:
            data_args = ', '.join(
                f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {_get_dtype_str(tzjsv__zfz[miuqz__rtl])}, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
                 if miuqz__rtl in tzjsv__zfz else
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
                 for i, miuqz__rtl in enumerate(df.columns))
    elif df.is_table_format:
        arr_typ = dtype_to_array_type(parse_dtype(dtype))
        gbysc__oijx = [arr_typ] * len(df.columns)
    else:
        data_args = ', '.join(
            f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), dtype, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
             for i in range(len(df.columns)))
    if df.is_table_format:
        yflwe__neadj = bodo.TableType(tuple(gbysc__oijx))
        extra_globals['out_table_typ'] = yflwe__neadj
        data_args = (
...
def overload_dataframe_copy(df, deep=True):
    ...
    if df.is_table_format:
        ...
        zkce__vcfbc = types.none
        extra_globals = {'output_arr_typ': zkce__vcfbc}
        ...
        for i in range(len(df.columns)):
            ...
            if is_overload_true(deep):
                cgb__aoh.append(arr + '.copy()')
            elif is_overload_false(deep):
                cgb__aoh.append(arr)
            else:
                cgb__aoh.append(f'{arr}.copy() if deep else {arr}')
        data_args = ', '.join(cgb__aoh)
    return _gen_init_df(header, df.columns, data_args, extra_globals=
        extra_globals)
...
def overload_dataframe_rename(df, mapper=None, index=None, columns=None,
    ...
    iivwh__fylkp = {'index': index, 'level': level, 'errors': errors}
    nfd__qmznb = {'index': None, 'level': None, 'errors': 'ignore'}
    check_unsupported_args('DataFrame.rename', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(mapper):
        ...
        dfl__rehw = get_overload_constant_dict(mapper)
        ...
        dfl__rehw = get_overload_constant_dict(columns)
    else:
        raise_bodo_error(
            "DataFrame.rename(): must pass columns either via 'mapper' and 'axis'=1 or 'columns'"
            )
    mbsr__hrbom = tuple([dfl__rehw.get(df.columns[i], df.columns[i]) for i in
...
"""
    ...
    if df.is_table_format:
        ...
        yrmu__rss = df.copy(columns=mbsr__hrbom)
        zkce__vcfbc = types.none
        extra_globals = {'output_arr_typ': zkce__vcfbc}
        ...
        for i in range(len(df.columns)):
            ...
            if is_overload_true(copy):
                ...
                cgb__aoh.append(f'{arr}.copy() if copy else {arr}')
        data_args = ', '.join(cgb__aoh)
    return _gen_init_df(header, mbsr__hrbom, data_args, extra_globals=
        extra_globals)
...
def overload_dataframe_filter(df, items=None, like=None, regex=None, axis=None
    ...
    dsoh__fpf = not is_overload_none(items)
    dbgxv__noyj = not is_overload_none(like)
    nbctp__tpxp = not is_overload_none(regex)
    nzmf__usqzu = dsoh__fpf ^ dbgxv__noyj ^ nbctp__tpxp
    tddo__rvple = not (dsoh__fpf or dbgxv__noyj or nbctp__tpxp)
    if tddo__rvple:
        raise BodoError(
    ...
    if is_overload_constant_str(axis):
        ...
        puexk__gkq = 0 if axis == 'index' else 1
    ...
    if puexk__gkq == 1:
        ...
        if dsoh__fpf:
            if is_overload_constant_list(items):
                qpgt__xjm = get_overload_const_list(items)
            else:
                raise BodoError(
        ...
        if dbgxv__noyj:
            if is_overload_constant_str(like):
                ymkd__txle = get_overload_const_str(like)
            else:
                raise BodoError(
        ...
        if nbctp__tpxp:
            if is_overload_constant_str(regex):
                rtyo__ehjg = get_overload_const_str(regex)
                jtcbj__ozn = re.compile(rtyo__ehjg)
            else:
                raise BodoError(
        ...
        for i, miuqz__rtl in enumerate(df.columns):
            if not is_overload_none(items
                ) and miuqz__rtl in qpgt__xjm or not is_overload_none(like
                ) and ymkd__txle in str(miuqz__rtl) or not is_overload_none(
                regex) and jtcbj__ozn.search(str(miuqz__rtl)):
                zntlw__pctc.append(miuqz__rtl)
                jsnif__wbize.append(i)
        for i in jsnif__wbize:
            var_name = f'data_{i}'
            xvku__tikx.append(var_name)
            jzy__mwbor += f"""  {var_name} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})
"""
        data_args = ', '.join(xvku__tikx)
        return _gen_init_df(jzy__mwbor, zntlw__pctc, data_args)
...
def overload_dataframe_isna(df):
    ...
    yrmu__rss = None
    if df.is_table_format:
        zkce__vcfbc = types.Array(types.bool_, 1, 'C')
        yrmu__rss = DataFrameType(tuple([zkce__vcfbc] * len(df.data)), df.
...
def overload_dataframe_select_dtypes(df, include=None, exclude=None):
    ...
    utda__fic = is_overload_none(include)
    rpvl__zad = is_overload_none(exclude)
    vcf__xzecx = 'DataFrame.select_dtypes'
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.select_dtypes()')
    if utda__fic and rpvl__zad:
        raise_bodo_error(
    ...
    if not utda__fic:
        if is_overload_constant_list(include):
            ...
            dkqj__efzc = [dtype_to_array_type(parse_dtype(elem, vcf__xzecx)
                ) for elem in include]
        elif is_legal_input(include):
            dkqj__efzc = [dtype_to_array_type(parse_dtype(include, vcf__xzecx))
                ]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        dkqj__efzc = get_nullable_and_non_nullable_types(dkqj__efzc)
        vba__hngy = tuple(miuqz__rtl for i, miuqz__rtl in enumerate(df.
            columns) if df.data[i] in dkqj__efzc)
    else:
        vba__hngy = df.columns
    if not rpvl__zad:
        if is_overload_constant_list(exclude):
            exclude = get_overload_const_list(exclude)
            hjqq__fxyk = [dtype_to_array_type(parse_dtype(elem, vcf__xzecx)
                ) for elem in exclude]
        elif is_legal_input(exclude):
            hjqq__fxyk = [dtype_to_array_type(parse_dtype(exclude, vcf__xzecx))
                ]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        hjqq__fxyk = get_nullable_and_non_nullable_types(hjqq__fxyk)
        vba__hngy = tuple(miuqz__rtl for miuqz__rtl in vba__hngy if df.data
            [df.column_index[miuqz__rtl]] not in hjqq__fxyk)
    data_args = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.column_index[miuqz__rtl]})'
         for miuqz__rtl in vba__hngy)
    header = 'def impl(df, include=None, exclude=None):\n'
    return _gen_init_df(header, vba__hngy, data_args)
...
def overload_dataframe_isin(df, values):
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.isin()')
    jzy__mwbor = 'def impl(df, values):\n'
    pjw__ogtj = {}
    duan__qbsq = False
    if isinstance(values, DataFrameType):
        duan__qbsq = True
        for i, miuqz__rtl in enumerate(df.columns):
            if miuqz__rtl in values.column_index:
                vash__omp = 'val{}'.format(i)
                jzy__mwbor += f"""  {vash__omp} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(values, {values.column_index[miuqz__rtl]})
"""
                pjw__ogtj[miuqz__rtl] = vash__omp
    elif is_iterable_type(values) and not isinstance(values, SeriesType):
        pjw__ogtj = {miuqz__rtl: 'values' for miuqz__rtl in df.columns}
    else:
        raise_bodo_error(f'pd.isin(): not supported for type {values}')
    data = []
    for i in range(len(df.columns)):
        vash__omp = 'data{}'.format(i)
        jzy__mwbor += (
            '  {} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {})\n'
            .format(vash__omp, i))
        data.append(vash__omp)
    msl__wku = ['out{}'.format(i) for i in range(len(df.columns))]
    uyd__mff = """
...
"""
  ...
  for i in numba.parfors.parfor.internal_prange(n):
    {2}[i] = {0}[i] in {1}
"""
    gugzw__yun = '  {} = np.zeros(len(df), np.bool_)\n'
    for i, (cname, hgq__qwurv) in enumerate(zip(df.columns, data)):
        if cname in pjw__ogtj:
            pleu__yxztz = pjw__ogtj[cname]
            if duan__qbsq:
                jzy__mwbor += uyd__mff.format(hgq__qwurv, pleu__yxztz,
                    msl__wku[i])
            else:
                jzy__mwbor += aghhi__vgo.format(hgq__qwurv, pleu__yxztz,
                    msl__wku[i])
        else:
            jzy__mwbor += gugzw__yun.format(msl__wku[i])
    return _gen_init_df(jzy__mwbor, df.columns, ','.join(msl__wku))
...
def overload_dataframe_abs(df):
    ...
    data_args = ', '.join(
        ...
        .format(i) for i in range(zhw__hwsos))
    header = 'def impl(df):\n'
    return _gen_init_df(header, df.columns, data_args)


def overload_dataframe_corr(df, method='pearson', min_periods=1):
    bgfbw__mwecp = [miuqz__rtl for miuqz__rtl, qunr__lphw in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(qunr__lphw.
        dtype)]
    assert len(bgfbw__mwecp) != 0
    fthu__ifczh = ''
    if not any(qunr__lphw == types.float64 for qunr__lphw in df.data):
        fthu__ifczh = '.astype(np.float64)'
    edmj__xtl = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.column_index[miuqz__rtl], '.astype(np.float64)' if 
        isinstance(df.data[df.column_index[miuqz__rtl]], IntegerArrayType) or
        df.data[df.column_index[miuqz__rtl]] == boolean_array else '') for
        miuqz__rtl in bgfbw__mwecp)
    lyfer__qzl = 'np.stack(({},), 1){}'.format(edmj__xtl, fthu__ifczh)
    data_args = ', '.join('res[:,{}]'.format(i) for i in range(len(
        bgfbw__mwecp)))
    index = f'{generate_col_to_index_func_text(bgfbw__mwecp)}\n'
    header = "def impl(df, method='pearson', min_periods=1):\n"
    header += '  mat = {}\n'.format(lyfer__qzl)
    header += '  res = bodo.libs.array_kernels.nancorr(mat, 0, min_periods)\n'
    return _gen_init_df(header, bgfbw__mwecp, data_args, index)
...
def overload_dataframe_cov(df, min_periods=None, ddof=1):
    ...
    stwl__masb = dict(ddof=ddof)
    ibz__jzc = dict(ddof=1)
    check_unsupported_args('DataFrame.cov', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    iha__eueht = '1' if is_overload_none(min_periods) else 'min_periods'
    bgfbw__mwecp = [miuqz__rtl for miuqz__rtl, qunr__lphw in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(qunr__lphw.
        dtype)]
    if len(bgfbw__mwecp) == 0:
...
def overload_dataframe_count(df, axis=0, level=None, numeric_only=False):
    ...
    stwl__masb = dict(axis=axis, level=level, numeric_only=numeric_only)
    ibz__jzc = dict(axis=0, level=None, numeric_only=False)
    check_unsupported_args('DataFrame.count', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}))'
         for i in range(len(df.columns)))
    jzy__mwbor = 'def impl(df, axis=0, level=None, numeric_only=False):\n'
    jzy__mwbor += '  data = np.array([{}])\n'.format(data_args)
    atfrq__sdhrz = (bodo.hiframes.dataframe_impl.
        generate_col_to_index_func_text(df.columns))
    jzy__mwbor += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {atfrq__sdhrz})\n'
        )
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'np': np}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
...
def overload_dataframe_nunique(df, axis=0, dropna=True):
    ...
    stwl__masb = dict(axis=axis)
    ibz__jzc = dict(axis=0)
    if not is_overload_bool(dropna):
        raise BodoError('DataFrame.nunique: dropna must be a boolean value')
    check_unsupported_args('DataFrame.nunique', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_kernels.nunique(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), dropna)'
         for i in range(len(df.columns)))
    jzy__mwbor = 'def impl(df, axis=0, dropna=True):\n'
    jzy__mwbor += '  data = np.asarray(({},))\n'.format(data_args)
    atfrq__sdhrz = (bodo.hiframes.dataframe_impl.
        generate_col_to_index_func_text(df.columns))
    jzy__mwbor += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {atfrq__sdhrz})\n'
...
def overload_dataframe_prod(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only,
        min_count=min_count)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.prod', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.product()')
...
def overload_dataframe_sum(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only,
        min_count=min_count)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.sum', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.sum()')
...
def overload_dataframe_max(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.max', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.max()')
...
def overload_dataframe_min(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.min', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.min()')
...
def overload_dataframe_mean(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.mean', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.mean()')
...
def overload_dataframe_var(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    stwl__masb = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    ibz__jzc = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.var', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.var()')
...
def overload_dataframe_std(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    stwl__masb = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    ibz__jzc = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.std', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.std()')
...
def overload_dataframe_median(df, axis=None, skipna=None, level=None,
    ...
    stwl__masb = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    ibz__jzc = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.median', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.median()')
...
def overload_dataframe_quantile(df, q=0.5, axis=0, numeric_only=True,
    ...
    stwl__masb = dict(numeric_only=numeric_only, interpolation=interpolation)
    ibz__jzc = dict(numeric_only=True, interpolation='linear')
    check_unsupported_args('DataFrame.quantile', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.quantile()')
...
def overload_dataframe_idxmax(df, axis=0, skipna=True):
    ...
    stwl__masb = dict(axis=axis, skipna=skipna)
    ibz__jzc = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmax', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.idxmax()')
    for ovgd__iafq in df.data:
        if not (bodo.utils.utils.is_np_array_typ(ovgd__iafq) and (
            ovgd__iafq.dtype in [bodo.datetime64ns, bodo.timedelta64ns] or
            isinstance(ovgd__iafq.dtype, (types.Number, types.Boolean))) or
            isinstance(ovgd__iafq, (bodo.IntegerArrayType, bodo.
            FloatingArrayType, bodo.CategoricalArrayType)) or ovgd__iafq in
            [bodo.boolean_array, bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmax() only supported for numeric column types. Column type: {ovgd__iafq} not supported.'
                )
        if isinstance(ovgd__iafq, bodo.CategoricalArrayType
            ) and not ovgd__iafq.dtype.ordered:
            raise BodoError(
...
def overload_dataframe_idxmin(df, axis=0, skipna=True):
    ...
    stwl__masb = dict(axis=axis, skipna=skipna)
    ibz__jzc = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmin', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.idxmin()')
    for ovgd__iafq in df.data:
        if not (bodo.utils.utils.is_np_array_typ(ovgd__iafq) and (
            ...
            [bodo.boolean_array, bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmin() only supported for numeric column types. Column type: {ovgd__iafq} not supported.'
                )
        if isinstance(ovgd__iafq, bodo.CategoricalArrayType
            ) and not ovgd__iafq.dtype.ordered:
            raise BodoError(
...
def _gen_reduce_impl(df, func_name, args=None, axis=None):
    ...
    if func_name in ('idxmax', 'idxmin'):
        ...
        bgfbw__mwecp = tuple(miuqz__rtl for miuqz__rtl, qunr__lphw in zip(
            df.columns, df.data) if bodo.utils.typing.
            _is_pandas_numeric_dtype(qunr__lphw.dtype))
        out_colnames = bgfbw__mwecp
    assert len(out_colnames) != 0
    try:
        if func_name in ('idxmax', 'idxmin') and axis == 0:
            comm_dtype = None
        else:
            lgva__htzxw = [numba.np.numpy_support.as_dtype(df.data[df.
                column_index[miuqz__rtl]].dtype) for miuqz__rtl in out_colnames
                ]
            comm_dtype = numba.np.numpy_support.from_dtype(np.
                find_common_type(lgva__htzxw, []))
    except NotImplementedError as lfht__iqcy:
        raise BodoError(
            f'Dataframe.{func_name}() with column types: {df.data} could not be merged to a common type.'
            )
    ajn__cpw = ''
    if func_name in ('sum', 'prod'):
        ajn__cpw = ', min_count=0'
    ddof = ''
    if func_name in ('var', 'std'):
        ddof = 'ddof=1, '
    jzy__mwbor = (
        'def impl(df, axis=None, skipna=None, level=None,{} numeric_only=None{}):\n'
        .format(ddof, ajn__cpw))
    if func_name == 'quantile':
        jzy__mwbor = (
            "def impl(df, q=0.5, axis=0, numeric_only=True, interpolation='linear'):\n"
            )
    if func_name in ('idxmax', 'idxmin'):
        jzy__mwbor = 'def impl(df, axis=0, skipna=True):\n'
    if axis == 0:
        jzy__mwbor += _gen_reduce_impl_axis0(df, func_name, out_colnames,
            comm_dtype, args)
    else:
        jzy__mwbor += _gen_reduce_impl_axis1(func_name, out_colnames,
            comm_dtype, df)
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba},
        cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def _gen_reduce_impl_axis0(df, func_name, out_colnames, comm_dtype, args):
    ...
    if func_name in ('min', 'max'):
        vyvug__ecxye = ', dtype=np.{}'.format(comm_dtype)
    if comm_dtype == types.float32 and func_name in ('sum', 'prod', 'mean',
        'var', 'std', 'median'):
        vyvug__ecxye = ', dtype=np.float32'
    xvl__llcb = f'bodo.libs.array_ops.array_op_{func_name}'
    juycl__hrn = ''
    if func_name in ['sum', 'prod']:
        juycl__hrn = 'True, min_count'
    elif func_name in ['idxmax', 'idxmin']:
        juycl__hrn = 'index'
    elif func_name == 'quantile':
        juycl__hrn = 'q'
    elif func_name in ['std', 'var']:
        juycl__hrn = 'True, ddof'
    elif func_name == 'median':
        juycl__hrn = 'True'
    data_args = ', '.join(
        f'{xvl__llcb}(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.column_index[miuqz__rtl]}), {juycl__hrn})'
         for miuqz__rtl in out_colnames)
    jzy__mwbor = ''
    if func_name in ('idxmax', 'idxmin'):
        jzy__mwbor += (
        ...
        jzy__mwbor += '  data = np.asarray(({},){})\n'.format(data_args,
            vyvug__ecxye)
    jzy__mwbor += f"""  return bodo.hiframes.pd_series_ext.init_series(data, pd.Index({out_colnames}))
"""
    return jzy__mwbor


def _gen_reduce_impl_axis1(func_name, out_colnames, comm_dtype, df_type):
    qosy__nvnmj = [df_type.column_index[miuqz__rtl] for miuqz__rtl in
        out_colnames]
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    data_args = '\n    '.join(
        'arr_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
        .format(i) for i in qosy__nvnmj)
    ygt__iqp = '\n        '.join(f'row[{i}] = arr_{qosy__nvnmj[i]}[i]' for
        i in range(len(out_colnames)))
    assert len(data_args) > 0, f'empty dataframe in DataFrame.{func_name}()'
    rkbn__shpui = f'len(arr_{qosy__nvnmj[0]})'
    gkng__ptfpb = {'max': 'np.nanmax', 'min': 'np.nanmin', 'sum':
    ...
    if func_name in gkng__ptfpb:
        hmrmh__jzhs = gkng__ptfpb[func_name]
        sufq__ldk = 'float64' if func_name in ['mean', 'median', 'std', 'var'
            ] else comm_dtype
        jzy__mwbor = f"""
    {data_args}
    numba.parfors.parfor.init_prange()
...
def overload_dataframe_pct_change(df, periods=1, fill_method='pad', limit=
    ...
    stwl__masb = dict(fill_method=fill_method, limit=limit, freq=freq)
    ibz__jzc = dict(fill_method='pad', limit=None, freq=None)
    check_unsupported_args('DataFrame.pct_change', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
...
def overload_dataframe_cumprod(df, axis=None, skipna=True):
    ...
    stwl__masb = dict(axis=axis, skipna=skipna)
    ibz__jzc = dict(axis=None, skipna=True)
    check_unsupported_args('DataFrame.cumprod', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.cumprod()')
    data_args = ', '.join(
...
def overload_dataframe_cumsum(df, axis=None, skipna=True):
    ...
    stwl__masb = dict(skipna=skipna)
    ibz__jzc = dict(skipna=True)
    check_unsupported_args('DataFrame.cumsum', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.cumsum()')
    data_args = ', '.join(
...
def overload_dataframe_describe(df, percentiles=None, include=None, exclude
    ...
    stwl__masb = dict(percentiles=percentiles, include=include, exclude=
        exclude, datetime_is_numeric=datetime_is_numeric)
    ibz__jzc = dict(percentiles=None, include=None, exclude=None,
        datetime_is_numeric=True)
    check_unsupported_args('DataFrame.describe', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.describe()')
    bgfbw__mwecp = [miuqz__rtl for miuqz__rtl, qunr__lphw in zip(df.columns,
        df.data) if _is_describe_type(qunr__lphw)]
    if len(bgfbw__mwecp) == 0:
        raise BodoError('df.describe() only supports numeric columns')
    bbw__miftj = sum(df.data[df.column_index[miuqz__rtl]].dtype == bodo.
        datetime64ns for miuqz__rtl in bgfbw__mwecp)

    def _get_describe(col_ind):
        ndazr__nyil = df.data[col_ind].dtype == bodo.datetime64ns
        if bbw__miftj and bbw__miftj != len(bgfbw__mwecp):
            if ndazr__nyil:
...
"""
    for miuqz__rtl in bgfbw__mwecp:
        col_ind = df.column_index[miuqz__rtl]
        header += f"""  des_{col_ind} = bodo.libs.array_ops.array_op_describe(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {col_ind}))
"""
    data_args = ', '.join(_get_describe(df.column_index[miuqz__rtl]) for
        miuqz__rtl in bgfbw__mwecp)
    mec__jwo = "['count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']"
    if bbw__miftj == len(bgfbw__mwecp):
        mec__jwo = "['count', 'mean', 'min', '25%', '50%', '75%', 'max']"
    elif bbw__miftj:
        mec__jwo = (
            "['count', 'mean', 'min', '25%', '50%', '75%', 'max', 'std']")
    index = f'bodo.utils.conversion.convert_to_index({mec__jwo})'
    return _gen_init_df(header, bgfbw__mwecp, data_args, index)
...
def overload_dataframe_take(df, indices, axis=0, convert=None, is_copy=True):
    ...
    stwl__masb = dict(axis=axis, convert=convert, is_copy=is_copy)
    ibz__jzc = dict(axis=0, convert=None, is_copy=True)
    check_unsupported_args('DataFrame.take', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_shift(df, periods=1, freq=None, axis=0, fill_value=None
    ...
    stwl__masb = dict(freq=freq, axis=axis)
    ibz__jzc = dict(freq=None, axis=0)
    check_unsupported_args('DataFrame.shift', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.shift()')
    for cgxit__bcx in df.data:
        if not is_supported_shift_array_type(cgxit__bcx):
            raise BodoError(
                f'Dataframe.shift() column input type {cgxit__bcx.dtype} not supported yet.'
                )
...
def overload_dataframe_diff(df, periods=1, axis=0):
    ...
    stwl__masb = dict(axis=axis)
    ibz__jzc = dict(axis=0)
    check_unsupported_args('DataFrame.diff', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.diff()')
    for cgxit__bcx in df.data:
        if not (isinstance(cgxit__bcx, types.Array) and (isinstance(
            cgxit__bcx.dtype, types.Number) or cgxit__bcx.dtype == bodo.
            datetime64ns)):
            raise BodoError(
                f'DataFrame.diff() column input type {cgxit__bcx.dtype} not supported.'
                )
    if not is_overload_int(periods):
...
def overload_dataframe_explode(df, column, ignore_index=False):
    ...
    if not is_literal_type(column):
        raise_bodo_error(vdk__xhyeu)
    if is_overload_constant_list(column) or is_overload_constant_tuple(column):
        asw__fynk = get_overload_const_list(column)
    else:
        asw__fynk = [get_literal_value(column)]
    wkyf__jazuj = [df.column_index[miuqz__rtl] for miuqz__rtl in asw__fynk]
    for i in wkyf__jazuj:
    ...
    header += (
        f'  counts = bodo.libs.array_kernels.get_arr_lens(data{wkyf__jazuj[0]})\n'
        )
    for i in range(n):
        if i in wkyf__jazuj:
            header += (
...
def overload_dataframe_set_index(df, keys, drop=True, append=False, inplace
    ...
    iivwh__fylkp = {'inplace': inplace, 'append': append,
        'verify_integrity': verify_integrity}
    nfd__qmznb = {'inplace': False, 'append': False, 'verify_integrity': False}
    check_unsupported_args('DataFrame.set_index', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
...
"""
    ...
    columns = tuple(miuqz__rtl for miuqz__rtl in df.columns if miuqz__rtl !=
        col_name)
    index = (
...
def overload_dataframe_query(df, expr, inplace=False):
    ...
    iivwh__fylkp = {'inplace': inplace}
    nfd__qmznb = {'inplace': False}
    check_unsupported_args('query', iivwh__fylkp, nfd__qmznb, package_name=
    ...
    def impl(df, expr, inplace=False):
        bsbmy__zbmt = bodo.hiframes.pd_dataframe_ext.query_dummy(df, expr)
        return df[bsbmy__zbmt]
    return impl
...
def overload_dataframe_duplicated(df, subset=None, keep='first'):
    ...
    iivwh__fylkp = {'subset': subset, 'keep': keep}
    nfd__qmznb = {'subset': None, 'keep': 'first'}
    check_unsupported_args('DataFrame.duplicated', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
    zhw__hwsos = len(df.columns)
    jzy__mwbor = "def impl(df, subset=None, keep='first'):\n"
    for i in range(zhw__hwsos):
        jzy__mwbor += (
            '  data_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})\n'
            .format(i))
    fte__zzm = ', '.join(f'data_{i}' for i in range(zhw__hwsos))
    fte__zzm += ',' if zhw__hwsos == 1 else ''
    jzy__mwbor += (
        f'  duplicated = bodo.libs.array_kernels.duplicated(({fte__zzm}))\n')
    jzy__mwbor += (
        '  index = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)\n')
    jzy__mwbor += (
...
def overload_dataframe_drop_duplicates(df, subset=None, keep='first',
    ...
    iivwh__fylkp = {'keep': keep, 'inplace': inplace, 'ignore_index':
        ignore_index}
    nfd__qmznb = {'keep': 'first', 'inplace': False, 'ignore_index': False}
    zlywd__veven = []
    if is_overload_constant_list(subset):
        zlywd__veven = get_overload_const_list(subset)
    elif is_overload_constant_str(subset):
        zlywd__veven = [get_overload_const_str(subset)]
    elif is_overload_constant_int(subset):
        zlywd__veven = [get_overload_const_int(subset)]
    ...
    for col_name in zlywd__veven:
        ...
        tyn__xhw.append(df.column_index[col_name])
    check_unsupported_args('DataFrame.drop_duplicates', iivwh__fylkp,
        nfd__qmznb, package_name='pandas', module_name='DataFrame')
    ouvtz__vid = []
    if tyn__xhw:
        for jgzib__buue in tyn__xhw:
            if isinstance(df.data[jgzib__buue], bodo.MapArrayType):
                ouvtz__vid.append(df.columns[jgzib__buue])
    else:
        for i, col_name in enumerate(df.columns):
            if isinstance(df.data[i], bodo.MapArrayType):
                ouvtz__vid.append(col_name)
    if ouvtz__vid:
        raise BodoError(
            f'DataFrame.drop_duplicates(): Columns {ouvtz__vid} ' +
    ...
    zhw__hwsos = len(df.columns)
    krl__aty = ['data_{}'.format(i) for i in tyn__xhw]
    igi__itc = ['data_{}'.format(i) for i in range(zhw__hwsos) if i not in
        tyn__xhw]
    if krl__aty:
        vmfh__msk = len(krl__aty)
    else:
        vmfh__msk = zhw__hwsos
    hipo__qrjt = ', '.join(krl__aty + igi__itc)
    data_args = ', '.join('data_{}'.format(i) for i in range(zhw__hwsos))
    jzy__mwbor = (
...
"""
        .format(hipo__qrjt, index, vmfh__msk))
    jzy__mwbor += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(jzy__mwbor, df.columns, data_args, 'index')
...
"""
        ...
        if not hasattr(other, 'ndim') or other.ndim == 1:
            ...
            if isinstance(other, DataFrameType):
                ...
                auik__xoyv = lambda i: f'other[:,{i}]'
        zhw__hwsos = len(df.columns)
        data_args = ', '.join(
            f'bodo.hiframes.series_impl.where_impl({cond_str(i, gen_all_false)}, bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {auik__xoyv(i)})'
             for i in range(zhw__hwsos))
        if gen_all_false[0]:
...
def _install_dataframe_mask_where_overload():
    for func_name in ('mask', 'where'):
        ops__acoa = create_dataframe_mask_where_overload(func_name)
        overload_method(DataFrameType, func_name, no_unliteral=True)(ops__acoa)
...
def _validate_arguments_mask_where(func_name, df, cond, other, inplace,
    ...
    stwl__masb = dict(inplace=inplace, level=level, errors=errors, try_cast
        =try_cast)
    ibz__jzc = dict(inplace=False, level=None, errors='raise', try_cast=False)
    check_unsupported_args(f'{func_name}', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
...
def _gen_init_df(header, columns, data_args, index=None, extra_globals=None):
    ...
    if extra_globals is None:
        extra_globals = {}
    ecepf__pii = ColNamesMetaType(tuple(columns))
    data_args = '({}{})'.format(data_args, ',' if data_args else '')
    jzy__mwbor = f"""{header}  return bodo.hiframes.pd_dataframe_ext.init_dataframe({data_args}, {index}, __col_name_meta_value_gen_init_df)
"""
    cnpf__vsgvm = {}
    lxt__fkfp = {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba,
        '__col_name_meta_value_gen_init_df': ecepf__pii}
    lxt__fkfp.update(extra_globals)
    exec(jzy__mwbor, lxt__fkfp, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def _get_binop_columns(lhs, rhs, is_inplace=False):
    if lhs.columns != rhs.columns:
        esejm__yoei = pd.Index(lhs.columns)
        qnmg__yfku = pd.Index(rhs.columns)
        nfgtj__nlk, zwcq__pevl, hegc__xjcj = esejm__yoei.join(qnmg__yfku,
            how='left' if is_inplace else 'outer', level=None,
            return_indexers=True)
        return tuple(nfgtj__nlk), zwcq__pevl, hegc__xjcj
...
def create_binary_op_overload(op):
    ...
    def overload_dataframe_binary_op(lhs, rhs):
        qpql__ynd = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        fvw__hee = operator.eq, operator.ne
        check_runtime_cols_unsupported(lhs, qpql__ynd)
        check_runtime_cols_unsupported(rhs, qpql__ynd)
        if isinstance(lhs, DataFrameType):
            if isinstance(rhs, DataFrameType):
                nfgtj__nlk, zwcq__pevl, hegc__xjcj = _get_binop_columns(lhs,
                    rhs)
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {tdmc__vsgs}) {qpql__ynd}bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {yyq__kmgz})'
                     if tdmc__vsgs != -1 and yyq__kmgz != -1 else
                    f'bodo.libs.array_kernels.gen_na_array(len(lhs), float64_arr_type)'
                     for tdmc__vsgs, yyq__kmgz in zip(zwcq__pevl, hegc__xjcj))
                header = 'def impl(lhs, rhs):\n'
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(lhs)')
                return _gen_init_df(header, nfgtj__nlk, data_args, index,
            ...
            jkk__udg = []
            if op in fvw__hee:
                for i, muv__nzj in enumerate(lhs.data):
                    if is_common_scalar_dtype([muv__nzj.dtype, rhs]):
                        lqsot__zlwlk.append(
                            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {qpql__ynd} rhs'
                            )
                    else:
                        wlxig__lms = f'arr{i}'
                        jkk__udg.append(wlxig__lms)
                        lqsot__zlwlk.append(wlxig__lms)
                data_args = ', '.join(lqsot__zlwlk)
            else:
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {qpql__ynd} rhs'
            ...
            if len(jkk__udg) > 0:
                ...
                header += '  n = len(lhs)\n'
                header += ''.join(
                    f'  {wlxig__lms} = np.empty(n, dtype=np.bool_)\n' for
                    wlxig__lms in jkk__udg)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(wlxig__lms, 
                    op == operator.ne) for wlxig__lms in jkk__udg)
        ...
        if isinstance(rhs, DataFrameType):
            ...
            jkk__udg = []
            if op in fvw__hee:
                for i, muv__nzj in enumerate(rhs.data):
                    if is_common_scalar_dtype([lhs, muv__nzj.dtype]):
                        lqsot__zlwlk.append(
                            f'lhs {qpql__ynd} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {i})'
                            )
                    else:
                        wlxig__lms = f'arr{i}'
                        jkk__udg.append(wlxig__lms)
                ...
                data_args = ', '.join(
                    ...
                    .format(i, qpql__ynd) for i in range(len(rhs.columns)))
            header = 'def impl(lhs, rhs):\n'
            if len(jkk__udg) > 0:
                header += '  numba.parfors.parfor.init_prange()\n'
                header += '  n = len(rhs)\n'
                header += ''.join('  {0} = np.empty(n, dtype=np.bool_)\n'.
                    format(wlxig__lms) for wlxig__lms in jkk__udg)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(wlxig__lms, 
                    op == operator.ne) for wlxig__lms in jkk__udg)
...
def _install_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_binary_ops:
        if op in skips:
            continue
        ops__acoa = create_binary_op_overload(op)
        overload(op)(ops__acoa)
...
def create_inplace_binary_op_overload(op):
    ...
    def overload_dataframe_inplace_binary_op(left, right):
        qpql__ynd = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        check_runtime_cols_unsupported(left, qpql__ynd)
        check_runtime_cols_unsupported(right, qpql__ynd)
        if isinstance(left, DataFrameType):
            if isinstance(right, DataFrameType):
                nfgtj__nlk, _, hegc__xjcj = _get_binop_columns(left, right,
                    True)
                jzy__mwbor = 'def impl(left, right):\n'
                for i, yyq__kmgz in enumerate(hegc__xjcj):
                    if yyq__kmgz == -1:
                        jzy__mwbor += f"""  df_arr{i} = bodo.libs.array_kernels.gen_na_array(len(left), float64_arr_type)
"""
                        continue
                    jzy__mwbor += f"""  df_arr{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(left, {i})
"""
                    jzy__mwbor += f"""  df_arr{i} {qpql__ynd} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(right, {yyq__kmgz})
"""
                data_args = ', '.join(f'df_arr{i}' for i in range(len(
                    nfgtj__nlk)))
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)')
                return _gen_init_df(jzy__mwbor, nfgtj__nlk, data_args,
...
"""
                    .format(i))
                jzy__mwbor += '  df_arr{0} {1} right\n'.format(i, qpql__ynd)
            data_args = ', '.join('df_arr{}'.format(i) for i in range(len(
                left.columns)))
            index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)'
            return _gen_init_df(jzy__mwbor, left.columns, data_args, index)
    return overload_dataframe_inplace_binary_op


def _install_inplace_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_inplace_binary_ops:
        ops__acoa = create_inplace_binary_op_overload(op)
        overload(op, no_unliteral=True)(ops__acoa)
...
def create_unary_op_overload(op):
    ...
    def overload_dataframe_unary_op(df):
        if isinstance(df, DataFrameType):
            qpql__ynd = numba.core.utils.OPERATORS_TO_BUILTINS[op]
            check_runtime_cols_unsupported(df, qpql__ynd)
            data_args = ', '.join(
                '{1} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
                .format(i, qpql__ynd) for i in range(len(df.columns)))
            header = 'def impl(df):\n'
            return _gen_init_df(header, df.columns, data_args)
...
def _install_unary_ops():
    for op in bodo.hiframes.pd_series_ext.series_unary_ops:
        ops__acoa = create_unary_op_overload(op)
        overload(op, no_unliteral=True)(ops__acoa)
...
def overload_isna(obj):
    ...
    if is_array_typ(obj):
        ...
        def impl(obj):
            numba.parfors.parfor.init_prange()
            n = len(obj)
            nhwr__uysh = np.empty(n, np.bool_)
            for i in numba.parfors.parfor.internal_prange(n):
                nhwr__uysh[i] = bodo.libs.array_kernels.isna(obj, i)
            return nhwr__uysh
        return impl
...
def overload_dataframe_replace(df, to_replace=None, value=None, inplace=
    ...
    iivwh__fylkp = {'inplace': inplace, 'limit': limit, 'regex': regex,
        'method': method}
    nfd__qmznb = {'inplace': False, 'limit': None, 'regex': False, 'method':
        'pad'}
    check_unsupported_args('replace', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def _is_col_access(expr_node):
    lmma__neycj = str(expr_node)
    return lmma__neycj.startswith('(left.') or lmma__neycj.startswith('(right.'
        )


def _insert_NA_cond(expr_node, left_columns, left_data, right_columns,
    right_data):
    nld__iwz = {'left': 0, 'right': 0, 'NOT_NA': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (nld__iwz,))
    krto__dtiob = pd.core.computation.parsing.clean_column_name

    def append_null_checks(expr_node, null_set):
        if not null_set:
            return expr_node
        kuaen__srzc = ' & '.join([('NOT_NA.`' + x + '`') for x in null_set])
        vajj__wyitn = {('NOT_NA', krto__dtiob(muv__nzj)): muv__nzj for
            muv__nzj in null_set}
        rjxjh__new, _, _ = _parse_query_expr(kuaen__srzc, env, [], [], None,
            join_cleaned_cols=vajj__wyitn)
        lugmw__bevf = (pd.core.computation.ops.BinOp.
            _disallow_scalar_only_bool_ops)
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        try:
            dzuc__qoksz = pd.core.computation.ops.BinOp('&', rjxjh__new,
                expr_node)
        finally:
    ...
    def _insert_NA_cond_body(expr_node, null_set):
        if isinstance(expr_node, pd.core.computation.ops.BinOp):
            if expr_node.op == '|':
                log__utgyb = set()
                nqqt__gexh = set()
                cugtm__wehz = _insert_NA_cond_body(expr_node.lhs, log__utgyb)
                fjp__wfs = _insert_NA_cond_body(expr_node.rhs, nqqt__gexh)
                scsxw__tjbku = log__utgyb.intersection(nqqt__gexh)
                log__utgyb.difference_update(scsxw__tjbku)
                nqqt__gexh.difference_update(scsxw__tjbku)
                null_set.update(scsxw__tjbku)
                expr_node.lhs = append_null_checks(cugtm__wehz, log__utgyb)
                expr_node.rhs = append_null_checks(fjp__wfs, nqqt__gexh)
            ...
            kvcq__ymmsq = expr_node.name
            nob__bnena, col_name = kvcq__ymmsq.split('.')
            if nob__bnena == 'left':
                hqm__xgtu = left_columns
                data = left_data
            else:
                hqm__xgtu = right_columns
                data = right_data
            wtpsa__xeew = data[hqm__xgtu.index(col_name)]
            if bodo.utils.typing.is_nullable(wtpsa__xeew):
                null_set.add(expr_node.name)
        return expr_node
    null_set = set()
    oidq__kmh = _insert_NA_cond_body(expr_node, null_set)
...
def _extract_equal_conds(expr_node):
    ...
    if expr_node.op == '==' and _is_col_access(expr_node.lhs
        ...
        uiq__qou = str(expr_node.lhs)
        upio__ouy = str(expr_node.rhs)
        if uiq__qou.startswith('(left.') and upio__ouy.startswith('(left.'
            ) or uiq__qou.startswith('(right.') and upio__ouy.startswith(
            '(right.'):
            return [], [], expr_node
        left_on = [uiq__qou.split('.')[1][:-1]]
        right_on = [upio__ouy.split('.')[1][:-1]]
        if uiq__qou.startswith('(right.'):
            return right_on, left_on, None
        return left_on, right_on, None
    if expr_node.op == '&':
        pqkr__ucayd, coy__sskf, dzx__wvprt = _extract_equal_conds(expr_node.lhs
            )
        uofl__fgyr, dgs__owypn, tly__nntb = _extract_equal_conds(expr_node.rhs)
        left_on = pqkr__ucayd + uofl__fgyr
        right_on = coy__sskf + dgs__owypn
        if dzx__wvprt is None:
            return left_on, right_on, tly__nntb
        if tly__nntb is None:
            return left_on, right_on, dzx__wvprt
        expr_node.lhs = dzx__wvprt
        expr_node.rhs = tly__nntb
        expr_node.operands = expr_node.lhs, expr_node.rhs
...
def _parse_merge_cond(on_str, left_columns, left_data, right_columns,
    right_data):
    nld__iwz = {'left': 0, 'right': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (nld__iwz,))
    dfl__rehw = dict()
    krto__dtiob = pd.core.computation.parsing.clean_column_name
    for name, ebe__xqwlf in (('left', left_columns), ('right', right_columns)):
        for muv__nzj in ebe__xqwlf:
            gjag__mxg = krto__dtiob(muv__nzj)
            mipd__pdxig = name, gjag__mxg
            if mipd__pdxig in dfl__rehw:
                raise_bodo_error(
                    f"pd.merge(): {name} table contains two columns that are escaped to the same Python identifier '{muv__nzj}' and '{dfl__rehw[gjag__mxg]}' Please rename one of these columns. To avoid this issue, please use names that are valid Python identifiers."
                    )
            dfl__rehw[mipd__pdxig] = muv__nzj
    ggaw__tfydm, _, _ = _parse_query_expr(on_str, env, [], [], None,
        join_cleaned_cols=dfl__rehw)
    left_on, right_on, xpvlv__addig = _extract_equal_conds(ggaw__tfydm.terms)
    return left_on, right_on, _insert_NA_cond(xpvlv__addig, left_columns,
...
def overload_dataframe_merge(left, right, how='inner', on=None, left_on=
    ...
    stwl__masb = dict(sort=sort, copy=copy, validate=validate)
    ibz__jzc = dict(sort=False, copy=True, validate=None)
    check_unsupported_args('DataFrame.merge', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    validate_merge_spec(left, right, how, on, left_on, right_on, left_index,
        right_index, sort, suffixes, copy, indicator, validate)
    how = get_overload_const_str(how)
    duo__iwb = tuple(sorted(set(left.columns) & set(right.columns), key=lambda
        k: str(k)))
    ound__nzq = ''
    if not is_overload_none(on):
        ...
        if is_overload_constant_str(on):
            ...
            if on_str not in duo__iwb and ('left.' in on_str or 'right.' in
                on_str):
                left_on, right_on, tfuk__evrs = _parse_merge_cond(on_str,
                    left.columns, left.data, right.columns, right.data)
                if tfuk__evrs is None:
                    ound__nzq = ''
                else:
                    ound__nzq = str(tfuk__evrs)
    ...
    uridu__dbz = get_overload_const_bool(_bodo_na_equal)
    ...
    if is_overload_constant_tuple(suffixes):
        evfo__vyiop = get_overload_const_tuple(suffixes)
    if is_overload_constant_list(suffixes):
        evfo__vyiop = list(get_overload_const_list(suffixes))
    suffix_x = evfo__vyiop[0]
    suffix_y = evfo__vyiop[1]
    ...
    jzy__mwbor = "def _impl(left, right, how='inner', on=None, left_on=None,\n"
    jzy__mwbor += (
        '    right_on=None, left_index=False, right_index=False, sort=False,\n'
        )
    jzy__mwbor += """    suffixes=('_x', '_y'), copy=True, indicator=False, validate=None, _bodo_na_equal=True):
"""
    jzy__mwbor += (
        """  return bodo.hiframes.pd_dataframe_ext.join_dummy(left, right, {}, {}, '{}', '{}', '{}', False, {}, {}, '{}')
"""
        .format(left_keys, right_keys, how, suffix_x, suffix_y,
        indicator_val, uridu__dbz, ound__nzq))
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo}, cnpf__vsgvm)
    _impl = cnpf__vsgvm['_impl']
    return _impl
...
def common_validate_merge_merge_asof_spec(name_func, left, right, on,
    ...
    oos__uwa = {string_array_type, dict_str_arr_type, binary_array_type,
        datetime_date_array_type, datetime_timedelta_array_type, boolean_array}
    dhctj__novyd = {get_overload_const_str(rgus__pyzr) for rgus__pyzr in (
        left_on, right_on, on) if is_overload_constant_str(rgus__pyzr)}
    for df in (left, right):
        for i, muv__nzj in enumerate(df.data):
            if not isinstance(muv__nzj, valid_dataframe_column_types
                ) and muv__nzj not in oos__uwa:
                raise BodoError(
                    f'{name_func}(): use of column with {type(muv__nzj)} in merge unsupported'
                    )
            if df.columns[i] in dhctj__novyd and isinstance(muv__nzj,
                MapArrayType):
                raise BodoError(
    ...
    if is_overload_constant_list(suffixes):
        evfo__vyiop = list(get_overload_const_list(suffixes))
    if len(evfo__vyiop) != 2:
        raise BodoError(name_func +
            '(): The number of suffixes should be exactly 2')
    duo__iwb = tuple(set(left.columns) & set(right.columns))
    if not is_overload_none(on):
        wky__kkqiw = False
        if is_overload_constant_str(on):
            on_str = get_overload_const_str(on)
            wky__kkqiw = on_str not in duo__iwb and ('left.' in on_str or 
                'right.' in on_str)
        if len(duo__iwb) == 0 and not wky__kkqiw:
            raise_bodo_error(name_func +
...
def validate_keys_dtypes(left, right, left_index, right_index, left_keys,
    right_keys):
    dbcwm__owb = numba.core.registry.cpu_target.typing_context
    if is_overload_true(left_index) or is_overload_true(right_index):
        if is_overload_true(left_index) and is_overload_true(right_index):
            ywrx__ukoj = left.index
            nnw__dyq = isinstance(ywrx__ukoj, StringIndexType)
            qad__crf = right.index
            ummqz__rwmz = isinstance(qad__crf, StringIndexType)
        elif is_overload_true(left_index):
            ywrx__ukoj = left.index
            nnw__dyq = isinstance(ywrx__ukoj, StringIndexType)
            qad__crf = right.data[right.columns.index(right_keys[0])]
            ummqz__rwmz = qad__crf.dtype == string_type
        elif is_overload_true(right_index):
            ywrx__ukoj = left.data[left.columns.index(left_keys[0])]
            nnw__dyq = ywrx__ukoj.dtype == string_type
            qad__crf = right.index
            ummqz__rwmz = isinstance(qad__crf, StringIndexType)
        if nnw__dyq and ummqz__rwmz:
            return
        ywrx__ukoj = ywrx__ukoj.dtype
        qad__crf = qad__crf.dtype
        try:
            fymzj__tdbkd = dbcwm__owb.resolve_function_type(operator.eq, (
                ywrx__ukoj, qad__crf), {})
        except:
            raise_bodo_error(
                'merge: You are trying to merge on {lk_dtype} and {rk_dtype} columns. If you wish to proceed you should use pd.concat'
                .format(lk_dtype=ywrx__ukoj, rk_dtype=qad__crf))
    else:
        for dxasv__uja, mzfgl__vkaz in zip(left_keys, right_keys):
            ywrx__ukoj = left.data[left.columns.index(dxasv__uja)].dtype
            fuptf__gjszs = left.data[left.columns.index(dxasv__uja)]
            qad__crf = right.data[right.columns.index(mzfgl__vkaz)].dtype
            usx__mvfsc = right.data[right.columns.index(mzfgl__vkaz)]
            if fuptf__gjszs == usx__mvfsc:
                continue
            gsv__xopr = (
                ...
                .format(lk=dxasv__uja, lk_dtype=ywrx__ukoj, rk=mzfgl__vkaz,
                rk_dtype=qad__crf))
            daeqv__lnm = ywrx__ukoj == string_type
            jvjp__ukcei = qad__crf == string_type
            if daeqv__lnm ^ jvjp__ukcei:
                raise_bodo_error(gsv__xopr)
            try:
                fymzj__tdbkd = dbcwm__owb.resolve_function_type(operator.eq,
                    (ywrx__ukoj, qad__crf), {})
            except:
...
def validate_keys(keys, df):
    doi__plsv = set(keys).difference(set(df.columns))
    if len(doi__plsv) > 0:
        if is_overload_constant_str(df.index.name_typ
            ) and get_overload_const_str(df.index.name_typ) in doi__plsv:
            raise_bodo_error(
                f'merge(): use of index {df.index.name_typ} as key for on/left_on/right_on is unsupported'
                )
        raise_bodo_error(
            f"""merge(): invalid key {doi__plsv} for on/left_on/right_on
...
def overload_dataframe_join(left, other, on=None, how='left', lsuffix='',
    ...
    stwl__masb = dict(lsuffix=lsuffix, rsuffix=rsuffix)
    ibz__jzc = dict(lsuffix='', rsuffix='')
    check_unsupported_args('DataFrame.join', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    ...
    left_keys = gen_const_tup(left_keys)
    right_keys = gen_const_tup(right_keys)
    jzy__mwbor = "def _impl(left, other, on=None, how='left',\n"
    jzy__mwbor += "    lsuffix='', rsuffix='', sort=False):\n"
    jzy__mwbor += (
...
def validate_join_spec(left, other, on, how, lsuffix, rsuffix, sort):
    ...
    if not is_overload_none(on):
        ynniw__lohp = get_overload_const_list(on)
        validate_keys(ynniw__lohp, left)
    if not is_overload_false(sort):
        raise BodoError(
            'join(): sort parameter only supports default value False')
    duo__iwb = tuple(set(left.columns) & set(other.columns))
    if len(duo__iwb) > 0:
        raise_bodo_error(
            'join(): not supporting joining on overlapping columns:{cols} Use DataFrame.merge() instead.'
            .format(cols=duo__iwb))


def validate_unicity_output_column_names(suffix_x, suffix_y, left_keys,
    right_keys, left_columns, right_columns, indicator_val):
    eiee__cda = set(left_keys) & set(right_keys)
    evk__wnmaz = set(left_columns) & set(right_columns)
    balb__kqaw = evk__wnmaz - eiee__cda
    dxd__lvjr = set(left_columns) - evk__wnmaz
    kvo__mvdwa = set(right_columns) - evk__wnmaz
    htn__buzko = {}

    def insertOutColumn(col_name):
        if col_name in htn__buzko:
            raise_bodo_error(
                'join(): two columns happen to have the same name : {}'.
                format(col_name))
        htn__buzko[col_name] = 0
    for kod__cdz in eiee__cda:
        insertOutColumn(kod__cdz)
    for kod__cdz in balb__kqaw:
        kxr__qfe = str(kod__cdz) + suffix_x
        rjrri__wqr = str(kod__cdz) + suffix_y
        insertOutColumn(kxr__qfe)
        insertOutColumn(rjrri__wqr)
    for kod__cdz in dxd__lvjr:
        insertOutColumn(kod__cdz)
    for kod__cdz in kvo__mvdwa:
        insertOutColumn(kod__cdz)
    if indicator_val:
...
def overload_dataframe_merge_asof(left, right, on=None, left_on=None,
    ...
    if isinstance(suffixes, tuple):
        evfo__vyiop = suffixes
    if is_overload_constant_list(suffixes):
        evfo__vyiop = list(get_overload_const_list(suffixes))
    if isinstance(suffixes, types.Omitted):
        evfo__vyiop = suffixes.value
    suffix_x = evfo__vyiop[0]
    suffix_y = evfo__vyiop[1]
    jzy__mwbor = (
    ...
    jzy__mwbor += "    right_by=None, suffixes=('_x', '_y'), tolerance=None,\n"
    jzy__mwbor += "    allow_exact_matches=True, direction='backward'):\n"
    jzy__mwbor += '  suffix_x = suffixes[0]\n'
    jzy__mwbor += '  suffix_y = suffixes[1]\n'
    jzy__mwbor += (
...
def validate_groupby_spec(df, by, axis, level, as_index, sort, group_keys,
    ...
    stwl__masb = dict(sort=sort, group_keys=group_keys, squeeze=squeeze,
        observed=observed)
    wnqo__iinmv = dict(sort=False, group_keys=True, squeeze=False, observed
        =True)
    check_unsupported_args('Dataframe.groupby', stwl__masb, wnqo__iinmv,
        package_name='pandas', module_name='GroupBy')


def pivot_error_checking(df, index, columns, values, func_name):
    gssuv__xifso = func_name == 'DataFrame.pivot_table'
    if gssuv__xifso:
    ...
    xfoof__ehr = get_literal_value(columns)
    if isinstance(xfoof__ehr, (list, tuple)):
        if len(xfoof__ehr) > 1:
            raise BodoError(
                f"{func_name}(): 'columns' argument must be a constant column label not a {xfoof__ehr}"
                )
        xfoof__ehr = xfoof__ehr[0]
    if xfoof__ehr not in df.columns:
        raise BodoError(
            f"{func_name}(): 'columns' column {xfoof__ehr} not found in DataFrame {df}."
            )
    bilg__clw = df.column_index[xfoof__ehr]
    if is_overload_none(index):
        elbt__tnqe = []
        dxpm__fir = []
    else:
        dxpm__fir = get_literal_value(index)
        if not isinstance(dxpm__fir, (list, tuple)):
            dxpm__fir = [dxpm__fir]
        elbt__tnqe = []
        for index in dxpm__fir:
            if index not in df.column_index:
                raise BodoError(
                    f"{func_name}(): 'index' column {index} not found in DataFrame {df}."
                    )
            elbt__tnqe.append(df.column_index[index])
    if not (all(isinstance(miuqz__rtl, int) for miuqz__rtl in dxpm__fir) or
        all(isinstance(miuqz__rtl, str) for miuqz__rtl in dxpm__fir)):
        raise BodoError(
    ...
    if is_overload_none(values):
        lyut__iarxg = []
        atqgo__wsgu = []
        ape__yvzdz = elbt__tnqe + [bilg__clw]
        for i, miuqz__rtl in enumerate(df.columns):
            if i not in ape__yvzdz:
                lyut__iarxg.append(i)
                atqgo__wsgu.append(miuqz__rtl)
    else:
        atqgo__wsgu = get_literal_value(values)
        if not isinstance(atqgo__wsgu, (list, tuple)):
            atqgo__wsgu = [atqgo__wsgu]
        lyut__iarxg = []
        for val in atqgo__wsgu:
            if val not in df.column_index:
                raise BodoError(
                    f"{func_name}(): 'values' column {val} not found in DataFrame {df}."
                    )
            lyut__iarxg.append(df.column_index[val])
    wwxdh__wfcm = set(lyut__iarxg) | set(elbt__tnqe) | {bilg__clw}
    if len(wwxdh__wfcm) != len(lyut__iarxg) + len(elbt__tnqe) + 1:
        raise BodoError(
    ...
    if len(elbt__tnqe) == 0:
        ...
        for rabg__ibj in elbt__tnqe:
            index_column = df.data[rabg__ibj]
            check_valid_index_typ(index_column)
    pvza__zoliz = df.data[bilg__clw]
    if isinstance(pvza__zoliz, (bodo.ArrayItemArrayType, bodo.MapArrayType,
    ...
    if isinstance(pvza__zoliz, bodo.CategoricalArrayType):
        raise BodoError(
            f"{func_name}(): 'columns' DataFrame column does not support categorical data"
            )
    for rilg__qbvpc in lyut__iarxg:
        jlbf__aazm = df.data[rilg__qbvpc]
        if isinstance(jlbf__aazm, (bodo.ArrayItemArrayType, bodo.
            MapArrayType, bodo.StructArrayType, bodo.TupleArrayType)
            ) or jlbf__aazm == bodo.binary_array_type:
            raise BodoError(
                f"{func_name}(): 'values' DataFrame column must have scalar rows"
                )
    return (dxpm__fir, xfoof__ehr, atqgo__wsgu, elbt__tnqe, bilg__clw,
        lyut__iarxg)
...
def overload_dataframe_pivot(data, index=None, columns=None, values=None):
    ...
    (dxpm__fir, xfoof__ehr, atqgo__wsgu, rabg__ibj, bilg__clw, htos__pwzkv) = (
        pivot_error_checking(data, index, columns, values, 'DataFrame.pivot'))
    if len(dxpm__fir) == 0:
        if is_overload_none(data.index.name_typ):
            eend__zhtrn = None,
        else:
            eend__zhtrn = get_literal_value(data.index.name_typ),
    else:
        eend__zhtrn = tuple(dxpm__fir)
    dxpm__fir = ColNamesMetaType(eend__zhtrn)
    atqgo__wsgu = ColNamesMetaType(tuple(atqgo__wsgu))
    xfoof__ehr = ColNamesMetaType((xfoof__ehr,))
    jzy__mwbor = 'def impl(data, index=None, columns=None, values=None):\n'
    jzy__mwbor += "    ev = tracing.Event('df.pivot')\n"
    jzy__mwbor += f'    pivot_values = data.iloc[:, {bilg__clw}].unique()\n'
    jzy__mwbor += '    result = bodo.hiframes.pd_dataframe_ext.pivot_impl(\n'
    if len(rabg__ibj) == 0:
        jzy__mwbor += f"""        (bodo.utils.conversion.index_to_array(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(data)),),
"""
    else:
        jzy__mwbor += '        (\n'
        for dun__rxizb in rabg__ibj:
            jzy__mwbor += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {dun__rxizb}),
"""
        jzy__mwbor += '        ),\n'
    jzy__mwbor += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {bilg__clw}),),
"""
    jzy__mwbor += '        (\n'
    for rilg__qbvpc in htos__pwzkv:
        jzy__mwbor += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {rilg__qbvpc}),
"""
    jzy__mwbor += '        ),\n'
    jzy__mwbor += '        pivot_values,\n'
    jzy__mwbor += '        index_lit,\n'
    jzy__mwbor += '        columns_lit,\n'
    jzy__mwbor += '        values_lit,\n'
    jzy__mwbor += '    )\n'
    jzy__mwbor += '    ev.finalize()\n'
    jzy__mwbor += '    return result\n'
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'index_lit': dxpm__fir, 'columns_lit':
        xfoof__ehr, 'values_lit': atqgo__wsgu, 'tracing': tracing}, cnpf__vsgvm
        )
    impl = cnpf__vsgvm['impl']
    return impl
...
def overload_dataframe_pivot_table(data, values=None, index=None, columns=
    ...
    stwl__masb = dict(fill_value=fill_value, margins=margins, dropna=dropna,
        margins_name=margins_name, observed=observed, sort=sort)
    ibz__jzc = dict(fill_value=None, margins=False, dropna=True,
        margins_name='All', observed=False, sort=True)
    check_unsupported_args('DataFrame.pivot_table', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    if not isinstance(data, DataFrameType):
        raise BodoError(
    ...
    (dxpm__fir, xfoof__ehr, atqgo__wsgu, rabg__ibj, bilg__clw, htos__pwzkv) = (
        ...
        'DataFrame.pivot_table'))
    bghmx__ivwb = dxpm__fir
    dxpm__fir = ColNamesMetaType(tuple(dxpm__fir))
    atqgo__wsgu = ColNamesMetaType(tuple(atqgo__wsgu))
    urxm__vli = xfoof__ehr
    xfoof__ehr = ColNamesMetaType((xfoof__ehr,))
    jzy__mwbor = 'def impl(\n'
    jzy__mwbor += '    data,\n'
    jzy__mwbor += '    values=None,\n'
    jzy__mwbor += '    index=None,\n'
    jzy__mwbor += '    columns=None,\n'
    jzy__mwbor += '    aggfunc="mean",\n'
    jzy__mwbor += '    fill_value=None,\n'
    jzy__mwbor += '    margins=False,\n'
    jzy__mwbor += '    dropna=True,\n'
    jzy__mwbor += '    margins_name="All",\n'
    jzy__mwbor += '    observed=False,\n'
    jzy__mwbor += '    sort=True,\n'
    jzy__mwbor += '    _pivot_values=None,\n'
    jzy__mwbor += '):\n'
    jzy__mwbor += "    ev = tracing.Event('df.pivot_table')\n"
    hde__hyu = rabg__ibj + [bilg__clw] + htos__pwzkv
    jzy__mwbor += f'    data = data.iloc[:, {hde__hyu}]\n'
    habaq__lwxrb = bghmx__ivwb + [urxm__vli]
    if not is_overload_none(_pivot_values):
        xztnv__tgpx = tuple(sorted(_pivot_values.meta))
        _pivot_values = ColNamesMetaType(xztnv__tgpx)
        jzy__mwbor += '    pivot_values = _pivot_values_arr\n'
        jzy__mwbor += (
            f'    data = data[data.iloc[:, {len(rabg__ibj)}].isin(pivot_values)]\n'
            )
        if all(isinstance(miuqz__rtl, str) for miuqz__rtl in xztnv__tgpx):
            zkxef__ukpp = pd.array(xztnv__tgpx, 'string')
        elif all(isinstance(miuqz__rtl, int) for miuqz__rtl in xztnv__tgpx):
            zkxef__ukpp = np.array(xztnv__tgpx, 'int64')
        else:
            raise BodoError(
        ...
        zkxef__ukpp = None
    dykyf__tpps = is_overload_constant_str(aggfunc) and get_overload_const_str(
        aggfunc) == 'nunique'
    wyqw__ugd = len(habaq__lwxrb) if dykyf__tpps else len(bghmx__ivwb)
    jzy__mwbor += f"""    data = data.groupby({habaq__lwxrb!r}, as_index=False, _bodo_num_shuffle_keys={wyqw__ugd}).agg(aggfunc)
"""
    if is_overload_none(_pivot_values):
        jzy__mwbor += (
            f'    pivot_values = data.iloc[:, {len(rabg__ibj)}].unique()\n')
    jzy__mwbor += '    result = bodo.hiframes.pd_dataframe_ext.pivot_impl(\n'
    jzy__mwbor += '        (\n'
    for i in range(0, len(rabg__ibj)):
        jzy__mwbor += (
            f'            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {i}),\n'
            )
    jzy__mwbor += '        ),\n'
    jzy__mwbor += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {len(rabg__ibj)}),),
"""
    jzy__mwbor += '        (\n'
    for i in range(len(rabg__ibj) + 1, len(htos__pwzkv) + len(rabg__ibj) + 1):
        jzy__mwbor += (
            f'            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {i}),\n'
            )
    jzy__mwbor += '        ),\n'
    ...
    jzy__mwbor += '        columns_lit,\n'
    jzy__mwbor += '        values_lit,\n'
    jzy__mwbor += '        check_duplicates=False,\n'
    jzy__mwbor += f'        is_already_shuffled={not dykyf__tpps},\n'
    jzy__mwbor += '        _constant_pivot_values=_constant_pivot_values,\n'
    jzy__mwbor += '    )\n'
    jzy__mwbor += '    ev.finalize()\n'
    jzy__mwbor += '    return result\n'
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'numba': numba, 'index_lit': dxpm__fir,
        'columns_lit': xfoof__ehr, 'values_lit': atqgo__wsgu,
        '_pivot_values_arr': zkxef__ukpp, '_constant_pivot_values':
        _pivot_values, 'tracing': tracing}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def overload_dataframe_melt(frame, id_vars=None, value_vars=None, var_name=
    ...
    stwl__masb = dict(col_level=col_level, ignore_index=ignore_index)
    ibz__jzc = dict(col_level=None, ignore_index=True)
    check_unsupported_args('DataFrame.melt', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    ...
    sjxuk__onwkr = get_literal_value(id_vars) if not is_overload_none(id_vars
        ) else []
    if not isinstance(sjxuk__onwkr, (list, tuple)):
        sjxuk__onwkr = [sjxuk__onwkr]
    for miuqz__rtl in sjxuk__onwkr:
        if miuqz__rtl not in frame.columns:
            raise BodoError(
                f"DataFrame.melt(): 'id_vars' column {miuqz__rtl} not found in {frame}."
                )
    cqt__ogt = [frame.column_index[i] for i in sjxuk__onwkr]
    if is_overload_none(value_vars):
        orsa__wpl = []
        yjwi__dzwx = []
        for i, miuqz__rtl in enumerate(frame.columns):
            if i not in cqt__ogt:
                orsa__wpl.append(i)
                yjwi__dzwx.append(miuqz__rtl)
    else:
        yjwi__dzwx = get_literal_value(value_vars)
        if not isinstance(yjwi__dzwx, (list, tuple)):
            yjwi__dzwx = [yjwi__dzwx]
        yjwi__dzwx = [v for v in yjwi__dzwx if v not in sjxuk__onwkr]
        if not yjwi__dzwx:
            raise BodoError(
                "DataFrame.melt(): currently empty 'value_vars' is unsupported."
                )
        orsa__wpl = []
        for val in yjwi__dzwx:
            if val not in frame.column_index:
                raise BodoError(
                    f"DataFrame.melt(): 'value_vars' column {val} not found in DataFrame {frame}."
                    )
            orsa__wpl.append(frame.column_index[val])
    for miuqz__rtl in yjwi__dzwx:
        if miuqz__rtl not in frame.columns:
            raise BodoError(
                f"DataFrame.melt(): 'value_vars' column {miuqz__rtl} not found in {frame}."
                )
    if not (all(isinstance(miuqz__rtl, int) for miuqz__rtl in yjwi__dzwx) or
        all(isinstance(miuqz__rtl, str) for miuqz__rtl in yjwi__dzwx)):
        raise BodoError(
    ...
    iya__cddz = frame.data[orsa__wpl[0]]
    sua__gctx = [frame.data[i].dtype for i in orsa__wpl]
    orsa__wpl = np.array(orsa__wpl, dtype=np.int64)
    cqt__ogt = np.array(cqt__ogt, dtype=np.int64)
    _, fah__lcl = bodo.utils.typing.get_common_scalar_dtype(sua__gctx)
    if not fah__lcl:
        raise BodoError(
            "DataFrame.melt(): columns selected in 'value_vars' must have a unifiable type."
            )
    extra_globals = {'np': np, 'value_lit': yjwi__dzwx, 'val_type': iya__cddz}
    header = 'def impl(\n'
    header += '  frame,\n'
    header += '  id_vars=None,\n'
    ...
    if frame.is_table_format and all(v == iya__cddz.dtype for v in sua__gctx):
    ...
    elif len(yjwi__dzwx) == 1:
        header += f"""  val_col = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(frame, {orsa__wpl[0]})
"""
    else:
        byj__vuuw = ', '.join(
            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(frame, {i})'
             for i in orsa__wpl)
        header += (
            f'  val_col = bodo.libs.array_kernels.concat(({byj__vuuw},))\n')
...
"""
    for i in cqt__ogt:
        ...
        header += (
            f'  out_id{i} = bodo.libs.array_kernels.concat([id{i}] * {len(yjwi__dzwx)})\n'
            )
    thyak__tbbn = ', '.join(f'out_id{i}' for i in cqt__ogt) + (', ' if len(
        cqt__ogt) > 0 else '')
    data_args = thyak__tbbn + 'var_col, val_col'
    columns = tuple(sjxuk__onwkr + [var_name, value_name])
    index = (
        f'bodo.hiframes.pd_index_ext.init_range_index(0, len(frame) * {len(yjwi__dzwx)}, 1, None)'
        )
...
def crosstab_overload(index, columns, values=None, rownames=None, colnames=
    ...
    stwl__masb = dict(values=values, rownames=rownames, colnames=colnames,
        aggfunc=aggfunc, margins=margins, margins_name=margins_name, dropna
        =dropna, normalize=normalize)
    ibz__jzc = dict(values=None, rownames=None, colnames=None, aggfunc=None,
        margins=False, margins_name='All', dropna=True, normalize=False)
    check_unsupported_args('pandas.crosstab', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_sort_values(df, by, axis=0, ascending=True, inplace=
    ...
    stwl__masb = dict(ignore_index=ignore_index, key=key)
    ibz__jzc = dict(ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_values', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
...
def validate_sort_values_spec(df, by, axis, ascending, inplace, kind,
    ...
    if is_overload_none(by) or not is_literal_type(by
        ...
        raise_bodo_error(
            ...
            .format(by))
    govqg__lou = set(df.columns)
    if is_overload_constant_str(df.index.name_typ):
        govqg__lou.add(get_overload_const_str(df.index.name_typ))
    if is_overload_constant_tuple(by):
        awx__sef = [get_overload_const_tuple(by)]
    else:
        awx__sef = get_overload_const_list(by)
    awx__sef = set((k, '') if (k, '') in govqg__lou else k for k in awx__sef)
    if len(awx__sef.difference(govqg__lou)) > 0:
        zby__lhdvp = list(set(get_overload_const_list(by)).difference(
            govqg__lou))
        raise_bodo_error(f'sort_values(): invalid keys {zby__lhdvp} for by.')
    if not is_overload_none(_bodo_chunk_bounds) and len(awx__sef) != 1:
        raise_bodo_error(
    ...
    if is_overload_constant_str(na_position):
        ...
        bie__dkfd = get_overload_const_list(na_position)
        for na_position in bie__dkfd:
            if na_position not in ('first', 'last'):
                raise BodoError(
...
def overload_dataframe_sort_index(df, axis=0, level=None, ascending=True,
    ...
    stwl__masb = dict(axis=axis, level=level, kind=kind, sort_remaining=
        sort_remaining, ignore_index=ignore_index, key=key)
    ibz__jzc = dict(axis=0, level=None, kind='quicksort', sort_remaining=
        True, ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_index', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_rank(df, axis=0, method='average', numeric_only=None,
    ...
    jzy__mwbor = """def impl(df, axis=0, method='average', numeric_only=None, na_option='keep', ascending=True, pct=False):
"""
    zhw__hwsos = len(df.columns)
    data_args = ', '.join(
...
def overload_dataframe_fillna(df, value=None, method=None, axis=None,
    ...
    stwl__masb = dict(limit=limit, downcast=downcast)
    ibz__jzc = dict(limit=None, downcast=None)
    check_unsupported_args('DataFrame.fillna', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.fillna()')
    if not (is_overload_none(axis) or is_overload_zero(axis)):
        raise BodoError("DataFrame.fillna(): 'axis' argument not supported.")
    fdc__lnif = not is_overload_none(value)
    hvz__vgs = not is_overload_none(method)
    if fdc__lnif and hvz__vgs:
        raise BodoError(
            "DataFrame.fillna(): Cannot specify both 'value' and 'method'.")
    if not fdc__lnif and not hvz__vgs:
        raise BodoError(
            "DataFrame.fillna(): Must specify one of 'value' and 'method'.")
    if fdc__lnif:
        btehz__iwx = 'value=value'
    else:
        btehz__iwx = 'method=method'
    data_args = [(
        f"df['{miuqz__rtl}'].fillna({btehz__iwx}, inplace=inplace)" if
        isinstance(miuqz__rtl, str) else
        f'df[{miuqz__rtl}].fillna({btehz__iwx}, inplace=inplace)') for
        miuqz__rtl in df.columns]
    jzy__mwbor = """def impl(df, value=None, method=None, axis=None, inplace=False, limit=None, downcast=None):
"""
    if is_overload_true(inplace):
        jzy__mwbor += '  ' + '  \n'.join(data_args) + '\n'
        cnpf__vsgvm = {}
        exec(jzy__mwbor, {}, cnpf__vsgvm)
        impl = cnpf__vsgvm['impl']
        return impl
    else:
        return _gen_init_df(jzy__mwbor, df.columns, ', '.join(qunr__lphw +
            '.values' for qunr__lphw in data_args))
...
def overload_dataframe_reset_index(df, level=None, drop=False, inplace=
    ...
    stwl__masb = dict(col_level=col_level, col_fill=col_fill)
    ibz__jzc = dict(col_level=0, col_fill='')
    check_unsupported_args('DataFrame.reset_index', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    ...
    jzy__mwbor = """def impl(df, level=None, drop=False, inplace=False, col_level=0, col_fill='', _bodo_transformed=False,):
"""
    jzy__mwbor += (
    ...
    if not drop:
        okj__frlnc = 'index' if 'index' not in columns else 'level_0'
        ...
        if isinstance(df.index, MultiIndexType):
            jzy__mwbor += """  m_index = bodo.hiframes.pd_index_ext.get_index_data(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df))
"""
            pri__mbg = ['m_index[{}]'.format(i) for i in range(df.index.
                nlevels)]
            data_args = pri__mbg + data_args
        else:
            xot__migbf = (
                'bodo.utils.conversion.index_to_array(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df))'
                )
            data_args = [xot__migbf] + data_args
    return _gen_init_df(jzy__mwbor, columns, ', '.join(data_args), 'index')


def _is_all_levels(df, level):
    auz__hajj = len(get_index_data_arr_types(df.index))
    return is_overload_none(level) or is_overload_constant_int(level
        ) and get_overload_const_int(level
        ) == 0 and auz__hajj == 1 or is_overload_constant_list(level) and list(
        get_overload_const_list(level)) == list(range(auz__hajj))
...
def overload_dataframe_dropna(df, axis=0, how='any', thresh=None, subset=
    ...
    if is_overload_none(subset):
        noa__tis = list(range(len(df.columns)))
    elif not is_overload_constant_list(subset):
        raise_bodo_error(
            f'df.dropna(): subset argument should a constant list, not {subset}'
            )
    else:
        zesqp__wqq = get_overload_const_list(subset)
        noa__tis = []
        for dag__syje in zesqp__wqq:
            if dag__syje not in df.column_index:
                raise_bodo_error(
                    f"df.dropna(): column '{dag__syje}' not in data frame columns {df}"
                    )
            noa__tis.append(df.column_index[dag__syje])
    zhw__hwsos = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(zhw__hwsos))
    jzy__mwbor = (
...
"""
        .format(data_args, index, ', '.join(str(a) for a in noa__tis)))
    jzy__mwbor += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(jzy__mwbor, df.columns, data_args, 'index')
...
def overload_dataframe_drop(df, labels=None, axis=0, index=None, columns=
    ...
    stwl__masb = dict(index=index, level=level, errors=errors)
    ibz__jzc = dict(index=None, level=None, errors='raise')
    check_unsupported_args('DataFrame.drop', stwl__masb, ibz__jzc,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(labels):
        ...
        if is_overload_constant_str(labels):
            cjrk__veue = get_overload_const_str(labels),
        elif is_overload_constant_list(labels):
            cjrk__veue = get_overload_const_list(labels)
        else:
            raise_bodo_error(
        ...
        if is_overload_constant_str(columns):
            cjrk__veue = get_overload_const_str(columns),
        elif is_overload_constant_list(columns):
            cjrk__veue = get_overload_const_list(columns)
        else:
            raise_bodo_error(
                'constant list of columns expected for labels in DataFrame.drop()'
                )
    for miuqz__rtl in cjrk__veue:
        if miuqz__rtl not in df.columns:
            raise_bodo_error(
                'DataFrame.drop(): column {} not in DataFrame columns {}'.
                format(miuqz__rtl, df.columns))
    if len(set(cjrk__veue)) == len(df.columns):
        raise BodoError('DataFrame.drop(): Dropping all columns not supported.'
            )
    inplace = is_overload_true(inplace)
    mbsr__hrbom = tuple(miuqz__rtl for miuqz__rtl in df.columns if 
        miuqz__rtl not in cjrk__veue)
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.column_index[miuqz__rtl], '.copy()' if not inplace else
        '') for miuqz__rtl in mbsr__hrbom)
    jzy__mwbor = (
    ...
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    return _gen_init_df(jzy__mwbor, mbsr__hrbom, data_args, index)
...
def overload_dataframe_sample(df, n=None, frac=None, replace=False, weights
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.sample()')
    stwl__masb = dict(random_state=random_state, weights=weights, axis=axis,
        ignore_index=ignore_index)
    skoqu__tfsxs = dict(random_state=None, weights=None, axis=None,
        ignore_index=False)
    check_unsupported_args('DataFrame.sample', stwl__masb, skoqu__tfsxs,
        package_name='pandas', module_name='DataFrame')
    ...
    zhw__hwsos = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(zhw__hwsos))
    hgkce__ksxc = ', '.join('rhs_data_{}'.format(i) for i in range(zhw__hwsos))
    jzy__mwbor = """def impl(df, n=None, frac=None, replace=False, weights=None, random_state=None, axis=None, ignore_index=False):
"""
    jzy__mwbor += '  if (frac == 1 or n == len(df)) and not replace:\n'
    jzy__mwbor += (
...
"""
            .format(i))
    jzy__mwbor += '  if frac is None:\n'
    jzy__mwbor += '    frac_d = -1.0\n'
    jzy__mwbor += '  else:\n'
    jzy__mwbor += '    frac_d = frac\n'
    jzy__mwbor += '  if n is None:\n'
    jzy__mwbor += '    n_i = 0\n'
    jzy__mwbor += '  else:\n'
    jzy__mwbor += '    n_i = n\n'
    index = (
    ...
    jzy__mwbor += f"""  ({data_args},), index_arr = bodo.libs.array_kernels.sample_table_operation(({hgkce__ksxc},), {index}, n_i, frac_d, replace)
"""
    jzy__mwbor += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return bodo.hiframes.dataframe_impl._gen_init_df(jzy__mwbor, df.columns,
        data_args, 'index')
...
def overload_dataframe_info(df, verbose=None, buf=None, max_cols=None,
    ...
    iivwh__fylkp = {'verbose': verbose, 'buf': buf, 'max_cols': max_cols,
        'memory_usage': memory_usage, 'show_counts': show_counts,
        'null_counts': null_counts}
    nfd__qmznb = {'verbose': None, 'buf': None, 'max_cols': None,
        'memory_usage': None, 'show_counts': None, 'null_counts': None}
    check_unsupported_args('DataFrame.info', iivwh__fylkp, nfd__qmznb,
        package_name='pandas', module_name='DataFrame')
    zoj__mnku = f"<class '{str(type(df)).split('.')[-1]}"
    if len(df.columns) == 0:

        def _info_impl(df, verbose=None, buf=None, max_cols=None,
            memory_usage=None, show_counts=None, null_counts=None):
            vzjk__ngxa = zoj__mnku + '\n'
            vzjk__ngxa += 'Index: 0 entries\n'
            vzjk__ngxa += 'Empty DataFrame'
            print(vzjk__ngxa)
        return _info_impl
    else:
        jzy__mwbor = """def _info_impl(df, verbose=None, buf=None, max_cols=None, memory_usage=None, show_counts=None, null_counts=None): #pragma: no cover
"""
        jzy__mwbor += '    ncols = df.shape[1]\n'
        jzy__mwbor += f'    lines = "{zoj__mnku}\\n"\n'
        jzy__mwbor += f'    lines += "{df.index}: "\n'
        jzy__mwbor += (
            '    index = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)\n'
            )
        if isinstance(df.index, bodo.hiframes.pd_index_ext.RangeIndexType):
            jzy__mwbor += """    lines += f"{len(index)} entries, {index.start} to {index.stop-1}\\n\"
"""
        elif isinstance(df.index, bodo.hiframes.pd_index_ext.StringIndexType):
            jzy__mwbor += """    lines += f"{len(index)} entries, {index[0]} to {index[len(index)-1]}\\n\"
"""
        else:
            jzy__mwbor += (
        ...
        jzy__mwbor += '    column_width = max(space, 7)\n'
        jzy__mwbor += '    column= "Column"\n'
        jzy__mwbor += '    underl= "------"\n'
        jzy__mwbor += (
        ...
        jzy__mwbor += '    mem_size = 0\n'
        jzy__mwbor += (
            '    col_name = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)\n'
            )
        jzy__mwbor += """    non_null_count = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)
"""
        jzy__mwbor += (
        ...
        spta__vrq = dict()
        for i in range(len(df.columns)):
            jzy__mwbor += f"""    non_null_count[{i}] = str(bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})))
"""
            sswo__uwh = f'{df.data[i].dtype}'
            if isinstance(df.data[i], bodo.CategoricalArrayType):
                sswo__uwh = 'category'
            elif isinstance(df.data[i], bodo.IntegerArrayType):
                uqbf__qdapn = bodo.libs.int_arr_ext.IntDtype(df.data[i].dtype
                    ).name
                sswo__uwh = f'{uqbf__qdapn[:-7]}'
            jzy__mwbor += f'    col_dtype[{i}] = "{sswo__uwh}"\n'
            if sswo__uwh in spta__vrq:
                spta__vrq[sswo__uwh] += 1
            else:
                spta__vrq[sswo__uwh] = 1
            jzy__mwbor += f'    col_name[{i}] = "{df.columns[i]}"\n'
            jzy__mwbor += f"""    mem_size += bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}).nbytes
"""
        jzy__mwbor += """    column_info = [f'{i:^3} {name:<{column_width}} {count} non-null      {dtype}' for i, (name, count, dtype) in enumerate(zip(col_name, non_null_count, col_dtype))]
"""
        jzy__mwbor += '    for i in column_info:\n'
        jzy__mwbor += "        lines += f'{i}\\n'\n"
        glcj__aike = ', '.join(f'{k}({spta__vrq[k]})' for k in sorted(
            spta__vrq))
        jzy__mwbor += f"    lines += 'dtypes: {glcj__aike}\\n'\n"
        jzy__mwbor += '    mem_size += df.index.nbytes\n'
        jzy__mwbor += '    total_size = _sizeof_fmt(mem_size)\n'
        jzy__mwbor += "    lines += f'memory usage: {total_size}'\n"
        jzy__mwbor += '    print(lines)\n'
        cnpf__vsgvm = {}
        exec(jzy__mwbor, {'_sizeof_fmt': _sizeof_fmt, 'pd': pd, 'bodo':
            bodo, 'np': np}, cnpf__vsgvm)
        _info_impl = cnpf__vsgvm['_info_impl']
        return _info_impl
...
def overload_dataframe_memory_usage(df, index=True, deep=False):
    ...
    jzy__mwbor = 'def impl(df, index=True, deep=False):\n'
    ppd__opm = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df).nbytes'
    sqycn__fwmhy = is_overload_true(index)
    columns = df.columns
    if sqycn__fwmhy:
        columns = ('Index',) + columns
    if len(columns) == 0:
        gme__waz = ()
    elif all(isinstance(miuqz__rtl, int) for miuqz__rtl in columns):
        gme__waz = np.array(columns, 'int64')
    elif all(isinstance(miuqz__rtl, str) for miuqz__rtl in columns):
        gme__waz = pd.array(columns, 'string')
    else:
        gme__waz = columns
    if df.is_table_format and len(df.columns) > 0:
        sgz__ozokp = int(sqycn__fwmhy)
        iyayo__wdlc = len(columns)
        jzy__mwbor += f'  nbytes_arr = np.empty({iyayo__wdlc}, np.int64)\n'
        jzy__mwbor += (
            '  table = bodo.hiframes.pd_dataframe_ext.get_dataframe_table(df)\n'
            )
        jzy__mwbor += f"""  bodo.utils.table_utils.generate_table_nbytes(table, nbytes_arr, {sgz__ozokp})
"""
        if sqycn__fwmhy:
            jzy__mwbor += f'  nbytes_arr[0] = {ppd__opm}\n'
        jzy__mwbor += f"""  return bodo.hiframes.pd_series_ext.init_series(nbytes_arr, pd.Index(column_vals), None)
"""
    else:
        data = ', '.join(
            f'bodo.libs.array_ops.array_op_nbytes(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}))'
             for i in range(len(df.columns)))
        if sqycn__fwmhy:
            data = f'{ppd__opm},{data}'
        else:
            trui__bosft = ',' if len(columns) == 1 else ''
            data = f'{data}{trui__bosft}'
        jzy__mwbor += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}), pd.Index(column_vals), None)
"""
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'np': np, 'pd': pd, 'column_vals':
        gme__waz}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def overload_read_excel(io, sheet_name=0, header=0, names=None, index_col=
    ...
    aed__gyecm = 'read_excel_df{}'.format(next_label())
    setattr(types, aed__gyecm, df_type)
    pkpza__fdl = False
    if is_overload_constant_list(parse_dates):
        pkpza__fdl = get_overload_const_list(parse_dates)
    uwrf__vuoe = ', '.join(["'{}':{}".format(cname, _get_pd_dtype_str(t)) for
...
def impl(
    ...
    with numba.objmode(df="{aed__gyecm}"):
        df = pd.read_excel(
            io=io,
...
"""
    cnpf__vsgvm = {}
    exec(jzy__mwbor, globals(), cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def overload_dataframe_plot(df, x=None, y=None, kind='line', figsize=None,
    ...
    except ImportError as lfht__iqcy:
    ...
    jzy__mwbor += '    ylabel=None, title=None, legend=True, fontsize=None, \n'
    jzy__mwbor += '    xticks=None, yticks=None, ax=None):\n'
    if is_overload_none(ax):
        jzy__mwbor += '   fig, ax = plt.subplots()\n'
    else:
        jzy__mwbor += '   fig = ax.get_figure()\n'
    if not is_overload_none(figsize):
        jzy__mwbor += '   fig.set_figwidth(figsize[0])\n'
        jzy__mwbor += '   fig.set_figheight(figsize[1])\n'
    if is_overload_none(xlabel):
        jzy__mwbor += '   xlabel = x\n'
    jzy__mwbor += '   ax.set_xlabel(xlabel)\n'
    if is_overload_none(ylabel):
        jzy__mwbor += '   ylabel = y\n'
    else:
        jzy__mwbor += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(title):
        jzy__mwbor += '   ax.set_title(title)\n'
    if not is_overload_none(fontsize):
        jzy__mwbor += '   ax.tick_params(labelsize=fontsize)\n'
    ...
    if kind == 'line':
        if is_overload_none(x) and is_overload_none(y):
            ...
            jzy__mwbor += '   ax.plot(df[y], label=y)\n'
        elif is_overload_none(y):
            vcp__cmv = get_overload_const_str(x)
            itqhw__gbn = df.columns.index(vcp__cmv)
            for i in range(len(df.columns)):
                if isinstance(df.data[i], (types.Array, IntegerArrayType)
                    ) and isinstance(df.data[i].dtype, (types.Integer,
                    types.Float)):
            ...
            jzy__mwbor += '   ax.plot(df[x], df[y], label=y)\n'
    elif kind == 'scatter':
        legend = False
        jzy__mwbor += '   ax.scatter(df[x], df[y], s=20)\n'
        jzy__mwbor += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(xticks):
        jzy__mwbor += '   ax.set_xticks(xticks)\n'
    if not is_overload_none(yticks):
        jzy__mwbor += '   ax.set_yticks(yticks)\n'
    if is_overload_true(legend):
        jzy__mwbor += '   ax.legend()\n'
    jzy__mwbor += '   return ax\n'
    cnpf__vsgvm = {}
    exec(jzy__mwbor, {'bodo': bodo, 'plt': plt}, cnpf__vsgvm)
    impl = cnpf__vsgvm['impl']
    return impl
...
def is_df_values_numpy_supported_dftyp(df_typ):
    for kvy__uqpzz in df_typ.data:
        if not (isinstance(kvy__uqpzz, (IntegerArrayType, FloatingArrayType
            )) or isinstance(kvy__uqpzz.dtype, types.Number) or kvy__uqpzz.
...
class SetDfColInfer(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        vupqi__wrgy = args[0]
        csw__rbdh = args[1].literal_value
        val = args[2]
        assert val != types.unknown
        mllz__rwy = vupqi__wrgy
        check_runtime_cols_unsupported(vupqi__wrgy, 'set_df_col()')
        if isinstance(vupqi__wrgy, DataFrameType):
            index = vupqi__wrgy.index
            if len(vupqi__wrgy.columns) == 0:
            ...
            if csw__rbdh in vupqi__wrgy.columns:
                mbsr__hrbom = vupqi__wrgy.columns
                mta__frduf = vupqi__wrgy.columns.index(csw__rbdh)
                yiw__rab = list(vupqi__wrgy.data)
                yiw__rab[mta__frduf] = val
                yiw__rab = tuple(yiw__rab)
            else:
                mbsr__hrbom = vupqi__wrgy.columns + (csw__rbdh,)
                yiw__rab = vupqi__wrgy.data + (val,)
            mllz__rwy = DataFrameType(yiw__rab, index, mbsr__hrbom,
                vupqi__wrgy.dist, vupqi__wrgy.is_table_format)
        return mllz__rwy(*args)
...
class BodoSQLReplaceColsInfer(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        vrwjy__rumzx = args[0]
        assert isinstance(vrwjy__rumzx, DataFrameType) and len(vrwjy__rumzx
            .columns
        ...
        oeoj__zho = args[2]
        assert len(col_names_to_replace) == len(oeoj__zho
        ...
        assert len(col_names_to_replace) <= len(vrwjy__rumzx.columns
        ...
        for col_name in col_names_to_replace:
            assert col_name in vrwjy__rumzx.columns, 'Error while typechecking __bodosql_replace_columns_dummy: All columns specified to be replaced should already be present in input dataframe'
        check_runtime_cols_unsupported(vrwjy__rumzx,
            '__bodosql_replace_columns_dummy()')
        index = vrwjy__rumzx.index
        mbsr__hrbom = vrwjy__rumzx.columns
        yiw__rab = list(vrwjy__rumzx.data)
        for i in range(len(col_names_to_replace)):
            col_name = col_names_to_replace[i]
            ixxrs__gwbay = oeoj__zho[i]
            assert isinstance(ixxrs__gwbay, SeriesType
            ...
            if isinstance(ixxrs__gwbay, SeriesType):
                ixxrs__gwbay = ixxrs__gwbay.data
            jgzib__buue = vrwjy__rumzx.column_index[col_name]
            yiw__rab[jgzib__buue] = ixxrs__gwbay
        yiw__rab = tuple(yiw__rab)
        mllz__rwy = DataFrameType(yiw__rab, index, mbsr__hrbom,
            vrwjy__rumzx.dist, vrwjy__rumzx.is_table_format)
        return mllz__rwy(*args)
...
def _parse_query_expr(expr, env, columns, cleaned_columns, index_name=None,
    ...
    def _rewrite_membership_op(self, node, left, right):
        fozr__asenp = node.op
        op = self.visit(fozr__asenp)
        return op, fozr__asenp, left, right
    ...
    class NewFuncNode(pd.core.computation.ops.FuncNode):
        ...
        def __init__(self, name):
            if (name not in pd.core.computation.ops.MATHOPS or pd.core.
                ...
                if name not in mbslb__foj:
                    raise BodoError('"{0}" is not a supported function'.
                        format(name))
            self.name = name
            if name in mbslb__foj:
                self.func = name
            else:
    ...
    def visit_Attribute(self, node, **kwargs):
        ihaqd__wbxn = node.attr
        value = node.value
        rtpx__gvjv = pd.core.computation.ops.LOCAL_TAG
        if ihaqd__wbxn in ('str', 'dt'):
            try:
                cou__oaeq = str(self.visit(value))
            except pd.core.computation.ops.UndefinedVariableError as npuf__gnrv:
                col_name = npuf__gnrv.args[0].split("'")[1]
                raise BodoError(
                    'df.query(): column {} is not found in dataframe columns {}'
                    .format(col_name, columns))
        else:
            cou__oaeq = str(self.visit(value))
        mipd__pdxig = cou__oaeq, ihaqd__wbxn
        if mipd__pdxig in join_cleaned_cols:
            ihaqd__wbxn = join_cleaned_cols[mipd__pdxig]
        name = cou__oaeq + '.' + ihaqd__wbxn
        if name.startswith(rtpx__gvjv):
            name = name[len(rtpx__gvjv):]
        if ihaqd__wbxn in ('str', 'dt'):
            tqv__tkti = columns[cleaned_columns.index(cou__oaeq)]
            grq__kdb[tqv__tkti] = cou__oaeq
            self.env.scope[name] = 0
            return self.term_type(rtpx__gvjv + name, self.env)
        mbslb__foj.append(name)
    ...
    def math__str__(self):
        ...
        return pd.io.formats.printing.pprint_thing(
            ...
            (op, ','.join(kdy__mdtg), csw__rbdh))

    def op__str__(self):
        bcoj__lmy = ('({0})'.format(pd.io.formats.printing.pprint_thing(
            rfcwv__dwvpi)) for rfcwv__dwvpi in self.operands)
        if self.op == 'in':
            return pd.io.formats.printing.pprint_thing(
        ...
        return pd.io.formats.printing.pprint_thing(' {0} '.format(self.op).
            join(bcoj__lmy))
    ufngb__bjiwx = (pd.core.computation.expr.BaseExprVisitor.
        _rewrite_membership_op)
    dsn__sjofi = pd.core.computation.expr.BaseExprVisitor._maybe_evaluate_binop
    wxvfg__nvxo = pd.core.computation.expr.BaseExprVisitor.visit_Attribute
    tstr__fcjfh = (pd.core.computation.expr.BaseExprVisitor.
        _maybe_downcast_constants)
    rljjv__fxas = pd.core.computation.ops.Term.__str__
    dfc__oqzha = pd.core.computation.ops.MathCall.__str__
    enxw__gxaea = pd.core.computation.ops.Op.__str__
    lugmw__bevf = pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops
    try:
        ...
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        ggaw__tfydm = pd.core.computation.expr.Expr(expr, env=env)
        afb__kvu = str(ggaw__tfydm)
    except pd.core.computation.ops.UndefinedVariableError as npuf__gnrv:
        if not is_overload_none(index_name) and get_overload_const_str(
            index_name) == npuf__gnrv.args[0].split("'")[1]:
            raise BodoError(
                "df.query(): Refering to named index ('{}') by name is not supported"
                .format(get_overload_const_str(index_name)))
        else:
            raise BodoError(f'df.query(): undefined variable, {npuf__gnrv}')
    finally:
        pd.core.computation.expr.BaseExprVisitor._rewrite_membership_op = (
            ufngb__bjiwx)
        pd.core.computation.expr.BaseExprVisitor._maybe_evaluate_binop = (
            dsn__sjofi)
        pd.core.computation.expr.BaseExprVisitor.visit_Attribute = wxvfg__nvxo
        (pd.core.computation.expr.BaseExprVisitor._maybe_downcast_constants
            ) = tstr__fcjfh
        pd.core.computation.ops.Term.__str__ = rljjv__fxas
        pd.core.computation.ops.MathCall.__str__ = dfc__oqzha
        pd.core.computation.ops.Op.__str__ = enxw__gxaea
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (
            lugmw__bevf)
    hzhci__oxa = pd.core.computation.parsing.clean_column_name
    grq__kdb.update({miuqz__rtl: hzhci__oxa(miuqz__rtl) for miuqz__rtl in
        columns if hzhci__oxa(miuqz__rtl) in ggaw__tfydm.names})
    return ggaw__tfydm, afb__kvu, grq__kdb
...
class DataFrameTupleIterator(types.SimpleIteratorType):
    ...
    def __init__(self, col_names, arr_typs):
        ...
        njq__mut = ['{}={}'.format(col_names[i], arr_typs[i]) for i in
            range(len(col_names))]
        name = 'itertuples({})'.format(','.join(njq__mut))
        vurj__jir = namedtuple('Pandas', col_names)
        mil__mfuia = types.NamedTuple([_get_series_dtype(a) for a in
            arr_typs], vurj__jir)
        super(DataFrameTupleIterator, self).__init__(name, mil__mfuia)
...
class TypeIterTuples(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        blpp__kcz = [if_series_to_array_type(a) for a in args[len(args) // 2:]]
        assert 'Index' not in col_names[0]
        col_names = ['Index'] + col_names
        blpp__kcz = [types.Array(types.int64, 1, 'C')] + blpp__kcz
        nlk__btmx = DataFrameTupleIterator(col_names, blpp__kcz)
        return nlk__btmx(*args)
...
class DataFrameTupleIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        mkpj__megd = [('index', types.EphemeralPointer(types.uintp))] + [(
...
def get_itertuples_impl(context, builder, sig, args):
    pufjs__oclyl = args[len(args) // 2:]
    hkvm__ldrlf = sig.args[len(sig.args) // 2:]
    xrs__cveg = context.make_helper(builder, sig.return_type)
    bmomn__qpa = context.get_constant(types.intp, 0)
    ftzy__pnfp = cgutils.alloca_once_value(builder, bmomn__qpa)
    xrs__cveg.index = ftzy__pnfp
    for i, arr in enumerate(pufjs__oclyl):
        setattr(xrs__cveg, 'array{}'.format(i), arr)
    for arr, arr_typ in zip(pufjs__oclyl, hkvm__ldrlf):
        context.nrt.incref(builder, arr_typ, arr)
    res = xrs__cveg._getvalue()
...
def iternext_itertuples(context, builder, sig, args, result):
    issm__lkwm, = sig.args
    qlj__kkys, = args
    xrs__cveg = context.make_helper(builder, issm__lkwm, value=qlj__kkys)
    uxaah__rgyk = signature(types.intp, issm__lkwm.array_types[1])
    xpf__ivp = context.compile_internal(builder, lambda a: len(a),
        uxaah__rgyk, [xrs__cveg.array0])
    index = builder.load(xrs__cveg.index)
    pmr__xzzud = builder.icmp_signed('<', index, xpf__ivp)
    result.set_valid(pmr__xzzud)
    with builder.if_then(pmr__xzzud):
        values = [index]
        for i, arr_typ in enumerate(issm__lkwm.array_types[1:]):
            qfbqq__gzoxg = getattr(xrs__cveg, 'array{}'.format(i))
            if arr_typ == types.Array(types.NPDatetime('ns'), 1, 'C'):
                cpx__wzt = signature(pd_timestamp_tz_naive_type, arr_typ,
                    types.intp)
                val = context.compile_internal(builder, lambda a, i: bodo.
                    ...
                    cpx__wzt, [qfbqq__gzoxg, index])
            else:
                cpx__wzt = signature(arr_typ.dtype, arr_typ, types.intp)
                val = context.compile_internal(builder, lambda a, i: a[i],
                    cpx__wzt, [qfbqq__gzoxg, index])
            values.append(val)
        value = context.make_tuple(builder, issm__lkwm.yield_type, values)
        result.yield_(value)
        fax__thomx = cgutils.increment_index(builder, index)
        builder.store(fax__thomx, xrs__cveg.index)
...
def _analyze_op_pair_first(self, scope, equiv_set, expr, lhs):
    ...
    avrp__pgls = ir.Assign(rhs, lhs, expr.loc)
    zbfx__ogcl = lhs
    jvl__vvo = []
    kfg__cjo = []
    hxqp__mdlvx = typ.count
    for i in range(hxqp__mdlvx):
        friml__zntl = ir.Var(zbfx__ogcl.scope, mk_unique_var('{}_size{}'.
            format(zbfx__ogcl.name, i)), zbfx__ogcl.loc)
        bvs__okpk = ir.Expr.static_getitem(lhs, i, None, zbfx__ogcl.loc)
        self.calltypes[bvs__okpk] = None
        jvl__vvo.append(ir.Assign(bvs__okpk, friml__zntl, zbfx__ogcl.loc))
        self._define(equiv_set, friml__zntl, types.intp, bvs__okpk)
        kfg__cjo.append(friml__zntl)
    xwa__depgv = tuple(kfg__cjo)
    return numba.parfors.array_analysis.ArrayAnalysis.AnalyzeResult(shape=
        xwa__depgv, pre=[avrp__pgls] + jvl__vvo)
