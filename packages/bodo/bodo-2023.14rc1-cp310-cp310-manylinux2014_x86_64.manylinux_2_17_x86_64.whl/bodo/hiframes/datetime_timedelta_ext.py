...
class PDTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dbs__szkmt = [('value', types.int64)]
        super(PDTimeDeltaModel, self).__init__(dmm, fe_type, dbs__szkmt)


@box(PDTimeDeltaType)
def box_pd_timedelta(typ, val, c):
    jqbj__qbwkh = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    qgc__wvgqu = c.pyapi.long_from_longlong(jqbj__qbwkh.value)
    mwwe__gar = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timedelta))
    res = c.pyapi.call_function_objargs(mwwe__gar, (qgc__wvgqu,))
    c.pyapi.decref(qgc__wvgqu)
    c.pyapi.decref(mwwe__gar)
    return res


@unbox(PDTimeDeltaType)
def unbox_pd_timedelta(typ, val, c):
    qgc__wvgqu = c.pyapi.object_getattr_string(val, 'value')
    kyke__ldlit = c.pyapi.long_as_longlong(qgc__wvgqu)
    jqbj__qbwkh = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    jqbj__qbwkh.value = kyke__ldlit
    c.pyapi.decref(qgc__wvgqu)
    gxw__vkjyx = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(jqbj__qbwkh._getvalue(), is_error=gxw__vkjyx)
...
def pd_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
    ...
    if value == _no_input:
        ...
        def impl_timedelta_kw(value=_no_input, unit='ns', days=0, seconds=0,
            ...
            vydkl__xabg = 1000 * microseconds
            return init_pd_timedelta(vydkl__xabg)
    ...
    sshve__llz, qnvcv__hmzh = pd._libs.tslibs.conversion.precision_from_unit(
        unit)

    def impl_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
        microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0):
        return init_pd_timedelta(value * sshve__llz)
...
def overload_add_operator_datetime_timedelta(lhs, rhs):
    ...
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            nrpbr__mnpvv = (rhs.microseconds + (rhs.seconds + rhs.days * 60 *
                60 * 24) * 1000 * 1000) * 1000
            val = lhs.value + nrpbr__mnpvv
            return pd.Timedelta(val)
        return impl
    if lhs == datetime_timedelta_type and rhs == pd_timedelta_type:

        def impl(lhs, rhs):
            ivm__nhe = (lhs.microseconds + (lhs.seconds + lhs.days * 60 * 
                60 * 24) * 1000 * 1000) * 1000
            val = ivm__nhe + rhs.value
            return pd.Timedelta(val)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            xybx__wdzz = rhs.toordinal()
            jyp__qrdlp = rhs.second + rhs.minute * 60 + rhs.hour * 3600
            ixydm__bta = rhs.microsecond
            hkcxu__kxeu = lhs.value // 1000
            soj__xrhcv = lhs.nanoseconds
            orm__zxuvb = ixydm__bta + hkcxu__kxeu
            cgqb__wamu = 1000000 * (xybx__wdzz * 86400 + jyp__qrdlp
                ) + orm__zxuvb
            zbi__nodbr = soj__xrhcv
            return compute_pd_timestamp(cgqb__wamu, zbi__nodbr)
        return impl
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            lsv__ulqiy = datetime.timedelta(rhs.toordinal(), hours=rhs.hour,
                minutes=rhs.minute, seconds=rhs.second, microseconds=rhs.
                microsecond)
            lsv__ulqiy = lsv__ulqiy + lhs
            cmytc__syt, jqs__pje = divmod(lsv__ulqiy.seconds, 3600)
            umrj__bpyle, eeau__uyu = divmod(jqs__pje, 60)
            if 0 < lsv__ulqiy.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(lsv__ulqiy
                    .days)
                return datetime.datetime(d.year, d.month, d.day, cmytc__syt,
                    umrj__bpyle, eeau__uyu, lsv__ulqiy.microseconds)
            raise OverflowError('result out of range')
        return impl
    if lhs == datetime_datetime_type and rhs == datetime_timedelta_type:

        def impl(lhs, rhs):
            lsv__ulqiy = datetime.timedelta(lhs.toordinal(), hours=lhs.hour,
                minutes=lhs.minute, seconds=lhs.second, microseconds=lhs.
                microsecond)
            lsv__ulqiy = lsv__ulqiy + rhs
            cmytc__syt, jqs__pje = divmod(lsv__ulqiy.seconds, 3600)
            umrj__bpyle, eeau__uyu = divmod(jqs__pje, 60)
            if 0 < lsv__ulqiy.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(lsv__ulqiy
...
def overload_sub_operator_datetime_timedelta(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            sdjt__gxisv = lhs.value - rhs.value
            return pd.Timedelta(sdjt__gxisv)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
    ...
    if lhs == datetime_timedelta_array_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            for dky__ootdk in numba.parfors.parfor.internal_prange(n):
                A[dky__ootdk] = ntfnj__hraka[dky__ootdk] - rhs
            return A
        return impl
...
def overload_mod_operator_timedeltas(lhs, rhs):
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            rbp__zgst = _to_microseconds(lhs) % _to_microseconds(rhs)
            return datetime.timedelta(0, 0, rbp__zgst)
        return impl
...
def pd_timedelta_divmod(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ffgb__paoe, rbp__zgst = divmod(lhs.value, rhs.value)
            return ffgb__paoe, pd.Timedelta(rbp__zgst)
        return impl
...
class DatetimeTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dbs__szkmt = [('days', types.int64), ('seconds', types.int64), (
            'microseconds', types.int64)]
        super(DatetimeTimeDeltaModel, self).__init__(dmm, fe_type, dbs__szkmt)
...
def box_datetime_timedelta(typ, val, c):
    jqbj__qbwkh = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    ngw__pel = c.pyapi.long_from_longlong(jqbj__qbwkh.days)
    pzl__qjk = c.pyapi.long_from_longlong(jqbj__qbwkh.seconds)
    bdwx__mou = c.pyapi.long_from_longlong(jqbj__qbwkh.microseconds)
    mwwe__gar = c.pyapi.unserialize(c.pyapi.serialize_object(datetime.
        timedelta))
    res = c.pyapi.call_function_objargs(mwwe__gar, (ngw__pel, pzl__qjk,
        bdwx__mou))
    c.pyapi.decref(ngw__pel)
    c.pyapi.decref(pzl__qjk)
    c.pyapi.decref(bdwx__mou)
    c.pyapi.decref(mwwe__gar)
    return res
...
def unbox_datetime_timedelta(typ, val, c):
    ngw__pel = c.pyapi.object_getattr_string(val, 'days')
    pzl__qjk = c.pyapi.object_getattr_string(val, 'seconds')
    bdwx__mou = c.pyapi.object_getattr_string(val, 'microseconds')
    nsv__hnnvm = c.pyapi.long_as_longlong(ngw__pel)
    ujhcq__adv = c.pyapi.long_as_longlong(pzl__qjk)
    aohde__jftwq = c.pyapi.long_as_longlong(bdwx__mou)
    jqbj__qbwkh = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    jqbj__qbwkh.days = nsv__hnnvm
    jqbj__qbwkh.seconds = ujhcq__adv
    jqbj__qbwkh.microseconds = aohde__jftwq
    c.pyapi.decref(ngw__pel)
    c.pyapi.decref(pzl__qjk)
    c.pyapi.decref(bdwx__mou)
    gxw__vkjyx = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
...
def _divide_and_round(a, b):
    ffgb__paoe, rbp__zgst = divmod(a, b)
    rbp__zgst *= 2
    qxpf__nwsr = rbp__zgst > b if b > 0 else rbp__zgst < b
    if qxpf__nwsr or rbp__zgst == b and ffgb__paoe % 2 == 1:
        ffgb__paoe += 1
    return ffgb__paoe
...
def create_cmp_op_overload(op):
    ...
    def overload_timedelta_cmp(lhs, rhs):
        if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
            ...
            def impl(lhs, rhs):
                kwl__sptdo = _cmp(_getstate(lhs), _getstate(rhs))
                return op(kwl__sptdo, 0)
            return impl
...
def timedelta_divmod(lhs, rhs):
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ffgb__paoe, rbp__zgst = divmod(_to_microseconds(lhs),
                _to_microseconds(rhs))
            return ffgb__paoe, datetime.timedelta(0, 0, rbp__zgst)
        return impl
...
def timedelta_to_bool(timedelta):
    if timedelta != datetime_timedelta_type:
        return
    dbhzn__tdzi = datetime.timedelta(0)

    def impl(timedelta):
        return timedelta != dbhzn__tdzi
    return impl
...
class DatetimeTimeDeltaArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dbs__szkmt = [('days_data', days_data_type), ('seconds_data',
            seconds_data_type), ('microseconds_data',
            microseconds_data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, dbs__szkmt)
...
def unbox_datetime_timedelta_array(typ, val, c):
    ...
    djqlx__rftk = types.Array(types.intp, 1, 'C')
    ceid__oqx = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        djqlx__rftk, [n])
    hmkib__yjgxh = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        djqlx__rftk, [n])
    jnqfo__zdiu = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        djqlx__rftk, [n])
    esw__sthnh = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    kvdw__iyv = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [esw__sthnh])
    xkrw__zlj = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(64), lir.IntType(64).as_pointer(), lir.IntType(64).
        as_pointer(), lir.IntType(64).as_pointer(), lir.IntType(8).
        as_pointer()])
    gruk__ndkr = cgutils.get_or_insert_function(c.builder.module, xkrw__zlj,
        name='unbox_datetime_timedelta_array')
    c.builder.call(gruk__ndkr, [val, n, ceid__oqx.data, hmkib__yjgxh.data,
        jnqfo__zdiu.data, kvdw__iyv.data])
    pgd__olzfh = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    pgd__olzfh.days_data = ceid__oqx._getvalue()
    pgd__olzfh.seconds_data = hmkib__yjgxh._getvalue()
    pgd__olzfh.microseconds_data = jnqfo__zdiu._getvalue()
    pgd__olzfh.null_bitmap = kvdw__iyv._getvalue()
    gxw__vkjyx = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(pgd__olzfh._getvalue(), is_error=gxw__vkjyx)


@box(DatetimeTimeDeltaArrayType)
def box_datetime_timedelta_array(typ, val, c):
    ntfnj__hraka = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    ceid__oqx = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ntfnj__hraka.days_data)
    hmkib__yjgxh = c.context.make_array(types.Array(types.int64, 1, 'C'))(c
        .context, c.builder, ntfnj__hraka.seconds_data).data
    jnqfo__zdiu = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ntfnj__hraka.microseconds_data).data
    tlv__vrk = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, ntfnj__hraka.null_bitmap).data
    n = c.builder.extract_value(ceid__oqx.shape, 0)
    xkrw__zlj = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(64).as_pointer(), lir.IntType
        (64).as_pointer(), lir.IntType(8).as_pointer()])
    qcvje__lytc = cgutils.get_or_insert_function(c.builder.module,
        xkrw__zlj, name='box_datetime_timedelta_array')
    turq__olk = c.builder.call(qcvje__lytc, [n, ceid__oqx.data,
        hmkib__yjgxh, jnqfo__zdiu, tlv__vrk])
...
def init_datetime_timedelta_array(typingctx, days_data, seconds_data,
    ...
    def codegen(context, builder, signature, args):
        yss__ovn, zue__xspx, kwcr__ryayx, ohxs__ajuac = args
        fwlrd__itcjt = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        fwlrd__itcjt.days_data = yss__ovn
        fwlrd__itcjt.seconds_data = zue__xspx
        fwlrd__itcjt.microseconds_data = kwcr__ryayx
        fwlrd__itcjt.null_bitmap = ohxs__ajuac
        context.nrt.incref(builder, signature.args[0], yss__ovn)
        context.nrt.incref(builder, signature.args[1], zue__xspx)
        context.nrt.incref(builder, signature.args[2], kwcr__ryayx)
        context.nrt.incref(builder, signature.args[3], ohxs__ajuac)
        return fwlrd__itcjt._getvalue()
    eoog__wow = datetime_timedelta_array_type(days_data, seconds_data,
        microseconds_data, nulls)
    return eoog__wow, codegen


@lower_constant(DatetimeTimeDeltaArrayType)
def lower_constant_datetime_timedelta_arr(context, builder, typ, pyval):
    n = len(pyval)
    ceid__oqx = np.empty(n, np.int64)
    hmkib__yjgxh = np.empty(n, np.int64)
    jnqfo__zdiu = np.empty(n, np.int64)
    xhmdw__dnh = np.empty(n + 7 >> 3, np.uint8)
    for dky__ootdk, s in enumerate(pyval):
        ljsmd__bxkl = pd.isna(s)
        bodo.libs.int_arr_ext.set_bit_to_arr(xhmdw__dnh, dky__ootdk, int(
            not ljsmd__bxkl))
        if not ljsmd__bxkl:
            ceid__oqx[dky__ootdk] = s.days
            hmkib__yjgxh[dky__ootdk] = s.seconds
            jnqfo__zdiu[dky__ootdk] = s.microseconds
    hlk__qce = context.get_constant_generic(builder, days_data_type, ceid__oqx)
    res__yxivd = context.get_constant_generic(builder, seconds_data_type,
        hmkib__yjgxh)
    nrc__gdg = context.get_constant_generic(builder, microseconds_data_type,
        jnqfo__zdiu)
    omhyh__cftf = context.get_constant_generic(builder, nulls_type, xhmdw__dnh)
    return lir.Constant.literal_struct([hlk__qce, res__yxivd, nrc__gdg,
        omhyh__cftf])


@numba.njit(no_cpython_wrapper=True)
def alloc_datetime_timedelta_array(n):
    ceid__oqx = np.empty(n, dtype=np.int64)
    hmkib__yjgxh = np.empty(n, dtype=np.int64)
    jnqfo__zdiu = np.empty(n, dtype=np.int64)
    nulls = np.full(n + 7 >> 3, 255, np.uint8)
    return init_datetime_timedelta_array(ceid__oqx, hmkib__yjgxh,
        jnqfo__zdiu, nulls)
...
def dt_timedelta_arr_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            fwny__htlli = bodo.utils.conversion.coerce_to_array(ind)
            txy__alou = A._null_bitmap
            sxm__lnxqh = A._days_data[fwny__htlli]
            ulxeq__oqehg = A._seconds_data[fwny__htlli]
            wgvde__hwn = A._microseconds_data[fwny__htlli]
            n = len(sxm__lnxqh)
            xqb__yme = get_new_null_mask_bool_index(txy__alou, ind, n)
            return init_datetime_timedelta_array(sxm__lnxqh, ulxeq__oqehg,
                wgvde__hwn, xqb__yme)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            ...
            wgvde__hwn = A._microseconds_data[fwny__htlli]
            n = len(sxm__lnxqh)
            xqb__yme = get_new_null_mask_int_index(txy__alou, fwny__htlli, n)
            return init_datetime_timedelta_array(sxm__lnxqh, ulxeq__oqehg,
                wgvde__hwn, xqb__yme)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            n = len(A._days_data)
            txy__alou = A._null_bitmap
            sxm__lnxqh = np.ascontiguousarray(A._days_data[ind])
            ulxeq__oqehg = np.ascontiguousarray(A._seconds_data[ind])
            wgvde__hwn = np.ascontiguousarray(A._microseconds_data[ind])
            xqb__yme = get_new_null_mask_slice_index(txy__alou, ind, n)
            return init_datetime_timedelta_array(sxm__lnxqh, ulxeq__oqehg,
                wgvde__hwn, xqb__yme)
        return impl_slice
    ...
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_arr_ind_scalar(A, ind, val):
                n = len(A)
                for dky__ootdk in range(n):
                    A._days_data[ind[dky__ootdk]] = val._days
                    A._seconds_data[ind[dky__ootdk]] = val._seconds
                    A._microseconds_data[ind[dky__ootdk]] = val._microseconds
            ...
            def impl_arr_ind(A, ind, val):
                ...
                for dky__ootdk in range(n):
                    A._days_data[ind[dky__ootdk]] = val._days_data[dky__ootdk]
                    A._seconds_data[ind[dky__ootdk]] = val._seconds_data[
                        dky__ootdk]
                    A._microseconds_data[ind[dky__ootdk]
                        ] = val._microseconds_data[dky__ootdk]
                    bseia__wnqyw = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, dky__ootdk)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        ind[dky__ootdk], bseia__wnqyw)
            return impl_arr_ind
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_bool_ind_mask_scalar(A, ind, val):
                n = len(ind)
                for dky__ootdk in range(n):
                    if not bodo.libs.array_kernels.isna(ind, dky__ootdk
                        ) and ind[dky__ootdk]:
                        A._days_data[dky__ootdk] = val._days
                        A._seconds_data[dky__ootdk] = val._seconds
                        A._microseconds_data[dky__ootdk] = val._microseconds
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            dky__ootdk, 1)
            ...
            def impl_bool_ind_mask(A, ind, val):
                ...
                for dky__ootdk in range(n):
                    if not bodo.libs.array_kernels.isna(ind, dky__ootdk
                        ...
                        A._days_data[dky__ootdk] = val._days_data[zgwwj__hekfg]
                        A._seconds_data[dky__ootdk] = val._seconds_data[
                            zgwwj__hekfg]
                        A._microseconds_data[dky__ootdk
                            ] = val._microseconds_data[zgwwj__hekfg]
                        bseia__wnqyw = (bodo.libs.int_arr_ext.
                            get_bit_bitmap_arr(val._null_bitmap, zgwwj__hekfg))
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            dky__ootdk, bseia__wnqyw)
                        zgwwj__hekfg += 1
    ...
    if isinstance(ind, types.SliceType):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_slice_scalar(A, ind, val):
                cdaiq__aqj = numba.cpython.unicode._normalize_slice(ind, len(A)
                    )
                for dky__ootdk in range(cdaiq__aqj.start, cdaiq__aqj.stop,
                    cdaiq__aqj.step):
                    A._days_data[dky__ootdk] = val._days
                    A._seconds_data[dky__ootdk] = val._seconds
                    A._microseconds_data[dky__ootdk] = val._microseconds
            ...
            def impl_slice_mask(A, ind, val):
                ...
                odnci__rpkj = val._null_bitmap.copy()
                setitem_slice_index_null_bits(A._null_bitmap, odnci__rpkj,
                    ind, n)
...
def overload_datetime_timedelta_arr_sub(arg1, arg2):
    if (arg1 == datetime_timedelta_array_type and arg2 ==
        ...
        def impl(arg1, arg2):
            ...
            A = alloc_datetime_timedelta_array(n)
            for dky__ootdk in numba.parfors.parfor.internal_prange(n):
                A[dky__ootdk] = ntfnj__hraka[dky__ootdk] - arg2
            return A
        return impl
    ...
    def overload_date_arr_cmp(lhs, rhs):
        ...
        if (lhs == datetime_timedelta_array_type and rhs ==
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                bmfpv__lnqbb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dky__ootdk in numba.parfors.parfor.internal_prange(n):
                    cavtm__hqned = bodo.libs.array_kernels.isna(lhs, dky__ootdk
                        )
                    uvxq__fib = bodo.libs.array_kernels.isna(rhs, dky__ootdk)
                    if cavtm__hqned or uvxq__fib:
                        tqv__lkqg = mxg__fbu
                    else:
                        tqv__lkqg = op(lhs[dky__ootdk], rhs[dky__ootdk])
                    bmfpv__lnqbb[dky__ootdk] = tqv__lkqg
                return bmfpv__lnqbb
            return impl
            ...
            def impl(lhs, rhs):
                ...
                bmfpv__lnqbb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dky__ootdk in numba.parfors.parfor.internal_prange(n):
                    bseia__wnqyw = bodo.libs.array_kernels.isna(lhs, dky__ootdk
                        )
                    if bseia__wnqyw:
                        tqv__lkqg = mxg__fbu
                    else:
                        tqv__lkqg = op(lhs[dky__ootdk], rhs)
                    bmfpv__lnqbb[dky__ootdk] = tqv__lkqg
                return bmfpv__lnqbb
            return impl
            ...
            def impl(lhs, rhs):
                ...
                bmfpv__lnqbb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dky__ootdk in numba.parfors.parfor.internal_prange(n):
                    bseia__wnqyw = bodo.libs.array_kernels.isna(rhs, dky__ootdk
                        )
                    if bseia__wnqyw:
                        tqv__lkqg = mxg__fbu
                    else:
                        tqv__lkqg = op(lhs, rhs[dky__ootdk])
                    bmfpv__lnqbb[dky__ootdk] = tqv__lkqg
                return bmfpv__lnqbb
            return impl
...
def _intstall_pd_timedelta_unsupported():
    ...
    for bcv__qadvp in timedelta_unsupported_attrs:
        vcix__diom = 'pandas.Timedelta.' + bcv__qadvp
        overload_attribute(PDTimeDeltaType, bcv__qadvp)(
            create_unsupported_overload(vcix__diom))
    for nfklg__haw in timedelta_unsupported_methods:
        vcix__diom = 'pandas.Timedelta.' + nfklg__haw
        overload_method(PDTimeDeltaType, nfklg__haw)(
            create_unsupported_overload(vcix__diom + '()'))
