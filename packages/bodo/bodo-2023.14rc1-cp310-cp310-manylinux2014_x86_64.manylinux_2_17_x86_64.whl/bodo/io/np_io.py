...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        udjo__itwc = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, udjo__itwc)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            btb__odwh = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(btb__odwh.dtype)
            file_write(unicode_to_utf8(fname), btb__odwh.ctypes, dtype_size *
                btb__odwh.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            btb__odwh = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(btb__odwh.dtype)
            oepbu__cnc = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                btb__odwh.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), btb__odwh.ctypes,
                start, count, oepbu__cnc)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            cndx__vyj = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                cndx__vyj = min(cndx__vyj, count * dtype_size)
            if cndx__vyj < 0:
                return -1
            return cndx__vyj
        return impl
