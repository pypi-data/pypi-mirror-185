...
def _global_except_hook(exctype, value, traceback):
    ...
    mxd__kkc = MPI.COMM_WORLD
    zwed__rxl = mxd__kkc.Get_rank()
    mlpfo__jkbu = 3.0
    sryf__uamq = True
    jnwnl__upnv = mxd__kkc.Ibarrier()
    vrwn__zml = time.time()
    while time.time() - vrwn__zml < mlpfo__jkbu:
        time.sleep(0.1)
        if jnwnl__upnv.Test():
            sryf__uamq = False
            break
    try:
        ...
        if sryf__uamq:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {zwed__rxl}. \n')
...
def set_numba_environ_vars():
    if (tzjxm__vut := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = tzjxm__vut
        os.environ['NUMBA_CACHE_DIR'] = tzjxm__vut
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    retp__orq = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(retp__orq)
