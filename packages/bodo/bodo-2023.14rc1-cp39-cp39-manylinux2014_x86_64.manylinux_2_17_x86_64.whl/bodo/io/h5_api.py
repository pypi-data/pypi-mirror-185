...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        lwo__fjfa = []
        vzacr__tizel = h5g_get_num_objs(obj_id)
        for staku__okztu in range(vzacr__tizel):
            onv__lea = h5g_get_objname_by_idx(obj_id, staku__okztu)
            lwo__fjfa.append(onv__lea)
        return lwo__fjfa
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    pbsuc__ejl = parse_dtype(dtype)
    jaly__nqmkl = np.int32(numba_to_c_type(pbsuc__ejl))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, jaly__nqmkl)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            qaxu__ekyz = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    vfm__tfe = tuple_to_ptr(starts)
    vevr__tsnbq = tuple_to_ptr(counts)
    rcw__edj = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, vfm__tfe, vevr__tsnbq,
        is_parallel, out_arr.ctypes, rcw__edj)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    vevr__tsnbq = tuple_to_ptr(counts)
    rcw__edj = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, vfm__tfe, vevr__tsnbq,
        is_parallel, out_arr.ctypes, rcw__edj)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        drar__mvh = types.Array(dtype, ndim, 'C')
        lgfw__yvbg = args[3]
        drar__mvh = self.context.resolve_function_type(operator.getitem, [
            drar__mvh, lgfw__yvbg], {}).return_type.copy(layout='C')
        return signature(drar__mvh, *args)
...
def get_filter_read_indices(bool_arr):
    wpue__nksv = bool_arr.nonzero()[0]
    neuui__shbpv = bodo.libs.distributed_api.get_rank()
    pxwqs__vnvgx = bodo.libs.distributed_api.get_size()
    znatr__nek = np.empty(pxwqs__vnvgx, np.int64)
    uverv__yljyt = len(bool_arr)
    bodo.libs.distributed_api.allgather(znatr__nek, uverv__yljyt)
    bjo__tuw = znatr__nek.cumsum()[neuui__shbpv] - uverv__yljyt
    wpue__nksv += bjo__tuw
    fiwsf__ohuf = bodo.libs.distributed_api.dist_reduce(len(wpue__nksv), np
        .int32(sum_op))
    xqsv__dnxz = bodo.libs.distributed_api.gatherv(wpue__nksv)
    if neuui__shbpv == 0:
        hsy__stkno = xqsv__dnxz
    else:
        hsy__stkno = np.empty(fiwsf__ohuf, wpue__nksv.dtype)
    bodo.libs.distributed_api.bcast(hsy__stkno)
    kncn__olfp = bodo.libs.distributed_api.get_start(fiwsf__ohuf,
        pxwqs__vnvgx, neuui__shbpv)
    mgyb__ciw = bodo.libs.distributed_api.get_end(fiwsf__ohuf, pxwqs__vnvgx,
        neuui__shbpv)
    return hsy__stkno[kncn__olfp:mgyb__ciw]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        veb__tgdul = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], veb__tgdul)
        return builder.bitcast(veb__tgdul, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    vevr__tsnbq = tuple_to_ptr(counts)
    rcw__edj = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, vfm__tfe,
        vevr__tsnbq, is_parallel, out_arr.ctypes, rcw__edj, read_indices.
