...
def box_regex_flag(typ, val, c):
    ntq__zhvni = c.box(types.int64, val)
    uotyl__duoyn = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(uotyl__duoyn, (ntq__zhvni,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    ntq__zhvni = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, ntq__zhvni)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    fpdn__zso = context.get_python_api(builder)
    qrnkn__jjc = builder.icmp_signed('==', val, fpdn__zso.borrow_none())
    with builder.if_then(qrnkn__jjc):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    ccxsr__fyu = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(ccxsr__fyu)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        sdnd__iom = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if sdnd__iom > 1:
            typ = types.List(types.Tuple([string_type] * sdnd__iom))
...
def overload_match_group(m, *args):
    ...
    xkqmv__aitw = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            hvl__pva = args[0]
            with numba.objmode(out=xkqmv__aitw):
                out = m.group(hvl__pva)
            return out
        return _match_group_impl_one
    ous__yoayu = 'tuple_str_{}'.format(len(args))
    setattr(types, ous__yoayu, types.Tuple([xkqmv__aitw] * len(args)))
    vrmss__tsvom = ', '.join('group{}'.format(xzgc__dkknm + 1) for
        xzgc__dkknm in range(len(args)))
    vipt__jcuyx = 'def _match_group_impl(m, *args):\n'
    vipt__jcuyx += '  ({}) = args\n'.format(vrmss__tsvom)
    vipt__jcuyx += "  with numba.objmode(out='{}'):\n".format(ous__yoayu)
    vipt__jcuyx += '    out = m.group({})\n'.format(vrmss__tsvom)
    vipt__jcuyx += '  return out\n'
    husxi__inq = {}
    exec(vipt__jcuyx, globals(), husxi__inq)
    wrm__vmedd = husxi__inq['_match_group_impl']
    return wrm__vmedd
...
def overload_match_groups(m, default=None):
    hrg__trw = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=hrg__trw):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(avs__umjyq is None for avs__umjyq in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=xkqmv__aitw):
