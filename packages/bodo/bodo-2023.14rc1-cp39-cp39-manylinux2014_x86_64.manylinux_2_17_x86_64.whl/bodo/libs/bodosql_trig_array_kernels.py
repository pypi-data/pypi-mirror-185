...
def create_trig_func_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_func(arr):
            ...
            yqb__xrz = 'def impl(arr):\n'
            yqb__xrz += (
                f'  return bodo.libs.bodosql_array_kernels.{func_name}_util(arr)'
                )
            aorg__npm = {}
            exec(yqb__xrz, {'bodo': bodo}, aorg__npm)
            return aorg__npm['impl']
    else:
        func_name = func_name.lower()

        def overload_func(arr0, arr1):
            args = [arr0, arr1]
            for jgr__yehjq in range(2):
                if isinstance(args[jgr__yehjq], types.optional):
                    return unopt_argument(
                        f'bodo.libs.bodosql_array_kernels.{func_name}', [
                        'arr0', 'arr1'], jgr__yehjq)
            yqb__xrz = 'def impl(arr0, arr1):\n'
            yqb__xrz += (
...
def create_trig_util_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_trig_util(arr):
            ...
            jmnhe__kujs = ''
            if func_name == 'ACOS':
                jmnhe__kujs += 'res[i] = np.arccos(arg0)'
            elif func_name == 'ACOSH':
                jmnhe__kujs += 'res[i] = np.arccosh(arg0)'
            elif func_name == 'ASIN':
                jmnhe__kujs += 'res[i] = np.arcsin(arg0)'
            elif func_name == 'ASINH':
                jmnhe__kujs += 'res[i] = np.arcsinh(arg0)'
            elif func_name == 'ATAN':
                jmnhe__kujs += 'res[i] = np.arctan(arg0)'
            elif func_name == 'ATANH':
                jmnhe__kujs += 'res[i] = np.arctanh(arg0)'
            elif func_name == 'COS':
                jmnhe__kujs += 'res[i] = np.cos(arg0)'
            elif func_name == 'COSH':
                jmnhe__kujs += 'res[i] = np.cosh(arg0)'
            elif func_name == 'SIN':
                jmnhe__kujs += 'res[i] = np.sin(arg0)'
            elif func_name == 'SINH':
                jmnhe__kujs += 'res[i] = np.sinh(arg0)'
            elif func_name == 'TAN':
                jmnhe__kujs += 'res[i] = np.tan(arg0)'
            elif func_name == 'TANH':
                jmnhe__kujs += 'res[i] = np.tanh(arg0)'
            elif func_name == 'RADIANS':
                jmnhe__kujs += 'res[i] = np.radians(arg0)'
            elif func_name == 'DEGREES':
                jmnhe__kujs += 'res[i] = np.degrees(arg0)'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            zwhx__lije = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(ixhxd__qqa, ybgf__omnu, pdtut__qaa,
                jmnhe__kujs, zwhx__lije)
    else:

        def overload_trig_util(arr0, arr1):
            verify_int_float_arg(arr0, func_name, 'arr0')
            verify_int_float_arg(arr1, func_name, 'arr1')
            ixhxd__qqa = ['arr0', 'arr1']
            ybgf__omnu = [arr0, arr1]
            pdtut__qaa = [True, True]
            jmnhe__kujs = ''
            if func_name == 'ATAN2':
                jmnhe__kujs += 'res[i] = np.arctan2(arg0, arg1)\n'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            zwhx__lije = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(ixhxd__qqa, ybgf__omnu, pdtut__qaa,
...
def _install_trig_overload(funcs_utils_names):
    for qukv__ppl, jysjm__kwilq, func_name in funcs_utils_names:
        jlrc__chy = create_trig_func_overload(func_name)
        overload(qukv__ppl)(jlrc__chy)
        dvdk__fvw = create_trig_util_overload(func_name)
        overload(jysjm__kwilq)(dvdk__fvw)
