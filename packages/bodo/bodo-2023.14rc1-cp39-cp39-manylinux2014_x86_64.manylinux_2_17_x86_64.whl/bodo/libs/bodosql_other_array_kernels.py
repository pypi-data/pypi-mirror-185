"""
Implements miscellaneous array kernels that are specific to BodoSQL
"""
import numba
from numba.core import types
import bodo
from bodo.libs.bodosql_array_kernel_utils import *
from bodo.utils.typing import raise_bodo_error


@numba.generated_jit(nopython=True)
def booland(A, B):
    args = [A, B]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.booland',
                ['A', 'B'], izik__mrzu)

    def impl(A, B):
        return booland_util(A, B)
    return impl


@numba.generated_jit(nopython=True)
def boolor(A, B):
    args = [A, B]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.boolor',
                ['A', 'B'], izik__mrzu)

    def impl(A, B):
        return boolor_util(A, B)
    return impl


@numba.generated_jit(nopython=True)
def boolxor(A, B):
    args = [A, B]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.boolxor',
                ['A', 'B'], izik__mrzu)

    def impl(A, B):
        return boolxor_util(A, B)
    return impl


@numba.generated_jit(nopython=True)
def boolnot(A):
    if isinstance(A, types.optional):
        return unopt_argument('bodo.libs.bodosql_array_kernels.boolnot_util',
            ['A'], 0)

    def impl(A):
        return boolnot_util(A)
    return impl


@numba.generated_jit(nopython=True)
def cond(arr, ifbranch, elsebranch):
    args = [arr, ifbranch, elsebranch]
    for izik__mrzu in range(3):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.cond', [
                'arr', 'ifbranch', 'elsebranch'], izik__mrzu)

    def impl(arr, ifbranch, elsebranch):
        return cond_util(arr, ifbranch, elsebranch)
    return impl


@numba.generated_jit(nopython=True)
def equal_null(A, B):
    args = [A, B]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.equal_null',
                ['A', 'B'], izik__mrzu)

    def impl(A, B):
        return equal_null_util(A, B)
    return impl


@numba.generated_jit(nopython=True)
def booland_util(A, B):
    verify_int_float_arg(A, 'BOOLAND', 'A')
    verify_int_float_arg(B, 'BOOLAND', 'B')
    wzqj__gmw = ['A', 'B']
    iqy__auo = [A, B]
    yhea__zhsd = [False] * 2
    if A == bodo.none:
        yhea__zhsd = [False, True]
        htza__qydv = 'if arg1 != 0:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = False\n'
    elif B == bodo.none:
        yhea__zhsd = [True, False]
        htza__qydv = 'if arg0 != 0:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = False\n'
    elif bodo.utils.utils.is_array_typ(A, True):
        if bodo.utils.utils.is_array_typ(B, True):
            htza__qydv = """if bodo.libs.array_kernels.isna(A, i) and bodo.libs.array_kernels.isna(B, i):
"""
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(A, i) and arg1 != 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(B, i) and arg0 != 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += 'else:\n'
            htza__qydv += '   res[i] = (arg0 != 0) and (arg1 != 0)'
        else:
            htza__qydv = (
                'if bodo.libs.array_kernels.isna(A, i) and arg1 != 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += 'else:\n'
            htza__qydv += '   res[i] = (arg0 != 0) and (arg1 != 0)'
    elif bodo.utils.utils.is_array_typ(B, True):
        htza__qydv = 'if bodo.libs.array_kernels.isna(B, i) and arg0 != 0:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = (arg0 != 0) and (arg1 != 0)'
    else:
        htza__qydv = 'res[i] = (arg0 != 0) and (arg1 != 0)'
    kgs__lfpw = bodo.libs.bool_arr_ext.boolean_array
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def boolor_util(A, B):
    verify_int_float_arg(A, 'BOOLOR', 'A')
    verify_int_float_arg(B, 'BOOLOR', 'B')
    wzqj__gmw = ['A', 'B']
    iqy__auo = [A, B]
    yhea__zhsd = [False] * 2
    if A == bodo.none:
        yhea__zhsd = [False, True]
        htza__qydv = 'if arg1 == 0:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = True\n'
    elif B == bodo.none:
        yhea__zhsd = [True, False]
        htza__qydv = 'if arg0 == 0:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = True\n'
    elif bodo.utils.utils.is_array_typ(A, True):
        if bodo.utils.utils.is_array_typ(B, True):
            htza__qydv = """if bodo.libs.array_kernels.isna(A, i) and bodo.libs.array_kernels.isna(B, i):
"""
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(A, i) and arg1 != 0:\n')
            htza__qydv += '   res[i] = True\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(A, i) and arg1 == 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(B, i) and arg0 != 0:\n')
            htza__qydv += '   res[i] = True\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(B, i) and arg0 == 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += 'else:\n'
            htza__qydv += '   res[i] = (arg0 != 0) or (arg1 != 0)'
        else:
            htza__qydv = (
                'if bodo.libs.array_kernels.isna(A, i) and arg1 != 0:\n')
            htza__qydv += '   res[i] = True\n'
            htza__qydv += (
                'elif bodo.libs.array_kernels.isna(A, i) and arg1 == 0:\n')
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += 'else:\n'
            htza__qydv += '   res[i] = (arg0 != 0) or (arg1 != 0)'
    elif bodo.utils.utils.is_array_typ(B, True):
        htza__qydv = 'if bodo.libs.array_kernels.isna(B, i) and arg0 != 0:\n'
        htza__qydv += '   res[i] = True\n'
        htza__qydv += (
            'elif bodo.libs.array_kernels.isna(B, i) and arg0 == 0:\n')
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   res[i] = (arg0 != 0) or (arg1 != 0)'
    else:
        htza__qydv = 'res[i] = (arg0 != 0) or (arg1 != 0)'
    kgs__lfpw = bodo.libs.bool_arr_ext.boolean_array
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def boolxor_util(A, B):
    verify_int_float_arg(A, 'BOOLXOR', 'A')
    verify_int_float_arg(B, 'BOOLXOR', 'B')
    wzqj__gmw = ['A', 'B']
    iqy__auo = [A, B]
    yhea__zhsd = [True] * 2
    htza__qydv = 'res[i] = (arg0 == 0) != (arg1 == 0)'
    kgs__lfpw = bodo.libs.bool_arr_ext.boolean_array
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def boolnot_util(A):
    verify_int_float_arg(A, 'BOOLNOT', 'A')
    wzqj__gmw = ['A']
    iqy__auo = [A]
    yhea__zhsd = [True]
    htza__qydv = 'res[i] = arg0 == 0'
    kgs__lfpw = bodo.libs.bool_arr_ext.boolean_array
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def nullif(arr0, arr1):
    args = [arr0, arr1]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.nullif',
                ['arr0', 'arr1'], izik__mrzu)

    def impl(arr0, arr1):
        return nullif_util(arr0, arr1)
    return impl


@numba.generated_jit(nopython=True)
def regr_valx(y, x):
    args = [y, x]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.regr_valx',
                ['y', 'x'], izik__mrzu)

    def impl(y, x):
        return regr_valx_util(y, x)
    return impl


@numba.generated_jit(nopython=True)
def regr_valy(y, x):
    args = [y, x]
    for izik__mrzu in range(2):
        if isinstance(args[izik__mrzu], types.optional):
            return unopt_argument('bodo.libs.bodosql_array_kernels.regr_valy',
                ['y', 'x'], izik__mrzu)

    def impl(y, x):
        return regr_valx(x, y)
    return impl


@numba.generated_jit(nopython=True)
def cond_util(arr, ifbranch, elsebranch):
    verify_boolean_arg(arr, 'cond', 'arr')
    if bodo.utils.utils.is_array_typ(arr, True
        ) and ifbranch == bodo.none and elsebranch == bodo.none:
        raise_bodo_error('Both branches of IF() cannot be scalar NULL')
    wzqj__gmw = ['arr', 'ifbranch', 'elsebranch']
    iqy__auo = [arr, ifbranch, elsebranch]
    yhea__zhsd = [False] * 3
    if bodo.utils.utils.is_array_typ(arr, True):
        htza__qydv = (
            'if (not bodo.libs.array_kernels.isna(arr, i)) and arg0:\n')
    elif arr != bodo.none:
        htza__qydv = 'if arg0:\n'
    else:
        htza__qydv = ''
    if arr != bodo.none:
        if bodo.utils.utils.is_array_typ(ifbranch, True):
            htza__qydv += '   if bodo.libs.array_kernels.isna(ifbranch, i):\n'
            htza__qydv += '      bodo.libs.array_kernels.setna(res, i)\n'
            htza__qydv += '   else:\n'
            htza__qydv += '      res[i] = arg1\n'
        elif ifbranch == bodo.none:
            htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
        else:
            htza__qydv += '   res[i] = arg1\n'
        htza__qydv += 'else:\n'
    if bodo.utils.utils.is_array_typ(elsebranch, True):
        htza__qydv += '   if bodo.libs.array_kernels.isna(elsebranch, i):\n'
        htza__qydv += '      bodo.libs.array_kernels.setna(res, i)\n'
        htza__qydv += '   else:\n'
        htza__qydv += '      res[i] = arg2\n'
    elif elsebranch == bodo.none:
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)\n'
    else:
        htza__qydv += '   res[i] = arg2\n'
    kgs__lfpw = get_common_broadcasted_type([ifbranch, elsebranch], 'IF')
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def equal_null_util(A, B):
    get_common_broadcasted_type([A, B], 'EQUAL_NULL')
    wzqj__gmw = ['A', 'B']
    iqy__auo = [A, B]
    yhea__zhsd = [False] * 2
    if A == bodo.none:
        if B == bodo.none:
            htza__qydv = 'res[i] = True'
        elif bodo.utils.utils.is_array_typ(B, True):
            htza__qydv = 'res[i] = bodo.libs.array_kernels.isna(B, i)'
        else:
            htza__qydv = 'res[i] = False'
    elif B == bodo.none:
        if bodo.utils.utils.is_array_typ(A, True):
            htza__qydv = 'res[i] = bodo.libs.array_kernels.isna(A, i)'
        else:
            htza__qydv = 'res[i] = False'
    elif bodo.utils.utils.is_array_typ(A, True):
        if bodo.utils.utils.is_array_typ(B, True):
            htza__qydv = """if bodo.libs.array_kernels.isna(A, i) and bodo.libs.array_kernels.isna(B, i):
"""
            htza__qydv += '   res[i] = True\n'
            htza__qydv += """elif bodo.libs.array_kernels.isna(A, i) or bodo.libs.array_kernels.isna(B, i):
"""
            htza__qydv += '   res[i] = False\n'
            htza__qydv += 'else:\n'
            htza__qydv += '   res[i] = arg0 == arg1'
        else:
            htza__qydv = (
                'res[i] = (not bodo.libs.array_kernels.isna(A, i)) and arg0 == arg1'
                )
    elif bodo.utils.utils.is_array_typ(B, True):
        htza__qydv = (
            'res[i] = (not bodo.libs.array_kernels.isna(B, i)) and arg0 == arg1'
            )
    else:
        htza__qydv = 'res[i] = arg0 == arg1'
    kgs__lfpw = bodo.libs.bool_arr_ext.boolean_array
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def nullif_util(arr0, arr1):
    wzqj__gmw = ['arr0', 'arr1']
    iqy__auo = [arr0, arr1]
    yhea__zhsd = [True, False]
    if arr1 == bodo.none:
        htza__qydv = 'res[i] = arg0\n'
    elif bodo.utils.utils.is_array_typ(arr1, True):
        htza__qydv = (
            'if bodo.libs.array_kernels.isna(arr1, i) or arg0 != arg1:\n')
        htza__qydv += '   res[i] = arg0\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)'
    else:
        htza__qydv = 'if arg0 != arg1:\n'
        htza__qydv += '   res[i] = arg0\n'
        htza__qydv += 'else:\n'
        htza__qydv += '   bodo.libs.array_kernels.setna(res, i)'
    kgs__lfpw = get_common_broadcasted_type([arr0, arr1], 'NULLIF')
    return gen_vectorized(wzqj__gmw, iqy__auo, yhea__zhsd, htza__qydv,
        kgs__lfpw)


@numba.generated_jit(nopython=True)
def regr_valx_util(y, x):
    verify_int_float_arg(y, 'regr_valx', 'y')
    verify_int_float_arg(x, 'regr_valx', 'x')
    wzqj__gmw = ['y', 'x']
    iqy__auo = [y, x]
    hjr__ngpnr = [True] * 2
    htza__qydv = 'res[i] = arg1'
    kgs__lfpw = types.Array(bodo.float64, 1, 'C')
    return gen_vectorized(wzqj__gmw, iqy__auo, hjr__ngpnr, htza__qydv,
        kgs__lfpw)
