...
def _column_filter_impl(B, ind):
    vefxx__ynb = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,), None)
    for oocqs__tgrwj in numba.parfors.parfor.internal_prange(len(vefxx__ynb)):
        if ind[oocqs__tgrwj]:
            vefxx__ynb[oocqs__tgrwj] = B[oocqs__tgrwj]
        else:
            bodo.libs.array_kernels.setna(vefxx__ynb, oocqs__tgrwj)
    return vefxx__ynb
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    gof__wywb = len(B)
    uilaa__iglrl = 0
    for oocqs__tgrwj in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, oocqs__tgrwj):
            uilaa__iglrl += 1
    reds__poi = gof__wywb - uilaa__iglrl
    apv__mcyd = bodo.libs.str_arr_ext.num_total_chars(B)
    vefxx__ynb = bodo.libs.str_arr_ext.pre_alloc_string_array(reds__poi,
        apv__mcyd)
    bodo.libs.str_arr_ext.copy_non_null_offsets(vefxx__ynb, B)
    bodo.libs.str_arr_ext.copy_data(vefxx__ynb, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(vefxx__ynb, -1)
    return vefxx__ynb
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    kqrk__ykttv = first_moment / count
    npol__nae = (third_moment - 3 * second_moment * kqrk__ykttv + 2 * count *
        kqrk__ykttv ** 3)
    xen__lpikr = second_moment - kqrk__ykttv * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * npol__nae / xen__lpikr ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    kqrk__ykttv = first_moment / count
    wmrrc__fgozq = (fourth_moment - 4 * third_moment * kqrk__ykttv + 6 *
        second_moment * kqrk__ykttv ** 2 - 3 * count * kqrk__ykttv ** 4)
    rcy__kwudh = second_moment - kqrk__ykttv * first_moment
    qolt__smmr = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    foxzw__ucwk = count * (count + 1) * (count - 1) * wmrrc__fgozq
    iem__ivq = (count - 2) * (count - 3) * rcy__kwudh ** 2
    s = (count - 1) * (foxzw__ucwk / iem__ivq - qolt__smmr)
    s = s / (count - 1)
    return s
