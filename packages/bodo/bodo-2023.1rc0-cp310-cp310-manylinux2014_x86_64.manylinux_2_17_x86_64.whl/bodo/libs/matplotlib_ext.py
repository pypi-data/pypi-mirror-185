...
def _install_mpl_types():
    yhgpx__fnxo = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for cdr__qwk, uxk__ezu in yhgpx__fnxo:
        install_py_obj_class(types_name=cdr__qwk, python_type=uxk__ezu,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    umr__oksrs = ', '.join(f'e{vdldn__llad}' for vdldn__llad in range(len(
        args)))
    if umr__oksrs:
        umr__oksrs += ', '
    ghem__celq = ', '.join(f"{jcm__bdt} = ''" for jcm__bdt in kws.keys())
    lna__baxf = 'matplotlib_obj, ' if obj_typ is not None else ''
    vzr__jrt = f'def mpl_stub({lna__baxf} {umr__oksrs} {ghem__celq}):\n'
    vzr__jrt += '    pass\n'
    zielq__wmofz = {}
    exec(vzr__jrt, {}, zielq__wmofz)
    zfa__dac = zielq__wmofz['mpl_stub']
    ndpgo__rwof = numba.core.utils.pysignature(zfa__dac)
    evyw__xaxlt = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(
        kws.values())
    return signature(return_typ, *unliteral_all(evyw__xaxlt)).replace(pysig
        =ndpgo__rwof)


def generate_axes_typing(mod_name, nrows, ncols):
    yiy__raiw = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(yiy__raiw.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(yiy__raiw.format(mod_name, 'ncols'))
    vrfjv__buhm = get_overload_const_int(nrows)
    uxc__fie = get_overload_const_int(ncols)
    if vrfjv__buhm < 1:
        raise BodoError(yiy__raiw.format(mod_name, 'nrows'))
    if uxc__fie < 1:
        raise BodoError(yiy__raiw.format(mod_name, 'ncols'))
    if vrfjv__buhm == 1 and uxc__fie == 1:
        uwcwk__xkwi = types.mpl_axes_type
    else:
        if uxc__fie == 1:
            qmffx__dqw = types.mpl_axes_type
        else:
            qmffx__dqw = types.Tuple([types.mpl_axes_type] * uxc__fie)
        uwcwk__xkwi = types.Tuple([qmffx__dqw] * vrfjv__buhm)
    return uwcwk__xkwi


def generate_pie_return_type(args, kws):
    wul__gva = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if wul__gva == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    sfzh__xlng = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(sfzh__xlng):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(sfzh__xlng):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        cwohi__abgpy = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, cwohi__abgpy]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    cwohi__abgpy = generate_axes_typing('matplotlib.figure.Figure', nrows,
        ncols)
    cdr__qwk = str(cwohi__abgpy)
    if not hasattr(types, cdr__qwk):
        cdr__qwk = f'objmode_type{ir_utils.next_label()}'
        setattr(types, cdr__qwk, cwohi__abgpy)
    vzr__jrt = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        ...
        with numba.objmode(axes="{cdr__qwk}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    zielq__wmofz = {}
    exec(vzr__jrt, {'numba': numba, 'np': np}, zielq__wmofz)
    impl = zielq__wmofz['impl']
    return impl
