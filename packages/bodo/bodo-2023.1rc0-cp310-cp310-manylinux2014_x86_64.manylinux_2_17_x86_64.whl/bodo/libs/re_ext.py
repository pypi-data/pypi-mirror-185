...
def box_regex_flag(typ, val, c):
    xki__idf = c.box(types.int64, val)
    bcayj__mctv = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(bcayj__mctv, (xki__idf,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    xki__idf = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, xki__idf)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    svzb__osy = context.get_python_api(builder)
    yjj__aaxcw = builder.icmp_signed('==', val, svzb__osy.borrow_none())
    with builder.if_then(yjj__aaxcw):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    ddj__jqprh = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(ddj__jqprh)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        rlj__fnv = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if rlj__fnv > 1:
            typ = types.List(types.Tuple([string_type] * rlj__fnv))
...
def overload_match_group(m, *args):
    ...
    vkzjj__ovth = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            arv__pnox = args[0]
            with numba.objmode(out=vkzjj__ovth):
                out = m.group(arv__pnox)
            return out
        return _match_group_impl_one
    ryqy__nmlp = 'tuple_str_{}'.format(len(args))
    setattr(types, ryqy__nmlp, types.Tuple([vkzjj__ovth] * len(args)))
    lpfd__ghycm = ', '.join('group{}'.format(feah__msid + 1) for feah__msid in
        range(len(args)))
    ttn__xluf = 'def _match_group_impl(m, *args):\n'
    ttn__xluf += '  ({}) = args\n'.format(lpfd__ghycm)
    ttn__xluf += "  with numba.objmode(out='{}'):\n".format(ryqy__nmlp)
    ttn__xluf += '    out = m.group({})\n'.format(lpfd__ghycm)
    ttn__xluf += '  return out\n'
    haqaa__tjs = {}
    exec(ttn__xluf, globals(), haqaa__tjs)
    izu__ouiay = haqaa__tjs['_match_group_impl']
    return izu__ouiay
...
def overload_match_groups(m, default=None):
    ucnfc__kmlhx = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ucnfc__kmlhx):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(clw__anmd is None for clw__anmd in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=vkzjj__ovth):
