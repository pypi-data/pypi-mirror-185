...
def format_iceberg_conn(conn_str: str) ->str:
    vviwi__vqihk = urlparse(conn_str)
    if not conn_str.startswith('iceberg+glue') and vviwi__vqihk.scheme not in (
...
def get_iceberg_type_info(table_name: str, con: str, database_schema: str,
    ...
    if bodo.get_rank() == 0:
        try:
            uaz__wzc, osij__ctds, acc__unrz = (bodo_iceberg_connector.
                get_iceberg_typing_schema(con, database_schema, table_name))
            if acc__unrz is None:
                raise BodoError('No such Iceberg table found')
        except bodo_iceberg_connector.IcebergError as qhisc__vucj:
            if isinstance(qhisc__vucj, bodo_iceberg_connector.IcebergJavaError
                ) and numba.core.config.DEVELOPER_MODE:
                uaz__wzc = BodoError(
                    f'{qhisc__vucj.message}: {qhisc__vucj.java_error}')
            else:
                uaz__wzc = BodoError(qhisc__vucj.message)
    vbc__jufrj = MPI.COMM_WORLD
    uaz__wzc = vbc__jufrj.bcast(uaz__wzc)
    if isinstance(uaz__wzc, Exception):
        raise uaz__wzc
    col_names = uaz__wzc
    osij__ctds = vbc__jufrj.bcast(osij__ctds)
    acc__unrz = vbc__jufrj.bcast(acc__unrz)
    rbah__gar = [_get_numba_typ_from_pa_typ(typ, False, True, None)[0] for
        typ in osij__ctds]
    if is_merge_into_cow:
        col_names.append('_bodo_row_id')
        rbah__gar.append(types.Array(types.int64, 1, 'C'))
    return col_names, rbah__gar, acc__unrz
...
def get_iceberg_file_list(table_name: str, conn: str, database_schema: str,
    ...
    try:
        hnrw__fbsmr = (bodo_iceberg_connector.
            bodo_connector_get_parquet_file_list(conn, database_schema,
            table_name, filters))
    except bodo_iceberg_connector.IcebergError as qhisc__vucj:
        if isinstance(qhisc__vucj, bodo_iceberg_connector.IcebergJavaError
            ) and numba.core.config.DEVELOPER_MODE:
            raise BodoError(f'{qhisc__vucj.message}:\n{qhisc__vucj.java_error}'
                )
        else:
            raise BodoError(qhisc__vucj.message)
    return hnrw__fbsmr
...
def get_iceberg_pq_dataset(conn: str, database_schema: str, table_name: str,
    ...
    gns__bbwjv = get_row_counts and tracing.is_tracing()
    if gns__bbwjv:
        unrt__vzu = tracing.Event('get_iceberg_pq_dataset')
    vbc__jufrj = MPI.COMM_WORLD
    oofip__zywy = None
    smuqp__mbnyi = None
    ejn__klzb = None
    if bodo.get_rank() == 0:
        if gns__bbwjv:
            zoqx__xpbbs = tracing.Event('get_iceberg_file_list',
                is_parallel=False)
            zoqx__xpbbs.add_attribute('g_dnf_filter', str(dnf_filters))
        try:
            oofip__zywy, ejn__klzb = get_iceberg_file_list(table_name, conn,
                database_schema, dnf_filters)
            if gns__bbwjv:
                iyqm__cvsxy = int(os.environ.get(
                    'BODO_ICEBERG_TRACING_NUM_FILES_TO_LOG', '50'))
                zoqx__xpbbs.add_attribute('num_files', len(oofip__zywy))
                zoqx__xpbbs.add_attribute(f'first_{iyqm__cvsxy}_files',
                    ', '.join(oofip__zywy[:iyqm__cvsxy]))
        except Exception as qhisc__vucj:
            oofip__zywy = qhisc__vucj
        if gns__bbwjv:
            zoqx__xpbbs.finalize()
            snf__ame = tracing.Event('get_snapshot_id', is_parallel=False)
        try:
            smuqp__mbnyi = get_iceberg_snapshot_id(table_name, conn,
                database_schema)
        except Exception as qhisc__vucj:
            smuqp__mbnyi = qhisc__vucj
        if gns__bbwjv:
            snf__ame.finalize()
    oofip__zywy, smuqp__mbnyi, ejn__klzb = vbc__jufrj.bcast((oofip__zywy,
        smuqp__mbnyi, ejn__klzb))
    if isinstance(oofip__zywy, Exception):
        pjabb__vhsm = oofip__zywy
        raise BodoError(
            f"""Error reading Iceberg Table: {type(pjabb__vhsm).__name__}: {str(pjabb__vhsm)}
"""
            )
    if isinstance(smuqp__mbnyi, Exception):
        pjabb__vhsm = smuqp__mbnyi
        raise BodoError(
            f"""Error reading Iceberg Table: {type(pjabb__vhsm).__name__}: {str(pjabb__vhsm)}
"""
            )
    uugsu__qljp: List[str] = oofip__zywy
    snapshot_id: int = smuqp__mbnyi
    if len(uugsu__qljp) == 0:
        pq_dataset = None
    else:
        try:
            pq_dataset = bodo.io.parquet_pio.get_parquet_dataset(uugsu__qljp,
                ...
                =tot_rows_to_read)
        except BodoError as qhisc__vucj:
            if re.search('Schema .* was different', str(qhisc__vucj), re.
                IGNORECASE):
                raise BodoError(
...
{qhisc__vucj}"""
                ...
                raise
    ofee__xfy = IcebergParquetDataset(conn, database_schema, table_name,
        typing_pa_table_schema, ejn__klzb, snapshot_id, pq_dataset)
    if gns__bbwjv:
        unrt__vzu.finalize()
    return ofee__xfy
...
def are_schemas_compatible(pa_schema: pa.Schema, df_schema: pa.Schema,
    ...
    if len(df_schema) < len(pa_schema):
        xlkx__pmjm = []
        for rboiu__psflz in pa_schema:
            fvmy__qjp = df_schema.field_by_name(rboiu__psflz.name)
            if not (fvmy__qjp is None and rboiu__psflz.nullable):
                xlkx__pmjm.append(rboiu__psflz)
        pa_schema = pa.schema(xlkx__pmjm)
    if len(pa_schema) != len(df_schema):
        return False
    for mryi__vcuee in range(len(df_schema)):
        fvmy__qjp = df_schema.field(mryi__vcuee)
        rboiu__psflz = pa_schema.field(mryi__vcuee)
        if fvmy__qjp.equals(rboiu__psflz):
            continue
        yeych__xud = fvmy__qjp.type
        hzuwv__glqay = rboiu__psflz.type
        if not yeych__xud.equals(hzuwv__glqay) and allow_downcasting and (
            pa.types.is_signed_integer(yeych__xud) and pa.types.
            is_signed_integer(hzuwv__glqay) or pa.types.is_floating(
            yeych__xud) and pa.types.is_floating(hzuwv__glqay)
            ) and yeych__xud.bit_width > hzuwv__glqay.bit_width:
            fvmy__qjp = fvmy__qjp.with_type(hzuwv__glqay)
        if not fvmy__qjp.nullable and rboiu__psflz.nullable:
            fvmy__qjp = fvmy__qjp.with_nullable(True)
        elif allow_downcasting and fvmy__qjp.nullable and not rboiu__psflz.nullable:
            fvmy__qjp = fvmy__qjp.with_nullable(False)
        df_schema = df_schema.set(mryi__vcuee, fvmy__qjp)
...
def get_table_details_before_write(table_name: str, conn: str,
    ...
    unrt__vzu = tracing.Event('iceberg_get_table_details_before_write')
    ...
    iceberg_schema_str = ''
    pa_schema = None
    uccx__qbm = {feid__fvkeu: tndhf__gyyto for tndhf__gyyto, feid__fvkeu in
        enumerate(df_schema.names)}
    if vbc__jufrj.Get_rank() == 0:
        try:
            ...
            for lunye__gschm, *qzav__duun in partition_spec:
                assert lunye__gschm in uccx__qbm, f'Iceberg Partition column {lunye__gschm} not found in dataframe'
            for lunye__gschm, *qzav__duun in sort_order:
                assert lunye__gschm in uccx__qbm, f'Iceberg Sort column {lunye__gschm} not found in dataframe'
            partition_spec = [(uccx__qbm[lunye__gschm], *ujy__vqe) for 
                lunye__gschm, *ujy__vqe in partition_spec]
            sort_order = [(uccx__qbm[lunye__gschm], *ujy__vqe) for 
                lunye__gschm, *ujy__vqe in sort_order]
...
"""
            ...
            if iceberg_schema_id is None:
                iceberg_schema_str = connector.pyarrow_to_iceberg_schema_str(
                    df_schema)
        except connector.IcebergError as qhisc__vucj:
            if isinstance(qhisc__vucj, connector.IcebergJavaError
                ) and numba.core.config.DEVELOPER_MODE:
                wocfl__lqy = BodoError(
                    f'{qhisc__vucj.message}: {qhisc__vucj.java_error}')
            else:
                wocfl__lqy = BodoError(qhisc__vucj.message)
        except Exception as qhisc__vucj:
            wocfl__lqy = qhisc__vucj
    wocfl__lqy = vbc__jufrj.bcast(wocfl__lqy)
    if isinstance(wocfl__lqy, Exception):
        raise wocfl__lqy
    table_loc = vbc__jufrj.bcast(table_loc)
    iceberg_schema_id = vbc__jufrj.bcast(iceberg_schema_id)
    partition_spec = vbc__jufrj.bcast(partition_spec)
    sort_order = vbc__jufrj.bcast(sort_order)
    iceberg_schema_str = vbc__jufrj.bcast(iceberg_schema_str)
    pa_schema = vbc__jufrj.bcast(pa_schema)
    if iceberg_schema_id is None:
...
def collect_file_info(iceberg_files_info) ->Tuple[List[str], List[int],
    ...
    from mpi4py import MPI
    vbc__jufrj = MPI.COMM_WORLD
    toioj__hmn = [vpp__luh[0] for vpp__luh in iceberg_files_info]
    etnh__zhe = vbc__jufrj.gather(toioj__hmn)
    fnames = [djd__bcq for hsbdd__qgcki in etnh__zhe for djd__bcq in
        hsbdd__qgcki] if vbc__jufrj.Get_rank() == 0 else None
    paof__npm = np.array([vpp__luh[1] for vpp__luh in iceberg_files_info],
        dtype=np.int64)
    yas__cunh = np.array([vpp__luh[2] for vpp__luh in iceberg_files_info],
        dtype=np.int64)
    kxyw__aofsg = bodo.gatherv(paof__npm).tolist()
    iglh__irny = bodo.gatherv(yas__cunh).tolist()
    return fnames, kxyw__aofsg, iglh__irny
...
def register_table_write(conn_str: str, db_name: str, table_name: str,
    ...
    unrt__vzu = tracing.Event('iceberg_register_table_write')
    import bodo_iceberg_connector
    vbc__jufrj = MPI.COMM_WORLD
    success = False
    if vbc__jufrj.Get_rank() == 0:
        iwk__uaak = None if iceberg_schema_id < 0 else iceberg_schema_id
        success = bodo_iceberg_connector.commit_write(conn_str, db_name,
            table_name, table_loc, fnames, all_metrics, iwk__uaak,
            pa_schema, partition_spec, sort_order, mode)
    success = vbc__jufrj.bcast(success)
    unrt__vzu.finalize()
    return success
...
def register_table_merge_cow(conn_str: str, db_name: str, table_name: str,
    ...
    unrt__vzu = tracing.Event('iceberg_register_table_merge_cow')
    import bodo_iceberg_connector
    vbc__jufrj = MPI.COMM_WORLD
    success = False
    if vbc__jufrj.Get_rank() == 0:
        success = bodo_iceberg_connector.commit_merge_cow(conn_str, db_name,
            ...
            snapshot_id)
    success: bool = vbc__jufrj.bcast(success)
    unrt__vzu.finalize()
    return success
...
def iceberg_pq_write(table_loc, bodo_table, col_names, partition_spec,
    ...
    iceberg_files_info = iceberg_pq_write_table_cpp(unicode_to_utf8(
        ...
        unicode_to_utf8(izjra__wnuc), is_parallel, unicode_to_utf8(
        bucket_region), nsd__qcw, unicode_to_utf8(iceberg_schema_str),
        expected_schema)
...
def iceberg_write(table_name, conn, database_schema, bodo_table, col_names,
    ...
    unrt__vzu = tracing.Event('iceberg_write_py', is_parallel)
    ...
    with numba.objmode(success='bool_'):
        fnames, kxyw__aofsg, iglh__irny = collect_file_info(iceberg_files_info)
        success = register_table_write(conn, database_schema, table_name,
            table_loc, fnames, {'size': iglh__irny, 'record_count':
            kxyw__aofsg}, iceberg_schema_id, df_pyarrow_schema,
...
def iceberg_merge_cow_py(table_name, conn, database_schema, bodo_df,
    ...
    df_pyarrow_schema = bodo.io.helpers.numba_to_pyarrow_schema(bodo_df,
        is_iceberg=True)
    aic__vcx = pd.array(bodo_df.columns)
    if bodo_df.is_table_format:
        qywch__fgpm = bodo_df.table_type
        ...
        def impl(table_name, conn, database_schema, bodo_df, snapshot_id,
            ...
            iceberg_merge_cow(table_name, format_iceberg_conn_njit(conn),
                ...
                pd_dataframe_ext.get_dataframe_table(bodo_df), qywch__fgpm),
                snapshot_id, old_fnames, array_to_info(aic__vcx),
                df_pyarrow_schema, is_parallel)
    else:
        qcqrv__qqcz = ', '.join(
            'array_to_info(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(bodo_df, {}))'
            .format(tndhf__gyyto) for tndhf__gyyto in range(len(bodo_df.
            columns)))
        ewog__hra = 'def impl(\n'
        ewog__hra += '    table_name,\n'
        ewog__hra += '    conn,\n'
        ewog__hra += '    database_schema,\n'
        ewog__hra += '    bodo_df,\n'
        ewog__hra += '    snapshot_id,\n'
        ewog__hra += '    old_fnames,\n'
        ewog__hra += '    is_parallel=False,\n'
        ewog__hra += '):\n'
        ewog__hra += '    info_list = [{}]\n'.format(qcqrv__qqcz)
        ewog__hra += '    table = arr_info_list_to_table(info_list)\n'
        ewog__hra += '    iceberg_merge_cow(\n'
        ewog__hra += '        table_name,\n'
        ewog__hra += '        format_iceberg_conn_njit(conn),\n'
        ewog__hra += '        database_schema,\n'
        ewog__hra += '        table,\n'
        ewog__hra += '        snapshot_id,\n'
        ewog__hra += '        old_fnames,\n'
        ewog__hra += '        array_to_info(col_names_py),\n'
        ewog__hra += '        df_pyarrow_schema,\n'
        ewog__hra += '        is_parallel,\n'
        ewog__hra += '    )\n'
        locals = dict()
        globals = {'bodo': bodo, 'array_to_info': array_to_info,
            ...
            'col_names_py': aic__vcx, 'df_pyarrow_schema': df_pyarrow_schema}
        exec(ewog__hra, globals, locals)
        impl = locals['impl']
    return impl
...
def iceberg_merge_cow(table_name, conn, database_schema, bodo_table,
    ...
    unrt__vzu = tracing.Event('iceberg_merge_cow_py', is_parallel)
    assert is_parallel, 'Iceberg Write only supported for distributed dataframes'
    with numba.objmode(already_exists='bool_', table_loc='unicode_type',
        ...
        (already_exists, table_loc, qzav__duun, partition_spec, sort_order,
    ...
    with numba.objmode(success='bool_'):
        ...
        success = register_table_merge_cow(conn, database_schema,
            table_name, table_loc, old_fnames, fnames, {'size': iglh__irny,
            'record_count': kxyw__aofsg}, snapshot_id)
    if not success:
...
def iceberg_pq_write_table_cpp(typingctx, table_data_loc_t, table_t,
    ...
    def codegen(context, builder, sig, args):
        tih__znby = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(1), lir.IntType(8).as_pointer(), lir.IntType(64), lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer()])
        vnqpr__ihd = cgutils.get_or_insert_function(builder.module,
            tih__znby, name='iceberg_pq_write')
        fyvr__yrun = builder.call(vnqpr__ihd, args)
