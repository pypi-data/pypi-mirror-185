...
def _from_nanos_codegen(c, val):
    jqtkl__cazt = c.pyapi.long_from_longlong(c.builder.udiv(val, lir.
        Constant(lir.IntType(64), _nanos_per_hour)))
    wxne__sydji = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_hour)), lir.Constant(
        lir.IntType(64), _nanos_per_minute)))
    wbawf__pac = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_minute)), lir.
        Constant(lir.IntType(64), _nanos_per_second)))
    sutbh__iqvw = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_second)), lir.
        Constant(lir.IntType(64), _nanos_per_milli)))
    jwwrk__wzp = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_milli)), lir.Constant
        (lir.IntType(64), _nanos_per_micro)))
    yckg__wavq = c.pyapi.long_from_longlong(c.builder.urem(val, lir.
        Constant(lir.IntType(64), _nanos_per_micro)))
    return (jqtkl__cazt, wxne__sydji, wbawf__pac, sutbh__iqvw, jwwrk__wzp,
        yckg__wavq)


@unbox(TimeType)
def unbox_time(typ, val, c):
    jqtkl__cazt = c.pyapi.object_getattr_string(val, 'hour')
    wxne__sydji = c.pyapi.object_getattr_string(val, 'minute')
    wbawf__pac = c.pyapi.object_getattr_string(val, 'second')
    sutbh__iqvw = c.pyapi.object_getattr_string(val, 'millisecond')
    jwwrk__wzp = c.pyapi.object_getattr_string(val, 'microsecond')
    yckg__wavq = c.pyapi.object_getattr_string(val, 'nanosecond')
    hour_ll = c.pyapi.long_as_longlong(jqtkl__cazt)
    minute_ll = c.pyapi.long_as_longlong(wxne__sydji)
    second_ll = c.pyapi.long_as_longlong(wbawf__pac)
    millisecond_ll = c.pyapi.long_as_longlong(sutbh__iqvw)
    microsecond_ll = c.pyapi.long_as_longlong(jwwrk__wzp)
    nanosecond_ll = c.pyapi.long_as_longlong(yckg__wavq)
    pne__smef = _to_nanos_codegen(c, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    c.pyapi.decref(jqtkl__cazt)
    c.pyapi.decref(wxne__sydji)
    c.pyapi.decref(wbawf__pac)
    c.pyapi.decref(sutbh__iqvw)
    c.pyapi.decref(jwwrk__wzp)
    c.pyapi.decref(yckg__wavq)
    rph__nmxk = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(pne__smef, is_error=rph__nmxk)
...
def lower_constant_time(context, builder, ty, pyval):
    ...
    pne__smef = _to_nanos_codegen(context, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    return pne__smef
...
def box_time(typ, val, c):
    (jqtkl__cazt, wxne__sydji, wbawf__pac, sutbh__iqvw, jwwrk__wzp, yckg__wavq
        ) = _from_nanos_codegen(c, val)
    cyrjm__hal = c.pyapi.unserialize(c.pyapi.serialize_object(Time))
    oxl__hzrqc = c.pyapi.call_function_objargs(cyrjm__hal, (jqtkl__cazt,
        wxne__sydji, wbawf__pac, sutbh__iqvw, jwwrk__wzp, yckg__wavq, c.
...
class TimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wsfj__kvdkd = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, wsfj__kvdkd)
...
def unbox_time_array(typ, val, c):
    ...
    sqjd__fyhp = types.Array(types.intp, 1, 'C')
    ffom__gbohz = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        sqjd__fyhp, [n])
    ovra__utumn = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    ornkt__ksr = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(types.uint8, 1, 'C'), [ovra__utumn])
    egwwh__tgsiv = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(64).as_pointer(), lir.
        IntType(8).as_pointer()])
    jkrn__qhzaq = cgutils.get_or_insert_function(c.builder.module,
        egwwh__tgsiv, name='unbox_time_array')
    c.builder.call(jkrn__qhzaq, [val, n, ffom__gbohz.data, ornkt__ksr.data])
    wofxs__dtnth = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    wofxs__dtnth.data = ffom__gbohz._getvalue()
    wofxs__dtnth.null_bitmap = ornkt__ksr._getvalue()
    rph__nmxk = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(wofxs__dtnth._getvalue(), is_error=rph__nmxk)


@box(TimeArrayType)
def box_time_array(typ, val, c):
    znl__lnbh = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    ffom__gbohz = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, znl__lnbh.data)
    nssm__owc = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, znl__lnbh.null_bitmap).data
    n = c.builder.extract_value(ffom__gbohz.shape, 0)
    egwwh__tgsiv = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType(8)])
    bmdup__hjni = cgutils.get_or_insert_function(c.builder.module,
        egwwh__tgsiv, name='box_time_array')
    qth__zma = c.builder.call(bmdup__hjni, [n, ffom__gbohz.data, nssm__owc,
...
def init_time_array(typingctx, data, nulls, precision):
    ...
    def codegen(context, builder, signature, args):
        rhlp__ipps, icqo__myaw, onrbq__wxgdi = args
        kfp__fxwqk = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        kfp__fxwqk.data = rhlp__ipps
        kfp__fxwqk.null_bitmap = icqo__myaw
        context.nrt.incref(builder, signature.args[0], rhlp__ipps)
        context.nrt.incref(builder, signature.args[1], icqo__myaw)
        return kfp__fxwqk._getvalue()
...
def lower_constant_time_arr(context, builder, typ, pyval):
    n = len(pyval)
    ffom__gbohz = np.full(n, 0, np.int64)
    klsm__qasfv = np.empty(n + 7 >> 3, np.uint8)
    for qub__zztn, pctye__dle in enumerate(pyval):
        jtnu__gaxo = pd.isna(pctye__dle)
        bodo.libs.int_arr_ext.set_bit_to_arr(klsm__qasfv, qub__zztn, int(
            not jtnu__gaxo))
        if not jtnu__gaxo:
            ffom__gbohz[qub__zztn] = (pctye__dle.hour * _nanos_per_hour + 
                pctye__dle.minute * _nanos_per_minute + pctye__dle.second *
                _nanos_per_second + pctye__dle.millisecond *
                _nanos_per_milli + pctye__dle.microsecond *
                _nanos_per_micro + pctye__dle.nanosecond)
    tps__fmcr = context.get_constant_generic(builder, data_type, ffom__gbohz)
    pess__ffebp = context.get_constant_generic(builder, nulls_type, klsm__qasfv
        )
    return lir.Constant.literal_struct([tps__fmcr, pess__ffebp])


@numba.njit(no_cpython_wrapper=True)
def alloc_time_array(n, precision):
    ffom__gbohz = np.empty(n, dtype=np.int64)
    nulls = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_time_array(ffom__gbohz, nulls, precision)
...
def time_arr_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            ymz__iwh, oyllv__hkka = array_getitem_bool_index(A, ind)
            return init_time_array(ymz__iwh, oyllv__hkka, precision)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):

        def impl(A, ind):
            ymz__iwh, oyllv__hkka = array_getitem_int_index(A, ind)
            return init_time_array(ymz__iwh, oyllv__hkka, precision)
        return impl
    if isinstance(ind, types.SliceType):

        def impl_slice(A, ind):
            ymz__iwh, oyllv__hkka = array_getitem_slice_index(A, ind)
            return init_time_array(ymz__iwh, oyllv__hkka, precision)
        return impl_slice
...
def create_cmp_op_overload(op):
    ...
    def overload_time_cmp(lhs, rhs):
        if isinstance(lhs, TimeType) and isinstance(rhs, TimeType):
            ...
            def impl(lhs, rhs):
                hobo__cmzjy = cast_time_to_int(lhs)
                xokps__vol = cast_time_to_int(rhs)
                return op(0 if hobo__cmzjy == xokps__vol else 1 if 
                    hobo__cmzjy > xokps__vol else -1, 0)
            return impl
