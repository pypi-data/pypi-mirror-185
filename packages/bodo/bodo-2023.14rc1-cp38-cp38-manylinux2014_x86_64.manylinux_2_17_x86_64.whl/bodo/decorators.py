...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        wtk__lng = self.overloads[sig]
        ziit__ivfk = wtk__lng.metadata.get('distributed_diagnostics', None)
        if ziit__ivfk is None:
            otn__bdot = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(otn__bdot)
        ziit__ivfk.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        nyru__oyigf = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(nyru__oyigf)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        fpwun__moiv = options.pop('distributed')
        pipeline_class = (pipeline_class if fpwun__moiv else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        bhien__ozu = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if bhien__ozu else
            pipeline_class)
    wtm__ebia = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(wtm__ebia, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(wtm__ebia)
        else:
            return master_mode_wrapper(wtm__ebia)
    else:
        return wtm__ebia
