...
class LazyTemporaryDirectory:
    ...
    def __init__(self, ignore_cleanup_errors=False, is_parallel=True):
        ...
        if self.is_parallel:
            pfpli__evamk = MPI.COMM_WORLD
            vqg__zenkz = None
            if pfpli__evamk.Get_rank() == 0:
                vqg__zenkz = str(uuid4())
            vqg__zenkz = pfpli__evamk.bcast(vqg__zenkz)
        else:
            vqg__zenkz = str(uuid4())
        self.name = os.path.join(gettempdir(), vqg__zenkz)
    ...
    def initialize(self):
        if not self.initialized:
            ...
            if self.active_rank:
                ...
                except Exception as vxn__jik:
                    yytv__wvsxv = vxn__jik
            if self.is_parallel:
                pfpli__evamk = MPI.COMM_WORLD
                gavy__hey = isinstance(yytv__wvsxv, Exception)
                ctjkd__cth = pfpli__evamk.allreduce(gavy__hey, op=MPI.LOR)
                if ctjkd__cth:
                    if gavy__hey:
                        raise yytv__wvsxv
                    else:
            ...
            elif isinstance(yytv__wvsxv, Exception):
                raise yytv__wvsxv
            if self.active_rank:
        ...
        def onerror(func, path, exc_info):
            if issubclass(exc_info[0], PermissionError):
                ...
                def resetperms(path):
                    try:
                        os.chflags(path, 0)
                    except AttributeError as yfl__nups:
                        pass
                    os.chmod(path, 448)
                try:
                    if path != name:
                    ...
                    try:
                        os.unlink(path)
                    except (IsADirectoryError, PermissionError) as yfl__nups:
                        cls._rmtree(path, ignore_errors=ignore_errors)
                except FileNotFoundError as yfl__nups:
                    pass
