...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    zue__gmww = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    cadc__snj = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    iauuw__aufz = cgutils.get_or_insert_function(builder.module, cadc__snj,
        name='decimal_to_str')
    builder.call(iauuw__aufz, [val, zue__gmww._get_ptr_by_name('meminfo'),
        zue__gmww._get_ptr_by_name('length'), scale])
    zue__gmww.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    zue__gmww.is_ascii = context.get_constant(types.int32, 1)
    zue__gmww.hash = context.get_constant(numba.cpython.unicode._Py_hash_t, -1)
    zue__gmww.data = context.nrt.meminfo_data(builder, zue__gmww.meminfo)
    zue__gmww.parent = cgutils.get_null_value(zue__gmww.parent.type)
    return zue__gmww._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, lyike__mjl, lyike__mjl = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    cadc__snj = lir.FunctionType(lir.IntType(128), [lir.IntType(8).
        as_pointer()])
    iauuw__aufz = cgutils.get_or_insert_function(builder.module, cadc__snj,
        name='str_to_decimal')
    njbm__nzmn = builder.call(iauuw__aufz, [val])
    return njbm__nzmn
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        evceg__ptofw = cgutils.alloca_once(builder, lir.ArrayType(lir.
            IntType(64), 2))
        builder.store(args[0], builder.bitcast(evceg__ptofw, lir.IntType(
            128).as_pointer()))
        return builder.load(evceg__ptofw)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    ismm__yjt = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, lyike__mjl = args
        cadc__snj = lir.FunctionType(lir.IntType(1), [lir.IntType(128), lir
            .IntType(64), lir.IntType(128), lir.IntType(64)])
        iauuw__aufz = cgutils.get_or_insert_function(builder.module,
            cadc__snj, name=ismm__yjt)
        return builder.call(iauuw__aufz, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            ismm__yjt = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, ismm__yjt)
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    uda__lma = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    vguqz__htmn = [context.get_constant_generic(builder, types.int64, v) for
        v in uda__lma]
    sdrsq__dik = cgutils.pack_array(builder, vguqz__htmn)
    evceg__ptofw = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(sdrsq__dik, builder.bitcast(evceg__ptofw, lir.ArrayType(
...
def unbox_decimal(typ, val, c):
    cadc__snj = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(128).as_pointer()])
    iauuw__aufz = cgutils.get_or_insert_function(c.builder.module,
        cadc__snj, name='unbox_decimal')
    evceg__ptofw = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(iauuw__aufz, [val, evceg__ptofw])
    pjl__ssr = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    rptl__chmi = c.builder.load(evceg__ptofw)
    return NativeValue(rptl__chmi, is_error=pjl__ssr)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    dzhph__pxaq = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    lttfl__towaf = c.pyapi.from_native_value(bodo.string_type, dzhph__pxaq,
        c.env_manager)
    vwkvz__ckqn = c.context.insert_const_string(c.builder.module, 'decimal')
    eoffc__imnfm = c.pyapi.import_module_noblock(vwkvz__ckqn)
    evceg__ptofw = c.pyapi.call_method(eoffc__imnfm, 'Decimal', (lttfl__towaf,)
        )
    c.pyapi.decref(lttfl__towaf)
    c.pyapi.decref(eoffc__imnfm)
    return evceg__ptofw
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        iga__pxiza = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, iga__pxiza)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        uyn__cmbl, pts__tec, lyike__mjl, lyike__mjl = args
        rahva__lvkmi = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        rahva__lvkmi.data = uyn__cmbl
        rahva__lvkmi.null_bitmap = pts__tec
        context.nrt.incref(builder, signature.args[0], uyn__cmbl)
        context.nrt.incref(builder, signature.args[1], pts__tec)
        return rahva__lvkmi._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    wjf__zdhyn = DecimalArrayType(precision, scale)
    bavkp__kafn = wjf__zdhyn(data, null_bitmap, precision_tp, scale_tp)
    return bavkp__kafn, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    dihha__tzll = context.get_constant(types.int64, n)
    giodf__snezs = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [dihha__tzll])
    hmlj__kul = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for jale__oxuyg, wvm__bjfys in enumerate(pyval):
        zslap__goqyu = pd.isna(wvm__bjfys)
        bodo.libs.int_arr_ext.set_bit_to_arr(hmlj__kul, jale__oxuyg, int(
            not zslap__goqyu))
        if not zslap__goqyu:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [giodf__snezs._getvalue(), context.
                get_constant(types.int64, jale__oxuyg), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), wvm__bjfys)])
    psu__mon = context.get_constant_generic(builder, nulls_type, hmlj__kul)
    rahva__lvkmi = context.make_helper(builder, typ)
    rahva__lvkmi.data = giodf__snezs._getvalue()
    rahva__lvkmi.null_bitmap = psu__mon
    return rahva__lvkmi._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    ontlw__zds = np.empty(n, dtype=int128_type)
    jzi__dgwd = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(ontlw__zds, jzi__dgwd, precision, scale)
...
def box_decimal_arr(typ, val, c):
    nuxu__raz = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    ontlw__zds = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, nuxu__raz.data)
    nhbu__sjyq = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, nuxu__raz.null_bitmap).data
    n = c.builder.extract_value(ontlw__zds.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    cadc__snj = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    rxn__nvevo = cgutils.get_or_insert_function(c.builder.module, cadc__snj,
        name='box_decimal_array')
    ptkq__oko = c.builder.call(rxn__nvevo, [n, ontlw__zds.data, nhbu__sjyq,
        scale])
...
def unbox_decimal_arr(typ, val, c):
    rahva__lvkmi = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    kngrh__gby = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(kngrh__gby)
    c.pyapi.decref(kngrh__gby)
    ixeb__szd = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(64
        ), 7)), lir.Constant(lir.IntType(64), 8))
    giodf__snezs = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(int128_type, 1, 'C'), [n])
    wwes__vff = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [ixeb__szd])
    cadc__snj = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(64), lir.IntType(128).as_pointer(), lir.IntType(8).
        as_pointer()])
    iauuw__aufz = cgutils.get_or_insert_function(c.builder.module,
        cadc__snj, name='unbox_decimal_array')
    c.builder.call(iauuw__aufz, [val, n, giodf__snezs.data, wwes__vff.data])
    rahva__lvkmi.null_bitmap = wwes__vff._getvalue()
    rahva__lvkmi.data = giodf__snezs._getvalue()
    pjl__ssr = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(rahva__lvkmi._getvalue(), is_error=pjl__ssr)
...
def decimal_arr_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl(A, ind):
            owao__vvdlk, qig__bwbc = array_getitem_bool_index(A, ind)
            return init_decimal_array(owao__vvdlk, qig__bwbc, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            owao__vvdlk, qig__bwbc = array_getitem_int_index(A, ind)
            return init_decimal_array(owao__vvdlk, qig__bwbc, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            owao__vvdlk, qig__bwbc = array_getitem_slice_index(A, ind)
            return init_decimal_array(owao__vvdlk, qig__bwbc, precision, scale)
        return impl_slice
