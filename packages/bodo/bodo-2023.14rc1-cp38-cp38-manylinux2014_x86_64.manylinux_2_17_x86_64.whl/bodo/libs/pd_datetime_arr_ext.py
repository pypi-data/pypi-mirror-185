...
def box_pd_datetime_tzdtype(typ, val, c):
    xooud__dkt = c.context.insert_const_string(c.builder.module, 'pandas')
    lbf__vaw = c.pyapi.import_module_noblock(xooud__dkt)
    ryw__lpcgn = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    htoki__byseq = c.pyapi.from_native_value(types.unicode_type, ryw__lpcgn,
        c.env_manager)
    if isinstance(typ.tz, str):
        htvzw__dcu = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        zsk__fxp = c.pyapi.from_native_value(types.unicode_type, htvzw__dcu,
            c.env_manager)
    else:
        qfsy__cvv = nanoseconds_to_offset(typ.tz)
        zsk__fxp = c.pyapi.unserialize(c.pyapi.serialize_object(qfsy__cvv))
    lptrq__rycb = c.pyapi.call_method(lbf__vaw, 'DatetimeTZDtype', (
        htoki__byseq, zsk__fxp))
    c.pyapi.decref(htoki__byseq)
    c.pyapi.decref(zsk__fxp)
    c.pyapi.decref(lbf__vaw)
...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        gln__cke = pd.Timedelta(pytz_type._offset).value
    else:
        gln__cke = pytz_type.zone
        if gln__cke not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    ijx__rptx = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(ijx__rptx)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        susvo__zrjrc = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, susvo__zrjrc)
...
def unbox_pd_datetime_array(typ, val, c):
    rmdfu__ulq = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    kwtdh__qpwnb = c.pyapi.string_from_constant_string('datetime64[ns]')
    mknlg__utgrr = c.pyapi.call_method(val, 'to_numpy', (kwtdh__qpwnb,))
    rmdfu__ulq.data = c.unbox(typ.data_array_type, mknlg__utgrr).value
    c.pyapi.decref(mknlg__utgrr)
    hoq__taatj = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(rmdfu__ulq._getvalue(), is_error=hoq__taatj)
...
def box_pd_datetime_array(typ, val, c):
    rmdfu__ulq = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, rmdfu__ulq.data)
    ape__nwtk = c.pyapi.from_native_value(typ.data_array_type, rmdfu__ulq.
        data, c.env_manager)
    ryw__lpcgn = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    htoki__byseq = c.pyapi.from_native_value(types.unicode_type, ryw__lpcgn,
    ...
    xooud__dkt = c.context.insert_const_string(c.builder.module, 'pandas')
    lbf__vaw = c.pyapi.import_module_noblock(xooud__dkt)
    jjdvd__zjt = c.pyapi.call_method(lbf__vaw, 'DatetimeTZDtype', (
        htoki__byseq, zsk__fxp))
    fhc__bltf = c.pyapi.object_getattr_string(lbf__vaw, 'arrays')
    lptrq__rycb = c.pyapi.call_method(fhc__bltf, 'DatetimeArray', (
        ape__nwtk, jjdvd__zjt))
    c.pyapi.decref(ape__nwtk)
    c.pyapi.decref(htoki__byseq)
    c.pyapi.decref(zsk__fxp)
    c.pyapi.decref(lbf__vaw)
    c.pyapi.decref(jjdvd__zjt)
    c.pyapi.decref(fhc__bltf)
    c.context.nrt.decref(c.builder, typ, val)
    return lptrq__rycb
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        pqa__jxjlu = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        pqa__jxjlu.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return pqa__jxjlu._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        htvzw__dcu = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    uwa__jjmtq = DatetimeArrayType(htvzw__dcu)
    sig = uwa__jjmtq(uwa__jjmtq.data_array_type, tz)
    return sig, codegen


@numba.njit(no_cpython_wrapper=True)
def alloc_pd_datetime_array(n, tz):
    sdkhq__ldu = np.empty(n, dtype='datetime64[ns]')
    return init_pandas_datetime_array(sdkhq__ldu, tz)
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    fneae__gdp = context.get_constant_generic(builder, typ.data_array_type,
        pyval.to_numpy('datetime64[ns]'))
    yylpk__rhmre = lir.Constant.literal_struct([fneae__gdp])
    return yylpk__rhmre
...
def overload_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            ...
            hmwlu__sbf = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(hmwlu__sbf, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    baou__autq = args[0]
    if equiv_set.has_shape(baou__autq):
        return ArrayAnalysis.AnalyzeResult(shape=baou__autq, pre=[])
    return None
    ...
    def overload_datetime_arr_cmp(lhs, rhs):
        ...
        if isinstance(lhs, DatetimeArrayType) and (isinstance(rhs,
            ...
            def impl(lhs, rhs):
                ...
                n = len(lhs)
                remm__wxo = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for hgskn__bgoe in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, hgskn__bgoe):
                        bodo.libs.array_kernels.setna(remm__wxo, hgskn__bgoe)
                    else:
                        remm__wxo[hgskn__bgoe] = op(lhs[hgskn__bgoe], rhs)
                return remm__wxo
            return impl
            ...
            def impl(lhs, rhs):
                ...
                remm__wxo = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for hgskn__bgoe in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(rhs, hgskn__bgoe):
                        bodo.libs.array_kernels.setna(remm__wxo, hgskn__bgoe)
                    else:
                        remm__wxo[hgskn__bgoe] = op(lhs, rhs[hgskn__bgoe])
                return remm__wxo
            return impl
            ...
            def impl(lhs, rhs):
                ...
                for hgskn__bgoe in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, hgskn__bgoe
                        ) or bodo.libs.array_kernels.isna(rhs, hgskn__bgoe):
                        bodo.libs.array_kernels.setna(remm__wxo, hgskn__bgoe)
                    else:
                        remm__wxo[hgskn__bgoe] = op(lhs[hgskn__bgoe], rhs[
                            hgskn__bgoe])
    ...
    if isinstance(lhs, DatetimeArrayType):
        if rhs == bodo.week_type:
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                remm__wxo = (bodo.libs.pd_datetime_arr_ext.
                    alloc_pd_datetime_array(n, ctnbl__ifwyp))
                for hgskn__bgoe in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, hgskn__bgoe):
                        bodo.libs.array_kernels.setna(remm__wxo, hgskn__bgoe)
                    else:
                        remm__wxo[hgskn__bgoe] = lhs[hgskn__bgoe] + rhs
                return remm__wxo
            return impl
        else:
            raise BodoError(
        ...
        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            n = len(rhs)
            remm__wxo = bodo.libs.pd_datetime_arr_ext.alloc_pd_datetime_array(n
                , ctnbl__ifwyp)
            for hgskn__bgoe in numba.parfors.parfor.internal_prange(n):
                if bodo.libs.array_kernels.isna(rhs, hgskn__bgoe):
                    bodo.libs.array_kernels.setna(remm__wxo, hgskn__bgoe)
                else:
                    remm__wxo[hgskn__bgoe] = lhs + rhs[hgskn__bgoe]
            return remm__wxo
        return impl
    else:
        raise BodoError(
...
def convert_months_offset_to_days(curr_year, curr_month, curr_day, num_months):
    fcb__mjf = curr_month + num_months - 1
    wugo__nzfpn = fcb__mjf % 12 + 1
    drbev__gmryc = fcb__mjf // 12
    ggh__pmc = curr_year + drbev__gmryc
    jbyy__lsw = bodo.hiframes.pd_timestamp_ext.get_days_in_month(ggh__pmc,
        wugo__nzfpn)
    gaeqj__yoy = min(jbyy__lsw, curr_day)
    bzdz__apzn = pd.Timestamp(year=curr_year, month=curr_month, day=curr_day)
    wpfo__mgq = pd.Timestamp(year=ggh__pmc, month=wugo__nzfpn, day=gaeqj__yoy)
    return wpfo__mgq - bzdz__apzn
