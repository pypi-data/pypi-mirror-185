...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(zzv__hbz.dtype for zzv__hbz in
            self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        uuoil__dxcuj = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, uuoil__dxcuj)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    rhnns__zwps = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        bdrgy__ebiuj, = args
        ghalb__jpgcp = context.make_helper(builder, rhnns__zwps)
        ghalb__jpgcp.data = bdrgy__ebiuj
        context.nrt.incref(builder, data_typ, bdrgy__ebiuj)
        return ghalb__jpgcp._getvalue()
    return rhnns__zwps(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    naxh__jqii = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    bdrgy__ebiuj = naxh__jqii.value
    ghalb__jpgcp = c.context.make_helper(c.builder, typ)
    ghalb__jpgcp.data = bdrgy__ebiuj
    gvhf__gadp = naxh__jqii.is_error
    return NativeValue(ghalb__jpgcp._getvalue(), is_error=gvhf__gadp)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    ghalb__jpgcp = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, ghalb__jpgcp.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        urrk__eagwi = 'def impl(arr, ind):\n'
        kxuzg__xfuss = ','.join(f'get_data(arr._data)[{wfsm__von}][ind]' for
            wfsm__von in range(len(arr.data)))
        urrk__eagwi += f'  return ({kxuzg__xfuss})\n'
        tfoz__omwg = {}
        exec(urrk__eagwi, {'get_data': bodo.libs.struct_arr_ext.get_data},
            tfoz__omwg)
        mpj__xkpz = tfoz__omwg['impl']
        return mpj__xkpz
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        cufj__gootw = len(arr.data)
        urrk__eagwi = 'def impl(arr, ind, val):\n'
        urrk__eagwi += '  data = get_data(arr._data)\n'
        urrk__eagwi += '  null_bitmap = get_null_bitmap(arr._data)\n'
        urrk__eagwi += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for wfsm__von in range(cufj__gootw):
            urrk__eagwi += f'  data[{wfsm__von}][ind] = val[{wfsm__von}]\n'
        tfoz__omwg = {}
        exec(urrk__eagwi, {'get_data': bodo.libs.struct_arr_ext.get_data,
            'get_null_bitmap': bodo.libs.struct_arr_ext.get_null_bitmap,
            'set_bit_to_arr': bodo.libs.int_arr_ext.set_bit_to_arr}, tfoz__omwg
            )
        mpj__xkpz = tfoz__omwg['impl']
        return mpj__xkpz
