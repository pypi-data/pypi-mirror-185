...
def get_constant(func_ir, var, default=NOT_CONSTANT):
    jwp__liuoo = guard(get_definition, func_ir, var)
    if jwp__liuoo is None:
        return default
    if isinstance(jwp__liuoo, ir.Const):
        return jwp__liuoo.value
    if isinstance(jwp__liuoo, ir.Var):
        return get_constant(func_ir, jwp__liuoo, default)
    return default
...
def find_build_tuple(func_ir, var):
    ...
    uuoaz__jjiyg = get_definition(func_ir, var)
    require(isinstance(uuoaz__jjiyg, ir.Expr))
    require(uuoaz__jjiyg.op == 'build_tuple')
    return uuoaz__jjiyg.items
...
def cprint_lower(context, builder, sig, args):
    for fmw__nkoaq, val in enumerate(args):
        typ = sig.args[fmw__nkoaq]
        if isinstance(typ, types.ArrayCTypes):
            cgutils.printf(builder, '%p ', val)
            continue
        bywi__aft = typ_to_format[typ]
        cgutils.printf(builder, '%{} '.format(bywi__aft), val)
...
def is_whole_slice(typemap, func_ir, var, accept_stride=False):
    ...
    eawx__gujjj = get_definition(func_ir, var)
    require(isinstance(eawx__gujjj, ir.Expr) and eawx__gujjj.op == 'call')
    assert len(eawx__gujjj.args) == 2 or accept_stride and len(eawx__gujjj.args
        ) == 3
    assert find_callname(func_ir, eawx__gujjj) == ('slice', 'builtins')
    ygc__dvwo = get_definition(func_ir, eawx__gujjj.args[0])
    sgha__dvxvq = get_definition(func_ir, eawx__gujjj.args[1])
    require(isinstance(ygc__dvwo, ir.Const) and ygc__dvwo.value == None)
    require(isinstance(sgha__dvxvq, ir.Const) and sgha__dvxvq.value == None)
    return True


def is_slice_equiv_arr(arr_var, index_var, func_ir, equiv_set,
    accept_stride=False):
    gkjv__bukt = get_definition(func_ir, index_var)
    require(find_callname(func_ir, gkjv__bukt) == ('slice', 'builtins'))
    require(len(gkjv__bukt.args) in (2, 3))
    require(find_const(func_ir, gkjv__bukt.args[0]) in (0, None))
    require(equiv_set.is_equiv(gkjv__bukt.args[1], arr_var.name + '#0'))
    require(accept_stride or len(gkjv__bukt.args) == 2 or find_const(
        func_ir, gkjv__bukt.args[2]) == 1)
    return True
...
def get_slice_step(typemap, func_ir, var):
    ...
    eawx__gujjj = get_definition(func_ir, var)
    require(isinstance(eawx__gujjj, ir.Expr) and eawx__gujjj.op == 'call')
    assert len(eawx__gujjj.args) == 3
    return eawx__gujjj.args[2]
...
def is_distributable_tuple_typ(var_typ):
    ...
    except ImportError as qdzw__hgsag:
    ...
    return isinstance(var_typ, types.BaseTuple) and any(
        ...
        BodoSQLContextType) and any([is_distributable_typ(umr__cri) for
        umr__cri in var_typ.dataframes])
...
def build_set_seen_na(A):
    ...
    def impl(A):
        s = dict()
        qudc__lqkx = False
        for fmw__nkoaq in range(len(A)):
            if bodo.libs.array_kernels.isna(A, fmw__nkoaq):
                qudc__lqkx = True
                continue
            s[A[fmw__nkoaq]] = 0
        return s, qudc__lqkx
...
def empty_like_type_overload(n, arr):
    ...
    if isinstance(arr, IntegerArrayType):
        rkmtd__lnee = arr.dtype

        def empty_like_type_int_arr(n, arr):
            return bodo.libs.int_arr_ext.alloc_int_array(n, rkmtd__lnee)
        return empty_like_type_int_arr
    if isinstance(arr, FloatingArrayType):
        rkmtd__lnee = arr.dtype

        def empty_like_type_float_arr(n, arr):
            return bodo.libs.float_arr_ext.alloc_float_array(n, rkmtd__lnee)
    ...
    def empty_like_type_str_arr(n, arr):
        mnxlh__muddz = 20
        if len(arr) != 0:
            mnxlh__muddz = num_total_chars(arr) // len(arr)
        return pre_alloc_string_array(n, n * mnxlh__muddz)
    return empty_like_type_str_arr


def _empty_nd_impl(context, builder, arrtype, shapes):
    lby__hdrjz = make_array(arrtype)
    sqcks__yoyog = lby__hdrjz(context, builder)
    mqrxz__urzs = context.get_data_type(arrtype.dtype)
    fzmfv__taxrc = context.get_constant(types.intp, get_itemsize(context,
        arrtype))
    njvu__jwkwq = context.get_constant(types.intp, 1)
    wdpo__tip = lir.Constant(lir.IntType(1), 0)
    for s in shapes:
        szhhq__zdv = builder.smul_with_overflow(njvu__jwkwq, s)
        njvu__jwkwq = builder.extract_value(szhhq__zdv, 0)
        wdpo__tip = builder.or_(wdpo__tip, builder.extract_value(szhhq__zdv, 1)
            )
    if arrtype.ndim == 0:
        agsa__pxho = ()
    elif arrtype.layout == 'C':
        agsa__pxho = [fzmfv__taxrc]
        for pok__bzzp in reversed(shapes[1:]):
            agsa__pxho.append(builder.mul(agsa__pxho[-1], pok__bzzp))
        agsa__pxho = tuple(reversed(agsa__pxho))
    elif arrtype.layout == 'F':
        agsa__pxho = [fzmfv__taxrc]
        for pok__bzzp in shapes[:-1]:
            agsa__pxho.append(builder.mul(agsa__pxho[-1], pok__bzzp))
        agsa__pxho = tuple(agsa__pxho)
    else:
        raise NotImplementedError(
            "Don't know how to allocate array with layout '{0}'.".format(
            arrtype.layout))
    jlqes__gxv = builder.smul_with_overflow(njvu__jwkwq, fzmfv__taxrc)
    ozyor__fvitn = builder.extract_value(jlqes__gxv, 0)
    wdpo__tip = builder.or_(wdpo__tip, builder.extract_value(jlqes__gxv, 1))
    with builder.if_then(wdpo__tip, likely=False):
        cgutils.printf(builder,
            'array is too big; `arr.size * arr.dtype.itemsize` is larger than the maximum possible size.'
            )
    dtype = arrtype.dtype
    klzdc__lrg = context.get_preferred_array_alignment(dtype)
    roz__kcaij = context.get_constant(types.uint32, klzdc__lrg)
    ekno__gqaqp = context.nrt.meminfo_alloc_aligned(builder, size=
        ozyor__fvitn, align=roz__kcaij)
    data = context.nrt.meminfo_data(builder, ekno__gqaqp)
    slr__omnr = context.get_value_type(types.intp)
    ebzq__uglj = cgutils.pack_array(builder, shapes, ty=slr__omnr)
    zwx__wrfx = cgutils.pack_array(builder, agsa__pxho, ty=slr__omnr)
    populate_array(sqcks__yoyog, data=builder.bitcast(data, mqrxz__urzs.
        as_pointer()), shape=ebzq__uglj, strides=zwx__wrfx, itemsize=
        fzmfv__taxrc, meminfo=ekno__gqaqp)
    return sqcks__yoyog
...
def alloc_arr_tup(n, arr_tup, init_vals=()):
    oltai__chntn = []
    for wowhn__vor in arr_tup:
        oltai__chntn.append(np.empty(n, wowhn__vor.dtype))
    return tuple(oltai__chntn)


@overload(alloc_arr_tup, no_unliteral=True)
def alloc_arr_tup_overload(n, data, init_vals=()):
    krzk__kjqb = data.count
    jkny__nan = ','.join(['empty_like_type(n, data[{}])'.format(fmw__nkoaq) for
        fmw__nkoaq in range(krzk__kjqb)])
    if init_vals != ():
        jkny__nan = ','.join(['np.full(n, init_vals[{}], data[{}].dtype)'.
            format(fmw__nkoaq, fmw__nkoaq) for fmw__nkoaq in range(krzk__kjqb)]
            )
    cvh__dhoot = 'def f(n, data, init_vals=()):\n'
    cvh__dhoot += '  return ({}{})\n'.format(jkny__nan, ',' if krzk__kjqb ==
        1 else '')
    axly__kyica = {}
    exec(cvh__dhoot, {'empty_like_type': empty_like_type, 'np': np},
        axly__kyica)
    ydch__btd = axly__kyica['f']
    return ydch__btd
...
def overload_full_type(n, val, t):
    ...
    if typ == string_array_type:
        ...
        def impl_str(n, val, t):
            poele__cizw = n * bodo.libs.str_arr_ext.get_utf8_size(val)
            A = pre_alloc_string_array(n, poele__cizw)
            for fmw__nkoaq in range(n):
                A[fmw__nkoaq] = val
            return A
        return impl_str
...
def is_null_pointer(typingctx, ptr_typ=None):
    ...
    def codegen(context, builder, signature, args):
        vsree__biwcu, = args
        azw__ltnrz = context.get_constant_null(ptr_typ)
        return builder.icmp_unsigned('==', vsree__biwcu, azw__ltnrz)
...
def is_null_value(typingctx, val_typ=None):
    ...
    def codegen(context, builder, signature, args):
        val, = args
        dgyoq__sri = cgutils.alloca_once_value(builder, val)
        tnfbo__wwkjq = cgutils.alloca_once_value(builder, context.
            get_constant_null(val_typ))
        return is_ll_eq(builder, dgyoq__sri, tnfbo__wwkjq)
...
def tuple_list_to_array(A, data, elem_type):
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(elem_type,
        'tuple_list_to_array()')
    cvh__dhoot = 'def impl(A, data, elem_type):\n'
    cvh__dhoot += '  for i, d in enumerate(data):\n'
    if elem_type == bodo.hiframes.pd_timestamp_ext.pd_timestamp_tz_naive_type:
        ...
        cvh__dhoot += '    A[i] = d\n'
    axly__kyica = {}
    exec(cvh__dhoot, {'bodo': bodo}, axly__kyica)
    impl = axly__kyica['impl']
    return impl


def object_length(c, obj):
    zfjc__nvuc = c.context.get_argument_type(types.pyobject)
    ycqxz__mzdiy = lir.FunctionType(lir.IntType(64), [zfjc__nvuc])
    uxob__oupsy = cgutils.get_or_insert_function(c.builder.module,
        ycqxz__mzdiy, name='PyObject_Length')
    return c.builder.call(uxob__oupsy, (obj,))
...
def incref(typingctx, data=None):
    ...
    def codegen(context, builder, signature, args):
        bnlwl__eqdnk, = args
        context.nrt.incref(builder, signature.args[0], bnlwl__eqdnk)
    return types.void(data), codegen


def gen_getitem(out_var, in_var, ind, calltypes, nodes):
    zjqct__tnvh = out_var.loc
    llas__hik = ir.Expr.static_getitem(in_var, ind, None, zjqct__tnvh)
    calltypes[llas__hik] = None
    nodes.append(ir.Assign(llas__hik, out_var, zjqct__tnvh))
...
def get_getsetitem_index_var(node, typemap, nodes):
    ...
    if index_var is None:
        ...
        try:
            vjkqk__vrfxf = types.literal(node.index)
        except:
            vjkqk__vrfxf = numba.typeof(node.index)
        index_var = ir.Var(node.value.scope, ir_utils.mk_unique_var(
            'dummy_index'), node.loc)
        typemap[index_var.name] = vjkqk__vrfxf
...
def sanitize_varname(varname):
    ...
    varname = str(varname)
    bouvy__rge = re.sub('\\W+', '_', varname)
    if not bouvy__rge or not bouvy__rge[0].isalpha():
        bouvy__rge = '_' + bouvy__rge
    if not bouvy__rge.isidentifier() or keyword.iskeyword(bouvy__rge):
        bouvy__rge = mk_unique_var('new_name').replace('.', '_')
    return bouvy__rge
...
def list_reverse(A):
    if isinstance(A, types.List):
        ...
        def impl_reversed(A):
            ptysb__fhp = len(A)
            for fmw__nkoaq in range(ptysb__fhp):
                yield A[ptysb__fhp - 1 - fmw__nkoaq]
...
@numba.njit
def nanvar_ddof1(a):
    eukz__njx = count_nonnan(a)
    if eukz__njx <= 1:
        return np.nan
    return np.nanvar(a) * (eukz__njx / (eukz__njx - 1))
...
def has_supported_h5py():
    try:
        ...
        sjr__ssmri = h5py.version.hdf5_version_tuple[1] in (10, 12)
    return sjr__ssmri
...
def check_and_propagate_cpp_exception(typingctx):
    ...
    def codegen(context, builder, sig, args):
        lmahm__qkikv = context.get_python_api(builder)
        hsmnc__jeui = lmahm__qkikv.err_occurred()
        iwsc__bbv = cgutils.is_not_null(builder, hsmnc__jeui)
        with builder.if_then(iwsc__bbv):
...
def lower_constant_list(context, builder, typ, pyval):
    ...
    for a in pyval:
        ...
        zsof__aju.append(context.get_constant_generic(builder, typ.dtype, a))
    dtly__rse = context.get_constant_generic(builder, types.int64, len(pyval))
    irw__pbmyo = context.get_constant_generic(builder, types.bool_, False)
    fnzyu__bwg = context.get_constant_null(types.pyobject)
    qayh__lcx = lir.Constant.literal_struct([dtly__rse, dtly__rse,
        irw__pbmyo] + zsof__aju)
    qayh__lcx = cgutils.global_constant(builder, '.const.payload', qayh__lcx
        ).bitcast(cgutils.voidptr_t)
    baqvw__jski = context.get_constant(types.int64, -1)
    hrmkj__jmc = context.get_constant_null(types.voidptr)
    ekno__gqaqp = lir.Constant.literal_struct([baqvw__jski, hrmkj__jmc,
        hrmkj__jmc, qayh__lcx, baqvw__jski])
    ekno__gqaqp = cgutils.global_constant(builder, '.const.meminfo',
        ekno__gqaqp).bitcast(cgutils.voidptr_t)
    return lir.Constant.literal_struct([ekno__gqaqp, fnzyu__bwg])
...
def lower_constant_set(context, builder, typ, pyval):
    ...
    yro__avrx = types.List(typ.dtype)
    ldtok__zuw = context.get_constant_generic(builder, yro__avrx, list(pyval))
    goqz__hec = context.compile_internal(builder, lambda l: set(l), types.
        Set(typ.dtype)(yro__avrx), [ldtok__zuw])
    return goqz__hec


def lower_const_dict_fast_path(context, builder, typ, pyval):
    from bodo.utils.typing import can_replace
    dqj__vzvlq = pd.Series(pyval.keys()).values
    lzyu__jfpid = pd.Series(pyval.values()).values
    uhhvy__jcji = bodo.typeof(dqj__vzvlq)
    hmn__tapyf = bodo.typeof(lzyu__jfpid)
    require(uhhvy__jcji.dtype == typ.key_type or can_replace(typ.key_type,
        uhhvy__jcji.dtype))
    require(hmn__tapyf.dtype == typ.value_type or can_replace(typ.
        value_type, hmn__tapyf.dtype))
    xcjs__rgymj = context.get_constant_generic(builder, uhhvy__jcji, dqj__vzvlq
        )
    kbety__gws = context.get_constant_generic(builder, hmn__tapyf, lzyu__jfpid)
    ...
    def create_dict(keys, vals):
        ...
        for k, v in zip(keys, vals):
            artvn__lfpaa[k] = v
        return artvn__lfpaa
    jcnrh__hpk = context.compile_internal(builder, create_dict, typ(
        uhhvy__jcji, hmn__tapyf), [xcjs__rgymj, kbety__gws])
    return jcnrh__hpk
...
def lower_constant_dict(context, builder, typ, pyval):
    ...
    ykfe__yyj = typ.key_type
    unyj__hshk = typ.value_type

    def make_dict():
        return numba.typed.Dict.empty(ykfe__yyj, unyj__hshk)
    jcnrh__hpk = context.compile_internal(builder, make_dict, typ(), [])

    def set_dict_val(d, k, v):
        d[k] = v
    for k, v in pyval.items():
        lmf__yrccw = context.get_constant_generic(builder, ykfe__yyj, k)
        wqn__zksza = context.get_constant_generic(builder, unyj__hshk, v)
        context.compile_internal(builder, set_dict_val, types.none(typ,
            ykfe__yyj, unyj__hshk), [jcnrh__hpk, lmf__yrccw, wqn__zksza])
    return jcnrh__hpk


def synchronize_error(exception_str, error_message):
    if exception_str == 'ValueError':
        nmc__ttvit = ValueError
    else:
        nmc__ttvit = RuntimeError
    jtvf__trhqg = MPI.COMM_WORLD
    if jtvf__trhqg.allreduce(error_message != '', op=MPI.LOR):
        for error_message in jtvf__trhqg.allgather(error_message):
            if error_message:
                raise nmc__ttvit(error_message)
