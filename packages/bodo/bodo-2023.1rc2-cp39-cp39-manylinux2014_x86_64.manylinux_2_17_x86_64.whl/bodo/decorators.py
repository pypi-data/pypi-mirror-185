...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        ict__ymqkh = self.overloads[sig]
        jrxz__ayq = ict__ymqkh.metadata.get('distributed_diagnostics', None)
        if jrxz__ayq is None:
            oaawx__lmnqs = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(oaawx__lmnqs)
        jrxz__ayq.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        gfsu__cdsaj = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(gfsu__cdsaj)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        qjtpu__cztic = options.pop('distributed')
        pipeline_class = (pipeline_class if qjtpu__cztic else bodo.compiler
            .BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        emp__fqniq = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if emp__fqniq else
            pipeline_class)
    tzhty__wwntf = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(tzhty__wwntf, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(tzhty__wwntf)
        else:
            return master_mode_wrapper(tzhty__wwntf)
    else:
        return tzhty__wwntf
