...
def overload_coerce_to_ndarray(data, error_on_nonarray=True,
    ...
    if isinstance(data, (types.List, types.UniTuple)):
        amxoo__psh = data.dtype
        if isinstance(amxoo__psh, types.Optional):
            amxoo__psh = amxoo__psh.type
            if bodo.utils.typing.is_scalar_type(amxoo__psh):
                use_nullable_array = True
        pju__zlt = dtype_to_array_type(amxoo__psh)
        if not is_overload_none(use_nullable_array):
            pju__zlt = to_nullable_type(pju__zlt)

        def impl(data, error_on_nonarray=True, use_nullable_array=None,
            scalar_to_arr_len=None):
            kto__zxnys = len(data)
            A = bodo.utils.utils.alloc_type(kto__zxnys, pju__zlt, (-1,))
            bodo.utils.utils.tuple_list_to_array(A, data, amxoo__psh)
            return A
        return impl
    ...
    if not is_overload_none(scalar_to_arr_len):
        if isinstance(data, Decimal128Type):
            dcld__elq = data.precision
            omfkv__jjzz = data.scale

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                kto__zxnys = scalar_to_arr_len
                A = bodo.libs.decimal_arr_ext.alloc_decimal_array(kto__zxnys,
                    dcld__elq, omfkv__jjzz)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    A[zgps__gosfh] = data
                return A
            return impl_ts
        if data == bodo.hiframes.datetime_datetime_ext.datetime_datetime_type:
            vkbua__luh = np.dtype('datetime64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                kto__zxnys = scalar_to_arr_len
                A = np.empty(kto__zxnys, vkbua__luh)
                namx__alooe = (bodo.hiframes.pd_timestamp_ext.
                    datetime_datetime_to_dt64(data))
                iyl__hmo = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
                    namx__alooe)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    A[zgps__gosfh] = iyl__hmo
                return A
            return impl_ts
        if (data == bodo.hiframes.datetime_timedelta_ext.
            datetime_timedelta_type):
            dgyns__bzbol = np.dtype('timedelta64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                kto__zxnys = scalar_to_arr_len
                A = np.empty(kto__zxnys, dgyns__bzbol)
                omwxf__iiwz = (bodo.hiframes.pd_timestamp_ext.
                    datetime_timedelta_to_timedelta64(data))
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    A[zgps__gosfh] = omwxf__iiwz
                return A
            return impl_ts
        ...
        if isinstance(data, bodo.hiframes.time_ext.TimeType):
            ...
            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                kto__zxnys = scalar_to_arr_len
                A = bodo.hiframes.time_ext.alloc_time_array(kto__zxnys,
                    dcld__elq)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    A[zgps__gosfh] = data
        ...
        if data == bodo.hiframes.pd_timestamp_ext.pd_timestamp_tz_naive_type:
            ...
            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                kto__zxnys = scalar_to_arr_len
                A = np.empty(scalar_to_arr_len, vkbua__luh)
                namx__alooe = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
                    data.value)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    A[zgps__gosfh] = namx__alooe
                return A
            return impl_ts
        ...
        if not is_overload_none(use_nullable_array) and isinstance(dtype,
            ...
            def impl_null_integer(data, error_on_nonarray=True,
                ...
                kto__zxnys = scalar_to_arr_len
                rvd__ysgxp = bodo.libs.int_arr_ext.alloc_int_array(kto__zxnys,
                    dtype)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    rvd__ysgxp[zgps__gosfh] = data
                return rvd__ysgxp
        ...
        if not is_overload_none(use_nullable_array) and isinstance(dtype,
            ...
            def impl_null_float(data, error_on_nonarray=True,
                ...
                numba.parfors.parfor.init_prange()
                kto__zxnys = scalar_to_arr_len
                rvd__ysgxp = bodo.libs.float_arr_ext.alloc_float_array(
                    kto__zxnys, dtype)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    rvd__ysgxp[zgps__gosfh] = data
        ...
        if not is_overload_none(use_nullable_array) and dtype == types.bool_:
            ...
            def impl_null_bool(data, error_on_nonarray=True,
                ...
                numba.parfors.parfor.init_prange()
                kto__zxnys = scalar_to_arr_len
                rvd__ysgxp = bodo.libs.bool_arr_ext.alloc_bool_array(kto__zxnys
                    )
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    rvd__ysgxp[zgps__gosfh] = data
        ...
        def impl_num(data, error_on_nonarray=True, use_nullable_array=None,
            ...
            numba.parfors.parfor.init_prange()
            kto__zxnys = scalar_to_arr_len
            rvd__ysgxp = np.empty(kto__zxnys, dtype)
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                rvd__ysgxp[zgps__gosfh] = data
            return rvd__ysgxp
        return impl_num
    if isinstance(data, types.BaseTuple) and all(isinstance(gvc__tpnz, (
        types.Float, types.Integer)) for gvc__tpnz in data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
...
def overload_coerce_scalar_to_array(scalar, length, arr_type):
    esx__qibx = to_nullable_type(unwrap_typeref(arr_type))
    if scalar == types.none:

        def impl(scalar, length, arr_type):
            return bodo.libs.array_kernels.gen_na_array(length, esx__qibx, True
                )
...
def overload_coerce_to_array(data, error_on_nonarray=True,
    ...
    if isinstance(data, types.UniTuple) and isinstance(data.dtype, (types.
        ...
        ) and all(isinstance(gvc__tpnz, types.StringLiteral) for gvc__tpnz in
        data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
            None, scalar_to_arr_len=None: bodo.libs.str_arr_ext.
            str_arr_from_sequence(data))
    ...
    if isinstance(data, (types.List, types.UniTuple)) and isinstance(data.
        ...
        mcaq__zhvz = tuple(dtype_to_array_type(gvc__tpnz) for gvc__tpnz in
            data.dtype.types)

        def impl_tuple_list(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            kto__zxnys = len(data)
            arr = bodo.libs.tuple_arr_ext.pre_alloc_tuple_array(kto__zxnys,
                (-1,), mcaq__zhvz)
            for zgps__gosfh in range(kto__zxnys):
                arr[zgps__gosfh] = data[zgps__gosfh]
            return arr
    ...
    if isinstance(data, types.List) and (bodo.utils.utils.is_array_typ(data
        ...
        vqen__biut = dtype_to_array_type(data.dtype.dtype)

        def impl_array_item_arr(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            kto__zxnys = len(data)
            qwd__cmems = init_nested_counts(vqen__biut)
            for zgps__gosfh in range(kto__zxnys):
                ujve__lkvq = bodo.utils.conversion.coerce_to_array(data[
                    zgps__gosfh], use_nullable_array=True)
                qwd__cmems = add_nested_counts(qwd__cmems, ujve__lkvq)
            rvd__ysgxp = (bodo.libs.array_item_arr_ext.
                pre_alloc_array_item_array(kto__zxnys, qwd__cmems, vqen__biut))
            uoptq__xzr = bodo.libs.array_item_arr_ext.get_null_bitmap(
                rvd__ysgxp)
            for uxzpk__itv in range(kto__zxnys):
                ujve__lkvq = bodo.utils.conversion.coerce_to_array(data[
                    uxzpk__itv], use_nullable_array=True)
                rvd__ysgxp[uxzpk__itv] = ujve__lkvq
                bodo.libs.int_arr_ext.set_bit_to_arr(uoptq__xzr, uxzpk__itv, 1)
            return rvd__ysgxp
    ...
    if not is_overload_none(scalar_to_arr_len) and isinstance(data, (types.
        ...
        def impl_str(data, error_on_nonarray=True, use_nullable_array=None,
            scalar_to_arr_len=None):
            kto__zxnys = scalar_to_arr_len
            imye__mxy = bodo.libs.str_arr_ext.str_arr_from_sequence([data])
            ylfz__quuq = bodo.libs.int_arr_ext.alloc_int_array(kto__zxnys,
                np.int32)
            numba.parfors.parfor.init_prange()
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                ylfz__quuq[zgps__gosfh] = 0
            A = bodo.libs.dict_arr_ext.init_dict_arr(imye__mxy, ylfz__quuq,
                True, True)
            return A
        return impl_str
    if isinstance(data, types.List) and isinstance(data.dtype, bodo.
        ...
        def impl_list_timestamp(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            kto__zxnys = len(data)
            A = np.empty(kto__zxnys, np.dtype('datetime64[ns]'))
            for zgps__gosfh in range(kto__zxnys):
                A[zgps__gosfh
    ...
    if isinstance(data, types.List) and data.dtype == bodo.pd_timedelta_type:
        ...
        def impl_list_timedelta(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            kto__zxnys = len(data)
            A = np.empty(kto__zxnys, np.dtype('timedelta64[ns]'))
            for zgps__gosfh in range(kto__zxnys):
                A[zgps__gosfh
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    data[zgps__gosfh].value)
            return A
    ...
    if isinstance(data, bodo.hiframes.pd_timestamp_ext.PandasTimestampType
        ...
        def impl_timestamp_tz_aware(data, error_on_nonarray=True,
            ...
            kmswm__atl = data.to_datetime64()
            for zgps__gosfh in numba.parfors.parfor.internal_prange(
                scalar_to_arr_len):
                A[zgps__gosfh] = kmswm__atl
...
def overload_fix_arr_dtype(data, new_dtype, copy=None, nan_to_str=True,
    ...
    if is_overload_none(new_dtype):
        ...
        lfq__afa = new_dtype.instance_type == data
    if not lfq__afa:
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(data,
            'fix_arr_dtype()')
    nmm__rppsh = is_overload_true(copy)
    bcfv__puls = is_overload_constant_str(new_dtype
        ) and get_overload_const_str(new_dtype) == 'object'
    if is_overload_none(new_dtype) or bcfv__puls:
        if nmm__rppsh:
    ...
    if isinstance(data, NullableTupleType):
        ...
        rxwgp__fannj = {types.unicode_type: '', boolean_dtype: False, types
        ...
        haz__opijh = {types.unicode_type: str, types.bool_: bool,
            ...
            pd.to_timedelta}
        opcz__brdbg = rxwgp__fannj.keys()
        ugw__vcrms = list(data._tuple_typ.types)
        if nb_dtype not in opcz__brdbg:
            raise BodoError(f'type conversion to {nb_dtype} types unsupported.'
                )
        for ixe__fchcq in ugw__vcrms:
            if ixe__fchcq == bodo.datetime64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
                    uint64, bodo.datetime64ns):
                    raise BodoError(
                        f'invalid type conversion from {ixe__fchcq} to {nb_dtype}.'
                        )
            elif ixe__fchcq == bodo.timedelta64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
        ...
        tqq__zwhi += '  data_tup = data._data\n'
        tqq__zwhi += '  null_tup = data._null_values\n'
        for zgps__gosfh in range(len(ugw__vcrms)):
            tqq__zwhi += f'  val_{zgps__gosfh} = convert_func(default_value)\n'
            tqq__zwhi += f'  if not null_tup[{zgps__gosfh}]:\n'
            tqq__zwhi += (
                f'    val_{zgps__gosfh} = convert_func(data_tup[{zgps__gosfh}])\n'
                )
        diydt__rvsvy = ', '.join(f'val_{zgps__gosfh}' for zgps__gosfh in
            range(len(ugw__vcrms)))
        tqq__zwhi += f'  vals_tup = ({diydt__rvsvy},)\n'
        tqq__zwhi += """  res_tup = bodo.libs.nullable_tuple_ext.build_nullable_tuple(vals_tup, null_tup)
"""
        tqq__zwhi += '  return res_tup\n'
        bzypp__dsx = {}
        ddxx__seess = haz__opijh[nb_dtype]
        iivk__gar = rxwgp__fannj[nb_dtype]
        exec(tqq__zwhi, {'bodo': bodo, 'np': np, 'pd': pd, 'default_value':
            iivk__gar, 'convert_func': ddxx__seess}, bzypp__dsx)
        impl = bzypp__dsx['impl']
        return impl
    if _is_str_dtype(new_dtype):
        if isinstance(data.dtype, types.Integer):
            ...
            def impl_int_str(data, new_dtype, copy=None, nan_to_str=True,
                ...
                numba.parfors.parfor.init_prange()
                kto__zxnys = len(data)
                A = bodo.libs.str_arr_ext.pre_alloc_string_array(kto__zxnys, -1
                    )
                for sdggy__jlhit in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    if bodo.libs.array_kernels.isna(data, sdggy__jlhit):
                        if nan_to_str:
                            bodo.libs.str_arr_ext.str_arr_setitem_NA_str(A,
                                sdggy__jlhit)
                        else:
                            bodo.libs.array_kernels.setna(A, sdggy__jlhit)
                    else:
                        bodo.libs.str_arr_ext.str_arr_setitem_int_to_str(A,
                            sdggy__jlhit, data[sdggy__jlhit])
                return A
            return impl_int_str
            ...
            def impl_binary(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for sdggy__jlhit in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, sdggy__jlhit):
                        ...
                        A[sdggy__jlhit] = ''.join([chr(oltzr__orcli) for
                            oltzr__orcli in data[sdggy__jlhit]])
                return A
            return impl_binary
                ...
                for sdggy__jlhit in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, sdggy__jlhit):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, sdggy__jlhit)
                        continue
                    A[sdggy__jlhit] = str(box_if_dt64(data[sdggy__jlhit]))
                return A
                ...
                for sdggy__jlhit in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, sdggy__jlhit):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, sdggy__jlhit)
                        continue
                    A[sdggy__jlhit] = str(data[sdggy__jlhit])
                return A
    ...
    if isinstance(new_dtype, bodo.hiframes.pd_categorical_ext.
        ...
        def impl_cat_dtype(data, new_dtype, copy=None, nan_to_str=True,
            ...
            kto__zxnys = len(data)
            numba.parfors.parfor.init_prange()
            jznit__kyzq = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories(new_dtype.categories.values))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                kto__zxnys, new_dtype)
            fksh__ccqq = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                    bodo.libs.array_kernels.setna(A, zgps__gosfh)
                    continue
                val = data[zgps__gosfh]
                if val not in jznit__kyzq:
                    bodo.libs.array_kernels.setna(A, zgps__gosfh)
                    continue
                fksh__ccqq[zgps__gosfh] = jznit__kyzq[val]
            return A
    ...
    if is_overload_constant_str(new_dtype) and get_overload_const_str(new_dtype
        ...
        def impl_category(data, new_dtype, copy=None, nan_to_str=True,
            from_series=False):
            zyvfh__kvylz = bodo.libs.array_kernels.unique(data, dropna=True)
            zyvfh__kvylz = pd.Series(zyvfh__kvylz).sort_values().values
            zyvfh__kvylz = bodo.allgatherv(zyvfh__kvylz, False)
            quca__lxlr = bodo.hiframes.pd_categorical_ext.init_cat_dtype(bodo
                .utils.conversion.index_from_array(zyvfh__kvylz, None), 
                False, None, None)
            kto__zxnys = len(data)
            numba.parfors.parfor.init_prange()
            jznit__kyzq = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories_no_duplicates(zyvfh__kvylz))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                kto__zxnys, quca__lxlr)
            fksh__ccqq = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
    ...
    nb_dtype = bodo.utils.typing.parse_dtype(new_dtype)
    if isinstance(data, bodo.libs.int_arr_ext.IntegerArrayType):
        ezzav__vhht = isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype
            ) and data.dtype == nb_dtype.dtype
    elif isinstance(data, bodo.libs.float_arr_ext.FloatingArrayType):
        ezzav__vhht = isinstance(nb_dtype, bodo.libs.float_arr_ext.FloatDtype
            ) and data.dtype == nb_dtype.dtype
    elif bodo.utils.utils.is_array_typ(nb_dtype, False):
        ezzav__vhht = data == nb_dtype
    else:
        ezzav__vhht = data.dtype == nb_dtype
    if nmm__rppsh and ezzav__vhht:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
            from_series=False: data.copy())
    if ezzav__vhht:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype):
        ...
        if isinstance(data.dtype, types.Float):
            ...
            def impl_float(data, new_dtype, copy=None, nan_to_str=True,
                ...
                kto__zxnys = len(data)
                numba.parfors.parfor.init_prange()
                inae__xrtof = bodo.libs.int_arr_ext.alloc_int_array(kto__zxnys,
                    unobr__kdrw)
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                        bodo.libs.array_kernels.setna(inae__xrtof, zgps__gosfh)
                    else:
                        inae__xrtof[zgps__gosfh] = int(data[zgps__gosfh])
                return inae__xrtof
            return impl_float
        else:
            ...
            if isinstance(data, bodo.hiframes.time_ext.TimeArrayType):
                ...
                def impl(data, new_dtype, copy=None, nan_to_str=True,
                    ...
                    kto__zxnys = len(data)
                    numba.parfors.parfor.init_prange()
                    inae__xrtof = bodo.libs.int_arr_ext.alloc_int_array(
                        kto__zxnys, unobr__kdrw)
                    for zgps__gosfh in numba.parfors.parfor.internal_prange(
                        kto__zxnys):
                        if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                            bodo.libs.array_kernels.setna(inae__xrtof,
                                zgps__gosfh)
                        else:
                            inae__xrtof[zgps__gosfh] = cast_time_to_int(data
                                [zgps__gosfh])
                    return inae__xrtof
                return impl
            ...
            def impl(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                        ...
                        inae__xrtof[zgps__gosfh] = np.int64(data[zgps__gosfh])
                return inae__xrtof
            return impl
    if isinstance(nb_dtype, bodo.libs.float_arr_ext.FloatDtype):
        ...
        def impl(data, new_dtype, copy=None, nan_to_str=True, from_series=False
            ...
            kto__zxnys = len(data)
            numba.parfors.parfor.init_prange()
            inae__xrtof = bodo.libs.float_arr_ext.alloc_float_array(kto__zxnys,
                unobr__kdrw)
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                    ...
                    inae__xrtof[zgps__gosfh] = float(data[zgps__gosfh])
            return inae__xrtof
        return impl
    ...
    if nb_dtype == bodo.libs.bool_arr_ext.boolean_dtype:
        ...
        def impl_bool(data, new_dtype, copy=None, nan_to_str=True,
            ...
            kto__zxnys = len(data)
            numba.parfors.parfor.init_prange()
            inae__xrtof = bodo.libs.bool_arr_ext.alloc_bool_array(kto__zxnys)
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                    bodo.libs.array_kernels.setna(inae__xrtof, zgps__gosfh)
                else:
                    inae__xrtof[zgps__gosfh] = bool(data[zgps__gosfh])
            return inae__xrtof
        return impl_bool
    if nb_dtype == bodo.datetime_date_type:
        if data.dtype == bodo.datetime64ns:
            ...
            def impl_date(data, new_dtype, copy=None, nan_to_str=True,
                from_series=False):
                kto__zxnys = len(data)
                rvd__ysgxp = (bodo.hiframes.datetime_date_ext.
                    alloc_datetime_date_array(kto__zxnys))
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                        bodo.libs.array_kernels.setna(rvd__ysgxp, zgps__gosfh)
                    else:
                        rvd__ysgxp[zgps__gosfh
    ...
    if nb_dtype == bodo.datetime64ns:
        ...
        if isinstance(data.dtype, types.Number) or data.dtype in [bodo.
            ...
            def impl_numeric(data, new_dtype, copy=None, nan_to_str=True,
                ...
                kto__zxnys = len(data)
                numba.parfors.parfor.init_prange()
                rvd__ysgxp = np.empty(kto__zxnys, dtype=np.dtype(
                    'datetime64[ns]'))
                for zgps__gosfh in numba.parfors.parfor.internal_prange(
                    kto__zxnys):
                    if bodo.libs.array_kernels.isna(data, zgps__gosfh):
            ...
            if nmm__rppsh:
                ...
                def impl_numeric(data, new_dtype, copy=None, nan_to_str=
                    ...
                    for zgps__gosfh in numba.parfors.parfor.internal_prange(
                        kto__zxnys):
                        if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                            bodo.libs.array_kernels.setna(rvd__ysgxp,
                                zgps__gosfh)
                        else:
                            rvd__ysgxp[zgps__gosfh] = (bodo.hiframes.
                                pd_timestamp_ext.integer_to_timedelta64(np.
                                int64(data[zgps__gosfh])))
                    return rvd__ysgxp
                return impl_numeric
            else:
                return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if nb_dtype == types.int64 and data.dtype in [bodo.datetime64ns, bodo.
        ...
        def impl_datelike_to_integer(data, new_dtype, copy=None, nan_to_str
            ...
            kto__zxnys = len(data)
            numba.parfors.parfor.init_prange()
            A = np.empty(kto__zxnys, types.int64)
            for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys
                ):
                if bodo.libs.array_kernels.isna(data, zgps__gosfh):
                    bodo.libs.array_kernels.setna(A, zgps__gosfh)
                else:
                    A[zgps__gosfh] = np.int64(data[zgps__gosfh])
            return A
...
def flatten_array(A):
    ...
    kto__zxnys = len(A)
    for zgps__gosfh in range(kto__zxnys):
        gwrdh__uhfak = A[zgps__gosfh]
        for copo__eifit in gwrdh__uhfak:
            uela__pbk.append(copo__eifit)
    return bodo.utils.conversion.coerce_to_array(uela__pbk)
...
def overload_parse_datetimes_from_strings(data):
    ...
    def parse_impl(data):
        numba.parfors.parfor.init_prange()
        kto__zxnys = len(data)
        iuy__phgm = np.empty(kto__zxnys, bodo.utils.conversion.NS_DTYPE)
        for zgps__gosfh in numba.parfors.parfor.internal_prange(kto__zxnys):
            iuy__phgm[zgps__gosfh
...
def overload_convert_to_index(data, name=None):
    ...
    def impl(data, name=None):
        diup__tdhbi = bodo.utils.conversion.coerce_to_array(data)
        return bodo.utils.conversion.index_from_array(diup__tdhbi, name)
    return impl
    ...
    if val == types.Optional(bodo.hiframes.pd_timestamp_ext.
        ...
        def impl_optional(val):
            if val is None:
                ...
                ffg__wbwd = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(bodo
    ...
    if val == types.Optional(bodo.hiframes.datetime_timedelta_ext.
        ...
        def impl_optional_td(val):
            if val is None:
                ...
                ffg__wbwd = (bodo.hiframes.pd_timestamp_ext.
...
def overload_to_tuple(val):
    if not isinstance(val, types.BaseTuple) and is_overload_constant_list(val):
        xtdxi__prz = len(val.types if isinstance(val, types.LiteralList) else
            get_overload_const_list(val))
        tqq__zwhi = 'def f(val):\n'
        sphrv__xnlm = ','.join(f'val[{zgps__gosfh}]' for zgps__gosfh in
            range(xtdxi__prz))
        tqq__zwhi += f'  return ({sphrv__xnlm},)\n'
        bzypp__dsx = {}
        exec(tqq__zwhi, {}, bzypp__dsx)
        impl = bzypp__dsx['f']
        return impl
...
def overload_get_array_if_series_or_index(data):
    ...
    if isinstance(data, bodo.hiframes.pd_index_ext.HeterogeneousIndexType):
        if not is_heterogeneous_tuple_type(data.data):
            ...
            def impl(data):
                ydsgw__kqcs = bodo.hiframes.pd_index_ext.get_index_data(data)
                return bodo.utils.conversion.coerce_to_array(ydsgw__kqcs)
            return impl
...
def overload_extract_index_array(A):
    ...
    if isinstance(A, SeriesType):
        ...
        def impl(A):
            ...
            tkul__wefa = bodo.utils.conversion.coerce_to_array(index)
            return tkul__wefa
        return impl
...
def struct_if_heter_dict(values, names):
    return {mao__ovsrn: namx__alooe for mao__ovsrn, namx__alooe in zip(
        names, values)}
...
def overload_struct_if_heter_dict(values, names):
    if not types.is_homogeneous(*values.types):
        return lambda values, names: bodo.libs.struct_arr_ext.init_struct(
            values, names)
    ieby__iei = len(values.types)
    tqq__zwhi = 'def f(values, names):\n'
    sphrv__xnlm = ','.join("'{}': values[{}]".format(get_overload_const_str
        (names.types[zgps__gosfh]), zgps__gosfh) for zgps__gosfh in range(
        ieby__iei))
    tqq__zwhi += '  return {{{}}}\n'.format(sphrv__xnlm)
    bzypp__dsx = {}
    exec(tqq__zwhi, {}, bzypp__dsx)
...
def overload_nullable_bool_to_bool_na_false(arr):
    if arr == bodo.boolean_array:
        ...
        def impl(arr):
            lxlvj__ods = bodo.libs.bool_arr_ext.get_bool_arr_data(arr)
            for zgps__gosfh in range(len(arr)):
                lxlvj__ods[zgps__gosfh] = lxlvj__ods[zgps__gosfh
                    ] and not bodo.libs.array_kernels.isna(arr, zgps__gosfh)
            return lxlvj__ods
        return impl
    else:
        return lambda arr: arr
