...
def box_regex_flag(typ, val, c):
    jomqp__krfjx = c.box(types.int64, val)
    imjo__sfjs = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(imjo__sfjs, (jomqp__krfjx,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    jomqp__krfjx = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, jomqp__krfjx)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    yvfoc__wez = context.get_python_api(builder)
    zfmfd__ffu = builder.icmp_signed('==', val, yvfoc__wez.borrow_none())
    with builder.if_then(zfmfd__ffu):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    rrcod__zgtv = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(rrcod__zgtv)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        oxdp__lul = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if oxdp__lul > 1:
            typ = types.List(types.Tuple([string_type] * oxdp__lul))
...
def overload_match_group(m, *args):
    ...
    ozht__cquyy = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            mikn__oskb = args[0]
            with numba.objmode(out=ozht__cquyy):
                out = m.group(mikn__oskb)
            return out
        return _match_group_impl_one
    kuczm__bsprq = 'tuple_str_{}'.format(len(args))
    setattr(types, kuczm__bsprq, types.Tuple([ozht__cquyy] * len(args)))
    xyo__qxlse = ', '.join('group{}'.format(itm__mgida + 1) for itm__mgida in
        range(len(args)))
    utfr__cfu = 'def _match_group_impl(m, *args):\n'
    utfr__cfu += '  ({}) = args\n'.format(xyo__qxlse)
    utfr__cfu += "  with numba.objmode(out='{}'):\n".format(kuczm__bsprq)
    utfr__cfu += '    out = m.group({})\n'.format(xyo__qxlse)
    utfr__cfu += '  return out\n'
    hil__evyf = {}
    exec(utfr__cfu, globals(), hil__evyf)
    jlni__duo = hil__evyf['_match_group_impl']
    return jlni__duo
...
def overload_match_groups(m, default=None):
    ttxag__qflas = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ttxag__qflas):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(tylhu__njqa is None for tylhu__njqa in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=ozht__cquyy):
