...
def _install_mpl_types():
    mew__mtpwl = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for eubeu__sph, nhiwh__djh in mew__mtpwl:
        install_py_obj_class(types_name=eubeu__sph, python_type=nhiwh__djh,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    souv__hst = ', '.join(f'e{nso__edrv}' for nso__edrv in range(len(args)))
    if souv__hst:
        souv__hst += ', '
    ciyga__obsgn = ', '.join(f"{kiwg__jrb} = ''" for kiwg__jrb in kws.keys())
    kgz__yaj = 'matplotlib_obj, ' if obj_typ is not None else ''
    keyj__ond = f'def mpl_stub({kgz__yaj} {souv__hst} {ciyga__obsgn}):\n'
    keyj__ond += '    pass\n'
    mfh__jdkdq = {}
    exec(keyj__ond, {}, mfh__jdkdq)
    xkmy__yuy = mfh__jdkdq['mpl_stub']
    pljgf__kcjtv = numba.core.utils.pysignature(xkmy__yuy)
    ebru__rbg = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(ebru__rbg)).replace(pysig=
        pljgf__kcjtv)


def generate_axes_typing(mod_name, nrows, ncols):
    xtko__polw = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(xtko__polw.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(xtko__polw.format(mod_name, 'ncols'))
    arbe__awdm = get_overload_const_int(nrows)
    hjm__pcwp = get_overload_const_int(ncols)
    if arbe__awdm < 1:
        raise BodoError(xtko__polw.format(mod_name, 'nrows'))
    if hjm__pcwp < 1:
        raise BodoError(xtko__polw.format(mod_name, 'ncols'))
    if arbe__awdm == 1 and hjm__pcwp == 1:
        apbs__jvd = types.mpl_axes_type
    else:
        if hjm__pcwp == 1:
            eiw__kddmd = types.mpl_axes_type
        else:
            eiw__kddmd = types.Tuple([types.mpl_axes_type] * hjm__pcwp)
        apbs__jvd = types.Tuple([eiw__kddmd] * arbe__awdm)
    return apbs__jvd


def generate_pie_return_type(args, kws):
    jnuf__posl = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if jnuf__posl == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    gowj__meyi = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(gowj__meyi):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(gowj__meyi):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        dbgo__fvd = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, dbgo__fvd]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    dbgo__fvd = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    eubeu__sph = str(dbgo__fvd)
    if not hasattr(types, eubeu__sph):
        eubeu__sph = f'objmode_type{ir_utils.next_label()}'
        setattr(types, eubeu__sph, dbgo__fvd)
    keyj__ond = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{eubeu__sph}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    mfh__jdkdq = {}
    exec(keyj__ond, {'numba': numba, 'np': np}, mfh__jdkdq)
    impl = mfh__jdkdq['impl']
    return impl
