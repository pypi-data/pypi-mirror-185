...
def _from_nanos_codegen(c, val):
    rchb__pgju = c.pyapi.long_from_longlong(c.builder.udiv(val, lir.
        Constant(lir.IntType(64), _nanos_per_hour)))
    uviuq__xqnqj = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem
        (val, lir.Constant(lir.IntType(64), _nanos_per_hour)), lir.Constant
        (lir.IntType(64), _nanos_per_minute)))
    cbld__abqmb = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_minute)), lir.
        Constant(lir.IntType(64), _nanos_per_second)))
    ska__ihkn = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_second)), lir.
        Constant(lir.IntType(64), _nanos_per_milli)))
    izw__ajla = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_milli)), lir.Constant
        (lir.IntType(64), _nanos_per_micro)))
    lqje__fstmu = c.pyapi.long_from_longlong(c.builder.urem(val, lir.
        Constant(lir.IntType(64), _nanos_per_micro)))
    return (rchb__pgju, uviuq__xqnqj, cbld__abqmb, ska__ihkn, izw__ajla,
        lqje__fstmu)


@unbox(TimeType)
def unbox_time(typ, val, c):
    rchb__pgju = c.pyapi.object_getattr_string(val, 'hour')
    uviuq__xqnqj = c.pyapi.object_getattr_string(val, 'minute')
    cbld__abqmb = c.pyapi.object_getattr_string(val, 'second')
    ska__ihkn = c.pyapi.object_getattr_string(val, 'millisecond')
    izw__ajla = c.pyapi.object_getattr_string(val, 'microsecond')
    lqje__fstmu = c.pyapi.object_getattr_string(val, 'nanosecond')
    hour_ll = c.pyapi.long_as_longlong(rchb__pgju)
    minute_ll = c.pyapi.long_as_longlong(uviuq__xqnqj)
    second_ll = c.pyapi.long_as_longlong(cbld__abqmb)
    millisecond_ll = c.pyapi.long_as_longlong(ska__ihkn)
    microsecond_ll = c.pyapi.long_as_longlong(izw__ajla)
    nanosecond_ll = c.pyapi.long_as_longlong(lqje__fstmu)
    rwryh__iuh = _to_nanos_codegen(c, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    c.pyapi.decref(rchb__pgju)
    c.pyapi.decref(uviuq__xqnqj)
    c.pyapi.decref(cbld__abqmb)
    c.pyapi.decref(ska__ihkn)
    c.pyapi.decref(izw__ajla)
    c.pyapi.decref(lqje__fstmu)
    rtqjc__kvj = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(rwryh__iuh, is_error=rtqjc__kvj)
...
def lower_constant_time(context, builder, ty, pyval):
    ...
    rwryh__iuh = _to_nanos_codegen(context, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    return rwryh__iuh
...
def box_time(typ, val, c):
    (rchb__pgju, uviuq__xqnqj, cbld__abqmb, ska__ihkn, izw__ajla, lqje__fstmu
        ) = _from_nanos_codegen(c, val)
    rbwv__lzukd = c.pyapi.unserialize(c.pyapi.serialize_object(Time))
    viz__xpj = c.pyapi.call_function_objargs(rbwv__lzukd, (rchb__pgju,
        uviuq__xqnqj, cbld__abqmb, ska__ihkn, izw__ajla, lqje__fstmu, c.
...
class TimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jssss__uiwlf = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, jssss__uiwlf)
...
def unbox_time_array(typ, val, c):
    ...
    van__wpv = types.Array(types.intp, 1, 'C')
    kqmwn__ffg = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        van__wpv, [n])
    yoy__img = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(64),
        7)), lir.Constant(lir.IntType(64), 8))
    kahg__pvn = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [yoy__img])
    hwcvd__zysj = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(64).as_pointer(), lir.
        IntType(8).as_pointer()])
    gbllm__smfm = cgutils.get_or_insert_function(c.builder.module,
        hwcvd__zysj, name='unbox_time_array')
    c.builder.call(gbllm__smfm, [val, n, kqmwn__ffg.data, kahg__pvn.data])
    vaw__eece = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    vaw__eece.data = kqmwn__ffg._getvalue()
    vaw__eece.null_bitmap = kahg__pvn._getvalue()
    rtqjc__kvj = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(vaw__eece._getvalue(), is_error=rtqjc__kvj)


@box(TimeArrayType)
def box_time_array(typ, val, c):
    qbft__rke = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    kqmwn__ffg = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, qbft__rke.data)
    elau__euqk = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, qbft__rke.null_bitmap).data
    n = c.builder.extract_value(kqmwn__ffg.shape, 0)
    hwcvd__zysj = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType(8)])
    wrpll__igz = cgutils.get_or_insert_function(c.builder.module,
        hwcvd__zysj, name='box_time_array')
    iqmtw__mtd = c.builder.call(wrpll__igz, [n, kqmwn__ffg.data, elau__euqk,
...
def init_time_array(typingctx, data, nulls, precision):
    ...
    def codegen(context, builder, signature, args):
        wbpnm__atvvj, pdtjp__arr, lxe__oyuky = args
        vvvhk__rkgd = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        vvvhk__rkgd.data = wbpnm__atvvj
        vvvhk__rkgd.null_bitmap = pdtjp__arr
        context.nrt.incref(builder, signature.args[0], wbpnm__atvvj)
        context.nrt.incref(builder, signature.args[1], pdtjp__arr)
        return vvvhk__rkgd._getvalue()
...
def lower_constant_time_arr(context, builder, typ, pyval):
    n = len(pyval)
    kqmwn__ffg = np.full(n, 0, np.int64)
    tsyt__ohsa = np.empty(n + 7 >> 3, np.uint8)
    for ekiu__sjrh, csfpn__esgj in enumerate(pyval):
        tnpr__urlt = pd.isna(csfpn__esgj)
        bodo.libs.int_arr_ext.set_bit_to_arr(tsyt__ohsa, ekiu__sjrh, int(
            not tnpr__urlt))
        if not tnpr__urlt:
            kqmwn__ffg[ekiu__sjrh] = (csfpn__esgj.hour * _nanos_per_hour + 
                csfpn__esgj.minute * _nanos_per_minute + csfpn__esgj.second *
                _nanos_per_second + csfpn__esgj.millisecond *
                _nanos_per_milli + csfpn__esgj.microsecond *
                _nanos_per_micro + csfpn__esgj.nanosecond)
    ghej__fyg = context.get_constant_generic(builder, data_type, kqmwn__ffg)
    rok__gbef = context.get_constant_generic(builder, nulls_type, tsyt__ohsa)
    return lir.Constant.literal_struct([ghej__fyg, rok__gbef])


@numba.njit(no_cpython_wrapper=True)
def alloc_time_array(n, precision):
    kqmwn__ffg = np.empty(n, dtype=np.int64)
    nulls = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_time_array(kqmwn__ffg, nulls, precision)
...
def time_arr_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            pcvht__bgsx, gmln__teza = array_getitem_bool_index(A, ind)
            return init_time_array(pcvht__bgsx, gmln__teza, precision)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):

        def impl(A, ind):
            pcvht__bgsx, gmln__teza = array_getitem_int_index(A, ind)
            return init_time_array(pcvht__bgsx, gmln__teza, precision)
        return impl
    if isinstance(ind, types.SliceType):

        def impl_slice(A, ind):
            pcvht__bgsx, gmln__teza = array_getitem_slice_index(A, ind)
            return init_time_array(pcvht__bgsx, gmln__teza, precision)
        return impl_slice
...
def create_cmp_op_overload(op):
    ...
    def overload_time_cmp(lhs, rhs):
        if isinstance(lhs, TimeType) and isinstance(rhs, TimeType):
            ...
            def impl(lhs, rhs):
                loz__kppyz = cast_time_to_int(lhs)
                rws__aryi = cast_time_to_int(rhs)
                return op(0 if loz__kppyz == rws__aryi else 1 if loz__kppyz >
                    rws__aryi else -1, 0)
            return impl
