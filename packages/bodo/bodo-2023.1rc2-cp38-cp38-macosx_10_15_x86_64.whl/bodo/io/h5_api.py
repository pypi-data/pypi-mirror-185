...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        ddz__ijl = []
        hxjgb__gdzt = h5g_get_num_objs(obj_id)
        for lmjni__ffjpg in range(hxjgb__gdzt):
            zxlfq__fngw = h5g_get_objname_by_idx(obj_id, lmjni__ffjpg)
            ddz__ijl.append(zxlfq__fngw)
        return ddz__ijl
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    ooxy__wkx = parse_dtype(dtype)
    bhy__mbi = np.int32(numba_to_c_type(ooxy__wkx))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            illdq__kjk = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    dlae__wzxk = tuple_to_ptr(starts)
    nohto__nil = tuple_to_ptr(counts)
    qvtug__tcqh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, dlae__wzxk, nohto__nil,
        is_parallel, out_arr.ctypes, qvtug__tcqh)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    nohto__nil = tuple_to_ptr(counts)
    qvtug__tcqh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, dlae__wzxk, nohto__nil,
        is_parallel, out_arr.ctypes, qvtug__tcqh)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        sikf__ytiv = types.Array(dtype, ndim, 'C')
        rrdt__fyv = args[3]
        sikf__ytiv = self.context.resolve_function_type(operator.getitem, [
            sikf__ytiv, rrdt__fyv], {}).return_type.copy(layout='C')
        return signature(sikf__ytiv, *args)
...
def get_filter_read_indices(bool_arr):
    xtq__alx = bool_arr.nonzero()[0]
    hojpr__ixkwi = bodo.libs.distributed_api.get_rank()
    elxrn__isu = bodo.libs.distributed_api.get_size()
    xmzs__duqpe = np.empty(elxrn__isu, np.int64)
    lge__yekx = len(bool_arr)
    bodo.libs.distributed_api.allgather(xmzs__duqpe, lge__yekx)
    bizs__svtl = xmzs__duqpe.cumsum()[hojpr__ixkwi] - lge__yekx
    xtq__alx += bizs__svtl
    wvwv__hyvgt = bodo.libs.distributed_api.dist_reduce(len(xtq__alx), np.
        int32(sum_op))
    pydct__gkf = bodo.libs.distributed_api.gatherv(xtq__alx)
    if hojpr__ixkwi == 0:
        upae__ftg = pydct__gkf
    else:
        upae__ftg = np.empty(wvwv__hyvgt, xtq__alx.dtype)
    bodo.libs.distributed_api.bcast(upae__ftg)
    xfn__uas = bodo.libs.distributed_api.get_start(wvwv__hyvgt, elxrn__isu,
        hojpr__ixkwi)
    hal__ckyhm = bodo.libs.distributed_api.get_end(wvwv__hyvgt, elxrn__isu,
        hojpr__ixkwi)
    return upae__ftg[xfn__uas:hal__ckyhm]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        hftns__tuy = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], hftns__tuy)
        return builder.bitcast(hftns__tuy, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    nohto__nil = tuple_to_ptr(counts)
    qvtug__tcqh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, dlae__wzxk,
        nohto__nil, is_parallel, out_arr.ctypes, qvtug__tcqh, read_indices.
