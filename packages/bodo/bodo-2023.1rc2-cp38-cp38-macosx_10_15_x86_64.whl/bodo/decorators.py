...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        mgab__pqh = self.overloads[sig]
        tnt__zko = mgab__pqh.metadata.get('distributed_diagnostics', None)
        if tnt__zko is None:
            bte__wkd = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(bte__wkd)
        tnt__zko.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        adcrh__pvt = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(adcrh__pvt)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        wfm__whvb = options.pop('distributed')
        pipeline_class = (pipeline_class if wfm__whvb else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        arm__miyvt = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if arm__miyvt else
            pipeline_class)
    jncjt__sgk = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(jncjt__sgk, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(jncjt__sgk)
        else:
            return master_mode_wrapper(jncjt__sgk)
    else:
        return jncjt__sgk
