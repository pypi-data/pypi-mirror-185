...
def generate_col_to_index_func_text(col_names: Tuple):
    if all(isinstance(a, str) for a in col_names) or all(isinstance(a,
        ...
        btyd__mqygc = f'bodo.utils.conversion.coerce_to_array({col_names})'
        return (
            f'bodo.hiframes.pd_index_ext.init_binary_str_index({btyd__mqygc})\n'
            )
...
def overload_dataframe_columns(df):
    kkj__mzrbr = 'def impl(df):\n'
    if df.has_runtime_cols:
        kkj__mzrbr += (
            '  return bodo.hiframes.pd_dataframe_ext.get_dataframe_column_names(df)\n'
            )
    else:
        qnz__qrt = (bodo.hiframes.dataframe_impl.
            generate_col_to_index_func_text(df.columns))
        kkj__mzrbr += f'  return {qnz__qrt}'
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def overload_dataframe_values(df):
    ...
    mnkxg__avk = len(df.columns)
    ylt__gyuz = set(i for i in range(mnkxg__avk) if isinstance(df.data[i],
        IntegerArrayType))
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(i, '.astype(float)' if i in ylt__gyuz else '') for i in
        range(mnkxg__avk))
    kkj__mzrbr = 'def f(df):\n'.format()
    kkj__mzrbr += '    return np.stack(({},), 1)\n'.format(data_args)
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'np': np}, vkbo__dapyc)
    vena__clbk = vkbo__dapyc['f']
    return vena__clbk
...
def overload_dataframe_to_numpy(df, dtype=None, copy=False, na_value=_no_input
    ...
    cft__ozzo = {'dtype': dtype, 'na_value': na_value}
    bckj__ejpg = {'dtype': None, 'na_value': _no_input}
    check_unsupported_args('DataFrame.to_numpy', cft__ozzo, bckj__ejpg,
...
def overload_dataframe_size(df):
    if df.has_runtime_cols:
        ...
        def impl(df):
            ...
            bpc__jptd = bodo.hiframes.table.compute_num_runtime_columns(t)
            return bpc__jptd * len(t)
        return impl
...
def overload_dataframe_dtypes(df):
    ...
    acwpk__unwhh = ',' if len(df.columns) == 1 else ''
    index = f'bodo.hiframes.pd_index_ext.init_heter_index({df.columns})'
    kkj__mzrbr += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}{acwpk__unwhh}), {index}, None)
"""
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
...
def overload_dataframe_astype(df, dtype, copy=True, errors='raise',
    ...
    cft__ozzo = {'errors': errors}
    bckj__ejpg = {'errors': 'raise'}
    check_unsupported_args('df.astype', cft__ozzo, bckj__ejpg, package_name
...
"""
    ...
    if _bodo_object_typeref is not None:
        ...
        lumi__ivhr = _bodo_object_typeref.instance_type
        assert isinstance(lumi__ivhr, DataFrameType
            ), 'Bodo schema used in DataFrame.astype is only supported for DataFrame schemas'
        if df.is_table_format:
            for i, name in enumerate(df.columns):
                if name in lumi__ivhr.column_index:
                    idx = lumi__ivhr.column_index[name]
                    arr_typ = lumi__ivhr.data[idx]
                else:
                    arr_typ = df.data[i]
                vfrrv__wqa.append(arr_typ)
        else:
            extra_globals = {}
            noykd__fcm = {}
            for i, name in enumerate(lumi__ivhr.columns):
                arr_typ = lumi__ivhr.data[i]
                extra_globals[f'_bodo_schema{i}'] = get_castable_arr_dtype(
                    arr_typ)
                noykd__fcm[name] = f'_bodo_schema{i}'
            data_args = ', '.join(
                f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {noykd__fcm[oqsb__jqghr]}, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
                 if oqsb__jqghr in noykd__fcm else
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
                 for i, oqsb__jqghr in enumerate(df.columns))
    elif is_overload_constant_dict(dtype) or is_overload_constant_series(dtype
        ):
        yvlzu__bxu = get_overload_constant_dict(dtype
            ) if is_overload_constant_dict(dtype) else dict(
            get_overload_constant_series(dtype))
        if df.is_table_format:
            yvlzu__bxu = {name: dtype_to_array_type(parse_dtype(dtype)) for
                name, dtype in yvlzu__bxu.items()}
            for i, name in enumerate(df.columns):
                if name in yvlzu__bxu:
                    arr_typ = yvlzu__bxu[name]
                else:
                    arr_typ = df.data[i]
                vfrrv__wqa.append(arr_typ)
        else:
            data_args = ', '.join(
                f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {_get_dtype_str(yvlzu__bxu[oqsb__jqghr])}, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
                 if oqsb__jqghr in yvlzu__bxu else
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
                 for i, oqsb__jqghr in enumerate(df.columns))
    elif df.is_table_format:
        arr_typ = dtype_to_array_type(parse_dtype(dtype))
        vfrrv__wqa = [arr_typ] * len(df.columns)
    else:
        data_args = ', '.join(
            f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), dtype, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
             for i in range(len(df.columns)))
    if df.is_table_format:
        bnnyc__nlfvc = bodo.TableType(tuple(vfrrv__wqa))
        extra_globals['out_table_typ'] = bnnyc__nlfvc
        data_args = (
...
def overload_dataframe_copy(df, deep=True):
    ...
    if df.is_table_format:
        ...
        sqo__lddbs = types.none
        extra_globals = {'output_arr_typ': sqo__lddbs}
        ...
        for i in range(len(df.columns)):
            ...
            if is_overload_true(deep):
                zirar__twfl.append(arr + '.copy()')
            elif is_overload_false(deep):
                zirar__twfl.append(arr)
            else:
                zirar__twfl.append(f'{arr}.copy() if deep else {arr}')
        data_args = ', '.join(zirar__twfl)
    return _gen_init_df(header, df.columns, data_args, extra_globals=
        extra_globals)
...
def overload_dataframe_rename(df, mapper=None, index=None, columns=None,
    ...
    cft__ozzo = {'index': index, 'level': level, 'errors': errors}
    bckj__ejpg = {'index': None, 'level': None, 'errors': 'ignore'}
    check_unsupported_args('DataFrame.rename', cft__ozzo, bckj__ejpg,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(mapper):
        ...
        ouol__ygl = get_overload_constant_dict(mapper)
        ...
        ouol__ygl = get_overload_constant_dict(columns)
    else:
        raise_bodo_error(
            "DataFrame.rename(): must pass columns either via 'mapper' and 'axis'=1 or 'columns'"
            )
    eqzq__luu = tuple([ouol__ygl.get(df.columns[i], df.columns[i]) for i in
...
"""
    ...
    if df.is_table_format:
        ...
        wsjxd__asqj = df.copy(columns=eqzq__luu)
        sqo__lddbs = types.none
        extra_globals = {'output_arr_typ': sqo__lddbs}
        ...
        for i in range(len(df.columns)):
            ...
            if is_overload_true(copy):
                ...
                zirar__twfl.append(f'{arr}.copy() if copy else {arr}')
        data_args = ', '.join(zirar__twfl)
    return _gen_init_df(header, eqzq__luu, data_args, extra_globals=
        extra_globals)
...
def overload_dataframe_filter(df, items=None, like=None, regex=None, axis=None
    ...
    szcu__cfn = not is_overload_none(items)
    pgep__ymj = not is_overload_none(like)
    wtbx__jeze = not is_overload_none(regex)
    vyfhj__bvczy = szcu__cfn ^ pgep__ymj ^ wtbx__jeze
    nsuzf__loff = not (szcu__cfn or pgep__ymj or wtbx__jeze)
    if nsuzf__loff:
        raise BodoError(
    ...
    if is_overload_constant_str(axis):
        ...
        jlqg__yeni = 0 if axis == 'index' else 1
    ...
    if jlqg__yeni == 1:
        ...
        if szcu__cfn:
            if is_overload_constant_list(items):
                obvzj__xkhx = get_overload_const_list(items)
            else:
                raise BodoError(
        ...
        if pgep__ymj:
            if is_overload_constant_str(like):
                qado__zqmwi = get_overload_const_str(like)
            else:
                raise BodoError(
        ...
        if wtbx__jeze:
            if is_overload_constant_str(regex):
                vkgju__bwvq = get_overload_const_str(regex)
                qjl__bjly = re.compile(vkgju__bwvq)
            else:
                raise BodoError(
        ...
        for i, oqsb__jqghr in enumerate(df.columns):
            if not is_overload_none(items
                ) and oqsb__jqghr in obvzj__xkhx or not is_overload_none(like
                ) and qado__zqmwi in str(oqsb__jqghr) or not is_overload_none(
                regex) and qjl__bjly.search(str(oqsb__jqghr)):
                tcaxo__lebor.append(oqsb__jqghr)
                zawr__xon.append(i)
        for i in zawr__xon:
            var_name = f'data_{i}'
            gqax__vkf.append(var_name)
            kkj__mzrbr += f"""  {var_name} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})
"""
        data_args = ', '.join(gqax__vkf)
        return _gen_init_df(kkj__mzrbr, tcaxo__lebor, data_args)
...
def overload_dataframe_isna(df):
    ...
    wsjxd__asqj = None
    if df.is_table_format:
        sqo__lddbs = types.Array(types.bool_, 1, 'C')
        wsjxd__asqj = DataFrameType(tuple([sqo__lddbs] * len(df.data)), df.
...
def overload_dataframe_select_dtypes(df, include=None, exclude=None):
    ...
    wahr__tli = is_overload_none(include)
    ydaai__biwvt = is_overload_none(exclude)
    yed__wzh = 'DataFrame.select_dtypes'
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.select_dtypes()')
    if wahr__tli and ydaai__biwvt:
        raise_bodo_error(
    ...
    if not wahr__tli:
        if is_overload_constant_list(include):
            ...
            dxnrk__nsm = [dtype_to_array_type(parse_dtype(elem, yed__wzh)) for
                elem in include]
        elif is_legal_input(include):
            dxnrk__nsm = [dtype_to_array_type(parse_dtype(include, yed__wzh))]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        dxnrk__nsm = get_nullable_and_non_nullable_types(dxnrk__nsm)
        jer__mgolu = tuple(oqsb__jqghr for i, oqsb__jqghr in enumerate(df.
            columns) if df.data[i] in dxnrk__nsm)
    else:
        jer__mgolu = df.columns
    if not ydaai__biwvt:
        if is_overload_constant_list(exclude):
            exclude = get_overload_const_list(exclude)
            qgn__tti = [dtype_to_array_type(parse_dtype(elem, yed__wzh)) for
                elem in exclude]
        elif is_legal_input(exclude):
            qgn__tti = [dtype_to_array_type(parse_dtype(exclude, yed__wzh))]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        qgn__tti = get_nullable_and_non_nullable_types(qgn__tti)
        jer__mgolu = tuple(oqsb__jqghr for oqsb__jqghr in jer__mgolu if df.
            data[df.column_index[oqsb__jqghr]] not in qgn__tti)
    data_args = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.column_index[oqsb__jqghr]})'
         for oqsb__jqghr in jer__mgolu)
    header = 'def impl(df, include=None, exclude=None):\n'
    return _gen_init_df(header, jer__mgolu, data_args)
...
def overload_dataframe_isin(df, values):
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.isin()')
    kkj__mzrbr = 'def impl(df, values):\n'
    rzo__knj = {}
    fgni__adqcy = False
    if isinstance(values, DataFrameType):
        fgni__adqcy = True
        for i, oqsb__jqghr in enumerate(df.columns):
            if oqsb__jqghr in values.column_index:
                plqq__iymm = 'val{}'.format(i)
                kkj__mzrbr += f"""  {plqq__iymm} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(values, {values.column_index[oqsb__jqghr]})
"""
                rzo__knj[oqsb__jqghr] = plqq__iymm
    elif is_iterable_type(values) and not isinstance(values, SeriesType):
        rzo__knj = {oqsb__jqghr: 'values' for oqsb__jqghr in df.columns}
    else:
        raise_bodo_error(f'pd.isin(): not supported for type {values}')
    data = []
    for i in range(len(df.columns)):
        plqq__iymm = 'data{}'.format(i)
        kkj__mzrbr += (
            '  {} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {})\n'
            .format(plqq__iymm, i))
        data.append(plqq__iymm)
    ohkpp__jdc = ['out{}'.format(i) for i in range(len(df.columns))]
    lfkk__msw = """
...
"""
  ...
  for i in numba.parfors.parfor.internal_prange(n):
    {2}[i] = {0}[i] in {1}
"""
    orrd__kesbd = '  {} = np.zeros(len(df), np.bool_)\n'
    for i, (cname, tkwc__plozr) in enumerate(zip(df.columns, data)):
        if cname in rzo__knj:
            fffyf__iatvy = rzo__knj[cname]
            if fgni__adqcy:
                kkj__mzrbr += lfkk__msw.format(tkwc__plozr, fffyf__iatvy,
                    ohkpp__jdc[i])
            else:
                kkj__mzrbr += tivu__yyr.format(tkwc__plozr, fffyf__iatvy,
                    ohkpp__jdc[i])
        else:
            kkj__mzrbr += orrd__kesbd.format(ohkpp__jdc[i])
    return _gen_init_df(kkj__mzrbr, df.columns, ','.join(ohkpp__jdc))
...
def overload_dataframe_abs(df):
    ...
    data_args = ', '.join(
        ...
        .format(i) for i in range(mnkxg__avk))
    header = 'def impl(df):\n'
    return _gen_init_df(header, df.columns, data_args)


def overload_dataframe_corr(df, method='pearson', min_periods=1):
    exkh__kil = [oqsb__jqghr for oqsb__jqghr, ufhh__uyo in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(ufhh__uyo.dtype)
        ]
    assert len(exkh__kil) != 0
    eqn__qkwtt = ''
    if not any(ufhh__uyo == types.float64 for ufhh__uyo in df.data):
        eqn__qkwtt = '.astype(np.float64)'
    zxc__prr = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.column_index[oqsb__jqghr], '.astype(np.float64)' if 
        isinstance(df.data[df.column_index[oqsb__jqghr]], IntegerArrayType) or
        df.data[df.column_index[oqsb__jqghr]] == boolean_array else '') for
        oqsb__jqghr in exkh__kil)
    ucjw__oqqe = 'np.stack(({},), 1){}'.format(zxc__prr, eqn__qkwtt)
    data_args = ', '.join('res[:,{}]'.format(i) for i in range(len(exkh__kil)))
    index = f'{generate_col_to_index_func_text(exkh__kil)}\n'
    header = "def impl(df, method='pearson', min_periods=1):\n"
    header += '  mat = {}\n'.format(ucjw__oqqe)
    header += '  res = bodo.libs.array_kernels.nancorr(mat, 0, min_periods)\n'
    return _gen_init_df(header, exkh__kil, data_args, index)
...
def overload_dataframe_cov(df, min_periods=None, ddof=1):
    ...
    rita__jbngr = dict(ddof=ddof)
    wrut__cabef = dict(ddof=1)
    check_unsupported_args('DataFrame.cov', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    mange__anpa = '1' if is_overload_none(min_periods) else 'min_periods'
    exkh__kil = [oqsb__jqghr for oqsb__jqghr, ufhh__uyo in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(ufhh__uyo.dtype)
        ]
    if len(exkh__kil) == 0:
...
def overload_dataframe_count(df, axis=0, level=None, numeric_only=False):
    ...
    rita__jbngr = dict(axis=axis, level=level, numeric_only=numeric_only)
    wrut__cabef = dict(axis=0, level=None, numeric_only=False)
    check_unsupported_args('DataFrame.count', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}))'
         for i in range(len(df.columns)))
    kkj__mzrbr = 'def impl(df, axis=0, level=None, numeric_only=False):\n'
    kkj__mzrbr += '  data = np.array([{}])\n'.format(data_args)
    qnz__qrt = bodo.hiframes.dataframe_impl.generate_col_to_index_func_text(df
        .columns)
    kkj__mzrbr += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {qnz__qrt})\n'
        )
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'np': np}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
...
def overload_dataframe_nunique(df, axis=0, dropna=True):
    ...
    rita__jbngr = dict(axis=axis)
    wrut__cabef = dict(axis=0)
    if not is_overload_bool(dropna):
        raise BodoError('DataFrame.nunique: dropna must be a boolean value')
    check_unsupported_args('DataFrame.nunique', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_kernels.nunique(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), dropna)'
         for i in range(len(df.columns)))
    kkj__mzrbr = 'def impl(df, axis=0, dropna=True):\n'
    kkj__mzrbr += '  data = np.asarray(({},))\n'.format(data_args)
    qnz__qrt = bodo.hiframes.dataframe_impl.generate_col_to_index_func_text(df
        .columns)
    kkj__mzrbr += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {qnz__qrt})\n'
...
def overload_dataframe_prod(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=
        numeric_only, min_count=min_count)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.prod', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.product()')
...
def overload_dataframe_sum(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=
        numeric_only, min_count=min_count)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.sum', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.sum()')
...
def overload_dataframe_max(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.max', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.max()')
...
def overload_dataframe_min(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.min', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.min()')
...
def overload_dataframe_mean(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.mean', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.mean()')
...
def overload_dataframe_var(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    wrut__cabef = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.var', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.var()')
...
def overload_dataframe_std(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    wrut__cabef = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.std', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.std()')
...
def overload_dataframe_median(df, axis=None, skipna=None, level=None,
    ...
    rita__jbngr = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    wrut__cabef = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.median', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.median()')
...
def overload_dataframe_quantile(df, q=0.5, axis=0, numeric_only=True,
    ...
    rita__jbngr = dict(numeric_only=numeric_only, interpolation=interpolation)
    wrut__cabef = dict(numeric_only=True, interpolation='linear')
    check_unsupported_args('DataFrame.quantile', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.quantile()')
...
def overload_dataframe_idxmax(df, axis=0, skipna=True):
    ...
    rita__jbngr = dict(axis=axis, skipna=skipna)
    wrut__cabef = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmax', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.idxmax()')
    for tzu__pvwd in df.data:
        if not (bodo.utils.utils.is_np_array_typ(tzu__pvwd) and (tzu__pvwd.
            dtype in [bodo.datetime64ns, bodo.timedelta64ns] or isinstance(
            tzu__pvwd.dtype, (types.Number, types.Boolean))) or isinstance(
            tzu__pvwd, (bodo.IntegerArrayType, bodo.FloatingArrayType, bodo
            .CategoricalArrayType)) or tzu__pvwd in [bodo.boolean_array,
            bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmax() only supported for numeric column types. Column type: {tzu__pvwd} not supported.'
                )
        if isinstance(tzu__pvwd, bodo.CategoricalArrayType
            ) and not tzu__pvwd.dtype.ordered:
            raise BodoError(
...
def overload_dataframe_idxmin(df, axis=0, skipna=True):
    ...
    rita__jbngr = dict(axis=axis, skipna=skipna)
    wrut__cabef = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmin', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.idxmin()')
    for tzu__pvwd in df.data:
        if not (bodo.utils.utils.is_np_array_typ(tzu__pvwd) and (tzu__pvwd.
            ...
            bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmin() only supported for numeric column types. Column type: {tzu__pvwd} not supported.'
                )
        if isinstance(tzu__pvwd, bodo.CategoricalArrayType
            ) and not tzu__pvwd.dtype.ordered:
            raise BodoError(
...
def _gen_reduce_impl(df, func_name, args=None, axis=None):
    ...
    if func_name in ('idxmax', 'idxmin'):
        ...
        exkh__kil = tuple(oqsb__jqghr for oqsb__jqghr, ufhh__uyo in zip(df.
            columns, df.data) if bodo.utils.typing._is_pandas_numeric_dtype
            (ufhh__uyo.dtype))
        out_colnames = exkh__kil
    assert len(out_colnames) != 0
    try:
        if func_name in ('idxmax', 'idxmin') and axis == 0:
            comm_dtype = None
        else:
            lcvu__blz = [numba.np.numpy_support.as_dtype(df.data[df.
                column_index[oqsb__jqghr]].dtype) for oqsb__jqghr in
                out_colnames]
            comm_dtype = numba.np.numpy_support.from_dtype(np.
                find_common_type(lcvu__blz, []))
    except NotImplementedError as aren__mgk:
        raise BodoError(
            f'Dataframe.{func_name}() with column types: {df.data} could not be merged to a common type.'
            )
    cjfkx__qjk = ''
    if func_name in ('sum', 'prod'):
        cjfkx__qjk = ', min_count=0'
    ddof = ''
    if func_name in ('var', 'std'):
        ddof = 'ddof=1, '
    kkj__mzrbr = (
        'def impl(df, axis=None, skipna=None, level=None,{} numeric_only=None{}):\n'
        .format(ddof, cjfkx__qjk))
    if func_name == 'quantile':
        kkj__mzrbr = (
            "def impl(df, q=0.5, axis=0, numeric_only=True, interpolation='linear'):\n"
            )
    if func_name in ('idxmax', 'idxmin'):
        kkj__mzrbr = 'def impl(df, axis=0, skipna=True):\n'
    if axis == 0:
        kkj__mzrbr += _gen_reduce_impl_axis0(df, func_name, out_colnames,
            comm_dtype, args)
    else:
        kkj__mzrbr += _gen_reduce_impl_axis1(func_name, out_colnames,
            comm_dtype, df)
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba},
        vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def _gen_reduce_impl_axis0(df, func_name, out_colnames, comm_dtype, args):
    ...
    if func_name in ('min', 'max'):
        uzya__ldesb = ', dtype=np.{}'.format(comm_dtype)
    if comm_dtype == types.float32 and func_name in ('sum', 'prod', 'mean',
        'var', 'std', 'median'):
        uzya__ldesb = ', dtype=np.float32'
    lli__noeyl = f'bodo.libs.array_ops.array_op_{func_name}'
    hzot__kyrvq = ''
    if func_name in ['sum', 'prod']:
        hzot__kyrvq = 'True, min_count'
    elif func_name in ['idxmax', 'idxmin']:
        hzot__kyrvq = 'index'
    elif func_name == 'quantile':
        hzot__kyrvq = 'q'
    elif func_name in ['std', 'var']:
        hzot__kyrvq = 'True, ddof'
    elif func_name == 'median':
        hzot__kyrvq = 'True'
    data_args = ', '.join(
        f'{lli__noeyl}(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.column_index[oqsb__jqghr]}), {hzot__kyrvq})'
         for oqsb__jqghr in out_colnames)
    kkj__mzrbr = ''
    if func_name in ('idxmax', 'idxmin'):
        kkj__mzrbr += (
        ...
        kkj__mzrbr += '  data = np.asarray(({},){})\n'.format(data_args,
            uzya__ldesb)
    kkj__mzrbr += f"""  return bodo.hiframes.pd_series_ext.init_series(data, pd.Index({out_colnames}))
"""
    return kkj__mzrbr


def _gen_reduce_impl_axis1(func_name, out_colnames, comm_dtype, df_type):
    tdpu__ivlnk = [df_type.column_index[oqsb__jqghr] for oqsb__jqghr in
        out_colnames]
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    data_args = '\n    '.join(
        'arr_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
        .format(i) for i in tdpu__ivlnk)
    uspt__psyyr = '\n        '.join(f'row[{i}] = arr_{tdpu__ivlnk[i]}[i]' for
        i in range(len(out_colnames)))
    assert len(data_args) > 0, f'empty dataframe in DataFrame.{func_name}()'
    dehjj__abs = f'len(arr_{tdpu__ivlnk[0]})'
    kxwl__lflrx = {'max': 'np.nanmax', 'min': 'np.nanmin', 'sum':
    ...
    if func_name in kxwl__lflrx:
        qvtns__uakgk = kxwl__lflrx[func_name]
        ktery__omjmt = 'float64' if func_name in ['mean', 'median', 'std',
            'var'] else comm_dtype
        kkj__mzrbr = f"""
    {data_args}
    numba.parfors.parfor.init_prange()
    ...
    A = np.empty(n, np.{ktery__omjmt})
    for i in numba.parfors.parfor.internal_prange(n):
        {uspt__psyyr}
        A[i] = {qvtns__uakgk}(row)
...
def overload_dataframe_pct_change(df, periods=1, fill_method='pad', limit=
    ...
    rita__jbngr = dict(fill_method=fill_method, limit=limit, freq=freq)
    wrut__cabef = dict(fill_method='pad', limit=None, freq=None)
    check_unsupported_args('DataFrame.pct_change', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
...
def overload_dataframe_cumprod(df, axis=None, skipna=True):
    ...
    rita__jbngr = dict(axis=axis, skipna=skipna)
    wrut__cabef = dict(axis=None, skipna=True)
    check_unsupported_args('DataFrame.cumprod', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.cumprod()')
    data_args = ', '.join(
...
def overload_dataframe_cumsum(df, axis=None, skipna=True):
    ...
    rita__jbngr = dict(skipna=skipna)
    wrut__cabef = dict(skipna=True)
    check_unsupported_args('DataFrame.cumsum', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.cumsum()')
    data_args = ', '.join(
...
def overload_dataframe_describe(df, percentiles=None, include=None, exclude
    ...
    rita__jbngr = dict(percentiles=percentiles, include=include, exclude=
        exclude, datetime_is_numeric=datetime_is_numeric)
    wrut__cabef = dict(percentiles=None, include=None, exclude=None,
        datetime_is_numeric=True)
    check_unsupported_args('DataFrame.describe', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.describe()')
    exkh__kil = [oqsb__jqghr for oqsb__jqghr, ufhh__uyo in zip(df.columns,
        df.data) if _is_describe_type(ufhh__uyo)]
    if len(exkh__kil) == 0:
        raise BodoError('df.describe() only supports numeric columns')
    pxzn__dwm = sum(df.data[df.column_index[oqsb__jqghr]].dtype == bodo.
        datetime64ns for oqsb__jqghr in exkh__kil)

    def _get_describe(col_ind):
        ize__umrn = df.data[col_ind].dtype == bodo.datetime64ns
        if pxzn__dwm and pxzn__dwm != len(exkh__kil):
            if ize__umrn:
...
"""
    for oqsb__jqghr in exkh__kil:
        col_ind = df.column_index[oqsb__jqghr]
        header += f"""  des_{col_ind} = bodo.libs.array_ops.array_op_describe(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {col_ind}))
"""
    data_args = ', '.join(_get_describe(df.column_index[oqsb__jqghr]) for
        oqsb__jqghr in exkh__kil)
    qljq__bisr = "['count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']"
    if pxzn__dwm == len(exkh__kil):
        qljq__bisr = "['count', 'mean', 'min', '25%', '50%', '75%', 'max']"
    elif pxzn__dwm:
        qljq__bisr = (
            "['count', 'mean', 'min', '25%', '50%', '75%', 'max', 'std']")
    index = f'bodo.utils.conversion.convert_to_index({qljq__bisr})'
    return _gen_init_df(header, exkh__kil, data_args, index)
...
def overload_dataframe_take(df, indices, axis=0, convert=None, is_copy=True):
    ...
    rita__jbngr = dict(axis=axis, convert=convert, is_copy=is_copy)
    wrut__cabef = dict(axis=0, convert=None, is_copy=True)
    check_unsupported_args('DataFrame.take', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_shift(df, periods=1, freq=None, axis=0, fill_value=None
    ...
    rita__jbngr = dict(freq=freq, axis=axis)
    wrut__cabef = dict(freq=None, axis=0)
    check_unsupported_args('DataFrame.shift', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.shift()')
    for hbm__gpye in df.data:
        if not is_supported_shift_array_type(hbm__gpye):
            raise BodoError(
                f'Dataframe.shift() column input type {hbm__gpye.dtype} not supported yet.'
                )
...
def overload_dataframe_diff(df, periods=1, axis=0):
    ...
    rita__jbngr = dict(axis=axis)
    wrut__cabef = dict(axis=0)
    check_unsupported_args('DataFrame.diff', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.diff()')
    for hbm__gpye in df.data:
        if not (isinstance(hbm__gpye, types.Array) and (isinstance(
            hbm__gpye.dtype, types.Number) or hbm__gpye.dtype == bodo.
            datetime64ns)):
            raise BodoError(
                f'DataFrame.diff() column input type {hbm__gpye.dtype} not supported.'
                )
    if not is_overload_int(periods):
...
def overload_dataframe_explode(df, column, ignore_index=False):
    ...
    if not is_literal_type(column):
        raise_bodo_error(dmkn__zix)
    if is_overload_constant_list(column) or is_overload_constant_tuple(column):
        osu__mjwht = get_overload_const_list(column)
    else:
        osu__mjwht = [get_literal_value(column)]
    oqw__dqa = [df.column_index[oqsb__jqghr] for oqsb__jqghr in osu__mjwht]
    for i in oqw__dqa:
    ...
    header += (
        f'  counts = bodo.libs.array_kernels.get_arr_lens(data{oqw__dqa[0]})\n'
        )
    for i in range(n):
        if i in oqw__dqa:
            header += (
...
def overload_dataframe_set_index(df, keys, drop=True, append=False, inplace
    ...
    cft__ozzo = {'inplace': inplace, 'append': append, 'verify_integrity':
        verify_integrity}
    bckj__ejpg = {'inplace': False, 'append': False, 'verify_integrity': False}
    check_unsupported_args('DataFrame.set_index', cft__ozzo, bckj__ejpg,
        package_name='pandas', module_name='DataFrame')
...
"""
    ...
    columns = tuple(oqsb__jqghr for oqsb__jqghr in df.columns if 
        oqsb__jqghr != col_name)
    index = (
...
def overload_dataframe_query(df, expr, inplace=False):
    ...
    cft__ozzo = {'inplace': inplace}
    bckj__ejpg = {'inplace': False}
    check_unsupported_args('query', cft__ozzo, bckj__ejpg, package_name=
        'pandas', module_name='DataFrame')
    ...
    def impl(df, expr, inplace=False):
        qmf__flck = bodo.hiframes.pd_dataframe_ext.query_dummy(df, expr)
        return df[qmf__flck]
    return impl
...
def overload_dataframe_duplicated(df, subset=None, keep='first'):
    ...
    cft__ozzo = {'subset': subset, 'keep': keep}
    bckj__ejpg = {'subset': None, 'keep': 'first'}
    check_unsupported_args('DataFrame.duplicated', cft__ozzo, bckj__ejpg,
        package_name='pandas', module_name='DataFrame')
    mnkxg__avk = len(df.columns)
    kkj__mzrbr = "def impl(df, subset=None, keep='first'):\n"
    for i in range(mnkxg__avk):
        kkj__mzrbr += (
            '  data_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})\n'
            .format(i))
    uif__vqg = ', '.join(f'data_{i}' for i in range(mnkxg__avk))
    uif__vqg += ',' if mnkxg__avk == 1 else ''
    kkj__mzrbr += (
        f'  duplicated = bodo.libs.array_kernels.duplicated(({uif__vqg}))\n')
    kkj__mzrbr += (
        '  index = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)\n')
    kkj__mzrbr += (
...
def overload_dataframe_drop_duplicates(df, subset=None, keep='first',
    ...
    cft__ozzo = {'keep': keep, 'inplace': inplace, 'ignore_index': ignore_index
        }
    bckj__ejpg = {'keep': 'first', 'inplace': False, 'ignore_index': False}
    srt__hpxul = []
    if is_overload_constant_list(subset):
        srt__hpxul = get_overload_const_list(subset)
    elif is_overload_constant_str(subset):
        srt__hpxul = [get_overload_const_str(subset)]
    elif is_overload_constant_int(subset):
        srt__hpxul = [get_overload_const_int(subset)]
    ...
    for col_name in srt__hpxul:
        ...
        hquqg__ggoj.append(df.column_index[col_name])
    check_unsupported_args('DataFrame.drop_duplicates', cft__ozzo,
        bckj__ejpg, package_name='pandas', module_name='DataFrame')
    qvo__hqyr = []
    if hquqg__ggoj:
        for xjbvv__vznd in hquqg__ggoj:
            if isinstance(df.data[xjbvv__vznd], bodo.MapArrayType):
                qvo__hqyr.append(df.columns[xjbvv__vznd])
    else:
        for i, col_name in enumerate(df.columns):
            if isinstance(df.data[i], bodo.MapArrayType):
                qvo__hqyr.append(col_name)
    if qvo__hqyr:
        raise BodoError(
            f'DataFrame.drop_duplicates(): Columns {qvo__hqyr} ' +
    ...
    mnkxg__avk = len(df.columns)
    hiwo__chldc = ['data_{}'.format(i) for i in hquqg__ggoj]
    sux__svqg = ['data_{}'.format(i) for i in range(mnkxg__avk) if i not in
        hquqg__ggoj]
    if hiwo__chldc:
        mypyc__ocgdf = len(hiwo__chldc)
    else:
        mypyc__ocgdf = mnkxg__avk
    gxhhc__esx = ', '.join(hiwo__chldc + sux__svqg)
    data_args = ', '.join('data_{}'.format(i) for i in range(mnkxg__avk))
    kkj__mzrbr = (
...
"""
        .format(gxhhc__esx, index, mypyc__ocgdf))
    kkj__mzrbr += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(kkj__mzrbr, df.columns, data_args, 'index')
...
"""
        ...
        if not hasattr(other, 'ndim') or other.ndim == 1:
            ...
            if isinstance(other, DataFrameType):
                ...
                vabgf__dzj = lambda i: f'other[:,{i}]'
        mnkxg__avk = len(df.columns)
        data_args = ', '.join(
            f'bodo.hiframes.series_impl.where_impl({cond_str(i, gen_all_false)}, bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {vabgf__dzj(i)})'
             for i in range(mnkxg__avk))
        if gen_all_false[0]:
...
def _install_dataframe_mask_where_overload():
    for func_name in ('mask', 'where'):
        zkm__tfgpl = create_dataframe_mask_where_overload(func_name)
        overload_method(DataFrameType, func_name, no_unliteral=True)(zkm__tfgpl
            )
...
def _validate_arguments_mask_where(func_name, df, cond, other, inplace,
    ...
    rita__jbngr = dict(inplace=inplace, level=level, errors=errors,
        try_cast=try_cast)
    wrut__cabef = dict(inplace=False, level=None, errors='raise', try_cast=
        False)
    check_unsupported_args(f'{func_name}', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
...
def _gen_init_df(header, columns, data_args, index=None, extra_globals=None):
    ...
    if extra_globals is None:
        extra_globals = {}
    rrnbc__duaiy = ColNamesMetaType(tuple(columns))
    data_args = '({}{})'.format(data_args, ',' if data_args else '')
    kkj__mzrbr = f"""{header}  return bodo.hiframes.pd_dataframe_ext.init_dataframe({data_args}, {index}, __col_name_meta_value_gen_init_df)
"""
    vkbo__dapyc = {}
    uyqcv__zoo = {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba,
        '__col_name_meta_value_gen_init_df': rrnbc__duaiy}
    uyqcv__zoo.update(extra_globals)
    exec(kkj__mzrbr, uyqcv__zoo, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def _get_binop_columns(lhs, rhs, is_inplace=False):
    if lhs.columns != rhs.columns:
        mnn__its = pd.Index(lhs.columns)
        zxlaa__crcxy = pd.Index(rhs.columns)
        nulml__czgrn, plpg__jkieu, kzlri__lfg = mnn__its.join(zxlaa__crcxy,
            how='left' if is_inplace else 'outer', level=None,
            return_indexers=True)
        return tuple(nulml__czgrn), plpg__jkieu, kzlri__lfg
...
def create_binary_op_overload(op):
    ...
    def overload_dataframe_binary_op(lhs, rhs):
        uxxpx__hyjdc = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        oxfve__zrlfl = operator.eq, operator.ne
        check_runtime_cols_unsupported(lhs, uxxpx__hyjdc)
        check_runtime_cols_unsupported(rhs, uxxpx__hyjdc)
        if isinstance(lhs, DataFrameType):
            if isinstance(rhs, DataFrameType):
                nulml__czgrn, plpg__jkieu, kzlri__lfg = _get_binop_columns(lhs,
                    rhs)
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {mcl__efyp}) {uxxpx__hyjdc}bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {rdrhs__cpmzl})'
                     if mcl__efyp != -1 and rdrhs__cpmzl != -1 else
                    f'bodo.libs.array_kernels.gen_na_array(len(lhs), float64_arr_type)'
                     for mcl__efyp, rdrhs__cpmzl in zip(plpg__jkieu,
                    kzlri__lfg))
                header = 'def impl(lhs, rhs):\n'
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(lhs)')
                return _gen_init_df(header, nulml__czgrn, data_args, index,
            ...
            dkyh__lfc = []
            jtg__whm = []
            if op in oxfve__zrlfl:
                for i, ehsm__qlma in enumerate(lhs.data):
                    if is_common_scalar_dtype([ehsm__qlma.dtype, rhs]):
                        dkyh__lfc.append(
                            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {uxxpx__hyjdc} rhs'
                            )
                    else:
                        yqew__boekv = f'arr{i}'
                        jtg__whm.append(yqew__boekv)
                        dkyh__lfc.append(yqew__boekv)
                data_args = ', '.join(dkyh__lfc)
            else:
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {uxxpx__hyjdc} rhs'
            ...
            if len(jtg__whm) > 0:
                ...
                header += '  n = len(lhs)\n'
                header += ''.join(
                    f'  {yqew__boekv} = np.empty(n, dtype=np.bool_)\n' for
                    yqew__boekv in jtg__whm)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(yqew__boekv, 
                    op == operator.ne) for yqew__boekv in jtg__whm)
        ...
        if isinstance(rhs, DataFrameType):
            ...
            jtg__whm = []
            if op in oxfve__zrlfl:
                for i, ehsm__qlma in enumerate(rhs.data):
                    if is_common_scalar_dtype([lhs, ehsm__qlma.dtype]):
                        dkyh__lfc.append(
                            f'lhs {uxxpx__hyjdc} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {i})'
                            )
                    else:
                        yqew__boekv = f'arr{i}'
                        jtg__whm.append(yqew__boekv)
                ...
                data_args = ', '.join(
                    ...
                    .format(i, uxxpx__hyjdc) for i in range(len(rhs.columns)))
            header = 'def impl(lhs, rhs):\n'
            if len(jtg__whm) > 0:
                header += '  numba.parfors.parfor.init_prange()\n'
                header += '  n = len(rhs)\n'
                header += ''.join('  {0} = np.empty(n, dtype=np.bool_)\n'.
                    format(yqew__boekv) for yqew__boekv in jtg__whm)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(yqew__boekv, 
                    op == operator.ne) for yqew__boekv in jtg__whm)
...
def _install_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_binary_ops:
        if op in skips:
            continue
        zkm__tfgpl = create_binary_op_overload(op)
        overload(op)(zkm__tfgpl)
...
def create_inplace_binary_op_overload(op):
    ...
    def overload_dataframe_inplace_binary_op(left, right):
        uxxpx__hyjdc = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        check_runtime_cols_unsupported(left, uxxpx__hyjdc)
        check_runtime_cols_unsupported(right, uxxpx__hyjdc)
        if isinstance(left, DataFrameType):
            if isinstance(right, DataFrameType):
                nulml__czgrn, _, kzlri__lfg = _get_binop_columns(left,
                    right, True)
                kkj__mzrbr = 'def impl(left, right):\n'
                for i, rdrhs__cpmzl in enumerate(kzlri__lfg):
                    if rdrhs__cpmzl == -1:
                        kkj__mzrbr += f"""  df_arr{i} = bodo.libs.array_kernels.gen_na_array(len(left), float64_arr_type)
"""
                        continue
                    kkj__mzrbr += f"""  df_arr{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(left, {i})
"""
                    kkj__mzrbr += f"""  df_arr{i} {uxxpx__hyjdc} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(right, {rdrhs__cpmzl})
"""
                data_args = ', '.join(f'df_arr{i}' for i in range(len(
                    nulml__czgrn)))
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)')
                return _gen_init_df(kkj__mzrbr, nulml__czgrn, data_args,
...
"""
                    .format(i))
                kkj__mzrbr += '  df_arr{0} {1} right\n'.format(i, uxxpx__hyjdc)
            data_args = ', '.join('df_arr{}'.format(i) for i in range(len(
                left.columns)))
            index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)'
            return _gen_init_df(kkj__mzrbr, left.columns, data_args, index)
    return overload_dataframe_inplace_binary_op


def _install_inplace_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_inplace_binary_ops:
        zkm__tfgpl = create_inplace_binary_op_overload(op)
        overload(op, no_unliteral=True)(zkm__tfgpl)
...
def create_unary_op_overload(op):
    ...
    def overload_dataframe_unary_op(df):
        if isinstance(df, DataFrameType):
            uxxpx__hyjdc = numba.core.utils.OPERATORS_TO_BUILTINS[op]
            check_runtime_cols_unsupported(df, uxxpx__hyjdc)
            data_args = ', '.join(
                '{1} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
                .format(i, uxxpx__hyjdc) for i in range(len(df.columns)))
            header = 'def impl(df):\n'
            return _gen_init_df(header, df.columns, data_args)
...
def _install_unary_ops():
    for op in bodo.hiframes.pd_series_ext.series_unary_ops:
        zkm__tfgpl = create_unary_op_overload(op)
        overload(op, no_unliteral=True)(zkm__tfgpl)
...
def overload_isna(obj):
    ...
    if is_array_typ(obj):
        ...
        def impl(obj):
            numba.parfors.parfor.init_prange()
            n = len(obj)
            umgsy__spj = np.empty(n, np.bool_)
            for i in numba.parfors.parfor.internal_prange(n):
                umgsy__spj[i] = bodo.libs.array_kernels.isna(obj, i)
            return umgsy__spj
        return impl
...
def overload_dataframe_replace(df, to_replace=None, value=None, inplace=
    ...
    cft__ozzo = {'inplace': inplace, 'limit': limit, 'regex': regex,
        'method': method}
    bckj__ejpg = {'inplace': False, 'limit': None, 'regex': False, 'method':
        'pad'}
    check_unsupported_args('replace', cft__ozzo, bckj__ejpg, package_name=
        'pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def _is_col_access(expr_node):
    hkvit__nygjd = str(expr_node)
    return hkvit__nygjd.startswith('(left.') or hkvit__nygjd.startswith(
        '(right.')


def _insert_NA_cond(expr_node, left_columns, left_data, right_columns,
    right_data):
    bupvf__umxwg = {'left': 0, 'right': 0, 'NOT_NA': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (bupvf__umxwg,))
    agz__sfqsf = pd.core.computation.parsing.clean_column_name

    def append_null_checks(expr_node, null_set):
        if not null_set:
            return expr_node
        pzapf__zxqzw = ' & '.join([('NOT_NA.`' + x + '`') for x in null_set])
        nqmtx__bein = {('NOT_NA', agz__sfqsf(ehsm__qlma)): ehsm__qlma for
            ehsm__qlma in null_set}
        cwzvw__csbi, _, _ = _parse_query_expr(pzapf__zxqzw, env, [], [],
            None, join_cleaned_cols=nqmtx__bein)
        evg__zdcg = (pd.core.computation.ops.BinOp.
            _disallow_scalar_only_bool_ops)
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        try:
            ishz__xupj = pd.core.computation.ops.BinOp('&', cwzvw__csbi,
                expr_node)
        finally:
    ...
    def _insert_NA_cond_body(expr_node, null_set):
        if isinstance(expr_node, pd.core.computation.ops.BinOp):
            if expr_node.op == '|':
                lraw__fkv = set()
                larsp__xudd = set()
                ruwq__xkgk = _insert_NA_cond_body(expr_node.lhs, lraw__fkv)
                mivoe__ckweb = _insert_NA_cond_body(expr_node.rhs, larsp__xudd)
                drx__adxy = lraw__fkv.intersection(larsp__xudd)
                lraw__fkv.difference_update(drx__adxy)
                larsp__xudd.difference_update(drx__adxy)
                null_set.update(drx__adxy)
                expr_node.lhs = append_null_checks(ruwq__xkgk, lraw__fkv)
                expr_node.rhs = append_null_checks(mivoe__ckweb, larsp__xudd)
            ...
            phmlk__mlcr = expr_node.name
            xilfa__fmzvy, col_name = phmlk__mlcr.split('.')
            if xilfa__fmzvy == 'left':
                slkb__zpkf = left_columns
                data = left_data
            else:
                slkb__zpkf = right_columns
                data = right_data
            obv__ufmdi = data[slkb__zpkf.index(col_name)]
            if bodo.utils.typing.is_nullable(obv__ufmdi):
                null_set.add(expr_node.name)
        return expr_node
    null_set = set()
    akfu__sab = _insert_NA_cond_body(expr_node, null_set)
...
def _extract_equal_conds(expr_node):
    ...
    if expr_node.op == '==' and _is_col_access(expr_node.lhs
        ...
        ikb__ubnxg = str(expr_node.lhs)
        ymgc__jwr = str(expr_node.rhs)
        if ikb__ubnxg.startswith('(left.') and ymgc__jwr.startswith('(left.'
            ) or ikb__ubnxg.startswith('(right.') and ymgc__jwr.startswith(
            '(right.'):
            return [], [], expr_node
        left_on = [ikb__ubnxg.split('.')[1][:-1]]
        right_on = [ymgc__jwr.split('.')[1][:-1]]
        if ikb__ubnxg.startswith('(right.'):
            return right_on, left_on, None
        return left_on, right_on, None
    if expr_node.op == '&':
        vxvo__iaztm, sbfsm__obzzy, dwv__lcset = _extract_equal_conds(expr_node
            .lhs)
        dqew__cwsi, xfnmj__vegol, hcnpx__lple = _extract_equal_conds(expr_node
            .rhs)
        left_on = vxvo__iaztm + dqew__cwsi
        right_on = sbfsm__obzzy + xfnmj__vegol
        if dwv__lcset is None:
            return left_on, right_on, hcnpx__lple
        if hcnpx__lple is None:
            return left_on, right_on, dwv__lcset
        expr_node.lhs = dwv__lcset
        expr_node.rhs = hcnpx__lple
        expr_node.operands = expr_node.lhs, expr_node.rhs
...
def _parse_merge_cond(on_str, left_columns, left_data, right_columns,
    right_data):
    bupvf__umxwg = {'left': 0, 'right': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (bupvf__umxwg,))
    ouol__ygl = dict()
    agz__sfqsf = pd.core.computation.parsing.clean_column_name
    for name, gkpom__wgdh in (('left', left_columns), ('right', right_columns)
        ):
        for ehsm__qlma in gkpom__wgdh:
            sur__hgnpz = agz__sfqsf(ehsm__qlma)
            ecmoj__ccxc = name, sur__hgnpz
            if ecmoj__ccxc in ouol__ygl:
                raise_bodo_error(
                    f"pd.merge(): {name} table contains two columns that are escaped to the same Python identifier '{ehsm__qlma}' and '{ouol__ygl[sur__hgnpz]}' Please rename one of these columns. To avoid this issue, please use names that are valid Python identifiers."
                    )
            ouol__ygl[ecmoj__ccxc] = ehsm__qlma
    rdr__kldqd, _, _ = _parse_query_expr(on_str, env, [], [], None,
        join_cleaned_cols=ouol__ygl)
    left_on, right_on, kvj__bqd = _extract_equal_conds(rdr__kldqd.terms)
    return left_on, right_on, _insert_NA_cond(kvj__bqd, left_columns,
...
def overload_dataframe_merge(left, right, how='inner', on=None, left_on=
    ...
    rita__jbngr = dict(sort=sort, copy=copy, validate=validate)
    wrut__cabef = dict(sort=False, copy=True, validate=None)
    check_unsupported_args('DataFrame.merge', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    validate_merge_spec(left, right, how, on, left_on, right_on, left_index,
        right_index, sort, suffixes, copy, indicator, validate)
    how = get_overload_const_str(how)
    euv__bvk = tuple(sorted(set(left.columns) & set(right.columns), key=lambda
        k: str(k)))
    xyalg__zjck = ''
    if not is_overload_none(on):
        ...
        if is_overload_constant_str(on):
            ...
            if on_str not in euv__bvk and ('left.' in on_str or 'right.' in
                on_str):
                left_on, right_on, xqc__bmc = _parse_merge_cond(on_str,
                    left.columns, left.data, right.columns, right.data)
                if xqc__bmc is None:
                    xyalg__zjck = ''
                else:
                    xyalg__zjck = str(xqc__bmc)
    ...
    dgaj__rroq = get_overload_const_bool(_bodo_na_equal)
    ...
    if is_overload_constant_tuple(suffixes):
        cyuan__xxctc = get_overload_const_tuple(suffixes)
    if is_overload_constant_list(suffixes):
        cyuan__xxctc = list(get_overload_const_list(suffixes))
    suffix_x = cyuan__xxctc[0]
    suffix_y = cyuan__xxctc[1]
    ...
    kkj__mzrbr = "def _impl(left, right, how='inner', on=None, left_on=None,\n"
    kkj__mzrbr += (
        '    right_on=None, left_index=False, right_index=False, sort=False,\n'
        )
    kkj__mzrbr += """    suffixes=('_x', '_y'), copy=True, indicator=False, validate=None, _bodo_na_equal=True):
"""
    kkj__mzrbr += (
        """  return bodo.hiframes.pd_dataframe_ext.join_dummy(left, right, {}, {}, '{}', '{}', '{}', False, {}, {}, '{}')
"""
        .format(left_keys, right_keys, how, suffix_x, suffix_y,
        indicator_val, dgaj__rroq, xyalg__zjck))
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo}, vkbo__dapyc)
    _impl = vkbo__dapyc['_impl']
    return _impl
...
def common_validate_merge_merge_asof_spec(name_func, left, right, on,
    ...
    zvgt__apog = {string_array_type, dict_str_arr_type, binary_array_type,
        datetime_date_array_type, datetime_timedelta_array_type, boolean_array}
    lyym__opy = {get_overload_const_str(cyqn__npses) for cyqn__npses in (
        left_on, right_on, on) if is_overload_constant_str(cyqn__npses)}
    for df in (left, right):
        for i, ehsm__qlma in enumerate(df.data):
            if not isinstance(ehsm__qlma, valid_dataframe_column_types
                ) and ehsm__qlma not in zvgt__apog:
                raise BodoError(
                    f'{name_func}(): use of column with {type(ehsm__qlma)} in merge unsupported'
                    )
            if df.columns[i] in lyym__opy and isinstance(ehsm__qlma,
                MapArrayType):
                raise BodoError(
    ...
    if is_overload_constant_list(suffixes):
        cyuan__xxctc = list(get_overload_const_list(suffixes))
    if len(cyuan__xxctc) != 2:
        raise BodoError(name_func +
            '(): The number of suffixes should be exactly 2')
    euv__bvk = tuple(set(left.columns) & set(right.columns))
    if not is_overload_none(on):
        lkapy__nnt = False
        if is_overload_constant_str(on):
            on_str = get_overload_const_str(on)
            lkapy__nnt = on_str not in euv__bvk and ('left.' in on_str or 
                'right.' in on_str)
        if len(euv__bvk) == 0 and not lkapy__nnt:
            raise_bodo_error(name_func +
...
def validate_keys_dtypes(left, right, left_index, right_index, left_keys,
    right_keys):
    glt__hpb = numba.core.registry.cpu_target.typing_context
    if is_overload_true(left_index) or is_overload_true(right_index):
        if is_overload_true(left_index) and is_overload_true(right_index):
            ubeub__rsfrw = left.index
            kmmv__cbh = isinstance(ubeub__rsfrw, StringIndexType)
            oil__weezc = right.index
            bsl__hmsoi = isinstance(oil__weezc, StringIndexType)
        elif is_overload_true(left_index):
            ubeub__rsfrw = left.index
            kmmv__cbh = isinstance(ubeub__rsfrw, StringIndexType)
            oil__weezc = right.data[right.columns.index(right_keys[0])]
            bsl__hmsoi = oil__weezc.dtype == string_type
        elif is_overload_true(right_index):
            ubeub__rsfrw = left.data[left.columns.index(left_keys[0])]
            kmmv__cbh = ubeub__rsfrw.dtype == string_type
            oil__weezc = right.index
            bsl__hmsoi = isinstance(oil__weezc, StringIndexType)
        if kmmv__cbh and bsl__hmsoi:
            return
        ubeub__rsfrw = ubeub__rsfrw.dtype
        oil__weezc = oil__weezc.dtype
        try:
            wsjjm__idjh = glt__hpb.resolve_function_type(operator.eq, (
                ubeub__rsfrw, oil__weezc), {})
        except:
            raise_bodo_error(
                'merge: You are trying to merge on {lk_dtype} and {rk_dtype} columns. If you wish to proceed you should use pd.concat'
                .format(lk_dtype=ubeub__rsfrw, rk_dtype=oil__weezc))
    else:
        for xwr__ehmr, ifkyi__myjw in zip(left_keys, right_keys):
            ubeub__rsfrw = left.data[left.columns.index(xwr__ehmr)].dtype
            frj__gxzhx = left.data[left.columns.index(xwr__ehmr)]
            oil__weezc = right.data[right.columns.index(ifkyi__myjw)].dtype
            siym__ekxl = right.data[right.columns.index(ifkyi__myjw)]
            if frj__gxzhx == siym__ekxl:
                continue
            tky__bxu = (
                ...
                .format(lk=xwr__ehmr, lk_dtype=ubeub__rsfrw, rk=ifkyi__myjw,
                rk_dtype=oil__weezc))
            fhjad__znl = ubeub__rsfrw == string_type
            ftcl__tjc = oil__weezc == string_type
            if fhjad__znl ^ ftcl__tjc:
                raise_bodo_error(tky__bxu)
            try:
                wsjjm__idjh = glt__hpb.resolve_function_type(operator.eq, (
                    ubeub__rsfrw, oil__weezc), {})
            except:
...
def validate_keys(keys, df):
    tmlj__kddjh = set(keys).difference(set(df.columns))
    if len(tmlj__kddjh) > 0:
        if is_overload_constant_str(df.index.name_typ
            ) and get_overload_const_str(df.index.name_typ) in tmlj__kddjh:
            raise_bodo_error(
                f'merge(): use of index {df.index.name_typ} as key for on/left_on/right_on is unsupported'
                )
        raise_bodo_error(
            f"""merge(): invalid key {tmlj__kddjh} for on/left_on/right_on
...
def overload_dataframe_join(left, other, on=None, how='left', lsuffix='',
    ...
    rita__jbngr = dict(lsuffix=lsuffix, rsuffix=rsuffix)
    wrut__cabef = dict(lsuffix='', rsuffix='')
    check_unsupported_args('DataFrame.join', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    ...
    left_keys = gen_const_tup(left_keys)
    right_keys = gen_const_tup(right_keys)
    kkj__mzrbr = "def _impl(left, other, on=None, how='left',\n"
    kkj__mzrbr += "    lsuffix='', rsuffix='', sort=False):\n"
    kkj__mzrbr += (
...
def validate_join_spec(left, other, on, how, lsuffix, rsuffix, sort):
    ...
    if not is_overload_none(on):
        pml__lhn = get_overload_const_list(on)
        validate_keys(pml__lhn, left)
    if not is_overload_false(sort):
        raise BodoError(
            'join(): sort parameter only supports default value False')
    euv__bvk = tuple(set(left.columns) & set(other.columns))
    if len(euv__bvk) > 0:
        raise_bodo_error(
            'join(): not supporting joining on overlapping columns:{cols} Use DataFrame.merge() instead.'
            .format(cols=euv__bvk))


def validate_unicity_output_column_names(suffix_x, suffix_y, left_keys,
    right_keys, left_columns, right_columns, indicator_val):
    llehy__cclfl = set(left_keys) & set(right_keys)
    rams__iajm = set(left_columns) & set(right_columns)
    udg__eou = rams__iajm - llehy__cclfl
    cbz__ljs = set(left_columns) - rams__iajm
    jcdy__yifg = set(right_columns) - rams__iajm
    hbb__hng = {}

    def insertOutColumn(col_name):
        if col_name in hbb__hng:
            raise_bodo_error(
                'join(): two columns happen to have the same name : {}'.
                format(col_name))
        hbb__hng[col_name] = 0
    for cykhb__gtpf in llehy__cclfl:
        insertOutColumn(cykhb__gtpf)
    for cykhb__gtpf in udg__eou:
        pzq__musof = str(cykhb__gtpf) + suffix_x
        mhnq__dyr = str(cykhb__gtpf) + suffix_y
        insertOutColumn(pzq__musof)
        insertOutColumn(mhnq__dyr)
    for cykhb__gtpf in cbz__ljs:
        insertOutColumn(cykhb__gtpf)
    for cykhb__gtpf in jcdy__yifg:
        insertOutColumn(cykhb__gtpf)
    if indicator_val:
...
def overload_dataframe_merge_asof(left, right, on=None, left_on=None,
    ...
    if isinstance(suffixes, tuple):
        cyuan__xxctc = suffixes
    if is_overload_constant_list(suffixes):
        cyuan__xxctc = list(get_overload_const_list(suffixes))
    if isinstance(suffixes, types.Omitted):
        cyuan__xxctc = suffixes.value
    suffix_x = cyuan__xxctc[0]
    suffix_y = cyuan__xxctc[1]
    kkj__mzrbr = (
    ...
    kkj__mzrbr += "    right_by=None, suffixes=('_x', '_y'), tolerance=None,\n"
    kkj__mzrbr += "    allow_exact_matches=True, direction='backward'):\n"
    kkj__mzrbr += '  suffix_x = suffixes[0]\n'
    kkj__mzrbr += '  suffix_y = suffixes[1]\n'
    kkj__mzrbr += (
...
def validate_groupby_spec(df, by, axis, level, as_index, sort, group_keys,
    ...
    rita__jbngr = dict(sort=sort, group_keys=group_keys, squeeze=squeeze,
        observed=observed)
    fday__fsolu = dict(sort=False, group_keys=True, squeeze=False, observed
        =True)
    check_unsupported_args('Dataframe.groupby', rita__jbngr, fday__fsolu,
        package_name='pandas', module_name='GroupBy')


def pivot_error_checking(df, index, columns, values, func_name):
    ikve__let = func_name == 'DataFrame.pivot_table'
    if ikve__let:
    ...
    dpc__cvgt = get_literal_value(columns)
    if isinstance(dpc__cvgt, (list, tuple)):
        if len(dpc__cvgt) > 1:
            raise BodoError(
                f"{func_name}(): 'columns' argument must be a constant column label not a {dpc__cvgt}"
                )
        dpc__cvgt = dpc__cvgt[0]
    if dpc__cvgt not in df.columns:
        raise BodoError(
            f"{func_name}(): 'columns' column {dpc__cvgt} not found in DataFrame {df}."
            )
    jjkx__fmnt = df.column_index[dpc__cvgt]
    if is_overload_none(index):
        nziod__ypy = []
        shcu__gqrc = []
    else:
        shcu__gqrc = get_literal_value(index)
        if not isinstance(shcu__gqrc, (list, tuple)):
            shcu__gqrc = [shcu__gqrc]
        nziod__ypy = []
        for index in shcu__gqrc:
            if index not in df.column_index:
                raise BodoError(
                    f"{func_name}(): 'index' column {index} not found in DataFrame {df}."
                    )
            nziod__ypy.append(df.column_index[index])
    if not (all(isinstance(oqsb__jqghr, int) for oqsb__jqghr in shcu__gqrc) or
        all(isinstance(oqsb__jqghr, str) for oqsb__jqghr in shcu__gqrc)):
        raise BodoError(
    ...
    if is_overload_none(values):
        yzjtg__ugcl = []
        lmhbt__luc = []
        btzew__onowu = nziod__ypy + [jjkx__fmnt]
        for i, oqsb__jqghr in enumerate(df.columns):
            if i not in btzew__onowu:
                yzjtg__ugcl.append(i)
                lmhbt__luc.append(oqsb__jqghr)
    else:
        lmhbt__luc = get_literal_value(values)
        if not isinstance(lmhbt__luc, (list, tuple)):
            lmhbt__luc = [lmhbt__luc]
        yzjtg__ugcl = []
        for val in lmhbt__luc:
            if val not in df.column_index:
                raise BodoError(
                    f"{func_name}(): 'values' column {val} not found in DataFrame {df}."
                    )
            yzjtg__ugcl.append(df.column_index[val])
    jokw__kkpu = set(yzjtg__ugcl) | set(nziod__ypy) | {jjkx__fmnt}
    if len(jokw__kkpu) != len(yzjtg__ugcl) + len(nziod__ypy) + 1:
        raise BodoError(
    ...
    if len(nziod__ypy) == 0:
        ...
        for amhov__wrwo in nziod__ypy:
            index_column = df.data[amhov__wrwo]
            check_valid_index_typ(index_column)
    cdwt__tcj = df.data[jjkx__fmnt]
    if isinstance(cdwt__tcj, (bodo.ArrayItemArrayType, bodo.MapArrayType,
    ...
    if isinstance(cdwt__tcj, bodo.CategoricalArrayType):
        raise BodoError(
            f"{func_name}(): 'columns' DataFrame column does not support categorical data"
            )
    for lysf__lhox in yzjtg__ugcl:
        ktv__mhme = df.data[lysf__lhox]
        if isinstance(ktv__mhme, (bodo.ArrayItemArrayType, bodo.
            MapArrayType, bodo.StructArrayType, bodo.TupleArrayType)
            ) or ktv__mhme == bodo.binary_array_type:
            raise BodoError(
                f"{func_name}(): 'values' DataFrame column must have scalar rows"
                )
    return (shcu__gqrc, dpc__cvgt, lmhbt__luc, nziod__ypy, jjkx__fmnt,
        yzjtg__ugcl)
...
def overload_dataframe_pivot(data, index=None, columns=None, values=None):
    ...
    (shcu__gqrc, dpc__cvgt, lmhbt__luc, amhov__wrwo, jjkx__fmnt, fns__nsgre
    ...
    if len(shcu__gqrc) == 0:
        if is_overload_none(data.index.name_typ):
            ...
            uyegd__abw = get_literal_value(data.index.name_typ),
    else:
        uyegd__abw = tuple(shcu__gqrc)
    shcu__gqrc = ColNamesMetaType(uyegd__abw)
    lmhbt__luc = ColNamesMetaType(tuple(lmhbt__luc))
    dpc__cvgt = ColNamesMetaType((dpc__cvgt,))
    kkj__mzrbr = 'def impl(data, index=None, columns=None, values=None):\n'
    kkj__mzrbr += "    ev = tracing.Event('df.pivot')\n"
    kkj__mzrbr += f'    pivot_values = data.iloc[:, {jjkx__fmnt}].unique()\n'
    kkj__mzrbr += '    result = bodo.hiframes.pd_dataframe_ext.pivot_impl(\n'
    if len(amhov__wrwo) == 0:
        kkj__mzrbr += f"""        (bodo.utils.conversion.index_to_array(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(data)),),
"""
    else:
        kkj__mzrbr += '        (\n'
        for ozyx__szw in amhov__wrwo:
            kkj__mzrbr += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {ozyx__szw}),
"""
        kkj__mzrbr += '        ),\n'
    kkj__mzrbr += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {jjkx__fmnt}),),
"""
    kkj__mzrbr += '        (\n'
    for lysf__lhox in fns__nsgre:
        kkj__mzrbr += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {lysf__lhox}),
"""
    kkj__mzrbr += '        ),\n'
    kkj__mzrbr += '        pivot_values,\n'
    kkj__mzrbr += '        index_lit,\n'
    kkj__mzrbr += '        columns_lit,\n'
    kkj__mzrbr += '        values_lit,\n'
    kkj__mzrbr += '    )\n'
    kkj__mzrbr += '    ev.finalize()\n'
    kkj__mzrbr += '    return result\n'
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'index_lit': shcu__gqrc, 'columns_lit':
        dpc__cvgt, 'values_lit': lmhbt__luc, 'tracing': tracing}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def overload_dataframe_pivot_table(data, values=None, index=None, columns=
    ...
    rita__jbngr = dict(fill_value=fill_value, margins=margins, dropna=
        dropna, margins_name=margins_name, observed=observed, sort=sort)
    wrut__cabef = dict(fill_value=None, margins=False, dropna=True,
        margins_name='All', observed=False, sort=True)
    check_unsupported_args('DataFrame.pivot_table', rita__jbngr,
        wrut__cabef, package_name='pandas', module_name='DataFrame')
    if not isinstance(data, DataFrameType):
        raise BodoError(
    ...
    (shcu__gqrc, dpc__cvgt, lmhbt__luc, amhov__wrwo, jjkx__fmnt, fns__nsgre
        ...
        'DataFrame.pivot_table'))
    doo__ducz = shcu__gqrc
    shcu__gqrc = ColNamesMetaType(tuple(shcu__gqrc))
    lmhbt__luc = ColNamesMetaType(tuple(lmhbt__luc))
    mxx__pjdh = dpc__cvgt
    dpc__cvgt = ColNamesMetaType((dpc__cvgt,))
    kkj__mzrbr = 'def impl(\n'
    kkj__mzrbr += '    data,\n'
    kkj__mzrbr += '    values=None,\n'
    kkj__mzrbr += '    index=None,\n'
    kkj__mzrbr += '    columns=None,\n'
    kkj__mzrbr += '    aggfunc="mean",\n'
    kkj__mzrbr += '    fill_value=None,\n'
    kkj__mzrbr += '    margins=False,\n'
    kkj__mzrbr += '    dropna=True,\n'
    kkj__mzrbr += '    margins_name="All",\n'
    kkj__mzrbr += '    observed=False,\n'
    kkj__mzrbr += '    sort=True,\n'
    kkj__mzrbr += '    _pivot_values=None,\n'
    kkj__mzrbr += '):\n'
    kkj__mzrbr += "    ev = tracing.Event('df.pivot_table')\n"
    fwjs__tcy = amhov__wrwo + [jjkx__fmnt] + fns__nsgre
    kkj__mzrbr += f'    data = data.iloc[:, {fwjs__tcy}]\n'
    sthcn__qcld = doo__ducz + [mxx__pjdh]
    if not is_overload_none(_pivot_values):
        sfgf__pomz = tuple(sorted(_pivot_values.meta))
        _pivot_values = ColNamesMetaType(sfgf__pomz)
        kkj__mzrbr += '    pivot_values = _pivot_values_arr\n'
        kkj__mzrbr += (
            f'    data = data[data.iloc[:, {len(amhov__wrwo)}].isin(pivot_values)]\n'
            )
        if all(isinstance(oqsb__jqghr, str) for oqsb__jqghr in sfgf__pomz):
            ocsl__waupl = pd.array(sfgf__pomz, 'string')
        elif all(isinstance(oqsb__jqghr, int) for oqsb__jqghr in sfgf__pomz):
            ocsl__waupl = np.array(sfgf__pomz, 'int64')
        else:
            raise BodoError(
        ...
        ocsl__waupl = None
    pde__wvqaq = is_overload_constant_str(aggfunc) and get_overload_const_str(
        aggfunc) == 'nunique'
    zye__kwxat = len(sthcn__qcld) if pde__wvqaq else len(doo__ducz)
    kkj__mzrbr += f"""    data = data.groupby({sthcn__qcld!r}, as_index=False, _bodo_num_shuffle_keys={zye__kwxat}).agg(aggfunc)
"""
    if is_overload_none(_pivot_values):
        kkj__mzrbr += (
            f'    pivot_values = data.iloc[:, {len(amhov__wrwo)}].unique()\n')
    kkj__mzrbr += '    result = bodo.hiframes.pd_dataframe_ext.pivot_impl(\n'
    kkj__mzrbr += '        (\n'
    for i in range(0, len(amhov__wrwo)):
        kkj__mzrbr += (
            f'            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {i}),\n'
            )
    kkj__mzrbr += '        ),\n'
    kkj__mzrbr += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {len(amhov__wrwo)}),),
"""
    kkj__mzrbr += '        (\n'
    for i in range(len(amhov__wrwo) + 1, len(fns__nsgre) + len(amhov__wrwo) + 1
        ):
        kkj__mzrbr += (
            f'            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(data, {i}),\n'
            )
    ...
    kkj__mzrbr += '        columns_lit,\n'
    kkj__mzrbr += '        values_lit,\n'
    kkj__mzrbr += '        check_duplicates=False,\n'
    kkj__mzrbr += f'        is_already_shuffled={not pde__wvqaq},\n'
    kkj__mzrbr += '        _constant_pivot_values=_constant_pivot_values,\n'
    kkj__mzrbr += '    )\n'
    kkj__mzrbr += '    ev.finalize()\n'
    kkj__mzrbr += '    return result\n'
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'numba': numba, 'index_lit': shcu__gqrc,
        'columns_lit': dpc__cvgt, 'values_lit': lmhbt__luc,
        '_pivot_values_arr': ocsl__waupl, '_constant_pivot_values':
        _pivot_values, 'tracing': tracing}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def overload_dataframe_melt(frame, id_vars=None, value_vars=None, var_name=
    ...
    rita__jbngr = dict(col_level=col_level, ignore_index=ignore_index)
    wrut__cabef = dict(col_level=None, ignore_index=True)
    check_unsupported_args('DataFrame.melt', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    ...
    izj__cuti = get_literal_value(id_vars) if not is_overload_none(id_vars
        ) else []
    if not isinstance(izj__cuti, (list, tuple)):
        izj__cuti = [izj__cuti]
    for oqsb__jqghr in izj__cuti:
        if oqsb__jqghr not in frame.columns:
            raise BodoError(
                f"DataFrame.melt(): 'id_vars' column {oqsb__jqghr} not found in {frame}."
                )
    tejqy__dbg = [frame.column_index[i] for i in izj__cuti]
    if is_overload_none(value_vars):
        hykr__qlq = []
        mgssq__nhrwt = []
        for i, oqsb__jqghr in enumerate(frame.columns):
            if i not in tejqy__dbg:
                hykr__qlq.append(i)
                mgssq__nhrwt.append(oqsb__jqghr)
    else:
        mgssq__nhrwt = get_literal_value(value_vars)
        if not isinstance(mgssq__nhrwt, (list, tuple)):
            mgssq__nhrwt = [mgssq__nhrwt]
        mgssq__nhrwt = [v for v in mgssq__nhrwt if v not in izj__cuti]
        if not mgssq__nhrwt:
            raise BodoError(
                "DataFrame.melt(): currently empty 'value_vars' is unsupported."
                )
        hykr__qlq = []
        for val in mgssq__nhrwt:
            if val not in frame.column_index:
                raise BodoError(
                    f"DataFrame.melt(): 'value_vars' column {val} not found in DataFrame {frame}."
                    )
            hykr__qlq.append(frame.column_index[val])
    for oqsb__jqghr in mgssq__nhrwt:
        if oqsb__jqghr not in frame.columns:
            raise BodoError(
                f"DataFrame.melt(): 'value_vars' column {oqsb__jqghr} not found in {frame}."
                )
    if not (all(isinstance(oqsb__jqghr, int) for oqsb__jqghr in
        mgssq__nhrwt) or all(isinstance(oqsb__jqghr, str) for oqsb__jqghr in
        mgssq__nhrwt)):
        raise BodoError(
    ...
    rkdi__asn = frame.data[hykr__qlq[0]]
    yktb__htjx = [frame.data[i].dtype for i in hykr__qlq]
    hykr__qlq = np.array(hykr__qlq, dtype=np.int64)
    tejqy__dbg = np.array(tejqy__dbg, dtype=np.int64)
    _, zats__oas = bodo.utils.typing.get_common_scalar_dtype(yktb__htjx)
    if not zats__oas:
        raise BodoError(
            "DataFrame.melt(): columns selected in 'value_vars' must have a unifiable type."
            )
    extra_globals = {'np': np, 'value_lit': mgssq__nhrwt, 'val_type': rkdi__asn
        }
    header = 'def impl(\n'
    header += '  frame,\n'
    header += '  id_vars=None,\n'
    ...
    if frame.is_table_format and all(v == rkdi__asn.dtype for v in yktb__htjx):
    ...
    elif len(mgssq__nhrwt) == 1:
        header += f"""  val_col = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(frame, {hykr__qlq[0]})
"""
    else:
        grxv__rdb = ', '.join(
            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(frame, {i})'
             for i in hykr__qlq)
        header += (
            f'  val_col = bodo.libs.array_kernels.concat(({grxv__rdb},))\n')
...
"""
    for i in tejqy__dbg:
        ...
        header += (
            f'  out_id{i} = bodo.libs.array_kernels.concat([id{i}] * {len(mgssq__nhrwt)})\n'
            )
    zawb__fzph = ', '.join(f'out_id{i}' for i in tejqy__dbg) + (', ' if len
        (tejqy__dbg) > 0 else '')
    data_args = zawb__fzph + 'var_col, val_col'
    columns = tuple(izj__cuti + [var_name, value_name])
    index = (
        f'bodo.hiframes.pd_index_ext.init_range_index(0, len(frame) * {len(mgssq__nhrwt)}, 1, None)'
        )
...
def crosstab_overload(index, columns, values=None, rownames=None, colnames=
    ...
    rita__jbngr = dict(values=values, rownames=rownames, colnames=colnames,
        aggfunc=aggfunc, margins=margins, margins_name=margins_name, dropna
        =dropna, normalize=normalize)
    wrut__cabef = dict(values=None, rownames=None, colnames=None, aggfunc=
        None, margins=False, margins_name='All', dropna=True, normalize=False)
    check_unsupported_args('pandas.crosstab', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_sort_values(df, by, axis=0, ascending=True, inplace=
    ...
    rita__jbngr = dict(ignore_index=ignore_index, key=key)
    wrut__cabef = dict(ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_values', rita__jbngr,
        wrut__cabef, package_name='pandas', module_name='DataFrame')
...
def validate_sort_values_spec(df, by, axis, ascending, inplace, kind,
    ...
    if is_overload_none(by) or not is_literal_type(by
        ...
        raise_bodo_error(
            ...
            .format(by))
    cnyjx__btpj = set(df.columns)
    if is_overload_constant_str(df.index.name_typ):
        cnyjx__btpj.add(get_overload_const_str(df.index.name_typ))
    if is_overload_constant_tuple(by):
        qptsd__pcbo = [get_overload_const_tuple(by)]
    else:
        qptsd__pcbo = get_overload_const_list(by)
    qptsd__pcbo = set((k, '') if (k, '') in cnyjx__btpj else k for k in
        qptsd__pcbo)
    if len(qptsd__pcbo.difference(cnyjx__btpj)) > 0:
        ixd__soeg = list(set(get_overload_const_list(by)).difference(
            cnyjx__btpj))
        raise_bodo_error(f'sort_values(): invalid keys {ixd__soeg} for by.')
    if not is_overload_none(_bodo_chunk_bounds) and len(qptsd__pcbo) != 1:
        raise_bodo_error(
    ...
    if is_overload_constant_str(na_position):
        ...
        ljrof__qjeqa = get_overload_const_list(na_position)
        for na_position in ljrof__qjeqa:
            if na_position not in ('first', 'last'):
                raise BodoError(
...
def overload_dataframe_sort_index(df, axis=0, level=None, ascending=True,
    ...
    rita__jbngr = dict(axis=axis, level=level, kind=kind, sort_remaining=
        sort_remaining, ignore_index=ignore_index, key=key)
    wrut__cabef = dict(axis=0, level=None, kind='quicksort', sort_remaining
        =True, ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_index', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_rank(df, axis=0, method='average', numeric_only=None,
    ...
    kkj__mzrbr = """def impl(df, axis=0, method='average', numeric_only=None, na_option='keep', ascending=True, pct=False):
"""
    mnkxg__avk = len(df.columns)
    data_args = ', '.join(
...
def overload_dataframe_fillna(df, value=None, method=None, axis=None,
    ...
    rita__jbngr = dict(limit=limit, downcast=downcast)
    wrut__cabef = dict(limit=None, downcast=None)
    check_unsupported_args('DataFrame.fillna', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.fillna()')
    if not (is_overload_none(axis) or is_overload_zero(axis)):
        raise BodoError("DataFrame.fillna(): 'axis' argument not supported.")
    bqsb__wsi = not is_overload_none(value)
    hyxb__btvq = not is_overload_none(method)
    if bqsb__wsi and hyxb__btvq:
        raise BodoError(
            "DataFrame.fillna(): Cannot specify both 'value' and 'method'.")
    if not bqsb__wsi and not hyxb__btvq:
        raise BodoError(
            "DataFrame.fillna(): Must specify one of 'value' and 'method'.")
    if bqsb__wsi:
        qtbeh__bql = 'value=value'
    else:
        qtbeh__bql = 'method=method'
    data_args = [(
        f"df['{oqsb__jqghr}'].fillna({qtbeh__bql}, inplace=inplace)" if
        isinstance(oqsb__jqghr, str) else
        f'df[{oqsb__jqghr}].fillna({qtbeh__bql}, inplace=inplace)') for
        oqsb__jqghr in df.columns]
    kkj__mzrbr = """def impl(df, value=None, method=None, axis=None, inplace=False, limit=None, downcast=None):
"""
    if is_overload_true(inplace):
        kkj__mzrbr += '  ' + '  \n'.join(data_args) + '\n'
        vkbo__dapyc = {}
        exec(kkj__mzrbr, {}, vkbo__dapyc)
        impl = vkbo__dapyc['impl']
        return impl
    else:
        return _gen_init_df(kkj__mzrbr, df.columns, ', '.join(ufhh__uyo +
            '.values' for ufhh__uyo in data_args))
...
def overload_dataframe_reset_index(df, level=None, drop=False, inplace=
    ...
    rita__jbngr = dict(col_level=col_level, col_fill=col_fill)
    wrut__cabef = dict(col_level=0, col_fill='')
    check_unsupported_args('DataFrame.reset_index', rita__jbngr,
        wrut__cabef, package_name='pandas', module_name='DataFrame')
    ...
    kkj__mzrbr = """def impl(df, level=None, drop=False, inplace=False, col_level=0, col_fill='', _bodo_transformed=False,):
"""
    kkj__mzrbr += (
    ...
    if not drop:
        aof__pxzye = 'index' if 'index' not in columns else 'level_0'
        ...
        if isinstance(df.index, MultiIndexType):
            kkj__mzrbr += """  m_index = bodo.hiframes.pd_index_ext.get_index_data(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df))
"""
            icp__zcm = ['m_index[{}]'.format(i) for i in range(df.index.
                nlevels)]
            data_args = icp__zcm + data_args
        else:
            jawnj__bgrb = (
                'bodo.utils.conversion.index_to_array(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df))'
                )
            data_args = [jawnj__bgrb] + data_args
    return _gen_init_df(kkj__mzrbr, columns, ', '.join(data_args), 'index')


def _is_all_levels(df, level):
    tlora__mtrfu = len(get_index_data_arr_types(df.index))
    return is_overload_none(level) or is_overload_constant_int(level
        ) and get_overload_const_int(level
        ) == 0 and tlora__mtrfu == 1 or is_overload_constant_list(level
        ) and list(get_overload_const_list(level)) == list(range(tlora__mtrfu))
...
def overload_dataframe_dropna(df, axis=0, how='any', thresh=None, subset=
    ...
    if is_overload_none(subset):
        mgk__bnm = list(range(len(df.columns)))
    elif not is_overload_constant_list(subset):
        raise_bodo_error(
            f'df.dropna(): subset argument should a constant list, not {subset}'
            )
    else:
        inv__xcime = get_overload_const_list(subset)
        mgk__bnm = []
        for sjbp__pqhjb in inv__xcime:
            if sjbp__pqhjb not in df.column_index:
                raise_bodo_error(
                    f"df.dropna(): column '{sjbp__pqhjb}' not in data frame columns {df}"
                    )
            mgk__bnm.append(df.column_index[sjbp__pqhjb])
    mnkxg__avk = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(mnkxg__avk))
    kkj__mzrbr = (
...
"""
        .format(data_args, index, ', '.join(str(a) for a in mgk__bnm)))
    kkj__mzrbr += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(kkj__mzrbr, df.columns, data_args, 'index')
...
def overload_dataframe_drop(df, labels=None, axis=0, index=None, columns=
    ...
    rita__jbngr = dict(index=index, level=level, errors=errors)
    wrut__cabef = dict(index=None, level=None, errors='raise')
    check_unsupported_args('DataFrame.drop', rita__jbngr, wrut__cabef,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(labels):
        ...
        if is_overload_constant_str(labels):
            oin__jvtn = get_overload_const_str(labels),
        elif is_overload_constant_list(labels):
            oin__jvtn = get_overload_const_list(labels)
        else:
            raise_bodo_error(
        ...
        if is_overload_constant_str(columns):
            oin__jvtn = get_overload_const_str(columns),
        elif is_overload_constant_list(columns):
            oin__jvtn = get_overload_const_list(columns)
        else:
            raise_bodo_error(
                'constant list of columns expected for labels in DataFrame.drop()'
                )
    for oqsb__jqghr in oin__jvtn:
        if oqsb__jqghr not in df.columns:
            raise_bodo_error(
                'DataFrame.drop(): column {} not in DataFrame columns {}'.
                format(oqsb__jqghr, df.columns))
    if len(set(oin__jvtn)) == len(df.columns):
        raise BodoError('DataFrame.drop(): Dropping all columns not supported.'
            )
    inplace = is_overload_true(inplace)
    eqzq__luu = tuple(oqsb__jqghr for oqsb__jqghr in df.columns if 
        oqsb__jqghr not in oin__jvtn)
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.column_index[oqsb__jqghr], '.copy()' if not inplace else
        '') for oqsb__jqghr in eqzq__luu)
    kkj__mzrbr = (
    ...
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    return _gen_init_df(kkj__mzrbr, eqzq__luu, data_args, index)
...
def overload_dataframe_sample(df, n=None, frac=None, replace=False, weights
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(df,
        'DataFrame.sample()')
    rita__jbngr = dict(random_state=random_state, weights=weights, axis=
        axis, ignore_index=ignore_index)
    nvp__fyfr = dict(random_state=None, weights=None, axis=None,
        ignore_index=False)
    check_unsupported_args('DataFrame.sample', rita__jbngr, nvp__fyfr,
        package_name='pandas', module_name='DataFrame')
    ...
    mnkxg__avk = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(mnkxg__avk))
    zjrxz__abmbd = ', '.join('rhs_data_{}'.format(i) for i in range(mnkxg__avk)
        )
    kkj__mzrbr = """def impl(df, n=None, frac=None, replace=False, weights=None, random_state=None, axis=None, ignore_index=False):
"""
    kkj__mzrbr += '  if (frac == 1 or n == len(df)) and not replace:\n'
    kkj__mzrbr += (
...
"""
            .format(i))
    kkj__mzrbr += '  if frac is None:\n'
    kkj__mzrbr += '    frac_d = -1.0\n'
    kkj__mzrbr += '  else:\n'
    kkj__mzrbr += '    frac_d = frac\n'
    kkj__mzrbr += '  if n is None:\n'
    kkj__mzrbr += '    n_i = 0\n'
    kkj__mzrbr += '  else:\n'
    kkj__mzrbr += '    n_i = n\n'
    index = (
    ...
    kkj__mzrbr += f"""  ({data_args},), index_arr = bodo.libs.array_kernels.sample_table_operation(({zjrxz__abmbd},), {index}, n_i, frac_d, replace)
"""
    kkj__mzrbr += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return bodo.hiframes.dataframe_impl._gen_init_df(kkj__mzrbr, df.columns,
        data_args, 'index')
...
def overload_dataframe_info(df, verbose=None, buf=None, max_cols=None,
    ...
    cft__ozzo = {'verbose': verbose, 'buf': buf, 'max_cols': max_cols,
        'memory_usage': memory_usage, 'show_counts': show_counts,
        'null_counts': null_counts}
    bckj__ejpg = {'verbose': None, 'buf': None, 'max_cols': None,
        'memory_usage': None, 'show_counts': None, 'null_counts': None}
    check_unsupported_args('DataFrame.info', cft__ozzo, bckj__ejpg,
        package_name='pandas', module_name='DataFrame')
    unvgt__irq = f"<class '{str(type(df)).split('.')[-1]}"
    if len(df.columns) == 0:

        def _info_impl(df, verbose=None, buf=None, max_cols=None,
            memory_usage=None, show_counts=None, null_counts=None):
            hcsgh__gwct = unvgt__irq + '\n'
            hcsgh__gwct += 'Index: 0 entries\n'
            hcsgh__gwct += 'Empty DataFrame'
            print(hcsgh__gwct)
        return _info_impl
    else:
        kkj__mzrbr = """def _info_impl(df, verbose=None, buf=None, max_cols=None, memory_usage=None, show_counts=None, null_counts=None): #pragma: no cover
"""
        kkj__mzrbr += '    ncols = df.shape[1]\n'
        kkj__mzrbr += f'    lines = "{unvgt__irq}\\n"\n'
        kkj__mzrbr += f'    lines += "{df.index}: "\n'
        kkj__mzrbr += (
            '    index = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)\n'
            )
        if isinstance(df.index, bodo.hiframes.pd_index_ext.RangeIndexType):
            kkj__mzrbr += """    lines += f"{len(index)} entries, {index.start} to {index.stop-1}\\n\"
"""
        elif isinstance(df.index, bodo.hiframes.pd_index_ext.StringIndexType):
            kkj__mzrbr += """    lines += f"{len(index)} entries, {index[0]} to {index[len(index)-1]}\\n\"
"""
        else:
            kkj__mzrbr += (
        ...
        kkj__mzrbr += '    column_width = max(space, 7)\n'
        kkj__mzrbr += '    column= "Column"\n'
        kkj__mzrbr += '    underl= "------"\n'
        kkj__mzrbr += (
        ...
        kkj__mzrbr += '    mem_size = 0\n'
        kkj__mzrbr += (
            '    col_name = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)\n'
            )
        kkj__mzrbr += """    non_null_count = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)
"""
        kkj__mzrbr += (
        ...
        azdoi__hjddc = dict()
        for i in range(len(df.columns)):
            kkj__mzrbr += f"""    non_null_count[{i}] = str(bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})))
"""
            qlas__bahhu = f'{df.data[i].dtype}'
            if isinstance(df.data[i], bodo.CategoricalArrayType):
                qlas__bahhu = 'category'
            elif isinstance(df.data[i], bodo.IntegerArrayType):
                cqgw__qmsp = bodo.libs.int_arr_ext.IntDtype(df.data[i].dtype
                    ).name
                qlas__bahhu = f'{cqgw__qmsp[:-7]}'
            kkj__mzrbr += f'    col_dtype[{i}] = "{qlas__bahhu}"\n'
            if qlas__bahhu in azdoi__hjddc:
                azdoi__hjddc[qlas__bahhu] += 1
            else:
                azdoi__hjddc[qlas__bahhu] = 1
            kkj__mzrbr += f'    col_name[{i}] = "{df.columns[i]}"\n'
            kkj__mzrbr += f"""    mem_size += bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}).nbytes
"""
        kkj__mzrbr += """    column_info = [f'{i:^3} {name:<{column_width}} {count} non-null      {dtype}' for i, (name, count, dtype) in enumerate(zip(col_name, non_null_count, col_dtype))]
"""
        kkj__mzrbr += '    for i in column_info:\n'
        kkj__mzrbr += "        lines += f'{i}\\n'\n"
        rpdjk__csuu = ', '.join(f'{k}({azdoi__hjddc[k]})' for k in sorted(
            azdoi__hjddc))
        kkj__mzrbr += f"    lines += 'dtypes: {rpdjk__csuu}\\n'\n"
        kkj__mzrbr += '    mem_size += df.index.nbytes\n'
        kkj__mzrbr += '    total_size = _sizeof_fmt(mem_size)\n'
        kkj__mzrbr += "    lines += f'memory usage: {total_size}'\n"
        kkj__mzrbr += '    print(lines)\n'
        vkbo__dapyc = {}
        exec(kkj__mzrbr, {'_sizeof_fmt': _sizeof_fmt, 'pd': pd, 'bodo':
            bodo, 'np': np}, vkbo__dapyc)
        _info_impl = vkbo__dapyc['_info_impl']
        return _info_impl
...
def overload_dataframe_memory_usage(df, index=True, deep=False):
    ...
    kkj__mzrbr = 'def impl(df, index=True, deep=False):\n'
    scg__wspz = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df).nbytes'
    pnrp__vop = is_overload_true(index)
    columns = df.columns
    if pnrp__vop:
        columns = ('Index',) + columns
    if len(columns) == 0:
        eyv__qgd = ()
    elif all(isinstance(oqsb__jqghr, int) for oqsb__jqghr in columns):
        eyv__qgd = np.array(columns, 'int64')
    elif all(isinstance(oqsb__jqghr, str) for oqsb__jqghr in columns):
        eyv__qgd = pd.array(columns, 'string')
    else:
        eyv__qgd = columns
    if df.is_table_format and len(df.columns) > 0:
        embl__dyg = int(pnrp__vop)
        bpc__jptd = len(columns)
        kkj__mzrbr += f'  nbytes_arr = np.empty({bpc__jptd}, np.int64)\n'
        kkj__mzrbr += (
            '  table = bodo.hiframes.pd_dataframe_ext.get_dataframe_table(df)\n'
            )
        kkj__mzrbr += f"""  bodo.utils.table_utils.generate_table_nbytes(table, nbytes_arr, {embl__dyg})
"""
        if pnrp__vop:
            kkj__mzrbr += f'  nbytes_arr[0] = {scg__wspz}\n'
        kkj__mzrbr += f"""  return bodo.hiframes.pd_series_ext.init_series(nbytes_arr, pd.Index(column_vals), None)
"""
    else:
        data = ', '.join(
            f'bodo.libs.array_ops.array_op_nbytes(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}))'
             for i in range(len(df.columns)))
        if pnrp__vop:
            data = f'{scg__wspz},{data}'
        else:
            acwpk__unwhh = ',' if len(columns) == 1 else ''
            data = f'{data}{acwpk__unwhh}'
        kkj__mzrbr += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}), pd.Index(column_vals), None)
"""
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'np': np, 'pd': pd, 'column_vals':
        eyv__qgd}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def overload_read_excel(io, sheet_name=0, header=0, names=None, index_col=
    ...
    eki__jjhf = 'read_excel_df{}'.format(next_label())
    setattr(types, eki__jjhf, df_type)
    edrq__dvn = False
    if is_overload_constant_list(parse_dates):
        edrq__dvn = get_overload_const_list(parse_dates)
    ydro__oozg = ', '.join(["'{}':{}".format(cname, _get_pd_dtype_str(t)) for
...
def impl(
    ...
    with numba.objmode(df="{eki__jjhf}"):
        df = pd.read_excel(
            io=io,
...
"""
    vkbo__dapyc = {}
    exec(kkj__mzrbr, globals(), vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def overload_dataframe_plot(df, x=None, y=None, kind='line', figsize=None,
    ...
    except ImportError as aren__mgk:
    ...
    kkj__mzrbr += '    ylabel=None, title=None, legend=True, fontsize=None, \n'
    kkj__mzrbr += '    xticks=None, yticks=None, ax=None):\n'
    if is_overload_none(ax):
        kkj__mzrbr += '   fig, ax = plt.subplots()\n'
    else:
        kkj__mzrbr += '   fig = ax.get_figure()\n'
    if not is_overload_none(figsize):
        kkj__mzrbr += '   fig.set_figwidth(figsize[0])\n'
        kkj__mzrbr += '   fig.set_figheight(figsize[1])\n'
    if is_overload_none(xlabel):
        kkj__mzrbr += '   xlabel = x\n'
    kkj__mzrbr += '   ax.set_xlabel(xlabel)\n'
    if is_overload_none(ylabel):
        kkj__mzrbr += '   ylabel = y\n'
    else:
        kkj__mzrbr += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(title):
        kkj__mzrbr += '   ax.set_title(title)\n'
    if not is_overload_none(fontsize):
        kkj__mzrbr += '   ax.tick_params(labelsize=fontsize)\n'
    ...
    if kind == 'line':
        if is_overload_none(x) and is_overload_none(y):
            ...
            kkj__mzrbr += '   ax.plot(df[y], label=y)\n'
        elif is_overload_none(y):
            qzk__wiku = get_overload_const_str(x)
            qmwrj__motyv = df.columns.index(qzk__wiku)
            for i in range(len(df.columns)):
                if isinstance(df.data[i], (types.Array, IntegerArrayType)
                    ) and isinstance(df.data[i].dtype, (types.Integer,
                    types.Float)):
            ...
            kkj__mzrbr += '   ax.plot(df[x], df[y], label=y)\n'
    elif kind == 'scatter':
        legend = False
        kkj__mzrbr += '   ax.scatter(df[x], df[y], s=20)\n'
        kkj__mzrbr += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(xticks):
        kkj__mzrbr += '   ax.set_xticks(xticks)\n'
    if not is_overload_none(yticks):
        kkj__mzrbr += '   ax.set_yticks(yticks)\n'
    if is_overload_true(legend):
        kkj__mzrbr += '   ax.legend()\n'
    kkj__mzrbr += '   return ax\n'
    vkbo__dapyc = {}
    exec(kkj__mzrbr, {'bodo': bodo, 'plt': plt}, vkbo__dapyc)
    impl = vkbo__dapyc['impl']
    return impl
...
def is_df_values_numpy_supported_dftyp(df_typ):
    for itkv__uwelx in df_typ.data:
        if not (isinstance(itkv__uwelx, (IntegerArrayType,
            FloatingArrayType)) or isinstance(itkv__uwelx.dtype, types.
            Number) or itkv__uwelx.dtype in (bodo.datetime64ns, bodo.
            timedelta64ns)):
            return False
    return True
...
class SetDfColInfer(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        arf__rzlso = args[0]
        tbrkq__dbmo = args[1].literal_value
        val = args[2]
        assert val != types.unknown
        ach__yagw = arf__rzlso
        check_runtime_cols_unsupported(arf__rzlso, 'set_df_col()')
        if isinstance(arf__rzlso, DataFrameType):
            index = arf__rzlso.index
            if len(arf__rzlso.columns) == 0:
            ...
            if tbrkq__dbmo in arf__rzlso.columns:
                eqzq__luu = arf__rzlso.columns
                khwi__jpyh = arf__rzlso.columns.index(tbrkq__dbmo)
                hfhf__xnk = list(arf__rzlso.data)
                hfhf__xnk[khwi__jpyh] = val
                hfhf__xnk = tuple(hfhf__xnk)
            else:
                eqzq__luu = arf__rzlso.columns + (tbrkq__dbmo,)
                hfhf__xnk = arf__rzlso.data + (val,)
            ach__yagw = DataFrameType(hfhf__xnk, index, eqzq__luu,
                arf__rzlso.dist, arf__rzlso.is_table_format)
        return ach__yagw(*args)
...
class BodoSQLReplaceColsInfer(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        hhlqv__sfpm = args[0]
        assert isinstance(hhlqv__sfpm, DataFrameType) and len(hhlqv__sfpm.
            columns
        ...
        tbums__wbxo = args[2]
        assert len(col_names_to_replace) == len(tbums__wbxo
        ...
        assert len(col_names_to_replace) <= len(hhlqv__sfpm.columns
        ...
        for col_name in col_names_to_replace:
            assert col_name in hhlqv__sfpm.columns, 'Error while typechecking __bodosql_replace_columns_dummy: All columns specified to be replaced should already be present in input dataframe'
        check_runtime_cols_unsupported(hhlqv__sfpm,
            '__bodosql_replace_columns_dummy()')
        index = hhlqv__sfpm.index
        eqzq__luu = hhlqv__sfpm.columns
        hfhf__xnk = list(hhlqv__sfpm.data)
        for i in range(len(col_names_to_replace)):
            col_name = col_names_to_replace[i]
            cpqgx__wyjnh = tbums__wbxo[i]
            assert isinstance(cpqgx__wyjnh, SeriesType
            ...
            if isinstance(cpqgx__wyjnh, SeriesType):
                cpqgx__wyjnh = cpqgx__wyjnh.data
            xjbvv__vznd = hhlqv__sfpm.column_index[col_name]
            hfhf__xnk[xjbvv__vznd] = cpqgx__wyjnh
        hfhf__xnk = tuple(hfhf__xnk)
        ach__yagw = DataFrameType(hfhf__xnk, index, eqzq__luu, hhlqv__sfpm.
            dist, hhlqv__sfpm.is_table_format)
        return ach__yagw(*args)
...
def _parse_query_expr(expr, env, columns, cleaned_columns, index_name=None,
    ...
    def _rewrite_membership_op(self, node, left, right):
        guhwo__ves = node.op
        op = self.visit(guhwo__ves)
        return op, guhwo__ves, left, right
    ...
    class NewFuncNode(pd.core.computation.ops.FuncNode):
        ...
        def __init__(self, name):
            if (name not in pd.core.computation.ops.MATHOPS or pd.core.
                ...
                if name not in lbax__bwrj:
                    raise BodoError('"{0}" is not a supported function'.
                        format(name))
            self.name = name
            if name in lbax__bwrj:
                self.func = name
            else:
    ...
    def visit_Attribute(self, node, **kwargs):
        hdmcn__arbu = node.attr
        value = node.value
        fume__wrh = pd.core.computation.ops.LOCAL_TAG
        if hdmcn__arbu in ('str', 'dt'):
            try:
                vie__qxzih = str(self.visit(value))
            except pd.core.computation.ops.UndefinedVariableError as zkji__plo:
                col_name = zkji__plo.args[0].split("'")[1]
                raise BodoError(
                    'df.query(): column {} is not found in dataframe columns {}'
                    .format(col_name, columns))
        else:
            vie__qxzih = str(self.visit(value))
        ecmoj__ccxc = vie__qxzih, hdmcn__arbu
        if ecmoj__ccxc in join_cleaned_cols:
            hdmcn__arbu = join_cleaned_cols[ecmoj__ccxc]
        name = vie__qxzih + '.' + hdmcn__arbu
        if name.startswith(fume__wrh):
            name = name[len(fume__wrh):]
        if hdmcn__arbu in ('str', 'dt'):
            hxia__jshgn = columns[cleaned_columns.index(vie__qxzih)]
            djbia__gerhh[hxia__jshgn] = vie__qxzih
            self.env.scope[name] = 0
            return self.term_type(fume__wrh + name, self.env)
        lbax__bwrj.append(name)
    ...
    def math__str__(self):
        ...
        return pd.io.formats.printing.pprint_thing(
            ...
            (op, ','.join(dgyu__byzb), tbrkq__dbmo))

    def op__str__(self):
        kek__ean = ('({0})'.format(pd.io.formats.printing.pprint_thing(
            hvrmn__wod)) for hvrmn__wod in self.operands)
        if self.op == 'in':
            return pd.io.formats.printing.pprint_thing(
        ...
        return pd.io.formats.printing.pprint_thing(' {0} '.format(self.op).
            join(kek__ean))
    gurvs__axy = (pd.core.computation.expr.BaseExprVisitor.
        _rewrite_membership_op)
    ppiz__svrzi = (pd.core.computation.expr.BaseExprVisitor.
        _maybe_evaluate_binop)
    sfdce__xyc = pd.core.computation.expr.BaseExprVisitor.visit_Attribute
    txl__acuc = (pd.core.computation.expr.BaseExprVisitor.
        _maybe_downcast_constants)
    nbazo__jhq = pd.core.computation.ops.Term.__str__
    vywj__gng = pd.core.computation.ops.MathCall.__str__
    pgbt__cbu = pd.core.computation.ops.Op.__str__
    evg__zdcg = pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops
    try:
        ...
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        rdr__kldqd = pd.core.computation.expr.Expr(expr, env=env)
        vagg__ucm = str(rdr__kldqd)
    except pd.core.computation.ops.UndefinedVariableError as zkji__plo:
        if not is_overload_none(index_name) and get_overload_const_str(
            index_name) == zkji__plo.args[0].split("'")[1]:
            raise BodoError(
                "df.query(): Refering to named index ('{}') by name is not supported"
                .format(get_overload_const_str(index_name)))
        else:
            raise BodoError(f'df.query(): undefined variable, {zkji__plo}')
    finally:
        pd.core.computation.expr.BaseExprVisitor._rewrite_membership_op = (
            gurvs__axy)
        pd.core.computation.expr.BaseExprVisitor._maybe_evaluate_binop = (
            ppiz__svrzi)
        pd.core.computation.expr.BaseExprVisitor.visit_Attribute = sfdce__xyc
        (pd.core.computation.expr.BaseExprVisitor._maybe_downcast_constants
            ) = txl__acuc
        pd.core.computation.ops.Term.__str__ = nbazo__jhq
        pd.core.computation.ops.MathCall.__str__ = vywj__gng
        pd.core.computation.ops.Op.__str__ = pgbt__cbu
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (
            evg__zdcg)
    vod__uqpvc = pd.core.computation.parsing.clean_column_name
    djbia__gerhh.update({oqsb__jqghr: vod__uqpvc(oqsb__jqghr) for
        oqsb__jqghr in columns if vod__uqpvc(oqsb__jqghr) in rdr__kldqd.names})
    return rdr__kldqd, vagg__ucm, djbia__gerhh
...
class DataFrameTupleIterator(types.SimpleIteratorType):
    ...
    def __init__(self, col_names, arr_typs):
        ...
        czj__idfd = ['{}={}'.format(col_names[i], arr_typs[i]) for i in
            range(len(col_names))]
        name = 'itertuples({})'.format(','.join(czj__idfd))
        glff__tcxz = namedtuple('Pandas', col_names)
        bix__zpaz = types.NamedTuple([_get_series_dtype(a) for a in
            arr_typs], glff__tcxz)
        super(DataFrameTupleIterator, self).__init__(name, bix__zpaz)
...
class TypeIterTuples(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        szk__kjzq = [if_series_to_array_type(a) for a in args[len(args) // 2:]]
        assert 'Index' not in col_names[0]
        col_names = ['Index'] + col_names
        szk__kjzq = [types.Array(types.int64, 1, 'C')] + szk__kjzq
        ftjch__ykmp = DataFrameTupleIterator(col_names, szk__kjzq)
        return ftjch__ykmp(*args)
...
class DataFrameTupleIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tpsg__viug = [('index', types.EphemeralPointer(types.uintp))] + [(
...
def get_itertuples_impl(context, builder, sig, args):
    wfo__kzvpf = args[len(args) // 2:]
    uikk__nwt = sig.args[len(sig.args) // 2:]
    mcn__jdnib = context.make_helper(builder, sig.return_type)
    qwqmu__vkzp = context.get_constant(types.intp, 0)
    kpr__gjv = cgutils.alloca_once_value(builder, qwqmu__vkzp)
    mcn__jdnib.index = kpr__gjv
    for i, arr in enumerate(wfo__kzvpf):
        setattr(mcn__jdnib, 'array{}'.format(i), arr)
    for arr, arr_typ in zip(wfo__kzvpf, uikk__nwt):
        context.nrt.incref(builder, arr_typ, arr)
    res = mcn__jdnib._getvalue()
...
def iternext_itertuples(context, builder, sig, args, result):
    pnji__ipsp, = sig.args
    glyqf__vqbms, = args
    mcn__jdnib = context.make_helper(builder, pnji__ipsp, value=glyqf__vqbms)
    jrfxj__rdywy = signature(types.intp, pnji__ipsp.array_types[1])
    vkdf__xhscs = context.compile_internal(builder, lambda a: len(a),
        jrfxj__rdywy, [mcn__jdnib.array0])
    index = builder.load(mcn__jdnib.index)
    obz__schbr = builder.icmp_signed('<', index, vkdf__xhscs)
    result.set_valid(obz__schbr)
    with builder.if_then(obz__schbr):
        values = [index]
        for i, arr_typ in enumerate(pnji__ipsp.array_types[1:]):
            qvbs__theu = getattr(mcn__jdnib, 'array{}'.format(i))
            if arr_typ == types.Array(types.NPDatetime('ns'), 1, 'C'):
                pyu__xwnx = signature(pd_timestamp_tz_naive_type, arr_typ,
                    types.intp)
                val = context.compile_internal(builder, lambda a, i: bodo.
                    ...
                    pyu__xwnx, [qvbs__theu, index])
            else:
                pyu__xwnx = signature(arr_typ.dtype, arr_typ, types.intp)
                val = context.compile_internal(builder, lambda a, i: a[i],
                    pyu__xwnx, [qvbs__theu, index])
            values.append(val)
        value = context.make_tuple(builder, pnji__ipsp.yield_type, values)
        result.yield_(value)
        owoxa__zts = cgutils.increment_index(builder, index)
        builder.store(owoxa__zts, mcn__jdnib.index)
...
def _analyze_op_pair_first(self, scope, equiv_set, expr, lhs):
    ...
    xivg__dfwd = ir.Assign(rhs, lhs, expr.loc)
    uiuzv__tutbh = lhs
    osnw__tluu = []
    mods__zwh = []
    qds__fhz = typ.count
    for i in range(qds__fhz):
        cguv__kojuc = ir.Var(uiuzv__tutbh.scope, mk_unique_var('{}_size{}'.
            format(uiuzv__tutbh.name, i)), uiuzv__tutbh.loc)
        pggrn__xhbpb = ir.Expr.static_getitem(lhs, i, None, uiuzv__tutbh.loc)
        self.calltypes[pggrn__xhbpb] = None
        osnw__tluu.append(ir.Assign(pggrn__xhbpb, cguv__kojuc, uiuzv__tutbh
            .loc))
        self._define(equiv_set, cguv__kojuc, types.intp, pggrn__xhbpb)
        mods__zwh.append(cguv__kojuc)
    osg__isio = tuple(mods__zwh)
    return numba.parfors.array_analysis.ArrayAnalysis.AnalyzeResult(shape=
        osg__isio, pre=[xivg__dfwd] + osnw__tluu)
