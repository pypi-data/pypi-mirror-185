...
class RollingCovType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        arr = args[0]
        mds__nfup = arr.copy(dtype=types.float64)
        return signature(mds__nfup, *unliteral_all(args))
...
def overload_rolling_fixed(arr, index_arr, win, minp, center, fname, raw=
    ...
    gby__wiya = get_overload_const_str(fname)
    if gby__wiya not in ('sum', 'mean', 'var', 'std', 'count', 'median',
        'min', 'max'):
        raise BodoError('invalid rolling (fixed window) function {}'.format
            (gby__wiya))
    if gby__wiya in ('median', 'min', 'max'):
        hccf__hjzcl = 'def kernel_func(A):\n'
        hccf__hjzcl += '  if np.isnan(A).sum() != 0: return np.nan\n'
        hccf__hjzcl += '  return np.{}(A)\n'.format(gby__wiya)
        nprvs__dbc = {}
        exec(hccf__hjzcl, {'np': np}, nprvs__dbc)
        kernel_func = register_jitable(nprvs__dbc['kernel_func'])
        return (lambda arr, index_arr, win, minp, center, fname, raw=True,
            parallel=False: roll_fixed_apply(arr, index_arr, win, minp,
...
def overload_rolling_variable(arr, on_arr, index_arr, win, minp, center,
    ...
    if gby__wiya in ('median', 'min', 'max'):
        hccf__hjzcl = 'def kernel_func(A):\n'
        hccf__hjzcl += '  arr  = dropna(A)\n'
        hccf__hjzcl += '  if len(arr) == 0: return np.nan\n'
        hccf__hjzcl += '  return np.{}(arr)\n'.format(gby__wiya)
        nprvs__dbc = {}
        exec(hccf__hjzcl, {'np': np, 'dropna': _dropna}, nprvs__dbc)
        kernel_func = register_jitable(nprvs__dbc['kernel_func'])
        return (lambda arr, on_arr, index_arr, win, minp, center, fname,
            raw=True, parallel=False: roll_variable_apply(arr, on_arr,
...
def roll_fixed_linear_generic(in_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        zgu__tpl = _border_icomm(in_arr, rank, n_pes, halo_size, True, center)
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req, jsc__ylc
            ) = zgu__tpl
    ...
    if parallel:
        ...
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(jsc__ylc, True)
            for lwf__bsvrq in range(0, halo_size):
                data = add_obs(r_recv_buff[lwf__bsvrq], *data)
                qlsf__eklz = in_arr[N + lwf__bsvrq - win]
                data = remove_obs(qlsf__eklz, *data)
                output[N + lwf__bsvrq - offset] = calc_out(minp, *data)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            data = init_data()
            for lwf__bsvrq in range(0, halo_size):
                data = add_obs(l_recv_buff[lwf__bsvrq], *data)
            for lwf__bsvrq in range(0, win - 1):
                data = add_obs(in_arr[lwf__bsvrq], *data)
                if lwf__bsvrq > offset:
                    qlsf__eklz = l_recv_buff[lwf__bsvrq - offset - 1]
                    data = remove_obs(qlsf__eklz, *data)
                if lwf__bsvrq >= offset:
                    output[lwf__bsvrq - offset] = calc_out(minp, *data)
    return output
...
def roll_fixed_linear_generic_seq(in_arr, win, minp, center, init_data,
    ...
    vzv__hpwn = max(minp, 1) - 1
    vzv__hpwn = min(vzv__hpwn, N)
    for lwf__bsvrq in range(0, vzv__hpwn):
        data = add_obs(in_arr[lwf__bsvrq], *data)
        if lwf__bsvrq >= offset:
            output[lwf__bsvrq - offset] = calc_out(minp, *data)
    for lwf__bsvrq in range(vzv__hpwn, N):
        val = in_arr[lwf__bsvrq]
        data = add_obs(val, *data)
        if lwf__bsvrq > win - 1:
            qlsf__eklz = in_arr[lwf__bsvrq - win]
            data = remove_obs(qlsf__eklz, *data)
        output[lwf__bsvrq - offset] = calc_out(minp, *data)
    gbr__frdco = data
    for lwf__bsvrq in range(N, N + offset):
        if lwf__bsvrq > win - 1:
            qlsf__eklz = in_arr[lwf__bsvrq - win]
            data = remove_obs(qlsf__eklz, *data)
        output[lwf__bsvrq - offset] = calc_out(minp, *data)
    return output, gbr__frdco
...
def roll_fixed_apply_impl(in_arr, index_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        if raw == False:
            mwk__ipu = _border_icomm(index_arr, rank, n_pes, halo_size, 
                True, center)
            (l_recv_buff_idx, r_recv_buff_idx, rfbd__tfzet, ydje__wqv,
                elayz__ydcyy, ftk__jvdw) = mwk__ipu
    output = roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
        kernel_func, raw)
    if parallel:
        _border_send_wait(r_send_req, l_send_req, rank, n_pes, True, center)
        if raw == False:
            _border_send_wait(ydje__wqv, rfbd__tfzet, rank, n_pes, True, center
                )
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(jsc__ylc, True)
            if raw == False:
                bodo.libs.distributed_api.wait(ftk__jvdw, True)
            recv_right_compute(output, in_arr, index_arr, N, win, minp,
                offset, r_recv_buff, r_recv_buff_idx, kernel_func, raw)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            if raw == False:
                bodo.libs.distributed_api.wait(elayz__ydcyy, True)
...
def overload_recv_right_compute(output, in_arr, index_arr, N, win, minp,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, N, win, minp, offset,
            r_recv_buff, r_recv_buff_idx, kernel_func, raw):
            gbr__frdco = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
            vkiof__mve = 0
            for lwf__bsvrq in range(max(N - offset, 0), N):
                data = gbr__frdco[vkiof__mve:vkiof__mve + win]
                if win - np.isnan(data).sum() < minp:
                    output[lwf__bsvrq] = np.nan
                else:
                    output[lwf__bsvrq] = kernel_func(data)
                vkiof__mve += 1
        return impl
    ...
    def impl_series(output, in_arr, index_arr, N, win, minp, offset,
        r_recv_buff, r_recv_buff_idx, kernel_func, raw):
        gbr__frdco = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
        uwvd__ibt = np.concatenate((index_arr[N - win + 1:], r_recv_buff_idx))
        vkiof__mve = 0
        for lwf__bsvrq in range(max(N - offset, 0), N):
            data = gbr__frdco[vkiof__mve:vkiof__mve + win]
            if win - np.isnan(data).sum() < minp:
                ...
                output[lwf__bsvrq] = kernel_func(pd.Series(data, uwvd__ibt[
                    vkiof__mve:vkiof__mve + win]))
            vkiof__mve += 1
    return impl_series
...
def overload_recv_left_compute(output, in_arr, index_arr, win, minp, offset,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, win, minp, offset, l_recv_buff,
            ...
            gbr__frdco = np.concatenate((l_recv_buff, in_arr[:win - 1]))
            for lwf__bsvrq in range(0, win - offset - 1):
                data = gbr__frdco[lwf__bsvrq:lwf__bsvrq + win]
                if win - np.isnan(data).sum() < minp:
                    output[lwf__bsvrq] = np.nan
                else:
                    output[lwf__bsvrq] = kernel_func(data)
    ...
    def impl_series(output, in_arr, index_arr, win, minp, offset,
        l_recv_buff, l_recv_buff_idx, kernel_func, raw):
        gbr__frdco = np.concatenate((l_recv_buff, in_arr[:win - 1]))
        uwvd__ibt = np.concatenate((l_recv_buff_idx, index_arr[:win - 1]))
        for lwf__bsvrq in range(0, win - offset - 1):
            data = gbr__frdco[lwf__bsvrq:lwf__bsvrq + win]
            if win - np.isnan(data).sum() < minp:
                output[lwf__bsvrq] = np.nan
...
def overload_roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
    ...
    def roll_fixed_apply_seq_impl(in_arr, index_arr, win, minp, center,
        ...
        output = np.empty(N, dtype=np.float64)
        offset = (win - 1) // 2 if center else 0
        for lwf__bsvrq in range(0, N):
            start = max(lwf__bsvrq - win + 1 + offset, 0)
            end = min(lwf__bsvrq + 1 + offset, N)
            data = in_arr[start:end]
            if end - start - np.isnan(data).sum() < minp:
                output[lwf__bsvrq] = np.nan
            else:
                output[lwf__bsvrq] = apply_func(kernel_func, data,
...
def roll_var_linear_generic(in_arr, on_arr_dt, win, minp, center, parallel,
    ...
    if parallel:
        ...
        zgu__tpl = _border_icomm_var(in_arr, on_arr, rank, n_pes, win)
        (l_recv_buff, l_recv_t_buff, r_send_req, wws__crp, l_recv_req,
            qfswc__szo) = zgu__tpl
    ...
    if parallel:
        ...
        _border_send_wait(wws__crp, wws__crp, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(qfswc__szo, True)
            num_zero_starts = 0
            for lwf__bsvrq in range(0, N):
                if start[lwf__bsvrq] != 0:
                    break
                num_zero_starts += 1
            ...
            recv_starts = _get_var_recv_starts(on_arr, l_recv_t_buff,
                num_zero_starts, win)
            data = init_data()
            for wlase__raca in range(recv_starts[0], len(l_recv_t_buff)):
                data = add_obs(l_recv_buff[wlase__raca], *data)
            if right_closed:
                data = add_obs(in_arr[0], *data)
            output[0] = calc_out(minp, *data)
            for lwf__bsvrq in range(1, num_zero_starts):
                s = recv_starts[lwf__bsvrq]
                cepm__xntxm = end[lwf__bsvrq]
                for wlase__raca in range(recv_starts[lwf__bsvrq - 1], s):
                    data = remove_obs(l_recv_buff[wlase__raca], *data)
                for wlase__raca in range(end[lwf__bsvrq - 1], cepm__xntxm):
                    data = add_obs(in_arr[wlase__raca], *data)
                output[lwf__bsvrq] = calc_out(minp, *data)
    return output
...
def _get_var_recv_starts(on_arr, l_recv_t_buff, num_zero_starts, win):
    ...
    dwbo__uvn = cast_dt64_arr_to_int(on_arr)
    left_closed = False
    doarf__fez = dwbo__uvn[0] - win
    if left_closed:
        doarf__fez -= 1
    recv_starts[0] = halo_size
    for wlase__raca in range(0, halo_size):
        if l_recv_t_buff[wlase__raca] > doarf__fez:
            recv_starts[0] = wlase__raca
            break
    for lwf__bsvrq in range(1, num_zero_starts):
        doarf__fez = dwbo__uvn[lwf__bsvrq] - win
        if left_closed:
            doarf__fez -= 1
        recv_starts[lwf__bsvrq] = halo_size
        for wlase__raca in range(recv_starts[lwf__bsvrq - 1], halo_size):
            if l_recv_t_buff[wlase__raca] > doarf__fez:
                recv_starts[lwf__bsvrq] = wlase__raca
                break
    return recv_starts
...
def roll_var_linear_generic_seq(in_arr, on_arr, win, minp, start, end,
    ...
    data = init_data()
    for wlase__raca in range(start[0], end[0]):
        data = add_obs(in_arr[wlase__raca], *data)
    output[0] = calc_out(minp, *data)
    for lwf__bsvrq in range(1, N):
        s = start[lwf__bsvrq]
        cepm__xntxm = end[lwf__bsvrq]
        for wlase__raca in range(start[lwf__bsvrq - 1], s):
            data = remove_obs(in_arr[wlase__raca], *data)
        for wlase__raca in range(end[lwf__bsvrq - 1], cepm__xntxm):
            data = add_obs(in_arr[wlase__raca], *data)
        output[lwf__bsvrq] = calc_out(minp, *data)
    return output
...
def roll_variable_apply_impl(in_arr, on_arr_dt, index_arr, win, minp,
    ...
    if parallel:
        ...
        (l_recv_buff, l_recv_t_buff, r_send_req, wws__crp, l_recv_req,
            qfswc__szo) = zgu__tpl
        if raw == False:
            mwk__ipu = _border_icomm_var(index_arr, on_arr, rank, n_pes, win)
            (l_recv_buff_idx, cox__uknn, ydje__wqv, uoc__tzq, elayz__ydcyy,
                bjozn__kgt) = mwk__ipu
    start, end = _build_indexer(on_arr, N, win, left_closed, right_closed)
    ...
    if parallel:
        ...
        _border_send_wait(wws__crp, wws__crp, rank, n_pes, True, False)
        if raw == False:
            _border_send_wait(ydje__wqv, ydje__wqv, rank, n_pes, True, False)
            _border_send_wait(uoc__tzq, uoc__tzq, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(qfswc__szo, True)
            if raw == False:
                bodo.libs.distributed_api.wait(elayz__ydcyy, True)
                bodo.libs.distributed_api.wait(bjozn__kgt, True)
            num_zero_starts = 0
            for lwf__bsvrq in range(0, N):
                if start[lwf__bsvrq] != 0:
                    break
...
def overload_recv_left_var_compute(output, in_arr, index_arr,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, num_zero_starts, recv_starts,
            ...
            for lwf__bsvrq in range(0, num_zero_starts):
                bubx__gkoh = recv_starts[lwf__bsvrq]
                svom__brrrh = np.concatenate((l_recv_buff[bubx__gkoh:],
                    in_arr[:lwf__bsvrq + 1]))
                if len(svom__brrrh) - np.isnan(svom__brrrh).sum() >= minp:
                    output[lwf__bsvrq] = kernel_func(svom__brrrh)
                else:
                    output[lwf__bsvrq] = np.nan
        return impl
    ...
    def impl_series(output, in_arr, index_arr, num_zero_starts, recv_starts,
        ...
        for lwf__bsvrq in range(0, num_zero_starts):
            bubx__gkoh = recv_starts[lwf__bsvrq]
            svom__brrrh = np.concatenate((l_recv_buff[bubx__gkoh:], in_arr[
                :lwf__bsvrq + 1]))
            tjgdp__azxbz = np.concatenate((l_recv_buff_idx[bubx__gkoh:],
                index_arr[:lwf__bsvrq + 1]))
            if len(svom__brrrh) - np.isnan(svom__brrrh).sum() >= minp:
                output[lwf__bsvrq] = kernel_func(pd.Series(svom__brrrh,
                    tjgdp__azxbz))
            else:
                output[lwf__bsvrq] = np.nan
    return impl_series
...
def roll_variable_apply_seq_impl(in_arr, on_arr, index_arr, win, minp,
    ...
    for lwf__bsvrq in range(0, N):
        s = start[lwf__bsvrq]
        cepm__xntxm = end[lwf__bsvrq]
        data = in_arr[s:cepm__xntxm]
        if cepm__xntxm - s - np.isnan(data).sum() >= minp:
            output[lwf__bsvrq] = kernel_func(data)
        else:
            output[lwf__bsvrq] = np.nan
    return output
...
def roll_variable_apply_seq_impl_series(in_arr, on_arr, index_arr, win,
    ...
    for lwf__bsvrq in range(0, N):
        ...
        data = in_arr[s:cepm__xntxm]
        if cepm__xntxm - s - np.isnan(data).sum() >= minp:
            output[lwf__bsvrq] = kernel_func(pd.Series(data, index_arr[s:
                cepm__xntxm]))
        else:
            output[lwf__bsvrq] = np.nan
    return output
...
def _build_indexer(on_arr, N, win, left_closed, right_closed):
    ...
    for lwf__bsvrq in range(1, N):
        inj__ginr = dwbo__uvn[lwf__bsvrq]
        doarf__fez = dwbo__uvn[lwf__bsvrq] - win
        if left_closed:
            doarf__fez -= 1
        start[lwf__bsvrq] = lwf__bsvrq
        for wlase__raca in range(start[lwf__bsvrq - 1], lwf__bsvrq):
            if dwbo__uvn[wlase__raca] > doarf__fez:
                start[lwf__bsvrq] = wlase__raca
                break
        if dwbo__uvn[end[lwf__bsvrq - 1]] <= inj__ginr:
            end[lwf__bsvrq] = lwf__bsvrq + 1
        else:
            end[lwf__bsvrq] = end[lwf__bsvrq - 1]
        if not right_closed:
            end[lwf__bsvrq] -= 1
    return start, end
...
def calc_mean(minp, nobs, sum_x, neg_ct):
    if nobs >= minp:
        zxzsd__ebp = sum_x / nobs
        if neg_ct == 0 and zxzsd__ebp < 0.0:
            zxzsd__ebp = 0
        elif neg_ct == nobs and zxzsd__ebp > 0.0:
            zxzsd__ebp = 0
    else:
        zxzsd__ebp = np.nan
    return zxzsd__ebp
...
def add_var(val, nobs, mean_x, ssqdm_x):
    if not np.isnan(val):
        nobs += 1
        ytk__zqoaa = val - mean_x
        mean_x += ytk__zqoaa / nobs
        ssqdm_x += (nobs - 1) * ytk__zqoaa ** 2 / nobs
...
def calc_var(minp, nobs, mean_x, ssqdm_x):
    pvfv__layus = 1.0
    zxzsd__ebp = np.nan
    if nobs >= minp and nobs > pvfv__layus:
        if nobs == 1:
            zxzsd__ebp = 0.0
        else:
            zxzsd__ebp = ssqdm_x / (nobs - pvfv__layus)
            if zxzsd__ebp < 0.0:
                zxzsd__ebp = 0.0
    return zxzsd__ebp
...
def calc_std(minp, nobs, mean_x, ssqdm_x):
    fgb__ncx = calc_var(minp, nobs, mean_x, ssqdm_x)
    return np.sqrt(fgb__ncx)
    ...
    if parallel:
        ...
        if _is_small_for_parallel(N, halo_size):
            return _handle_small_data_shift(in_arr, shift, rank, n_pes,
                default_fill_value)
        zgu__tpl = _border_icomm(in_arr, rank, n_pes, halo_size, send_right,
            send_left)
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req, jsc__ylc
            ) = zgu__tpl
    ...
    if parallel:
        if send_right:
            ...
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(jsc__ylc, True)
                for lwf__bsvrq in range(0, halo_size):
                    if bodo.libs.array_kernels.isna(r_recv_buff, lwf__bsvrq):
                        bodo.libs.array_kernels.setna(output, N - halo_size +
                            lwf__bsvrq)
                        continue
                    output[N - halo_size + lwf__bsvrq] = r_recv_buff[lwf__bsvrq
                        ]
    return output
...
def shift_seq(in_arr, shift, output, is_parallel_str=False,
    ...
    N = len(in_arr)
    sfdm__oqx = 1 if shift > 0 else -1
    shift = sfdm__oqx * min(abs(shift), N)
    if shift > 0 and (not is_parallel_str or bodo.get_rank() == 0):
        if default_fill_value is None:
            bodo.libs.array_kernels.setna_slice(output, slice(None, shift))
        else:
            for lwf__bsvrq in range(shift):
                output[lwf__bsvrq
                    ] = bodo.utils.conversion.unbox_if_tz_naive_timestamp(
                    default_fill_value)
    start = max(shift, 0)
    end = min(N, N + shift)
    for lwf__bsvrq in range(start, end):
        if bodo.libs.array_kernels.isna(in_arr, lwf__bsvrq - shift):
            bodo.libs.array_kernels.setna(output, lwf__bsvrq)
            continue
        output[lwf__bsvrq] = in_arr[lwf__bsvrq - shift]
    if shift < 0:
        if default_fill_value is None:
            bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
        else:
            for lwf__bsvrq in range(end, N):
                output[lwf__bsvrq
                    ] = bodo.utils.conversion.unbox_if_tz_naive_timestamp(
                    default_fill_value)
    return output
...
def shift_left_recv(r_send_req, l_send_req, rank, n_pes, halo_size,
    ...
    if rank != 0:
        bodo.libs.distributed_api.wait(l_recv_req, True)
        for lwf__bsvrq in range(0, halo_size):
            if bodo.libs.array_kernels.isna(l_recv_buff, lwf__bsvrq):
                bodo.libs.array_kernels.setna(output, lwf__bsvrq)
                continue
            output[lwf__bsvrq] = l_recv_buff[lwf__bsvrq]
    ...
    if parallel:
        if send_right:
            ...
            if rank != 0:
                bodo.libs.distributed_api.wait(l_recv_req, True)
                for lwf__bsvrq in range(0, halo_size):
                    xuehy__hvjc = l_recv_buff[lwf__bsvrq]
                    output[lwf__bsvrq] = (in_arr[lwf__bsvrq] - xuehy__hvjc
                        ) / xuehy__hvjc
        else:
            _border_send_wait(r_send_req, l_send_req, rank, n_pes, False, True)
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(jsc__ylc, True)
                for lwf__bsvrq in range(0, halo_size):
                    xuehy__hvjc = r_recv_buff[lwf__bsvrq]
                    output[N - halo_size + lwf__bsvrq] = (in_arr[N -
                        halo_size + lwf__bsvrq] - xuehy__hvjc) / xuehy__hvjc
    return output
...
def get_first_non_na(arr):
    ...
    if arr.dtype == types.float32:
        rfwm__pbjbe = np.float32('nan')

    def impl(arr):
        for lwf__bsvrq in range(len(arr)):
            if not bodo.libs.array_kernels.isna(arr, lwf__bsvrq):
                return arr[lwf__bsvrq]
        return rfwm__pbjbe
    return impl
...
def get_last_non_na(arr):
    ...
    def impl(arr):
        blfm__pimmp = len(arr)
        for lwf__bsvrq in range(len(arr)):
            vkiof__mve = blfm__pimmp - lwf__bsvrq - 1
            if not bodo.libs.array_kernels.isna(arr, vkiof__mve):
                return arr[vkiof__mve]
        return rfwm__pbjbe
    return impl
...
def pct_change_seq(in_arr, shift):
    ...
    if shift > 0:
        ...
        bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
    if shift > 0:
        nzi__mcjbo = get_first_non_na(in_arr[:shift])
        oyea__ayb = get_last_non_na(in_arr[:shift])
    else:
        nzi__mcjbo = get_last_non_na(in_arr[:-shift])
        oyea__ayb = get_first_non_na(in_arr[:-shift])
    one = get_one_from_arr_dtype(output)
    start = max(shift, 0)
    end = min(N, N + shift)
    for lwf__bsvrq in range(start, end):
        xuehy__hvjc = in_arr[lwf__bsvrq - shift]
        if np.isnan(xuehy__hvjc):
            xuehy__hvjc = nzi__mcjbo
        else:
            nzi__mcjbo = xuehy__hvjc
        val = in_arr[lwf__bsvrq]
        if np.isnan(val):
            val = oyea__ayb
        else:
            oyea__ayb = val
        output[lwf__bsvrq] = val / xuehy__hvjc - one
    return output
...
def _border_icomm(in_arr, rank, n_pes, halo_size, send_right=True,
    send_left=False):
    cuo__rybcy = np.int32(comm_border_tag)
...
def _border_icomm_var(in_arr, on_arr, rank, n_pes, win_size):
    ...
    for wlase__raca in range(-2, -N, -1):
        fytuf__vvyp = on_arr[wlase__raca]
        if end - fytuf__vvyp >= win_size:
            halo_size = -wlase__raca
            break
    if rank != n_pes - 1:
        bodo.libs.distributed_api.send(halo_size, np.int32(rank + 1),
            cuo__rybcy)
        r_send_req = bodo.libs.distributed_api.isend(in_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), cuo__rybcy, True)
        wws__crp = bodo.libs.distributed_api.isend(on_arr[-halo_size:], np.
            int32(halo_size), np.int32(rank + 1), cuo__rybcy, True)
    if rank != 0:
        ...
        qfswc__szo = bodo.libs.distributed_api.irecv(l_recv_t_buff, np.
            int32(halo_size), np.int32(rank - 1), cuo__rybcy, True)
    return (l_recv_buff, l_recv_t_buff, r_send_req, wws__crp, l_recv_req,
        qfswc__szo)
...
def _is_small_for_parallel(N, halo_size):
    ooio__skdh = bodo.libs.distributed_api.dist_reduce(int(N <= 2 *
...
def _handle_small_data(in_arr, win, minp, center, rank, n_pes, init_data,
    add_obs, remove_obs, calc_out):
    N = len(in_arr)
    lqxn__diqt = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    qpzm__tqmk = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        aolgt__feew, vejmt__gcogc = roll_fixed_linear_generic_seq(qpzm__tqmk,
            win, minp, center, init_data, add_obs, remove_obs, calc_out)
    else:
        aolgt__feew = np.empty(lqxn__diqt, np.float64)
    bodo.libs.distributed_api.bcast(aolgt__feew)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
    return aolgt__feew[start:end]
...
def _handle_small_data_apply(in_arr, index_arr, win, minp, center, rank,
    ...
    lqxn__diqt = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    qpzm__tqmk = bodo.libs.distributed_api.gatherv(in_arr)
    pvgj__psvii = bodo.libs.distributed_api.gatherv(index_arr)
    if rank == 0:
        aolgt__feew = roll_fixed_apply_seq(qpzm__tqmk, pvgj__psvii, win,
...
def _handle_small_data_shift(in_arr, shift, rank, n_pes, default_fill_value):
    ...
    qpzm__tqmk = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        aolgt__feew = alloc_shift(len(qpzm__tqmk), qpzm__tqmk, (-1,),
            fill_value=default_fill_value)
        shift_seq(qpzm__tqmk, shift, aolgt__feew, default_fill_value=
            default_fill_value)
        srib__oji = bcast_n_chars_if_str_binary_arr(aolgt__feew)
    else:
        srib__oji = bcast_n_chars_if_str_binary_arr(in_arr)
        aolgt__feew = alloc_shift(lqxn__diqt, in_arr, (srib__oji,),
            fill_value=default_fill_value)
    bodo.libs.distributed_api.bcast(aolgt__feew)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
...
def _handle_small_data_pct_change(in_arr, shift, rank, n_pes):
    N = len(in_arr)
    lqxn__diqt = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    qpzm__tqmk = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        aolgt__feew = pct_change_seq(qpzm__tqmk, shift)
    else:
        aolgt__feew = alloc_pct_change(lqxn__diqt, in_arr)
    bodo.libs.distributed_api.bcast(aolgt__feew)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _is_small_for_parallel_variable(on_arr, win_size):
    if len(on_arr) < 2:
        ...
        grp__fjzfj = end - start
        cbyvh__wnnkb = int(grp__fjzfj <= win_size)
    ooio__skdh = bodo.libs.distributed_api.dist_reduce(cbyvh__wnnkb, np.
        int32(Reduce_Type.Sum.value))
    return ooio__skdh != 0
...
def _handle_small_data_variable(in_arr, on_arr, win, minp, rank, n_pes,
    ...
    lqxn__diqt = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    qpzm__tqmk = bodo.libs.distributed_api.gatherv(in_arr)
    fblm__xvmk = bodo.libs.distributed_api.gatherv(on_arr)
    if rank == 0:
        start, end = _build_indexer(fblm__xvmk, lqxn__diqt, win, False, True)
        aolgt__feew = roll_var_linear_generic_seq(qpzm__tqmk, fblm__xvmk,
...
def _handle_small_data_variable_apply(in_arr, on_arr, index_arr, win, minp,
    ...
    if rank == 0:
        start, end = _build_indexer(fblm__xvmk, lqxn__diqt, win, False, True)
        aolgt__feew = roll_variable_apply_seq(qpzm__tqmk, fblm__xvmk,
            pvgj__psvii, win, minp, start, end, kernel_func, raw)
    else:
        aolgt__feew = np.empty(lqxn__diqt, np.float64)
    bodo.libs.distributed_api.bcast(aolgt__feew)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
...
def _dropna(arr):
    zwjd__jmas = len(arr)
    ocz__atnzb = zwjd__jmas - np.isnan(arr).sum()
    A = np.empty(ocz__atnzb, arr.dtype)
    hbkr__raj = 0
    for lwf__bsvrq in range(zwjd__jmas):
        val = arr[lwf__bsvrq]
        if not np.isnan(val):
            A[hbkr__raj] = val
            hbkr__raj += 1
