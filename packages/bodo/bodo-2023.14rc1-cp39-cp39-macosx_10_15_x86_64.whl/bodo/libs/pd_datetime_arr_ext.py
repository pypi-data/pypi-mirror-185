...
def box_pd_datetime_tzdtype(typ, val, c):
    uhrc__nprod = c.context.insert_const_string(c.builder.module, 'pandas')
    mnxy__ddg = c.pyapi.import_module_noblock(uhrc__nprod)
    qsw__tuha = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    lyz__svzkw = c.pyapi.from_native_value(types.unicode_type, qsw__tuha, c
        .env_manager)
    if isinstance(typ.tz, str):
        lqycg__xluzg = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        ppkls__tkpqa = c.pyapi.from_native_value(types.unicode_type,
            lqycg__xluzg, c.env_manager)
    else:
        hwgb__byf = nanoseconds_to_offset(typ.tz)
        ppkls__tkpqa = c.pyapi.unserialize(c.pyapi.serialize_object(hwgb__byf))
    dgby__hsbqm = c.pyapi.call_method(mnxy__ddg, 'DatetimeTZDtype', (
        lyz__svzkw, ppkls__tkpqa))
    c.pyapi.decref(lyz__svzkw)
    c.pyapi.decref(ppkls__tkpqa)
    c.pyapi.decref(mnxy__ddg)
...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        vlxa__jrzf = pd.Timedelta(pytz_type._offset).value
    else:
        vlxa__jrzf = pytz_type.zone
        if vlxa__jrzf not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    jym__murim = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(jym__murim)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        axvr__wabv = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, axvr__wabv)
...
def unbox_pd_datetime_array(typ, val, c):
    nftdp__zxbw = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    cece__fizry = c.pyapi.string_from_constant_string('datetime64[ns]')
    kgvq__yyh = c.pyapi.call_method(val, 'to_numpy', (cece__fizry,))
    nftdp__zxbw.data = c.unbox(typ.data_array_type, kgvq__yyh).value
    c.pyapi.decref(kgvq__yyh)
    xyjl__csmw = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(nftdp__zxbw._getvalue(), is_error=xyjl__csmw)
...
def box_pd_datetime_array(typ, val, c):
    nftdp__zxbw = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, nftdp__zxbw.data)
    lqug__lrlyo = c.pyapi.from_native_value(typ.data_array_type,
        nftdp__zxbw.data, c.env_manager)
    qsw__tuha = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    lyz__svzkw = c.pyapi.from_native_value(types.unicode_type, qsw__tuha, c
        .env_manager)
    ...
    uhrc__nprod = c.context.insert_const_string(c.builder.module, 'pandas')
    mnxy__ddg = c.pyapi.import_module_noblock(uhrc__nprod)
    zla__jdugm = c.pyapi.call_method(mnxy__ddg, 'DatetimeTZDtype', (
        lyz__svzkw, ppkls__tkpqa))
    nfdl__ynf = c.pyapi.object_getattr_string(mnxy__ddg, 'arrays')
    dgby__hsbqm = c.pyapi.call_method(nfdl__ynf, 'DatetimeArray', (
        lqug__lrlyo, zla__jdugm))
    c.pyapi.decref(lqug__lrlyo)
    c.pyapi.decref(lyz__svzkw)
    c.pyapi.decref(ppkls__tkpqa)
    c.pyapi.decref(mnxy__ddg)
    c.pyapi.decref(zla__jdugm)
    c.pyapi.decref(nfdl__ynf)
    c.context.nrt.decref(c.builder, typ, val)
    return dgby__hsbqm
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        hcjjn__ibz = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        hcjjn__ibz.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return hcjjn__ibz._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        lqycg__xluzg = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    xgvw__rit = DatetimeArrayType(lqycg__xluzg)
    sig = xgvw__rit(xgvw__rit.data_array_type, tz)
    return sig, codegen


@numba.njit(no_cpython_wrapper=True)
def alloc_pd_datetime_array(n, tz):
    gxsys__she = np.empty(n, dtype='datetime64[ns]')
    return init_pandas_datetime_array(gxsys__she, tz)
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    jki__yayv = context.get_constant_generic(builder, typ.data_array_type,
        pyval.to_numpy('datetime64[ns]'))
    tsglh__nzgn = lir.Constant.literal_struct([jki__yayv])
    return tsglh__nzgn
...
def overload_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            ...
            jkrlu__sfuh = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(jkrlu__sfuh, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    luv__oem = args[0]
    if equiv_set.has_shape(luv__oem):
        return ArrayAnalysis.AnalyzeResult(shape=luv__oem, pre=[])
    return None
    ...
    def overload_datetime_arr_cmp(lhs, rhs):
        ...
        if isinstance(lhs, DatetimeArrayType) and (isinstance(rhs,
            ...
            def impl(lhs, rhs):
                ...
                n = len(lhs)
                hjz__trkth = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for his__zrte in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, his__zrte):
                        bodo.libs.array_kernels.setna(hjz__trkth, his__zrte)
                    else:
                        hjz__trkth[his__zrte] = op(lhs[his__zrte], rhs)
                return hjz__trkth
            return impl
            ...
            def impl(lhs, rhs):
                ...
                hjz__trkth = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for his__zrte in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(rhs, his__zrte):
                        bodo.libs.array_kernels.setna(hjz__trkth, his__zrte)
                    else:
                        hjz__trkth[his__zrte] = op(lhs, rhs[his__zrte])
                return hjz__trkth
            return impl
            ...
            def impl(lhs, rhs):
                ...
                for his__zrte in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, his__zrte
                        ) or bodo.libs.array_kernels.isna(rhs, his__zrte):
                        bodo.libs.array_kernels.setna(hjz__trkth, his__zrte)
                    else:
                        hjz__trkth[his__zrte] = op(lhs[his__zrte], rhs[
                            his__zrte])
    ...
    if isinstance(lhs, DatetimeArrayType):
        if rhs == bodo.week_type:
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                hjz__trkth = (bodo.libs.pd_datetime_arr_ext.
                    alloc_pd_datetime_array(n, vwv__oze))
                for his__zrte in numba.parfors.parfor.internal_prange(n):
                    if bodo.libs.array_kernels.isna(lhs, his__zrte):
                        bodo.libs.array_kernels.setna(hjz__trkth, his__zrte)
                    else:
                        hjz__trkth[his__zrte] = lhs[his__zrte] + rhs
                return hjz__trkth
            return impl
        else:
            raise BodoError(
        ...
        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            n = len(rhs)
            hjz__trkth = bodo.libs.pd_datetime_arr_ext.alloc_pd_datetime_array(
                n, vwv__oze)
            for his__zrte in numba.parfors.parfor.internal_prange(n):
                if bodo.libs.array_kernels.isna(rhs, his__zrte):
                    bodo.libs.array_kernels.setna(hjz__trkth, his__zrte)
                else:
                    hjz__trkth[his__zrte] = lhs + rhs[his__zrte]
            return hjz__trkth
        return impl
    else:
        raise BodoError(
...
def convert_months_offset_to_days(curr_year, curr_month, curr_day, num_months):
    fmzqs__dwzom = curr_month + num_months - 1
    lot__rjyu = fmzqs__dwzom % 12 + 1
    rtb__paft = fmzqs__dwzom // 12
    pdk__jmc = curr_year + rtb__paft
    gjnny__wtzac = bodo.hiframes.pd_timestamp_ext.get_days_in_month(pdk__jmc,
        lot__rjyu)
    pbabh__bkxk = min(gjnny__wtzac, curr_day)
    pgi__gwh = pd.Timestamp(year=curr_year, month=curr_month, day=curr_day)
    zdeyw__tnt = pd.Timestamp(year=pdk__jmc, month=lot__rjyu, day=pbabh__bkxk)
    return zdeyw__tnt - pgi__gwh
