...
def _install_mpl_types():
    frun__itwmv = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for jwcly__qzk, tqsce__rep in frun__itwmv:
        install_py_obj_class(types_name=jwcly__qzk, python_type=tqsce__rep,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    eai__vbkg = ', '.join(f'e{xvin__rzapx}' for xvin__rzapx in range(len(args))
        )
    if eai__vbkg:
        eai__vbkg += ', '
    mncn__lxkf = ', '.join(f"{ebmj__bht} = ''" for ebmj__bht in kws.keys())
    fnli__gbcwl = 'matplotlib_obj, ' if obj_typ is not None else ''
    xmhnf__eoygl = f'def mpl_stub({fnli__gbcwl} {eai__vbkg} {mncn__lxkf}):\n'
    xmhnf__eoygl += '    pass\n'
    bax__owjl = {}
    exec(xmhnf__eoygl, {}, bax__owjl)
    jup__xyai = bax__owjl['mpl_stub']
    rqw__wwby = numba.core.utils.pysignature(jup__xyai)
    naz__usndb = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(naz__usndb)).replace(pysig=
        rqw__wwby)


def generate_axes_typing(mod_name, nrows, ncols):
    sjyc__ron = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(sjyc__ron.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(sjyc__ron.format(mod_name, 'ncols'))
    cjokz__esx = get_overload_const_int(nrows)
    tkv__bkzm = get_overload_const_int(ncols)
    if cjokz__esx < 1:
        raise BodoError(sjyc__ron.format(mod_name, 'nrows'))
    if tkv__bkzm < 1:
        raise BodoError(sjyc__ron.format(mod_name, 'ncols'))
    if cjokz__esx == 1 and tkv__bkzm == 1:
        ekt__tjrp = types.mpl_axes_type
    else:
        if tkv__bkzm == 1:
            img__dzp = types.mpl_axes_type
        else:
            img__dzp = types.Tuple([types.mpl_axes_type] * tkv__bkzm)
        ekt__tjrp = types.Tuple([img__dzp] * cjokz__esx)
    return ekt__tjrp


def generate_pie_return_type(args, kws):
    qcekd__zfo = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if qcekd__zfo == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    jfvne__wuxh = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(jfvne__wuxh):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(jfvne__wuxh):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        jqnj__wtr = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, jqnj__wtr]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    jqnj__wtr = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    jwcly__qzk = str(jqnj__wtr)
    if not hasattr(types, jwcly__qzk):
        jwcly__qzk = f'objmode_type{ir_utils.next_label()}'
        setattr(types, jwcly__qzk, jqnj__wtr)
    xmhnf__eoygl = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{jwcly__qzk}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    bax__owjl = {}
    exec(xmhnf__eoygl, {'numba': numba, 'np': np}, bax__owjl)
    impl = bax__owjl['impl']
    return impl
