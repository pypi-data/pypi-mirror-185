...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        cyo__kysts = self.overloads[sig]
        mxsmx__hida = cyo__kysts.metadata.get('distributed_diagnostics', None)
        if mxsmx__hida is None:
            nqz__payca = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(nqz__payca)
        mxsmx__hida.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        pzlz__zhdfa = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(pzlz__zhdfa)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        wmgo__zwz = options.pop('distributed')
        pipeline_class = (pipeline_class if wmgo__zwz else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        uze__bhk = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if uze__bhk else
            pipeline_class)
    kspt__hianz = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(kspt__hianz, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(kspt__hianz)
        else:
            return master_mode_wrapper(kspt__hianz)
    else:
        return kspt__hianz
