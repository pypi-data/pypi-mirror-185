...
def _global_except_hook(exctype, value, traceback):
    ...
    fofg__nmedg = MPI.COMM_WORLD
    hixx__exfby = fofg__nmedg.Get_rank()
    gap__vxje = 3.0
    krv__icti = True
    mbts__dlrt = fofg__nmedg.Ibarrier()
    xnuf__dzuy = time.time()
    while time.time() - xnuf__dzuy < gap__vxje:
        time.sleep(0.1)
        if mbts__dlrt.Test():
            krv__icti = False
            break
    try:
        ...
        if krv__icti:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {hixx__exfby}. \n')
...
def set_numba_environ_vars():
    if (qdms__ftj := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = qdms__ftj
        os.environ['NUMBA_CACHE_DIR'] = qdms__ftj
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    ydy__gtakv = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(ydy__gtakv)
