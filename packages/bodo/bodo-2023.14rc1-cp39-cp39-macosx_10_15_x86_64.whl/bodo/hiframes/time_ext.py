...
def _from_nanos_codegen(c, val):
    cvvuh__qlmhz = c.pyapi.long_from_longlong(c.builder.udiv(val, lir.
        Constant(lir.IntType(64), _nanos_per_hour)))
    nleh__lrq = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_hour)), lir.Constant(
        lir.IntType(64), _nanos_per_minute)))
    enld__mdcd = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_minute)), lir.
        Constant(lir.IntType(64), _nanos_per_second)))
    zbjzt__oswu = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_second)), lir.
        Constant(lir.IntType(64), _nanos_per_milli)))
    fdpb__teljh = c.pyapi.long_from_longlong(c.builder.udiv(c.builder.urem(
        val, lir.Constant(lir.IntType(64), _nanos_per_milli)), lir.Constant
        (lir.IntType(64), _nanos_per_micro)))
    hov__jzqlu = c.pyapi.long_from_longlong(c.builder.urem(val, lir.
        Constant(lir.IntType(64), _nanos_per_micro)))
    return (cvvuh__qlmhz, nleh__lrq, enld__mdcd, zbjzt__oswu, fdpb__teljh,
        hov__jzqlu)


@unbox(TimeType)
def unbox_time(typ, val, c):
    cvvuh__qlmhz = c.pyapi.object_getattr_string(val, 'hour')
    nleh__lrq = c.pyapi.object_getattr_string(val, 'minute')
    enld__mdcd = c.pyapi.object_getattr_string(val, 'second')
    zbjzt__oswu = c.pyapi.object_getattr_string(val, 'millisecond')
    fdpb__teljh = c.pyapi.object_getattr_string(val, 'microsecond')
    hov__jzqlu = c.pyapi.object_getattr_string(val, 'nanosecond')
    hour_ll = c.pyapi.long_as_longlong(cvvuh__qlmhz)
    minute_ll = c.pyapi.long_as_longlong(nleh__lrq)
    second_ll = c.pyapi.long_as_longlong(enld__mdcd)
    millisecond_ll = c.pyapi.long_as_longlong(zbjzt__oswu)
    microsecond_ll = c.pyapi.long_as_longlong(fdpb__teljh)
    nanosecond_ll = c.pyapi.long_as_longlong(hov__jzqlu)
    vwv__lpfj = _to_nanos_codegen(c, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    c.pyapi.decref(cvvuh__qlmhz)
    c.pyapi.decref(nleh__lrq)
    c.pyapi.decref(enld__mdcd)
    c.pyapi.decref(zbjzt__oswu)
    c.pyapi.decref(fdpb__teljh)
    c.pyapi.decref(hov__jzqlu)
    aahn__uurp = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(vwv__lpfj, is_error=aahn__uurp)
...
def lower_constant_time(context, builder, ty, pyval):
    ...
    vwv__lpfj = _to_nanos_codegen(context, hour_ll, minute_ll, second_ll,
        millisecond_ll, microsecond_ll, nanosecond_ll)
    return vwv__lpfj
...
def box_time(typ, val, c):
    (cvvuh__qlmhz, nleh__lrq, enld__mdcd, zbjzt__oswu, fdpb__teljh, hov__jzqlu
        ) = _from_nanos_codegen(c, val)
    iglh__ixm = c.pyapi.unserialize(c.pyapi.serialize_object(Time))
    tfvnl__ltc = c.pyapi.call_function_objargs(iglh__ixm, (cvvuh__qlmhz,
        nleh__lrq, enld__mdcd, zbjzt__oswu, fdpb__teljh, hov__jzqlu, c.
...
class TimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        fqpcj__olpte = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, fqpcj__olpte)
...
def unbox_time_array(typ, val, c):
    ...
    euyq__dkg = types.Array(types.intp, 1, 'C')
    utzfy__abh = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        euyq__dkg, [n])
    cavwj__rkzs = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    oosdk__fwirl = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(types.uint8, 1, 'C'), [cavwj__rkzs])
    idtiy__myw = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(64).as_pointer(), lir.
        IntType(8).as_pointer()])
    ycef__udi = cgutils.get_or_insert_function(c.builder.module, idtiy__myw,
        name='unbox_time_array')
    c.builder.call(ycef__udi, [val, n, utzfy__abh.data, oosdk__fwirl.data])
    depye__tiyxn = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    depye__tiyxn.data = utzfy__abh._getvalue()
    depye__tiyxn.null_bitmap = oosdk__fwirl._getvalue()
    aahn__uurp = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(depye__tiyxn._getvalue(), is_error=aahn__uurp)


@box(TimeArrayType)
def box_time_array(typ, val, c):
    icu__uema = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    utzfy__abh = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, icu__uema.data)
    rojad__cdz = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, icu__uema.null_bitmap).data
    n = c.builder.extract_value(utzfy__abh.shape, 0)
    idtiy__myw = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType(8)])
    hey__oka = cgutils.get_or_insert_function(c.builder.module, idtiy__myw,
        name='box_time_array')
    ttol__ubcpq = c.builder.call(hey__oka, [n, utzfy__abh.data, rojad__cdz,
...
def init_time_array(typingctx, data, nulls, precision):
    ...
    def codegen(context, builder, signature, args):
        zuxd__fmy, uwv__ope, uxvx__szxy = args
        nrre__tysfm = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        nrre__tysfm.data = zuxd__fmy
        nrre__tysfm.null_bitmap = uwv__ope
        context.nrt.incref(builder, signature.args[0], zuxd__fmy)
        context.nrt.incref(builder, signature.args[1], uwv__ope)
        return nrre__tysfm._getvalue()
...
def lower_constant_time_arr(context, builder, typ, pyval):
    n = len(pyval)
    utzfy__abh = np.full(n, 0, np.int64)
    sll__yskr = np.empty(n + 7 >> 3, np.uint8)
    for fbub__tqitk, ptc__rnqw in enumerate(pyval):
        fcvvx__jjqkd = pd.isna(ptc__rnqw)
        bodo.libs.int_arr_ext.set_bit_to_arr(sll__yskr, fbub__tqitk, int(
            not fcvvx__jjqkd))
        if not fcvvx__jjqkd:
            utzfy__abh[fbub__tqitk] = (ptc__rnqw.hour * _nanos_per_hour + 
                ptc__rnqw.minute * _nanos_per_minute + ptc__rnqw.second *
                _nanos_per_second + ptc__rnqw.millisecond *
                _nanos_per_milli + ptc__rnqw.microsecond * _nanos_per_micro +
                ptc__rnqw.nanosecond)
    cnov__olxo = context.get_constant_generic(builder, data_type, utzfy__abh)
    nofig__fbj = context.get_constant_generic(builder, nulls_type, sll__yskr)
    return lir.Constant.literal_struct([cnov__olxo, nofig__fbj])


@numba.njit(no_cpython_wrapper=True)
def alloc_time_array(n, precision):
    utzfy__abh = np.empty(n, dtype=np.int64)
    nulls = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_time_array(utzfy__abh, nulls, precision)
...
def time_arr_getitem(A, ind):
    ...
    if ind != bodo.boolean_array and is_list_like_index_type(ind
        ...
        def impl_bool(A, ind):
            mubwe__qxnrr, gwym__rjzi = array_getitem_bool_index(A, ind)
            return init_time_array(mubwe__qxnrr, gwym__rjzi, precision)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):

        def impl(A, ind):
            mubwe__qxnrr, gwym__rjzi = array_getitem_int_index(A, ind)
            return init_time_array(mubwe__qxnrr, gwym__rjzi, precision)
        return impl
    if isinstance(ind, types.SliceType):

        def impl_slice(A, ind):
            mubwe__qxnrr, gwym__rjzi = array_getitem_slice_index(A, ind)
            return init_time_array(mubwe__qxnrr, gwym__rjzi, precision)
        return impl_slice
...
def create_cmp_op_overload(op):
    ...
    def overload_time_cmp(lhs, rhs):
        if isinstance(lhs, TimeType) and isinstance(rhs, TimeType):
            ...
            def impl(lhs, rhs):
                gshs__ghqe = cast_time_to_int(lhs)
                abyd__rsgi = cast_time_to_int(rhs)
                return op(0 if gshs__ghqe == abyd__rsgi else 1 if 
                    gshs__ghqe > abyd__rsgi else -1, 0)
            return impl
