...
def _column_filter_impl(B, ind):
    gkm__wflv = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,), None)
    for tkrm__efdhs in numba.parfors.parfor.internal_prange(len(gkm__wflv)):
        if ind[tkrm__efdhs]:
            gkm__wflv[tkrm__efdhs] = B[tkrm__efdhs]
        else:
            bodo.libs.array_kernels.setna(gkm__wflv, tkrm__efdhs)
    return gkm__wflv
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    szv__feppm = len(B)
    feo__zoe = 0
    for tkrm__efdhs in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, tkrm__efdhs):
            feo__zoe += 1
    jpim__vnart = szv__feppm - feo__zoe
    nck__dzsrq = bodo.libs.str_arr_ext.num_total_chars(B)
    gkm__wflv = bodo.libs.str_arr_ext.pre_alloc_string_array(jpim__vnart,
        nck__dzsrq)
    bodo.libs.str_arr_ext.copy_non_null_offsets(gkm__wflv, B)
    bodo.libs.str_arr_ext.copy_data(gkm__wflv, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(gkm__wflv, -1)
    return gkm__wflv
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    txqh__lozz = first_moment / count
    ygbdb__jhrh = (third_moment - 3 * second_moment * txqh__lozz + 2 *
        count * txqh__lozz ** 3)
    rwpb__avfbw = second_moment - txqh__lozz * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * ygbdb__jhrh / rwpb__avfbw ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    txqh__lozz = first_moment / count
    dpbam__veu = (fourth_moment - 4 * third_moment * txqh__lozz + 6 *
        second_moment * txqh__lozz ** 2 - 3 * count * txqh__lozz ** 4)
    czvf__crt = second_moment - txqh__lozz * first_moment
    fbn__psos = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    vpzo__fkl = count * (count + 1) * (count - 1) * dpbam__veu
    skah__eld = (count - 2) * (count - 3) * czvf__crt ** 2
    s = (count - 1) * (vpzo__fkl / skah__eld - fbn__psos)
    s = s / (count - 1)
    return s
