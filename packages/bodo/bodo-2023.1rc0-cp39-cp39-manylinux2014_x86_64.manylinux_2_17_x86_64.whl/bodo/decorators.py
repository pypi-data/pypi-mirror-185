...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        jalzz__lwxp = self.overloads[sig]
        vze__afik = jalzz__lwxp.metadata.get('distributed_diagnostics', None)
        if vze__afik is None:
            ieq__stcb = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(ieq__stcb)
        vze__afik.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        kje__gst = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(kje__gst)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        roeu__dup = options.pop('distributed')
        pipeline_class = (pipeline_class if roeu__dup else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        njmp__opj = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if njmp__opj else
            pipeline_class)
    ivnlh__ygw = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(ivnlh__ygw, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(ivnlh__ygw)
        else:
            return master_mode_wrapper(ivnlh__ygw)
    else:
        return ivnlh__ygw
