...
def _install_mpl_types():
    zejcj__ywoko = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for mbt__zpt, ggi__asvfx in zejcj__ywoko:
        install_py_obj_class(types_name=mbt__zpt, python_type=ggi__asvfx,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    pjuyw__yqz = ', '.join(f'e{tmq__pyzke}' for tmq__pyzke in range(len(args)))
    if pjuyw__yqz:
        pjuyw__yqz += ', '
    mhrz__idgbg = ', '.join(f"{qxk__rgqk} = ''" for qxk__rgqk in kws.keys())
    mdbxj__qaijz = 'matplotlib_obj, ' if obj_typ is not None else ''
    zmc__plv = f'def mpl_stub({mdbxj__qaijz} {pjuyw__yqz} {mhrz__idgbg}):\n'
    zmc__plv += '    pass\n'
    pxl__oryih = {}
    exec(zmc__plv, {}, pxl__oryih)
    gngcn__ithnd = pxl__oryih['mpl_stub']
    rfxb__vvrw = numba.core.utils.pysignature(gngcn__ithnd)
    zhu__dqzeu = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(zhu__dqzeu)).replace(pysig=
        rfxb__vvrw)


def generate_axes_typing(mod_name, nrows, ncols):
    mvybh__pfyzc = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(mvybh__pfyzc.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(mvybh__pfyzc.format(mod_name, 'ncols'))
    gcr__llttm = get_overload_const_int(nrows)
    aoqc__qwuiq = get_overload_const_int(ncols)
    if gcr__llttm < 1:
        raise BodoError(mvybh__pfyzc.format(mod_name, 'nrows'))
    if aoqc__qwuiq < 1:
        raise BodoError(mvybh__pfyzc.format(mod_name, 'ncols'))
    if gcr__llttm == 1 and aoqc__qwuiq == 1:
        sfan__iuwk = types.mpl_axes_type
    else:
        if aoqc__qwuiq == 1:
            pzl__uglc = types.mpl_axes_type
        else:
            pzl__uglc = types.Tuple([types.mpl_axes_type] * aoqc__qwuiq)
        sfan__iuwk = types.Tuple([pzl__uglc] * gcr__llttm)
    return sfan__iuwk


def generate_pie_return_type(args, kws):
    yib__uwz = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if yib__uwz == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    wneh__tqy = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(wneh__tqy):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(wneh__tqy):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ncg__gle = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, ncg__gle]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    ncg__gle = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    mbt__zpt = str(ncg__gle)
    if not hasattr(types, mbt__zpt):
        mbt__zpt = f'objmode_type{ir_utils.next_label()}'
        setattr(types, mbt__zpt, ncg__gle)
    zmc__plv = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        ...
        with numba.objmode(axes="{mbt__zpt}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    pxl__oryih = {}
    exec(zmc__plv, {'numba': numba, 'np': np}, pxl__oryih)
    impl = pxl__oryih['impl']
    return impl
