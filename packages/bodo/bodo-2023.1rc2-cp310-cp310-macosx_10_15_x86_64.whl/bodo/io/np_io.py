...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        zzo__zgnhw = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, zzo__zgnhw)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            hbpb__uvkl = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(hbpb__uvkl.dtype)
            file_write(unicode_to_utf8(fname), hbpb__uvkl.ctypes, 
                dtype_size * hbpb__uvkl.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            hbpb__uvkl = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(hbpb__uvkl.dtype)
            yhrp__ngp = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                hbpb__uvkl.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), hbpb__uvkl.ctypes,
                start, count, yhrp__ngp)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            mbfuw__tagbp = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                mbfuw__tagbp = min(mbfuw__tagbp, count * dtype_size)
            if mbfuw__tagbp < 0:
                return -1
            return mbfuw__tagbp
        return impl
