...
def _global_except_hook(exctype, value, traceback):
    ...
    xppj__tbwc = MPI.COMM_WORLD
    gdeu__vvmn = xppj__tbwc.Get_rank()
    mlw__zoakb = 3.0
    mtop__lqlj = True
    eirb__dqea = xppj__tbwc.Ibarrier()
    uavy__tdd = time.time()
    while time.time() - uavy__tdd < mlw__zoakb:
        time.sleep(0.1)
        if eirb__dqea.Test():
            mtop__lqlj = False
            break
    try:
        ...
        if mtop__lqlj:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {gdeu__vvmn}. \n')
...
def set_numba_environ_vars():
    if (hkk__nhdg := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = hkk__nhdg
        os.environ['NUMBA_CACHE_DIR'] = hkk__nhdg
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    rywpz__fxyd = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(rywpz__fxyd)
