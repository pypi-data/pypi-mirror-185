...
def _global_except_hook(exctype, value, traceback):
    ...
    lug__ymir = MPI.COMM_WORLD
    vxk__qsia = lug__ymir.Get_rank()
    wvfwb__xkrro = 3.0
    omli__gyt = True
    mjr__efvy = lug__ymir.Ibarrier()
    wdaph__zpd = time.time()
    while time.time() - wdaph__zpd < wvfwb__xkrro:
        time.sleep(0.1)
        if mjr__efvy.Test():
            omli__gyt = False
            break
    try:
        ...
        if omli__gyt:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {vxk__qsia}. \n')
...
def set_numba_environ_vars():
    if (gwlz__esxws := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = gwlz__esxws
        os.environ['NUMBA_CACHE_DIR'] = gwlz__esxws
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    hhz__xnip = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(hhz__xnip)
