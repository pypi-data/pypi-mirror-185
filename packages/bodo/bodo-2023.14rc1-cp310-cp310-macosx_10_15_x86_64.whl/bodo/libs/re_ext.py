...
def box_regex_flag(typ, val, c):
    jhfl__zrtp = c.box(types.int64, val)
    ckw__ogjc = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(ckw__ogjc, (jhfl__zrtp,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    jhfl__zrtp = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, jhfl__zrtp)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    sgzbr__wiazg = context.get_python_api(builder)
    ihwv__uguh = builder.icmp_signed('==', val, sgzbr__wiazg.borrow_none())
    with builder.if_then(ihwv__uguh):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    tijg__kclrk = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(tijg__kclrk)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        uxyw__duao = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if uxyw__duao > 1:
            typ = types.List(types.Tuple([string_type] * uxyw__duao))
...
def overload_match_group(m, *args):
    ...
    bigb__fuze = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            wdvap__eci = args[0]
            with numba.objmode(out=bigb__fuze):
                out = m.group(wdvap__eci)
            return out
        return _match_group_impl_one
    fzphz__smm = 'tuple_str_{}'.format(len(args))
    setattr(types, fzphz__smm, types.Tuple([bigb__fuze] * len(args)))
    fft__yuzte = ', '.join('group{}'.format(phgx__hgwzt + 1) for
        phgx__hgwzt in range(len(args)))
    qak__uuwv = 'def _match_group_impl(m, *args):\n'
    qak__uuwv += '  ({}) = args\n'.format(fft__yuzte)
    qak__uuwv += "  with numba.objmode(out='{}'):\n".format(fzphz__smm)
    qak__uuwv += '    out = m.group({})\n'.format(fft__yuzte)
    qak__uuwv += '  return out\n'
    jxi__zwvd = {}
    exec(qak__uuwv, globals(), jxi__zwvd)
    ndjx__eag = jxi__zwvd['_match_group_impl']
    return ndjx__eag
...
def overload_match_groups(m, default=None):
    ijv__fvwe = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ijv__fvwe):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(exhud__tllcn is None for exhud__tllcn in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=bigb__fuze):
