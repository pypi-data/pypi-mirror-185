...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        vye__oqmg = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, vye__oqmg)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            nuls__dfqq = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(nuls__dfqq.dtype)
            file_write(unicode_to_utf8(fname), nuls__dfqq.ctypes, 
                dtype_size * nuls__dfqq.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            nuls__dfqq = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(nuls__dfqq.dtype)
            gqf__hjz = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                nuls__dfqq.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), nuls__dfqq.ctypes,
                start, count, gqf__hjz)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            komvu__zianq = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                komvu__zianq = min(komvu__zianq, count * dtype_size)
            if komvu__zianq < 0:
                return -1
            return komvu__zianq
        return impl
