...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        nrfgr__qtj = len(A.types)
        sgl__jph = 'def f(A):\n'
        cblx__ktd = ','.join(f'decode_if_dict_array(A[{i}])' for i in range
            (nrfgr__qtj))
        sgl__jph += '  return ({}{})\n'.format(cblx__ktd, ',' if nrfgr__qtj ==
            1 else '')
        qktfe__xuox = {}
        exec(sgl__jph, {'decode_if_dict_array': decode_if_dict_array},
            qktfe__xuox)
        impl = qktfe__xuox['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                nrfgr__qtj += 1
            hso__kmx = []
            for i in range(nrfgr__qtj):
                hso__kmx.append(decode_if_dict_array(A[i]))
            return hso__kmx
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            wzs__kkozx = bodo.hiframes.pd_series_ext.get_series_data(A)
            fbi__ksp = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            jqymz__chq = decode_if_dict_array(wzs__kkozx)
            return bodo.hiframes.pd_series_ext.init_series(jqymz__chq,
                fbi__ksp, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        nov__jxrom = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(nov__jxrom, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        vfr__ucvj = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(vfr__ucvj(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        qibg__ucji = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=qibg__ucji)
...
class FileInfo:
    ...
    def get_schema(self, fname: str):
        kbwy__znmyy = self.get_full_filename(fname)
        return self._get_schema(kbwy__znmyy)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vjcu__yqft = dmm.lookup(bodo.typeof(fe_type.fname))
        rshaj__wfgf = [(a, xny__afb) for a, xny__afb in zip(vjcu__yqft.
            _fields, vjcu__yqft._members)]
        super().__init__(dmm, fe_type, rshaj__wfgf)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    aot__oxg = t.initial_value
    if isinstance(aot__oxg, dict):
        aot__oxg = aot__oxg.values()
    return not any(isinstance(lsug__ite, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for lsug__ite in aot__oxg)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (lsug__ite) for lsug__ite in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (lsug__ite) for lsug__ite in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        xha__aib = []
        for lsug__ite in val.literal_value:
            tvv__jcec = get_overload_const(lsug__ite)
            if tvv__jcec == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                xha__aib.append(tvv__jcec)
        return xha__aib
    ...
    if isinstance(val, types.BaseTuple):
        xha__aib = []
        for lsug__ite in val.types:
            tvv__jcec = get_overload_const(lsug__ite)
            if tvv__jcec == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                xha__aib.append(tvv__jcec)
        return tuple(xha__aib)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    tvv__jcec = get_overload_const(val)
    ywkgd__tda = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(tvv__jcec, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {ywkgd__tda} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    tvv__jcec = get_overload_const(val)
    if tvv__jcec not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{tvv__jcec}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    pchz__kxgr = False
    for a in args_dict:
        kst__ztiq = get_overload_const(args_dict[a])
        pbh__vns = arg_defaults_dict[a]
        if (kst__ztiq is NOT_CONSTANT or kst__ztiq is not None and pbh__vns is
            None or kst__ztiq is None and pbh__vns is not None or kst__ztiq !=
            pbh__vns or kst__ztiq is not _no_input and pbh__vns is
            _no_input or kst__ztiq is _no_input and pbh__vns is not _no_input):
            uboi__zcd = (
                f'{fn_str}: {a} parameter only supports default value {pbh__vns}'
                )
            pchz__kxgr = True
            break
    if pchz__kxgr and package_name == 'pandas':
        if module_name == 'IO':
            uboi__zcd += """
...
def get_overload_constant_dict(val) ->Dict:
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(qmsai__jkjqv): get_literal_value(
            lsug__ite) for qmsai__jkjqv, lsug__ite in val.literal_value.items()
            }
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    zfdx__hrhr = [get_overload_const(lsug__ite) for lsug__ite in val.types[1:]]
    return {zfdx__hrhr[2 * i]: zfdx__hrhr[2 * i + 1] for i in range(len(
        zfdx__hrhr) // 2)}
...
def get_overload_const_list(val) ->Union[List[Any], Tuple[Any, ...], None]:
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(lsug__ite) for lsug__ite in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        brgb__mlgz = get_overload_const_str(dtype)
        if brgb__mlgz.startswith('Int') or brgb__mlgz.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(brgb__mlgz), None)
        if brgb__mlgz.startswith('Float'):
            return bodo.libs.float_arr_ext.typeof_pd_float_dtype(pd.api.
                types.pandas_dtype(brgb__mlgz), None)
        if brgb__mlgz == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if brgb__mlgz == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(brgb__mlgz))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    bvtvn__lfp = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        gymn__ovc = []
        for i, nloa__vdu in enumerate(t.names_typ):
            if nloa__vdu == types.none:
                gymn__ovc.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(nloa__vdu):
                raise BodoError(bvtvn__lfp)
            gymn__ovc.append(get_overload_const_str(nloa__vdu))
        return tuple(gymn__ovc)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        kghtl__rhtr = typ.literal_value
        tqdx__tnaou = []
        for nrqhm__crcu in ('start', 'stop', 'step'):
            azgx__wte = getattr(typ.literal_value, nrqhm__crcu)
            evdkl__ouzci = c.pyapi.make_none(
                ) if azgx__wte is None else c.pyapi.from_native_value(types
                .literal(azgx__wte), azgx__wte, c.env_manager)
            tqdx__tnaou.append(evdkl__ouzci)
        rviaf__aaq = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        ijrbx__jczx = c.pyapi.call_function_objargs(rviaf__aaq, tqdx__tnaou)
        for a in tqdx__tnaou:
            c.pyapi.decref(a)
        c.pyapi.decref(rviaf__aaq)
        return ijrbx__jczx
...
def box_list_literal(typ, val, c):
    mtlfg__ggq = typ.literal_value
    pph__tesq = [c.pyapi.from_native_value(types.literal(lsug__ite),
        lsug__ite, c.env_manager) for lsug__ite in mtlfg__ggq]
    xarjg__whkas = c.pyapi.list_pack(pph__tesq)
    for a in pph__tesq:
        c.pyapi.decref(a)
    return xarjg__whkas
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    btiw__gqhg = types.List(toty.dtype)
...
def is_literal_type(t):
    return isinstance(t, types.TypeRef) or isinstance(t, (types.Literal,
        ...
        , types.BaseTuple) and all(is_literal_type(lsug__ite) for lsug__ite in
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(qmsai__jkjqv): get_literal_value(
                lsug__ite) for qmsai__jkjqv, lsug__ite in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(lsug__ite) for lsug__ite in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(lsug__ite) for lsug__ite in t.types)
...
def dtype_to_array_type(dtype, convert_nullable=False):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        wzs__kkozx = types.Array(dtype, 1, 'C')
        if convert_nullable:
            return to_nullable_type(wzs__kkozx)
        return wzs__kkozx
...
def types_equality_exists(t1, t2):
    ula__nto = numba.core.registry.cpu_target.typing_context
    try:
        ula__nto.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    dwmmd__hgufh = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns,
        bodo.pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in dwmmd__hgufh:
        return True
    ula__nto = numba.core.registry.cpu_target.typing_context
    try:
        ula__nto.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        yxvw__eyutw = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(yxvw__eyutw, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    uzp__nnc, swalt__rajmh = get_common_scalar_dtype(scalar_types)
    return swalt__rajmh
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        pfwr__ewk = np.find_common_type([numba.np.numpy_support.as_dtype(t) for
            t in scalar_types], [])
        if pfwr__ewk != object:
            return numba.np.numpy_support.from_dtype(pfwr__ewk), True
    except numba.core.errors.NumbaNotImplementedError as vjlk__ude:
        pass
    ...
    mjpig__xcdv = itertools.groupby(scalar_types)
    if next(mjpig__xcdv, True) and not next(mjpig__xcdv, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            zofra__khic.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, (bodo.libs.int_arr_ext.IntegerArrayType, bodo.
            libs.float_arr_ext.FloatingArrayType)):
            zofra__khic.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                zofra__khic.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                zofra__khic.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
            if isinstance(typ.dtype, types.Float):
                zofra__khic.append(bodo.libs.float_arr_ext.
                    FloatingArrayType(typ.dtype))
        zofra__khic.append(typ)
    return zofra__khic
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        wgzv__ewx = getfullargspec(func)
        assert wgzv__ewx.varargs is None, 'varargs not supported'
        assert wgzv__ewx.varkw is None, 'varkw not supported'
        defaults = [] if wgzv__ewx.defaults is None else wgzv__ewx.defaults
        hfzt__epf = len(wgzv__ewx.args) - len(defaults)
        args = wgzv__ewx.args[1:] if attr_name else wgzv__ewx.args[:]
        ykkhe__cgz = []
        for i, ctpnn__liy in enumerate(wgzv__ewx.args):
            if i < hfzt__epf:
                ykkhe__cgz.append(ctpnn__liy)
            elif str(defaults[i - hfzt__epf]) != '<deprecated parameter>':
                ykkhe__cgz.append(ctpnn__liy + '=' + str(defaults[i -
                    hfzt__epf]))
            else:
                args.remove(ctpnn__liy)
        if wgzv__ewx.kwonlyargs is not None:
            for ctpnn__liy in wgzv__ewx.kwonlyargs:
                args.append(f'{ctpnn__liy}={ctpnn__liy}')
                ykkhe__cgz.append(
                    f'{ctpnn__liy}={str(wgzv__ewx.kwonlydefaults[ctpnn__liy])}'
                    )
        sig = ', '.join(ykkhe__cgz)
        args = ', '.join(args)
    else:
    ...
    avhc__xalxn = f'self.{attr_name}' if attr_name else f'{func_name}'
    sgl__jph = f'def overload_impl({sig}):\n'
    sgl__jph += f'    def impl({sig}):\n'
    if single_rank:
        sgl__jph += f'        if bodo.get_rank() == 0:\n'
        zfqq__cqav = '    '
    else:
        zfqq__cqav = ''
    sgl__jph += f"        {zfqq__cqav}with numba.objmode(res='{type_name}'):\n"
    if is_function:
        sgl__jph += f'            {zfqq__cqav}res = {avhc__xalxn}({args})\n'
    else:
        sgl__jph += f'            {zfqq__cqav}res = {avhc__xalxn}\n'
    sgl__jph += f'        return res\n'
    sgl__jph += f'    return impl\n'
    qktfe__xuox = {}
    wezyp__txaej = globals()
    if not attr_name:
        wezyp__txaej[func_name] = func
    exec(sgl__jph, wezyp__txaej, qktfe__xuox)
    fnmb__pkfr = qktfe__xuox['overload_impl']
    return fnmb__pkfr


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        fnmb__pkfr = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(fnmb__pkfr)
    except Exception as vjlk__ude:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        fnmb__pkfr = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(fnmb__pkfr)
    except Exception as vjlk__ude:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        fnmb__pkfr = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(fnmb__pkfr)
    except Exception as vjlk__ude:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, ekorb__rmr = args
        if isinstance(ekorb__rmr, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[ekorb__rmr]),
                *args)
...
def construct_pysig(arg_names, defaults):
    sgl__jph = f'def stub('
    for ctpnn__liy in arg_names:
        sgl__jph += ctpnn__liy
        if ctpnn__liy in defaults:
            if isinstance(defaults[ctpnn__liy], str):
                sgl__jph += f"='{defaults[ctpnn__liy]}'"
            else:
                sgl__jph += f'={defaults[ctpnn__liy]}'
        sgl__jph += ', '
    sgl__jph += '):\n'
    sgl__jph += '    pass\n'
    qktfe__xuox = {}
    exec(sgl__jph, {}, qktfe__xuox)
    qrj__fil = qktfe__xuox['stub']
    return numba.core.utils.pysignature(qrj__fil)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    kvgej__ajtub = len(arg_names)
    actuh__roff = len(args) + len(kws)
    if actuh__roff > kvgej__ajtub:
        pdalx__qlg = 'argument' if kvgej__ajtub == 1 else 'arguments'
        vuu__elek = 'was' if actuh__roff == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {kvgej__ajtub} {pdalx__qlg}, but {actuh__roff} {vuu__elek} provided.'
            )
    jtrt__cqzsq = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        aio__zztyu = bodo.utils.transform.fold_argument_types(jtrt__cqzsq,
            args, kws)
    except Exception as ora__uifl:
        raise_bodo_error(f'{func_name}(): {ora__uifl}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                zzjm__rukym[arg_name] = aio__zztyu[i]
                upei__zeaqr[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, zzjm__rukym, upei__zeaqr)
    return jtrt__cqzsq, aio__zztyu
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        rshaj__wfgf = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, rshaj__wfgf)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [suo__wlws] = sig.args
    [ujkm__igif] = args
    bjg__esil = context.make_helper(builder, suo__wlws, value=ujkm__igif)
    gqlhx__shb = signature(types.intp, suo__wlws.arr_type)
    gxarw__hsbyy = context.compile_internal(builder, lambda a: len(a),
        gqlhx__shb, [bjg__esil.array])
    fbi__ksp = builder.load(bjg__esil.index)
    vkxm__ulv = builder.icmp_signed('<', fbi__ksp, gxarw__hsbyy)
    result.set_valid(vkxm__ulv)
    with builder.if_then(vkxm__ulv):
        rmcg__pxw = signature(suo__wlws.yield_type, suo__wlws.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            rmcg__pxw, [bjg__esil.array, fbi__ksp])
        result.yield_(value)
        bjrly__efqwm = cgutils.increment_index(builder, fbi__ksp)
        builder.store(bjrly__efqwm, bjg__esil.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    zzaoa__mdhk = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        dpdv__mmn = None
    else:
        dpdv__mmn = types.StringLiteral(name)
    if zzaoa__mdhk == bodo.hiframes.pd_index_ext.NumericIndexType:
        zyrch__djqs = zzaoa__mdhk(elem_dtype, dpdv__mmn, arr_typ)
    elif zzaoa__mdhk == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        zyrch__djqs = zzaoa__mdhk(bodo.CategoricalArrayType(elem_dtype),
            dpdv__mmn, arr_typ)
    else:
        zyrch__djqs = zzaoa__mdhk(dpdv__mmn, arr_typ)
    return zyrch__djqs
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(lsug__ite, t) for lsug__ite, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        fapz__skt = pd.Index(val_typ.columns)
        fllw__dhj = pd.Index(typ.columns)
        ranj__nxha = fapz__skt.argsort()
        mis__uryce = fllw__dhj.argsort()
        gfe__fwd = val_typ.copy(data=tuple(np.array(val_typ.data)[
            ranj__nxha]), columns=tuple(fapz__skt[ranj__nxha]))
        owb__uvwzi = typ.copy(data=tuple(np.array(typ.data)[mis__uryce]),
            columns=tuple(fllw__dhj[mis__uryce]))
        if gfe__fwd == owb__uvwzi:
            val_typ = typ
