...
class Intercept(Term):
    ...
    def build_columns(self, X, verbose=False, return_sparse: bool = True):
        """construct the model matrix columns for the term
...
class LinearTerm(Term):
    ...
    def build_columns(self, X, verbose=False, return_sparse: bool = True):
        ...
        if return_sparse:
            ...
            return X[:, self.feature][:, np.newaxis]
...
class SplineTerm(Term):
    ...
    def _validate_arguments(self):
        ...
        if (self.n_splines < 1) or not isinstance(self.n_splines, numbers.Integral):
            raise ValueError('n_splines must be int >= 1')

        if (self.spline_order < 0) or not isinstance(self.spline_order, numbers.Integral):
    ...
    def compile(self, X, verbose=False):
        ...
        if not hasattr(self, 'edge_knots_'):
            self.edge_knots_ = np.sort(gen_edge_knots(X[:, self.feature],
                                              self.dtype,
                                              verbose=verbose))
        return self
...
class FactorTerm(SplineTerm):
    ...
    def build_columns(self, X, verbose=False, return_sparse: bool = True):
        ...
        scipy sparse array with n rows
        """
        columns = super(FactorTerm, self).build_columns(X, verbose=verbose, return_sparse=return_sparse)
...
class TensorTerm(SplineTerm, MetaTermMixin):
    ...
    def build_columns(self, X, verbose=False, return_sparse: bool = True):
        ...
        scipy sparse array with n rows
        """
        splines = self._terms[0].build_columns(X, verbose=verbose, return_sparse=return_sparse)
        for term in self._terms[1:]:
            marginal_splines = term.build_columns(X, verbose=verbose, return_sparse=return_sparse)
...
class TermList(Core, MetaTermMixin):
    ...
    def build_columns(self, X, term=-1, verbose=False, return_sparse: bool = True):
        """construct the model matrix columns for the term
        ...
        columns = []
        for term_id in term:
            columns.append(self._terms[term_id].build_columns(X, verbose=verbose, return_sparse=return_sparse))
        if return_sparse:
