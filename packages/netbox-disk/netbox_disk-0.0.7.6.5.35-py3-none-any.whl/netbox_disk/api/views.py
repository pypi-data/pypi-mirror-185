...
class PhysicalVolumeViewSet(NetBoxModelViewSet):
    ...
    @action(detail=True, methods=["get"])
    def physical_volume(self, request, pk=None):
        physical_volume = PhysicalVolume.objects.filter(physical_volume__id=pk)
        serializer = PhysicalVolumeSerializer(physical_volume, many=True, context={"request": request})
        return Response(serializer.data)
