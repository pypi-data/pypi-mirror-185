...
class FiniteDifference1D():
    ...
    def iterate_left(self):
        fd_coeffcient = self.finit_coefficient.central()
        fd_coeffcient = {k: v for k, v in fd_coeffcient.items() if k < 0}

        for offset, value in fd_coeffcient.items():
            yield {'offset': offset, 'value': value, 'boundary': self.boundaries['left'], 'type': 1}
    ...
    def iterate_right(self):
        ...
        for offset, value in fd_coeffcient.items():
            yield {'offset': offset, 'value': value, 'boundary': self.boundaries['right'], 'type': 2}

    def iterate_center(self):
        fd_coeffcient = self.finit_coefficient.central()
        fd_coeffcient = {k: v for k, v in fd_coeffcient.items() if k == 0}
    ...
    def _construct_central_triplet_(self):
        ...
        for parameters in self.iterate_left():
            center_diagonals.append(
                (1 / self._dx) * Diagonal1D(size=self.n_x, **parameters).triplet
            )
