...
from ..models import EnrollRecord, Enroll, EnrollSubitemRecord, EnrollRecordExtendField
...
class EnrollRecordServices:
    ...
    @staticmethod
    def appoint(enroll_id, record_id):
        appointed_code = 356  # 报名记录被指派的状态码，PS：当前认为356是报名的状态码
        make_up_difference_code = 242  # 补差价状态码
        rough_draft_code = 124  # 草稿状态码
        try:
            # 获取报名主体，确认传的enroll_id是否存在
            enroll_query_set = Enroll.objects.filter(id=enroll_id).first()
            ...
            if not this_record_obj:
                return None, "找不到该报名记录"
            if this_record_obj.to_json().get("enroll_status_code", appointed_code) == appointed_code:
                return None, "当前记录已经指派过了，请勿重复指派"

            # 判断是否可以报名
            need_count = enroll_query_set.to_json().get("count", 0)  # 需求份数
            enrolled_count = EnrollRecord.objects.filter(enroll_id=enroll_id, enroll_status_code=appointed_code).count()  # 报名成功的数量
            if enrolled_count >= int(need_count):
                return None, "已经指派完成了，无法再次指派"

            # 修改当前的报名记录为被指派状态
            EnrollRecord.objects.filter(id=record_id).update(enroll_status_code=appointed_code)

            # 再次查询指派成功的数量, 判断是否可以进入补差补差价
            enrolled_count = EnrollRecord.objects.filter(enroll_id=enroll_id, enroll_status_code=appointed_code).count()  # 再次查询 报名成功的数量
            if enrolled_count >= need_count:
                EnrollRecord.objects.filter(enroll_id=enroll_id).exclude(enroll_status_code=appointed_code).update(enroll_status_code=rough_draft_code)
                Enroll.objects.filter(id=Enroll).update(enroll_status_code=make_up_difference_code)
            return None, None
        except Exception as e:
            return None, str(e)
