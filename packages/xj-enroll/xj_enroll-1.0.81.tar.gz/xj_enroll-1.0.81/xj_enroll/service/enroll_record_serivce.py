...
class EnrollRecordServices:
    ...
    @staticmethod
    def appoint(enroll_id, record_id, subitem_id, user_id):
        """
        指派报名记录
        :param enroll_id:
        :param record_id:
        ...
        try:
            ...
            if not enroll_subitem_record_obj:
                return None, "该任务已经指派过了，无法在指派给他人。"

            # 判断是否可以报名
            need_count = enroll_query_set.to_json().get("count", 0)  # 需求份数
            enrolled_count = EnrollRecord.objects.filter(enroll_id=enroll_id, enroll_status_code__regex=appointed_start).count()  # 报名成功的数量
            if enrolled_count >= int(need_count):
                return None, "任务已经指派完成了，无法再次指派"
            ...
            enrolled_count = EnrollRecord.objects.filter(enroll_id=enroll_id, enroll_status_code__regex=appointed_start).count()  # 再次查询 报名成功的数量
