...
class GoogleOAuth2LoginHandler(tornado.web.RequestHandler,
    ...
    async def get(self):
        log.write_logs("1.", str(self.current_user), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        log.write_logs("2.", str(self.get_argument('code', False)), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')    
        if not self.get_argument('code', False):
            log.write_logs("3.", str(self.current_user), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        ...
        log.write_logs("4.", str(user), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        self.set_secure_cookie("user", str(user))

        log.write_logs("5.", str(self.current_user), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        self.redirect("/")
    
class Userform(BaseHandler, tornado.web.RequestHandler):
    @tornado.web.authenticated
    async def get(self):
        # c = self.get_secure_cookie("user")
        # log.write_logs("1.", str(c), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        # log.write_logs("2.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        
        # if not self.get_current_user():
        #     self.redirect("/login")
        #     return
        
        log.write_logs("6.", "Authenticated!", "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
