...
class Common:
    ...
    def get_company_info_and_model(psql, jwt_claims, company_hashid=None):
        ...
        if company_hashid is not None:
            ...
            model = psql(
                ...
                os.environ["db_password"],
                os.environ["clients_db"],
                company_info[0]["schema_name"],
                os.environ["db_port"],
                company_info[0]["timezone"],
            )

        return company_info, model

    def local_run(email, pathParameters={}, queryStringParameters={}, body={}):
        ...
        context = data(
            invoked_function_arn="arn:aws:lambda:region:account_id:function:function_name",
            log_group_name="log",
        ...
        return json.dumps(handler(event, context), indent=2)
