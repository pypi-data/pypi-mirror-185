...
Busy is a task, plan, and queue management tool, designed to help us all through our crazy busy days with as little stress as possible. It's is simple, fast, and fun to use. One of the core principles is _monotasking_, which is the idea that we each focus better when we are working on exactly one task at a time. So busy only shows you one task. Also, it's designed to run on your laptop or desktop computer, without needing an internet connection, so it works extremely fast under any conditions. Also, at the core is a traditional BASH-type command line utility, which should work anywhere that Python runs. I use a Mac, but in principle it ought to work just as well on Linux or Windows machines (not yet tested). Enjoy!

The instructions below assume basic familiarity with a terminal program. On the Mac it works with Terminal (included with the OS) or iTerm.
...
The following commands apply to all queues (explained later).
...
There are three commands related to plan management.
...
The task commands listed abover accept item designations. The `defer` and `finish` commands reference the `tasks` queue; the `activate` command references the `plans` queue. The default for `defer` and `finish` is the top item in the `tasks` queue; the default for `activate` is to activate nothing.
...
But it's also possible to designate tasks to be managed. The `manage` command does its best to replace the edited items in place in the list order. So if you `manage` a tag whose items are recently popeed (at the top of the queue), then the edited items will still appear at the top. Even if you add items, they will be inserted after the last item in the managed set, not at the end of the queue. But all the items brought up in the editor will be managed. So if you remove an item in the editor, it will be deleted and the others will be moved up to take its place.

### For faster daily use - the Curses UI

_Under Development_

Busy suports multiple user interfaces. The command line interface described above is the Shell interface. It's the default, though it can also be specified with:

```
busy --ui shell <command...>
```

The alternative is the Curses UI, which draws an entirely new terminal in the same window. As of now, the Curses UI only supports the `tasks` queue, and has a limited set of commands. The commands (`add`, `pop`, `drop`, `defer`, and `finish`) can all be triggered with single keystrokes. The UI always displays the current queue (which is always `tasks` for now) and the current (top) task at the top of the screen. More functionality coming later.
...
Although it requires Python 3.6.5 or higher, Busy is designed to function with the Python standard library without any additional pip modules. But we use Pip packages in the devops pipeline, so:
...
To publish a new build, use 'vernum' with 'major', 'minor', or 'patch' depending on how major the changes were since the last build. Then push. GitLab allows you to publish to PyPI in its UI.