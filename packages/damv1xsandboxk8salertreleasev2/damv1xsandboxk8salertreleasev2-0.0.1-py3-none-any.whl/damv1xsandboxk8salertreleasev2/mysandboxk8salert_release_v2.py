...
def execution_k8s_alert_release_v2( _number, _nameof_msg_rpt, _namespace, _sincelast, _lst_patterns, _lst_target=[], **kwargs):
    ALLOW_PARAM= False
    ...
    idAirtable='';DiffMinSecondsSilentLogger_forUpdate=0;threadNumber, \
    ...
    if ALLOW_PARAM == True:
        ...
        Q.logger(time7.currentTime7(),'      [ Begin k8s_alert_release - {0} ] シミュレーション'.format(_number),_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
        ...
        Q.logger(time7.currentTime7(),'       (1) - Loading Process ( ローディング )',_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
        ...
        if len(DICT_RELEASE_PodC) !=0 and len(_lst_patterns)!= 0:
            ...
            Q.logger(time7.currentTime7(),'       (2) - Start Process ( はじめる )',_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
            try:
                    ...
                    if len(dict_cont)!=0:
                        for cont in dict_cont:
                                ...
                                for pattern in _lst_patterns:
                                    ...
                                    if bAirtableReact == False:
                                        ...
                                        get_bStatus_complete_sshcmd, get_exception_message_handler, outstr = fCls.getLst_log_pod_by_pattern_andTarget_v2(   _sincelast,pod_name, cont_name, _namespace, pattern, _lst_target, \
                                                                                                ...
                                                                                                _argThreadNumber=threadNumber, _argIdAirtable=idAirtable, _argShowCommandkubectl=bShowCommand_for_getLogInPodsContainer)
                                    ...
                                    if mpl.variable_type.list.value in str(type(outstr)):
                                        if len(outstr)!=0:
                                            ...
                                            Q.logger('\n{0}'.format(str(time7.currentTime7())),f'             Detect a ( 見つける ) {_namespace}::{pod_name} | {cont_name}',_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
                                            Q.logger('{0}'.format(str(time7.currentTime7())),f'             --> pattern ( パタン ): "{pattern}"',_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
                                        else:
                                            print('\r',end='')
                            ...
                            if len(lst_line)!=0:
                                ...
                                Q.logger('{0}'.format(str(time7.currentTime7())),f'             ..[ Line ( ライン ) - {str(number)} : Done ( ダン ) ]',_argThreadNumber = threadNumber, _argIdAirtable = idAirtable)
