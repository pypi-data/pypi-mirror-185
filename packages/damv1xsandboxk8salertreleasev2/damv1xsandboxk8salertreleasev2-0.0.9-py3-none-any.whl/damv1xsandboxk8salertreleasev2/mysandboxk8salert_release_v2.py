...
def execution( _number, _nameof_msg_rpt, _namespace, _sincelast, _lst_patterns, _lst_target=[], **kwargs):
    ...
    print("BEFORE KWARGS DEF FUNCTION IN EXECUTE ----> TYPE :", str(type(kwargs.get('_argFunctAirtable_update'))) )

    idAirtable=''; threadNumber=0; DiffMinSecondsSilentLogger_forUpdate=0; funct_pyairtable_update, \
    bparam = mpl.kwargs().getValueAllowed(kwargs, '_argFunctAirtable_update', mpl.variable_type.function.value, None)   
    if bparam==True:
        threadNumber, \
    ...
    print("AFTER KWARGS DEF FUNCTION IN EXECUTE ----> TYPE :", str(type(eval(funct_pyairtable_update))))
