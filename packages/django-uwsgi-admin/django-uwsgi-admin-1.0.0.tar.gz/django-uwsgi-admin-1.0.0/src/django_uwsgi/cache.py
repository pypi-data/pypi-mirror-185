...
try:
    ...
    class UwsgiCache(BaseCache):
        ...
        def get(self, key, default=None, version=None):
            full_key = self.make_key(key, version=version)
            val = self._cache.cache_get(smart_str(full_key), self._server)
            if val is None:
                return default
            val = smart_str(val)
        ...
        def _set(self, full_key, value, timeout):
            ...
            self._cache.cache_update(smart_str(full_key), pickle.dumps(value), uwsgi_timeout, self._server)
        ...
        def delete(self, key, version=None):
            full_key = self.make_key(key, version=version)
            self._cache.cache_del(smart_str(full_key), self._server)
