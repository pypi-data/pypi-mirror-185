...
class ExergenicsApi:
    ...
    ********************************
    ...
    def setJobData(self, jobId, fieldName, fieldValue):
        activityUrl = "{}/{}?fieldName={}&fieldValue={}".format(
            self.ex__setJobData, jobId, fieldName, urllib.parse.quote_plus(fieldValue))
        self.lastStatus, self.lastResponse = self.doRequest(
            self.getExchangeEndpoint(), activityUrl, {})
        if self.noResponse():
            return False
    ...
    def portalChart_bowlChart2D(self, plantCode, chartName, portalCategory, data, title="title",
                                ...
                                legendBg="LightSteelBlue", titleFont="Arial", titleColor="black", equipCode="", jobId=""):
