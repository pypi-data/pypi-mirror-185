...
@_dispatch.add_fallback_dispatch_list
@_dispatch.add_type_based_api_dispatcher
@tf_export('interleave_tensors_to_xla_tuple')
def interleave_tensors_to_xla_tuple(indices, values, name=None):
  r"""TODO: add doc.

  Args:
    indices: A `Tensor`. Must be one of the following types: `int32`, `int64`.
    values: A `Tensor`. Must be one of the following types: `half`, `bfloat16`, `float32`, `int32`, `uint32`, `int64`.
    name: A name for the operation (optional).
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "InterleaveTensorsToXLATuple", name, indices, values)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
      pass
    try:
      _result = _dispatcher_for_interleave_tensors_to_xla_tuple(
          (indices, values, name,), None)
      if _result is not NotImplemented:
        return _result
      return interleave_tensors_to_xla_tuple_eager_fallback(
          indices, values, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            interleave_tensors_to_xla_tuple, (), dict(indices=indices,
                                                      values=values,
                                                      name=name)
          )
      if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "InterleaveTensorsToXLATuple", indices=indices, values=values,
                                       name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          interleave_tensors_to_xla_tuple, (), dict(indices=indices,
  ...
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("indices_type", _op._get_attr_type("indices_type"),
              "values_type", _op._get_attr_type("values_type"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "InterleaveTensorsToXLATuple", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

InterleaveTensorsToXLATuple = tf_export("raw_ops.InterleaveTensorsToXLATuple")(_ops.to_raw_op(interleave_tensors_to_xla_tuple))
_dispatcher_for_interleave_tensors_to_xla_tuple = interleave_tensors_to_xla_tuple._tf_type_based_dispatcher.Dispatch


def interleave_tensors_to_xla_tuple_eager_fallback(indices, values, name, ctx):
  _attr_indices_type, (indices,) = _execute.args_to_matching_eager([indices], ctx, [_dtypes.int32, _dtypes.int64, ], _dtypes.int32)
  _attr_values_type, (values,) = _execute.args_to_matching_eager([values], ctx, [_dtypes.half, _dtypes.bfloat16, _dtypes.float32, _dtypes.int32, _dtypes.uint32, _dtypes.int64, ])
  _inputs_flat = [indices, values]
  _attrs = ("indices_type", _attr_indices_type, "values_type",
  _attr_values_type)
  _result = _execute.execute(b"InterleaveTensorsToXLATuple", 1,
                             inputs=_inputs_flat, attrs=_attrs, ctx=ctx,
                             name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
...
_SplitXLATupleToTensorsOutput = collections.namedtuple(
    "SplitXLATupleToTensors",
    ["indices", "values"])


@_dispatch.add_fallback_dispatch_list
@_dispatch.add_type_based_api_dispatcher
@tf_export('split_xla_tuple_to_tensors')
def split_xla_tuple_to_tensors(input, values_type, output_shape, indices_type=_dtypes.int32, name=None):
  r"""TODO: add doc.

  Args:
    input: A `Tensor` of type `variant`.
    values_type: A `tf.DType` from: `tf.half, tf.bfloat16, tf.float32, tf.int32, tf.uint32, tf.int64`.
    output_shape: A `tf.TensorShape` or list of `ints`.
    indices_type: An optional `tf.DType` from: `tf.int32, tf.int64`. Defaults to `tf.int32`.
    name: A name for the operation (optional).

  Returns:
    A tuple of `Tensor` objects (indices, values).

    indices: A `Tensor` of type `indices_type`.
    values: A `Tensor` of type `values_type`.
  """
  _ctx = _context._context or _context.context()
  tld = _ctx._thread_local_data
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "SplitXLATupleToTensors", name, input, "indices_type",
        indices_type, "values_type", values_type, "output_shape",
        output_shape)
      _result = _SplitXLATupleToTensorsOutput._make(_result)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
      pass
    try:
      _result = _dispatcher_for_split_xla_tuple_to_tensors(
          (input, values_type, output_shape, indices_type, name,), None)
      if _result is not NotImplemented:
        return _result
      return split_xla_tuple_to_tensors_eager_fallback(
          input, indices_type=indices_type, values_type=values_type,
          output_shape=output_shape, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            split_xla_tuple_to_tensors, (), dict(input=input,
    ...
    if _result is not NotImplemented:
      return _result
  # Add nodes to the TensorFlow graph.
  values_type = _execute.make_type(values_type, "values_type")
  output_shape = _execute.make_shape(output_shape, "output_shape")
  if indices_type is None:
    indices_type = _dtypes.int32
  indices_type = _execute.make_type(indices_type, "indices_type")
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "SplitXLATupleToTensors", input=input, values_type=values_type,
                                  output_shape=output_shape,
                                  indices_type=indices_type, name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          split_xla_tuple_to_tensors, (), dict(input=input,
                                               values_type=values_type,
  ...
  if _execute.must_record_gradient():
    _attrs = ("indices_type", _op._get_attr_type("indices_type"),
              "values_type", _op._get_attr_type("values_type"),
              "output_shape", _op.get_attr("output_shape"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "SplitXLATupleToTensors", _inputs_flat, _attrs, _result)
  _result = _SplitXLATupleToTensorsOutput._make(_result)
  return _result

SplitXLATupleToTensors = tf_export("raw_ops.SplitXLATupleToTensors")(_ops.to_raw_op(split_xla_tuple_to_tensors))
_dispatcher_for_split_xla_tuple_to_tensors = split_xla_tuple_to_tensors._tf_type_based_dispatcher.Dispatch


def split_xla_tuple_to_tensors_eager_fallback(input, values_type, output_shape, indices_type, name, ctx):
  values_type = _execute.make_type(values_type, "values_type")
  output_shape = _execute.make_shape(output_shape, "output_shape")
  if indices_type is None:
    indices_type = _dtypes.int32
  ...
  _inputs_flat = [input]
  _attrs = ("indices_type", indices_type, "values_type", values_type,
  "output_shape", output_shape)
  _result = _execute.execute(b"SplitXLATupleToTensors", 2,
                             inputs=_inputs_flat, attrs=_attrs, ctx=ctx,
                             name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
