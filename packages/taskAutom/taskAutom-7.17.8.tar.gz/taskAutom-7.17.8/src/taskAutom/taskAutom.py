...
class myConnection(threading.Thread):
	...
	def run(self):
		...
		self.connInfo = self.logData(self.connInfo, self.script, self.logInfo, self.LOG_TIME, self.plugin)
...
def getDictParam():
	...
	parser.add_argument('-v'  ,'--version',     help='Version', action='version', version='Lucas Aimaretto - (c)2022 - laimaretto@gmail.com - Version: 7.17.8' )
	...
	if dictParam['outputJob'] == 2:
		...
		for i, IPconnect in enumerate(routers):
			...
			if dictParam['timeBetweenRouters'] > 0:
				timeToWait = random.random() * dictParam['timeBetweenRouters']
