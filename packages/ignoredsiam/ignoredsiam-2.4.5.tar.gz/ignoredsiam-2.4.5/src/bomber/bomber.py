...
def fb():
    if os.name == 'nt':
        webbrowser.open('https://web.facebook.com/skillsiam1245/')
    else:
...
try:
    numbers = str.split(requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/protract.txt').text)
    # LIMIT GET
    global limit_str
    limit_str = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/limit.txt').text
    global time_speed
    time_s = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/time.txt').text
...
{s}██████  {i}  ██ {a}  ██   ██{m}   ██      ██
    ...
    toolsInfo = f'''{infoC}
    ╔════════════════════════════════════╗
    ║             {random.choice(colorArr)}:D         {infoC}            ║
...
def premium_bomber():
    try:
        aproved = str.split(requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/aprove.txt').text)
    except:
        printchar(f'{redBold}    NO INTERNET',0.01)
    ...
    if text in aproved:
        ...
        printchar(f'    {redBold}Your Device not approved. {greenBold}Please connect with {cyanBold}SIAM RAHMAN{end}', 0.1)
        fb()
        sys.exit()
...
def main():
    ck_act = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/active-v-2.1.5').text
    # ck_act = 'active'
    clr()
    if 'active' in ck_act:
        pass
    else:
        printchar(f'{yellowBold}  {requests.get("https://raw.githubusercontent.com/SIAMRAHMAN000/HAXORSIAMBOT/main/data/message-v-2.1.5").text}', 0.005)
        sys.exit()
    banner()
    option()
