...
def InsertColumns(D): #(D es la tabla) 
    D=D.astype({'nro_dormitorios': 'uint8','nro_banos':'uint8','nro_estacionamientos':'uint8','latitud':'float64','longitud':'float64','sup_util':'float64',
...
def Selec_Comuna(D1,cve): #(D1 es la tabla, cve: es el codigo de la comuna)
    D_comuna=D1.loc[(D1.loc[:,'cve_comuna']==cve) & (D1.loc[:,'valor_uf']>=1000)]
    return D_comuna
...
def Muestra(df1,df2,cve,ant):
    ...
    MuestraAVM=pd.merge(df2,Muestras[1], how="right", 
                        on=["cve_comuna",'nro_dormitorios','nro_banos','nro_estacionamientos',"longitud","latitud","sup_util","sup_total","valor_uf"])
    MuestraML=MuestraML.astype({'nro_dormitorios':'uint8','nro_banos':'uint8'})
    MuestraAVM=datosduplicados(MuestraAVM,False)
    MuestraAVM=MuestraAVM.astype({'nro_dormitorios':'uint8','nro_banos':'uint8'})
    nombre1='Muestra_OfertaDept_ML'+str(cve)+'.xlsx'
    nombre2='Muestra_OfertaDept_AVM'+str(cve)+'.xlsx'
...
def Muestra_Total(A):
    if A=="ML":
        LR = pd.read_excel('Muestra_OfertaDept_ML11.xlsx');
        LC = pd.read_excel('Muestra_OfertaDept_ML21.xlsx');
        LB = pd.read_excel('Muestra_OfertaDept_ML29.xlsx');
        V = pd.read_excel('Muestra_OfertaDept_ML49.xlsx');
        Muestra = pd.concat([LR,LC,LB,V], ignore_index=True, sort=False)
        Muestra.to_excel('Muestra_Total_OfertaDept_ML.xlsx')
    elif A=="AVM":
        LR = pd.read_excel('Muestra_OfertaDept_AVM11.xlsx');
        LC = pd.read_excel('Muestra_OfertaDept_AVM21.xlsx');
        LB = pd.read_excel('Muestra_OfertaDept_AVM29.xlsx');
        V = pd.read_excel('Muestra_OfertaDept_AVM49.xlsx');
        Muestra = pd.concat([LR,LC,LB,V], ignore_index=True, sort=False)
        Muestra.to_excel('Muestra_Total_OfertaDept_AVM.xlsx')
    return Muestra
...
def Entrenamiento(df2, t_s):
    Xrlinter = np.array(df2[['nro_dormitorios','nro_banos','nro_estacionamientos','longitud','latitud','sup_util','sup_total']])
...
def RForestEleccion(df2,cve,k):
    ...
    scaler1_file = "escalaRF_OfertaDept_"+str(cve)+".save"
    ...
    joblib.dump(rforest1,"RForest_OfertaDept_"+str(cve)+".joblib")
...
def XGB_Eleccion(df2,cve,k):
    ...
    scaler1_file = "escalaXGB_OfertaDept_"+str(cve)+".save"
    ...
    joblib.dump(xgb1,"XGB_OfertaDept_"+str(cve)+".joblib")
...
def Error_ML(reml2):
    ...
    col = ['Intervalo','%_RL_Acumulado','Cantidad_RL', '%_EN_Acumulado','Cantidad_EN',
    ...
    for lim in range(0,len(k)):
        ...
        tablaux = pd.DataFrame([[inter,porcentaje1,cantidad1,porcentaje2,cantidad2,
...
def Tabla_Estadistica_Error2(df):
    ...
    Sobreestimados=[Sobreestimados2,Sobreestimados3,Sobreestimados4,Sobreestimados5]
    Subestimados=[Subestimados2,Subestimados3,Subestimados4,Subestimados5]
...
def ModeloFinal(reml2,scls,models,cve):
    ...
    if k==0:
        ...
        scaler_file = "escala_Final_OfertaDept_"+str(cve)+".save"
        joblib.dump(scls[0], scaler_file)
        joblib.dump(models[0],"Modelo_Final_OfertaDept_"+str(cve)+".joblib")
        ...
        joblib.dump(models[1],"Modelo_Final_OfertaDept_"+str(cve)+".joblib")
        ...
        joblib.dump(models[2],"Modelo_Final_OfertaDept_"+str(cve)+".joblib")
        ...
        joblib.dump(models[3],"Modelo_Final_OfertaDept_"+str(cve)+".joblib")
