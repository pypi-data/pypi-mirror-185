...
class AFPEntitiesParameters(ProcessorParameters):
    ...
    white_label: Optional[str] = Field("white", description="Label name of the white list")
    eurovoc_label: Optional[str] = Field(
        "eurovoc", description="Label name of the Eurovoc thesaurus"
    )
    wikidata_label: Optional[str] = Field("wikidata", description="Label name of Wikidata entities")
        ...
        for document in documents:
            with add_logging_context(docid=document.identifier):
                if document.annotations:
                    mark_whitelisted(document, params.white_label)
                    ...
                    conso_anns = consolidate_and_link(ann_groups, params.kill_label, params.white_label,
                                                      params.wikidata_label, params.eurovoc_label)
...
def mark_whitelisted(document, white_label):
    for a in document.annotations:
        if a.labelName == white_label:  # Consider whitelisted terms as entities coming from the model
            a.terms = None


def consolidate_and_link(ann_groups, kill_label, white_label, wikidata_label, eurovoc_label):
    conso_anns = []
    kb_names = [item for item in ann_groups.keys() if item not in ["", kill_label, white_label, wikidata_label, eurovoc_label]]
