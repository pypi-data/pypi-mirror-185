    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole) -> QVariant:
        ...
        if index.column() == 0 and role in (Qt.DisplayRole, Qt.EditRole):
            ...
            if isinstance(tree_item.element, Account):
                # if name exists...
                if tree_item.element.name is not None:
                    # display account "address name"
                    data = QVariant(
                        f"{tree_item.element.address} - {tree_item.element.name}"
                    )
                else:
                    # display only account address
