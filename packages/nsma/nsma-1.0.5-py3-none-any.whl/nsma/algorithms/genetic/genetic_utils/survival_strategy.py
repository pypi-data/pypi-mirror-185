...
from nsma.algorithms.genetic.genetic_utils.general_utils import non_dominated_sorting, calc_crowding_distance, split_by_feasibility
...
class GeneticSurvivalStrategy:
    ...
    def least_infeasible_survival(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, n_survive: int):
        """
        Use the infeasible solutions survival strategy
    ...
    def ranking_and_crowding_feasible_survival(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, n_survive: int):
        """
        Use the feasible solutions survival strategy
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
    ...
    def get_survivals(self, p_list: np.array, f_list: np.array, constraint_violations: np.array):
        """
        Execute the survival strategy
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
        ...
        Notes:  It divides the solutions in feasible and infeasible ones through the function split_by_feasibility(np.array)
                If there are no feasible solutions, then the function least_infeasible_survival(np.array, np.array, np.array, int) is called to get the survivals among the infeasible points
                Otherwise, the function ranking_and_crowding_feasible_survival(np.array, np.array, np.array, int) is first called to get the survivals among the feasible solutions
                Then, the function least_infeasible_survival(np.array, np.array, np.array, int) is called to get the last survivals among the infeasible points
        ...
        if len(feasible) == 0:
            p_list_inf, f_list_inf, constraint_violations_inf = self.least_infeasible_survival(p_list[infeasible], f_list[infeasible], constraint_violations[infeasible], min(len(infeasible), n_survive))
        else:
            p_list_f, f_list_f, constraint_violations_f, rank_list_f, crowding_list_f = self.ranking_and_crowding_feasible_survival(p_list[feasible], f_list[feasible], constraint_violations[feasible], min(len(feasible), n_survive))

            n_infeasible_survive = n_survive - len(p_list_f)

            if n_infeasible_survive > 0:
                p_list_inf, f_list_inf, constraint_violations_inf = self.least_infeasible_survival(p_list[infeasible], f_list[infeasible], constraint_violations[infeasible], min(len(infeasible), n_infeasible_survive))
