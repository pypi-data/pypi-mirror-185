...
class FeatureService(Fco):
    ...
    def _from_proto(cls, feature_service: FeatureServiceProto, fco_container: FcoContainer):
        ...
        for item in feature_service.feature_set_items:
            if item.HasField("feature_view_id"):
                ...
                if isinstance(fv, specs.OnDemandFeatureViewSpec):
                ...
                elif isinstance(fv, (specs.MaterializedFeatureViewSpec, specs.FeatureTableSpec)):
                    # Feature Tables and Batch/Stream Feature Views cannot have other Feature Views as inputs.
                    inputs = []
                else:
                    TypeError(f"Expected a feature view type. {fv}")
    ...
    def feature_views(self) -> List[Union[FeatureView, FeatureTable]]:
        ...
        for d in self.feature_set_config.feature_definitions:
            if d.is_feature_table:
                fvs.append(FeatureTable(d.feature_view_proto, self._fco_container))
            else:
                fvs.append(get_feature_view_by_type(d.feature_view_proto, self._fco_container))
        return fvs
    ...
    def get_historical_features(
        ...
        validate_spine_dataframe(spine, timestamp_key, list(self._request_context.schema.keys()))
...
def get_feature_service(name: str, workspace_name: Optional[str] = None) -> FeatureService:
    ...
    return FeatureService._from_proto(feature_service_spec.data_proto, fco_container)
