...
class Logger:
    ...
    def __check_logfile(self) -> None:
        ...
        if self.max_logfile_lifetime != -1:
            ...
            for name in names:
                if name != self.log_file and self.today() - date.fromtimestamp(path.getctime(name)) > timedelta(days=self.max_logfile_lifetime):
                    remove(name)
