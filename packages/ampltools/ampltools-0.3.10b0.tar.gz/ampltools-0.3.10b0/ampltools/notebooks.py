...
def ampl_notebook(
    ...
    if license_uuid in (None, "default", "your-license-uuid"):
        if _handle_default_uuid():
            if show_prompt:
                open_modules = ["highs", "cbc", "coin", "open", "plugins", "ampl"]
                show_prompt = len(set(modules) - set(open_modules)) >= 1
            if not show_prompt:
                print(
                    "Using default Community Edition License (get yours at https://ampl.com/ce)"
                )
