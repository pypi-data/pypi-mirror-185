...
def jsonToDataFrame(jsonString):
    ...
    # Is a json Report
    isJsonReport = '"variant"' in jsonString or '/bfi/input/reports' in jsonString or jsonString == '{}'
    if isJsonValue:
...
def jsonReportsToDataFrame(jsonString):
    try:
        _jsonReportsToDataFrame(jsonString)
    except:
        dataFrameData = []
        author = 'Pacifico'
        document = 'Error'
...
def jsonValuesToDataFrame(jsonString):
    try:
        _jsonValuesToDataFrame(jsonString)
    except:
        dataFrameData = []
        scenario = 'Pacifico'
