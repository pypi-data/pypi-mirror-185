...
setup(
    ...
    extras_require={
        'build': ['wheel', 'twine'],
        'flask':
        [  # TODO Needs attention if we're going to keep this functionality
        ...
        [  # TODO Needs attention if we're going to keep this functionality
            'celery[redis]>=5.1.2',
            'celery-dyrygent==0.8.0',  # Library to address some celery issues
            'networkx>=2.4',
            'redis>=3.3.11',
            'rhodb[redis]>=5.1.1',
            'sermos-tools>=0.3.0'
        ],
        'generators': ['boto3>=1.11'],
        'dev':
        ['honcho>=1.0.1', 'awscli>=1.11', 'pylint>=2.5.3', 'pip-licenses'],
        'docs': ['sphinx>=3.0.2', 'boto3>=1.11', 'celery==5.1.2'],
        'test': [
            'pytest-cov>=2.6.1,<3', 'tox>=3.14.1,<4', 'mock>=1,<2',
            'moto>=1.3.16,<2', 'responses>=0.10.16,<0.11', 'fakeredis>=1.0.5',
