...
try:
    ...
    secho("no ghidra module found...not in Ghidrathon??")
...
def setglobs():
    ...
    if conf.config.Ghidra.manager == "program":
        ...
        if conf.VERBOSE:
            secho("working with data type manager %s" % dtm, fg="blue")
        ...
        dtm = StandAloneDataTypeManager(conf.config.Ghidra.category)
        if conf.VERBOSE:
            secho(
                "working with standalone data type manager", fg="blue"
            )
