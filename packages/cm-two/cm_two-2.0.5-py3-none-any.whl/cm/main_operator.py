...
class Operator:
    ...
    def __init__(self, root, settings, scale, can, deffaultScreenSize,
        ...
        if self.settings.mirrored:
            self.auto_exit_x, self.auto_exit_y = 1615, 837
            self.gross_x, self.gross_y = 417, 837
        else:
            self.auto_exit_x, self.auto_exit_y = 417, 822
            self.gross_x, self.gross_y = 1615, 822
    ...
    def create_video_streams(self):
        for cam in self.cameras_info:
            if cam['type'] == 'main' and cam['enable']:
            ...
            if cam['type'] == 'cad_gross' and cam['enable']:
                cam_ip = self.get_cam_ip(cam)
                port = self.get_cam_port(cam)
                inst = start_video_stream(self.root, self.terminal.can,
                ...
                cam['video_worker_inst'] = inst
            if cam['type'] == 'auto_exit' and cam['enable']:
                cam_ip = self.get_cam_ip(cam)
                port = self.get_cam_port(cam)
                inst = start_video_stream(self.root, self.terminal.can,
    ...
    def drawStatus(self):
        ...
        if self.current == 'MainPage' and self.mainPage.orupState == False:
            ...
            notes = f"Гос. номер: {self.road_anim_info['carnum']}\n" \
                    f"Cтатус: {self.road_anim_info['status']}\n" \
                    f"Примечания: {self.road_anim_info['notes']}\n"
