...
## :Copyright: © 2008–2010, 2013, 2014, 2016, 2018–2023 Lele Gaifax
##
...
<table class="ui compact unstackable definition table">
  <tbody>
    ...
    % if entity.currentturn:
      <tr>
        ...
        <td>
          ...
          % for i in range(1,entity.currentturn+1):
            ...
            <a class="ui ${' disabled black' if turn==i else ''} button" href="${href|n}">
              ${i}
            </a>
          % endfor
          </div>
      ...
      % if turn is not None and entity.rankedturn:
        <tr>
          <td class="right aligned">${_('Ranked round')}</td>
          <td>
            <a class="ui mini compact button"
               href="${request.route_path('lit_tourney', guid=entity.guid) | n}">
              ${entity.rankedturn}
            </a>
          </td>
        </tr>
...
<%def name="training_boards_header()">
  <thead>
    <tr>
      <th class="center aligned competitor1${match.score1>match.score2 and ' winner' or ''}"
          width="45%">
        <%
      ...
      % if match.idcompetitor2:
        ...
        players = [ctor.player1, ctor.player2, ctor.player3, ctor.player4]
        %>
        <th class="center aligned competitor2${match.score1<match.score2 and ' winner' or ''}"
            width="45%">
          ${njoin(players, stringify=lambda p: '<a href="%s" title="%s">%s</a>' % (
            request.route_path('lit_tourney', guid=match.tourney.guid, _query=dict(player=p.guid)),
            escape(_('Show matches played by %s') % p.caption(html=False)),
...
<%def name="training_boards_body()">
  <tbody>
    % for i, row in enumerate(match.boards, 1):
      ${training_boards_row(i, row)}
    % endfor
    % if match.score1 is not None and match.score2 is not None:
    <tr>
      <td class="center aligned${match.score1>match.score2 and ' winner' or ''}">
        ${match.score2}
      </td>
      <td></td>
      <td class="center aligned${match.score1<match.score2 and ' winner' or ''}">
...
<%def name="training_boards_row(rank, row)">
  <tr>
    <td class="center aligned${row.score1>row.score2 and ' winner' or ''}">
      ${row.score1}
    </td>
...
<%def name="boards_header()">
  ...
  <thead>
    ...
    <tr>
      ...
      <th class="center aligned round-number-header" colspan="2">#</th>
      <th class="center aligned scores-header">${_('Queen')}</th>
      <th class="center aligned scores-header">${_('Coins')}</th>
      <th class="center aligned scores-header">${_('Score')}</th>
    </tr>
...
<%def name="boards_body()">
  ...
  <tfoot>
    ...
    <tr>
      <td colspan="3" class="center aligned${match.score1>match.score2 and ' winner' or ''}">
        ${match.score1}
      </td>
      <td colspan="2"></td>
      <td colspan="3" class="center aligned${match.score1<match.score2 and ' winner' or ''}">
    ...
    <tr class="partial-score">
      <td class="right aligned ${match.partial_score1>match.partial_score2 and ' winner' or ''}">
...
<%def name="boards_row(rank, row)">
  <tr>
    <td class="right aligned${row.score1>row.score2 and ' winner' or ''}">
      ${row.score1}
    </td>
    ...
    <td class="center aligned round-number" colspan="2">${rank}</td>
    <td class="center aligned">
      ${'✔' if row.queen == '2' else ''}
    </td>
...
<table class="ui compact unstackable table boards">
  <caption>
    <a href="${request.route_path('lit_tourney', guid=entity.guid, _query=dict(turn=match.turn))}">
      ${_('Round $round', mapping=dict(round=match.turn))}
    </a>, ${_('carromboard $num', mapping=dict(num=match.board))}
    <%
