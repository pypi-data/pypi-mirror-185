...
class CreateWorkingCopyFromRevision(flow.Action):
    ...
    def get_buttons(self):
        ...
        if self._file.has_working_copy(from_current_user=True):
            msg += (
                "<font color=#D66700>WARNING: You already have "
...
class OpenRevision(GenericRunAction):
    ...
    def needs_dialog(self):
        ...
        return not available or not exists
    ...
    def run(self, button):
        ...
        if default_tasks.has_mapped_name(self._task.name()):
            ...
            if dt.files.edits.has_mapped_name(self._file.name()):
                df = df.files.edits[self._file.name()]
                print('Default File found')
                if df.is_primary_file.get():
                    df.is_primary_file.set(False)
                    print('Status removed')
                else:
                    df.is_primary_file.set(True)
                    print('Status added')
            else:
                print('Not a default file')
                primary_files = dt.primary_files.get()
                if self._file.display_name.get() in primary_files:
                    primary_files.remove(self._file.display_name.get())
                    print('Status removed')
                else:
                    primary_files.append(self._file.display_name.get())
                    print('Status added')
                dt.primary_files.set(primary_files)
