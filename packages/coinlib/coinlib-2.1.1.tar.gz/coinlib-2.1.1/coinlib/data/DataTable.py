...
class DataTable():
    ...
    def getColumn(self, name, index=None, limit=None, with_dates=False,
        ...
        if clear_nan and targetColumn is not None:
            #i = np.argwhere(pd.isna(data))
            idx = 0
            for e in targetColumn:
                if ((type(e) == int or type(e) == float) and e is not None and not np.isnan(e)) \
                        or ((type(e) != int and type(e) != float) and e is not None):
                    break
                else:
                    idx = idx + 1
            targetColumn = targetColumn[idx:]
            indexColumn = indexColumn[idx:]
            #datetimes = np.delete(datetimes, i)
