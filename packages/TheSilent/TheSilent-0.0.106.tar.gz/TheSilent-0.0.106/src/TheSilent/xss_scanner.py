...
def xss_scanner(url, secure = True, tor = False, my_file = " ",):
    ...
    #malicious script
    mal_payloads = ['<scipt>alert("TheSilent")</script>']

    my_random = random.randint(0, 1000000000)
    md5 = hashlib.md5(my_random.encode("utf8")).hexdigest()
    mal_payloads.append("toString(" + str(md5) + ")")

    my_random = random.randint(0, 1000000000)
    md5_1 = hashlib.md5(my_random.encode("utf8")).hexdigest()
    md5_2 = hashlib.md5(my_random.encode("utf8")).hexdigest()
