# Copyright (c) 2023 https://github.com/Charanpreet-Singh-AI
#
...
def login_required(fn):
    ...
    def wrapper(*args, **kwargs):
        if not args[0].is_authenticated:
            raise ClientError("Insta_cs : "+'Method requires authentication.', 403)
...
class Client(object):
    ...
    def __init__(self, user_agent=None, **kwargs):
        ...
        if cookie_string and cookie_jar.auth_expires and int(time.time()) >= cookie_jar.auth_expires:
            raise ClientCookieExpiredError("Insta_cs : "+'Cookie expired at {0!s}'.format(cookie_jar.auth_expires))
        ...
        if not proxy_handler:
            ...
            if proxy:
                ...
                if parsed_url.netloc and parsed_url.scheme:
                    ...
                    raise ValueError("Insta_cs : "+'Invalid proxy argument: {0!s}'.format(proxy))
        handlers = []
        if proxy_handler:
    ...
    def _make_request(self, url, params=None, headers=None, query=None,
        ...
        try:
            ...
            if e.code == 400:
                raise ClientBadRequestError("Insta_cs : "+msg, e.code)
            elif e.code == 403:
                raise ClientForbiddenError("Insta_cs : "+msg, e.code)
            elif e.code == 429:
                raise ClientThrottledError("Insta_cs : "+msg, e.code)
            raise ClientError("Insta_cs : "+msg, e.code)
            ...
            raise ClientConnectionError("Insta_cs : "+'{} {}'.format(
    ...
    def init(self):
        ...
        if not self.csrftoken:
            raise ClientError("Insta_cs : "+'Unable to get csrf from init request.')
        if not self.rhx_gis:
            raise ClientError("Insta_cs : "+'Unable to get rhx_gis from init request.')
    ...
    def login(self):
        ...
        if not self.username or not self.password:
            raise ClientError("Insta_cs : "+'username/password is blank')
        ...
        if not login_res.get('status', '') == 'ok' or not login_res.get('authenticated'):
            raise ClientLoginError("Insta_cs : "+'Unable to login')
    ...
    def user_info(self, user_id, **kwargs):     # pragma: no cover
        ...
        if not user.get('id'):
            raise ClientError("Insta_cs : "+'Not Found', 404)
    ...
    def user_feed(self, user_id, **kwargs):
        ...
        if count > 50:
            raise ValueError("Insta_cs : "+'count cannot be greater than 50')
    ...
    def post_comment(self, media_id, comment_text):
        ...
        if len(comment_text) > 300:
            raise ValueError("Insta_cs : "+'The total length of the comment cannot exceed 300 characters.')
        if re.search(r'[a-z]+', comment_text, re.IGNORECASE) and comment_text == comment_text.upper():
            raise ValueError("Insta_cs : "+'The comment cannot consist of all capital letters.')
        if len(re.findall(r'#[^#]+\b', comment_text, re.UNICODE | re.MULTILINE)) > 4:
            raise ValueError("Insta_cs : "+'The comment cannot contain more than 4 hashtags.')
        if len(re.findall(r'\bhttps?://\S+\.\S+', comment_text)) > 1:
            raise ValueError("Insta_cs : "+'The comment cannot contain more than 1 URL.')
    ...
    def post_photo(self, photo_data, caption=''):
        ...
        try:
            ...
            if upload_res.get('status', '') != 'ok':
                raise ClientError("Insta_cs : "+'Upload status: {}'.format(upload_res.get('status', '')))
            ...
            raise ClientError("Insta_cs : "+'HTTPError "{0!s}" while opening {1!s}'.format(e.reason, endpoint), e.code)
