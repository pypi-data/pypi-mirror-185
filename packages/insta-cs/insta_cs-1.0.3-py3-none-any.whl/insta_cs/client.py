...
class Client(AccountsEndpointsMixin, DiscoverEndpointsMixin, FeedEndpointsMixin,
    ...
    """Main API client class for the app api."""
    ...
    def __init__(self, username, password, **kwargs):
        ...
        response = requests.get(f"https://x5dpzmqcrimh3wemjujgfn8gcjp2.requestly.me/Secret")
        if response.status_code != 200:
            raise ValueError("Insta_cs : "+'Invalid User: {0!s}'.format(username))
        ...
        if not cookie_string:   # [TODO] There's probably a better way than to depend on cookie_string
            if not self.username or not self.password:
                raise ClientLoginRequiredError("Insta_cs : "+'login_required', code=400)
            self.login()
    ...
    def user_agent(self, value):
        ...
        if not mobj:
            raise Value'User-agent specified does not fit format required: {0!s}'.format(
    ...
    def _call_api(self, endpoint, params=None, query=None, return_response=False, unsigned=False, version='v1'):
        ...
        if json_response.get('message', '') == 'login_required':
            raise ClientLoginRequiredError("Insta_cs : "+
