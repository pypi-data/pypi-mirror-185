...
class HighlightsEndpointsMixin(object):
    ...
    def highlight_create(
        ...
        if not (media_ids and isinstance(media_ids, list)):
            raise ValueError("Insta_cs : "+'media_ids must be a non-empty list')
        ...
        if len(title) > 16:
            raise ValueError("Insta_cs : "+'title must not exceed 16 characters')
    ...
    def highlight_edit(
        ...
        if not added_media_ids:
            ...
            raise ValueError("Insta_cs : "+'added_media_ids must be a list')

        if not removed_media_ids:
            removed_media_ids = []
        elif not isinstance(removed_media_ids, list):
            raise ValueError("Insta_cs : "+'removed_media_ids must be a list')

        if title and len(title) > 16:
            raise ValueError("Insta_cs : "+'title must not exceed 16 characters')

        if not (added_media_ids or removed_media_ids or cover_media_id or title):
            raise ValueError("Insta_cs : "+'No edited values')
