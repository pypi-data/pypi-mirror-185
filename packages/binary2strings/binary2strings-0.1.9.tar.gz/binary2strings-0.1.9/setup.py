#! /usr/bin/0env python3
# Templated from https://github.com/pybind/python_example
import sys
import os
from glob import glob

from pybind11 import get_cmake_dir, get_include
from pybind11.setup_helpers import Pybind11Extension, build_ext
from setuptools import setup, Extension, find_packages

__version__ = "0.1.9"

ext_modules = [
    Pybind11Extension("binary2strings",
        sorted(glob("src/*.cpp")),  # Sort source files for reproducibility
        define_macros = [('VERSION_INFO', __version__)],
        include_dirs = ["src"],
        language='c++',
        ),
]

# Note to self to build and upload skip existing:
#   python setup.py sdist bdist_wheel
#   WORKAROUND NOTE: Manually copy .hpp files into the /src folder of sdist if building on windows.
#   python setup.py sdist bdist_wheel & pip install ./dist/binary2strings-0.1.8.tar.gz
#   twine upload dist/* --skip-existing
with open("README.md", "r") as fh:
    long_description = fh.read()
setup(
    name='binary2strings',
    version=__version__,
    author='Geoff McDonald',
    author_email='glmcdona@gmail.com',
    url='https://github.com/glmcdona/binary2strings',
    license='MIT',
    description='Fast string extraction from binary buffers.',
    long_description=long_description,
    long_description_content_type="text/markdown",
    ext_modules=ext_modules,
    packages=["src"],
    include_package_data=True,
    cmdclass={"build_ext": build_ext},
    test_suite='tests',
    zip_safe=False,
    python_requires=">=3.7",
)