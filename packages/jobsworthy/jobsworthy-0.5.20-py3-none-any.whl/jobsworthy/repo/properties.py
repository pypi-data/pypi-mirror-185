...
class DbPropertyManager(PropertyManager):
    ...
    def to_all_properties(self) -> List[DbProperty]:
        return ([DbProperty(key=prop.info_name, value=prop.info_value, convert_to_urn=False) for prop in
                 self.standard_properties()]
                + self.to_properties())

    def standard_properties(self) -> List[dataframe.Row]:
        return (self.properties_from_db().filter(F.col('info_name') != "Properties")
                .collect())

    def parse_custom_properties(self, row: dataframe.Row) -> List[DbProperty]:
        return fn.remove_none(
            [self.parse_custom_property(prop) for prop in self.split_into_key_pairs(row.info_value[1:-1])])
    ...
    def custom_properties(self) -> dataframe.Row:
        ...
        :return:
        """
        return self.properties_from_db().filter(F.col('info_name') == "Properties").collect()[0]
