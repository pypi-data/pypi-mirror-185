    ...
    def pack(self, objet_name: [str, list[str]], **kwargs):
        ...
        if "Y" in expand.upper():
            if pady is None:
                pady = 2
            for o in objet_name:
                self.objet[o].height = round((self.height- decy-(len(objet_name)+1)*pady)/len(objet_name))
        elif pady is None:
        ...
        if "X" in expand.upper():
            for o in objet_name:
                self.objet[o].width = self.width - padx*2
...
class AskYesNo(MessageBox):
    def __init__(self, parent, message="", title="Question"):
        super().__init__(parent, title, message, ["Oui", "Non"], loadImage(processing.get_path() + "question.png"))
