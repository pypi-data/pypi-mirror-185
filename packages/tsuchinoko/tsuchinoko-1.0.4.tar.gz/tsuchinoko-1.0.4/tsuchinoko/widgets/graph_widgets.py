    ...
    def fileSaveDialog(self, filter=None, opts=None):
        ...
        if filter is not None:
            if isinstance(filter, str):
                ...
                self.file_dialog.setNameFilters(filter)
        global last_export_directory
        exportDir = last_export_directory
        if exportDir is not None:
            self.file_dialog.setDirectory(exportDir)
        self.file_dialog.show()
        self.file_dialog.opts = opts
        self.file_dialog.fileSelected.connect(self.files_save_finished)
        return

    def files_save_finished(self, filename):
        global last_export_directory
        last_export_directory = os.path.split(filename)[0]

        ## If file name does not match selected extension, append it now
        ext = os.path.splitext(filename)[1].lower().lstrip('.')
        selectedExt = re.search(r'\*\.(\w+)\b', self.file_dialog.selectedNameFilter())
        if selectedExt is not None:
            selectedExt = selectedExt.groups()[0].lower()
            if ext != selectedExt:
                filename = filename + '.' + selectedExt.lstrip('.')

        self.export_video(fileName=filename, **self.file_dialog.opts)

    def export_video(self, fileName=None, toBytes=False, copy=False, fps=10):
        if fileName is None and not toBytes and not copy:
            filter = ['*.mp4']
            self.fileSaveDialog(filter=filter)
            return

        exporter = ImageExporter(self.cloud.scene())
        tempdir = Path(tempfile.gettempdir())
        start_frame = self.timeline.getXPos()

        for i in range(self.nframes()):
            self.set_current_index(i)
            exporter.export(str(tempdir / Path(f"export_{i}.png")))

        os.system(f"ffmpeg -r {fps} -i {str(tempdir)}/export_%d.png -vcodec mpeg4 -y {fileName}")  # Now ffmpeg and ffprobe will use static_ffmpeg versions
