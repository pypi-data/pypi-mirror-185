...
class ConfDB(IConf):
    ...
    def _set_item(self, id_, value, update_sys_value: bool):
        if isinstance(value, ConfItem):
            ...
            value_type = self._check_value_type(sys_value, user_value)
        ...
        if update_sys_value:
            ...
            value_type = self._check_value_type(self.get(id_, value_only=False).sys_value, value)
    ...
    def _add_item(self, id_, value, update_sys_value):
        if isinstance(value, ConfItem):
            ...
            user_value = value.raw_user_value
            item = value
            item.value_type = self._check_value_type(sys_value, user_value)
    ...
    @classmethod
    def _check_value_type(cls, sys_val, user_val):
        ...
        raise Exception('Invalid value type %r, type %r required.' % (user_type, sys_type))
