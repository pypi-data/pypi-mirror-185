...
class Yolox:
    ...
    def __init__(self, classes_path: str = 'voc_classes.txt',
                 VOCdevkit_path: str = '.', fonts: str = ".", phi: str = "l",
                 input_shape: list = [640, 640]) -> None:
    ...
    def train(self, cuda: bool = True, distributed: bool = False,
        ...
        #   创建yolo模型
        #------------------------------------------------------#
        model = YoloBody(num_classes, self.phi)
        weights_init(model)
        if model_path != '':
            #------------------------------------------------------#
            #   权值文件请看README，百度网盘下载
        ...
        if local_rank == 0:
            show_config(
                classes_path=self.classes_path, model_path=model_path, input_shape=self.input_shape,
        ...
        if True:
            ...
            #   构建数据集加载器。
            #---------------------------------------#
            train_dataset = YoloDataset(train_lines, self.input_shape, num_classes, epoch_length=UnFreeze_Epoch,
                                        mosaic=mosaic, mixup=mixup, mosaic_prob=mosaic_prob, mixup_prob=mixup_prob, train=True, special_aug_ratio=special_aug_ratio)
            val_dataset = YoloDataset(val_lines, self.input_shape, num_classes, epoch_length=UnFreeze_Epoch,
            ...
            #----------------------#
            if local_rank == 0:
                eval_callback = EvalCallback(model, self.input_shape, class_names, num_classes, val_lines, log_dir, cuda,
    ...
    def predict(self, cuda: bool = True, model_path: str = "", mode: str = "predict",
        ...
        yolo = YOLO(model_path=model_path, classes_path=self.classes_path,
                    input_shape=self.input_shape, phi=self.phi, fonts=self.fonts,
                    confidence=confidence, nms_iou=nms_iou, letterbox_image=letterbox_image, fonts=self.fonts, cuda=cuda)
    ...
    def compute_map(self, model_path: str = '', cuda: bool = True, map_mode: int = 0, min_overlap: float = 0.5, confidence: float = 0.001,
        ...
        if map_mode == 0 or map_mode == 1:
            print("Load model.")
            yolo = YOLO(model_path=model_path, classes_path=self.classes_path,
                        input_shape=self.input_shape, phi=self.phi,
    ...
    def summary(self) -> None:
        ...
        m       = YoloBody(self.num_classes, self.phi).to(device)
        for i in m.children():
            print(i)
            print('==============================')
