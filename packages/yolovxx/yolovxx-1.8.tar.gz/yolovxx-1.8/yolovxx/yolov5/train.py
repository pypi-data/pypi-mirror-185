    ...
    def train(self, cuda: bool = True, distributed: bool = False,
        ...
        if model_path != '':
            ...
            if local_rank == 0:
                ...
                print(
                    "\n\033[1;33;44m温馨提示，head部分没有载入是正常现象，Backbone部分没有载入是错误的。As a reminder, if the head part of the model is not loaded, thats normal given that you probably using a pretrained model that was trained on a dataset with different number of classes. But if the backbone (or some part of it) of your model is not loaded, thats an error and you should make sure you are usinmg the right pretrained model and the right parameter phi (which can either be 'nano' for nano yolov5, 's' for small yolov5, 'm' for medium yolov5, 'l' for large yolo, and 'x' for extra large yolo).\033[0m")

        #----------------------#
        #   获得损失函数
        #----------------------#
        yolo_loss = YOLOLoss(anchors=anchors, num_classes=num_classes,
                             input_shape=self.input_shape, anchors_mask=anchors_mask,
                             cuda=cuda, label_smoothing=label_smoothing)
        #----------------------#
        #   记录Loss
        #----------------------#
        if local_rank == 0:
