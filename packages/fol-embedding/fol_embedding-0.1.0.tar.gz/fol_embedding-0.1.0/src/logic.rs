    ...
    match (term1, term2) {
        (Term::Variable(x), Term::Variable(y)) => {
            if x == y {
                ...
                Some((vec![(x.clone(), Term::Variable(y.clone()))].into_iter().collect(), HashMap::new()))
            }
        }
        (Term::Variable(x), t) => {
                Some((vec![(x.clone(), t.clone())].into_iter().collect(), HashMap::new()))
        }
        (t, Term::Variable(x)) => {
                Some((HashMap::new(), vec![(x.clone(), t.clone())].into_iter().collect()))
        }
...
impl Environment {
    ...
    fn read_equation(&mut self, eq: &str) -> Result<Equation> {
        ...
        let lhs = self.read_term(tokens.next().unwrap(), &mut variables)?;
        let rhs = self.read_term(tokens.next().unwrap(), &mut variables)?;
        Ok(Equation { lhs, rhs })
    }

    fn show_equation(&self, eq: &Equation) -> String {
        format!("(= {} {})", self.show_term(&eq.lhs), self.show_term(&eq.rhs))
    }
    ...
    fn read_sequent(&mut self, s: &str) -> Result<Sequent> {
        ...
        let lhs = self.read_equations(tokens.next().unwrap())?;
        let rhs = self.read_equations(tokens.next().unwrap())?;
        Ok(Sequent { lhs, rhs })
    }
        ...
        for (name, id) in self.context.functionid.iter() {
            s.push_str(&format!(
                ...
                self.context
                    ...
                    .collect::<Vec<String>>()
                    .join(" "),
                self.context.sortname.get(self.context.resultsort.get(id).unwrap()).unwrap()
            ));
        }
