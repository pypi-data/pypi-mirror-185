...
import low_code_assistant.assistant.notebook
...
SOLARA_APP=low_code_assistant.app_entrypoint SOLARA_TELEMETRY_MIXPANEL_ENABLE=False SOLARA_THEME_VARIANT={theme_variant}\\
    SOLARA_THEME_LOADER={theme_loader} uvicorn solara.server.starlette:app\\
    --loop asyncio\\
    --host=0.0.0.0 --port=8888"""


logger = logging.getLogger("lca.deploy")
...
APP_END_STATES = (AppStatus.NON_EXISTENT, AppStatus.STOPPED, AppStatus.FAILED, AppStatus.RUNNING)
...
def deploy(notebook_saved: threading.Event, filesytem_synced: threading.Event, notebook_browser_path: str, theme_variant="light"):
    ...
    with step("Validating app"):
        log(f"Running python -m low_code_assistant.app_entrypoint with working directory: {settings.domino_working_dir}")
        proc = validate_app()
    ...
    with step("Syncing files"):
        ...
        get_domino_api().sync("Auto sync from Low Code Assistant for deployment")
    ...
    with step("Getting initial app state"):
    ...
    if status == AppStatus.RUNNING:
        with step("Stopping previous app"):
    ...
    with step("Starting new app... (this may take a while)"):
...
def Advanced():
    ...
    def get_logs() -> Iterator[List[str]]:
        while True:
    ...
    with v.Tabs() as main:
        v.Tab(children=["LCA deploy logs"])
        ...
        with v.TabItem():
            if server_log_result.error:
                ...
                server_log = "\n".join(server_log_result.value or [""])
                ...
                solara.Preformatted(server_log, attributes={"class": "lca-server-logs"}).meta(ref="server_logs")
...
def Deployer(app: Callable[[], reacton.core.Element]):
    ...
    low_code_assistant.assistant.notebook.notebook_browser_path.use()
    deploy_step.use()
    ...
    def run_deploy():
        if deploy_request:
            return deploy(
                ...
                notebook_browser_path=low_code_assistant.assistant.notebook.notebook_browser_path.value,
            )
    ...
    disable_view = deploy_request and not online
    with v.Sheet() as main:
        with v.Dialog(v_model=open_preview, on_v_model=set_open_preview):
        ...
        with sol.Div(class_="d-flex justify-center"):
            ...
            redeploy_possible = deployed.error or (deployed.state == solara.ResultState.FINISHED and deploy_request)
            loading = not redeploy_possible and ((app_status not in APP_END_STATES) or deploy_request or not filesystem_watcher_connected)
            solara.Button(
                "Deploy app" if not redeploy_possible else "Re-Deploy app",
            ...
            with v.Card():
                with v.CardText(class_="pt-2"):
                    if open_advanced:
                        Advanced()
                with v.CardActions():
                    sol.Button("Close", icon_name="mdi-close", on_click=lambda: set_open_advanced(False))
        ...
        if app_state.state == solara.ResultState.ERROR:
            ...
            if deployed.state == solara.ResultState.ERROR:
                ...
                if app_status == AppStatus.RUNNING and deployed.state == solara.ResultState.FINISHED and filesystem_watcher_connected:
                    ...
                    if not deploy_request and not filesystem_watcher_connected:
                        info_message = "Waiting for file system watcher..."

        if deployed.value and deployed.state == solara.ResultState.FINISHED:
            if online:
                fresh_deploy = False
        ...
        if current_env.value != default_env.value and deployed.value is False and deployed.state == solara.ResultState.FINISHED:
            with solara.Warning(
                ...
                solara.Markdown(f"[Changing your default environment to {current_env.value!r} in your settings may solve this problem.]({settings_urls})")
        ...
        if deployed.value is not False:
            ...
            FileSystemWatcher(
                server_path=server_path, on_connected=set_filesystem_watcher_connected, on_message=on_message, dev=bool(settings.low_code_assistant_dev)
        ...
        with v.Dialog(v_model=show_app_deployed_dialog, on_v_model=lambda v: set_show_app_deployed_dialog(v), width="400px").meta(ref="popup").key("popup"):
            with v.Card():
                with v.CardText(class_="pt-2"):
                    sol.Info(sol.HTML(unsafe_innerHTML="<h2>Your app is deployed! 🎉<h2>"), icon=False)
                ...
                with v.CardActions():
                    ...
                    sol.Button("Close", icon_name="mdi-close", on_click=lambda: set_show_app_deployed_dialog(False))
