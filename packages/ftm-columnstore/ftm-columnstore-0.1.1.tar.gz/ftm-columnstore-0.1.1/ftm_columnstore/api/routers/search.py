...
from ftm_columnstore.search import get_entity_match_query
...
from ..search.nested import serialize_entity
from ..search.queries import FilterDict, entities_query, parse_sorts
...
from .util import PATH_DATASET, get_dataset, get_datasets, get_ftm_filters
...
async def search(
    ...
    target: bool | None = Query(None, title="Include only targeted entities"),
    ...
    """
    if simple:
        raise HTTPException(status_code=400, detail="`simple` not implemented")
    if offset > 0:
        raise HTTPException(status_code=400, detail="`offset` not implemented")
    ...
    * **Company**: ``name``, ``jurisdiction``, ``registrationNumber``, ``address``,
    ...
    for name, example in match.queries.items():
        try:
            ...
            query = get_entity_match_query(ds, entity)[: limit * 3]
    ...
    for (name, entity), resp in zip(entities, queries):
