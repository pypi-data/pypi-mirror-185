...
from yente.util import limit_window, match_prefix

from ftm_columnstore.search import get_entity_match_query

from ..routers.util import PATH_DATASET, QUERY_PREFIX, get_datasets
...
async def reconcile(
    ...
    return FreebaseManifest(
        ...
        view={"url": (DOMAIN + "/entities/{{id}}/")},
        preview=FreebaseManifestPreview(
            url=DOMAIN + "/entities/preview/{{id}}/",
            width=430,
            height=600,
        ),
...
async def reconcile_query(
    ...
    query = get_entity_match_query(dataset, proxy)
...
async def reconcile_suggest_entity(
    ...
    ds = get_datasets(dataset)
    results = []
    query = ds.EQ.where(prop_type="name", value__ilike=f"{prefix}%")
    limit, offset = limit_window(limit, 0, settings.MATCH_PAGE)
    resp = await search_entities(ds, query, prefix, limit=limit, offset=offset)
