...
async def diff_command(filename, fmt, client, no_color=False):
    ...
    for resource in remote_datasources + remote_pipes:
        ...
        if name:
            ...
            diff_lines = diff_files(file, target, with_format=fmt, with_color=(not no_color))
            sys.stdout.writelines(diff_lines)
            click.echo('  ')


def diff_files(from_file, to_file, from_file_suffix='[local]', to_file_suffix='[remote]', with_format=False, with_color=False):
    def file_lines(filename):
        with open(filename) as file:
            return file.readlines()

    def parse(filename, with_format=False):
