...
LIMIT_NUMBER_OF_IDS_PUBCHEM = 500
...
class Downloader:
    ...
    def download(self, limit=None, result_path=None):
        ...
        chunksize = LIMIT_NUMBER_OF_IDS_PUBCHEM
        max_num_futures = 100
        ...
        return _parse_and_compress_from_temp(download_folder, result_path)


def _parse_and_compress_from_temp(folder, target_file):
    if target_file is None:
        target_file = os.path.join(folder, "pubchem.zip")

    print("start compress files at", folder)

    with zipfile.ZipFile(
        target_file, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9
    ) as zipfile_handle:
        for p in glob.glob(os.path.join(folder, "pubchem*.json")):
            with open(p, "r") as fh_in:
                data = fh_in.read()
            with zipfile_handle.open(os.path.basename(p), "w") as fh_out:
                fh_out.write(data.encode("ascii"))
    print("compressed files into", target_file)
    return target_file


def _read_zip_file_entries_iter(zip_file):
    with zipfile.ZipFile(zip_file, "r") as zipfile_handle:
        for name in zipfile_handle.namelist():
            with zipfile_handle.open(name, "r") as fh_in:
                yield json.load(fh_in)


def assemble_table(zip_file, path=None):
    ...
    for block in _read_zip_file_entries_iter(zip_file):
        rows = []
        for data in block.values():
            data["synonymlist"] = ", ".join(data["synonymlist"])
            rows.append([data[field_name] for field_name in IN_SUMMARY])
...
if __name__ == "__main__":
    ...
    path = datetime.now().strftime("pubchem_%Y-%m-%d_%H-%M.zip")
    target_file = d.download(result_path=path)
    print()
    print("final result at", target_file)
    print()
