...
from emzed.utils.sqlite import copy_table
    ...
    return (
        df.style.set_table_styles(
            [
                {
                    "selector": "td",
                    "props": [("white-space", "nowrap"), ("text-align", "left")],
                },
                {
                    "selector": "th",
                    "props": [("white-space", "nowrap"), ("text-align", "left")],
...
def copy_all_refered_tables(source_model, target_conn):
    ...
    for table_name, _ in list_data_tables(source_model._conn) + list_peakmap_tables(
        source_model._conn
    ):
        reference = table_name.split("_")[1]
        if reference not in references:
            continue
        copy_table(source_model._conn, target_conn, table_name, table_name)
