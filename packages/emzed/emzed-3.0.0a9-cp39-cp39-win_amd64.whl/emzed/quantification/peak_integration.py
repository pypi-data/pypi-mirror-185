...
def integrate(
    ...
    **model_extra_args,
    ...
    messages, n_cores = check_num_cores(
        n_cores, len(peak_table), min_size_for_parallel_execution, max_cores
    )
...
def _integrate(args):
    ...
    peak_shape_model = available_peak_shape_models[peak_shape_model]
...
def check_num_cores(n_cores, table_size, min_table_size, max_cores):
    messages = []
...
def create_result_table(
    peak_table, indices, peak_shape_model, post_fixes, result, path, overwrite
):
    result_table = peak_table.copy(path=path, overwrite=overwrite)

    for post_fix in post_fixes:
        result_table.add_or_replace_column_with_constant_value(
            "peak_shape_model" + post_fix, peak_shape_model, str
        )
