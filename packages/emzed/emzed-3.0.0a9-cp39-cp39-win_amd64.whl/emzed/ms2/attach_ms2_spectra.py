...
def attach_ms2_spectra(
    ...
    *print_final_report*: prints some final report with diagnosis messages for testing
    ...
    rtmax = peak_table.rtmin.max().eval() + 5
    ...
    for i, row in enumerate(peak_table):
        ...
        if merged_spectra is None:
            ...
            charges = set(charges)
            assert len(charges) == 1, "can not handle different charges"
            charge = charges.pop()
...
def _overlay(mz_vecs, intensity_vecs, scalings, mz_tolerance, n):
    data_mz = []
    data_ii = []
    ...
    labels = sklearn.cluster.MeanShift(
        seeds=data_mz, bandwidth=mz_tolerance
    ).fit_predict(data_mz)

    cc = Counter(labels)
    label_common_peaks = set(l for l in labels if cc[l] == len(scalings))

    n_common_peaks = len(label_common_peaks)

    clustering_mismatch = n - n_common_peaks

    peak_list = [
        (np.mean(data_mz[labels == l]), np.mean(data_ii[labels == l]))
        for l in set(labels)
    ]
    peak_list.sort()
    return peak_list, clustering_mismatch
