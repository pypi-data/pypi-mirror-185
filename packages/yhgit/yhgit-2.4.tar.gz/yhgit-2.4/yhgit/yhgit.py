    ...
    for a in conver_deplist:
        for mo_re in branch_result:
            if a.module and mo_re.module and a.module == mo_re.module and mo_re.result == 1:
                module_dict = {"module": mo_re.module, "pod": a.pod, "git": a.git, "branch": n_branch, "configurations": a.configurations,
...
def main(argvs=None):
    ...
    if command == "install":
        ...
        debugInfo("version 2.4")
    elif command == "help" or command == "-h":

        """
            文档指令
            0. install 组件  根据本地PodfileLocal.yaml中的配置，获取新分支branch，然后基于的组件新建分支branch
            1. install -b branch 组件 根据分支branch，然后基于组件新建分支branch
            2. status 根据本地PodfileLocal.yaml中配置的仓库，获取所有仓库的开发分支及仓库状态
            3. commit 根据本地PodfileLocal.yaml中配置的仓库，提交本地仓库中的修改
            4. pull   根据本地PodfileLocal.yaml中配置的仓库，拉取远端仓库中代码
            5. merge   根据本地PodfileLocal.yaml中配置的仓库，然后基于组件merge 分支到当前开发分支，并自动提交
            6. push   根据本地PodfileLocal.yaml中配置的仓库，推送本地代码到远端
            5. push   根据本地PodfileLocal.yaml中配置的仓库，推送本地代码到远端

        """

        table = PrettyTable(["编号", "指令", "参数", "说明"])
        table.add_row(
