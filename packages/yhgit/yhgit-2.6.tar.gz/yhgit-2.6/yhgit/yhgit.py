    ...
    if command == "install":
        ...
        table.add_row(
            [0, "install", "-b 指定分支名; 组件 指定 新建分支的组件",
             "根据分支branch，然后基于组件或者PodfileLocal.yaml中的配置，新建分支branch", "yhgit install -b test abc \n yhgit install"])
        table.add_row([1, "status", "", "根据本地PodfileLocal.yaml中配置的仓库，获取所有仓库的开发分支及仓库状态", "yhgit status"])
        table.add_row([2, "commit", "-m 指定提交的信息", "根据本地PodfileLocal.yaml中配置的仓库，提交本地仓库中的修改", "yhgit commit -m \'提交信息\'"])
        ...
        table.add_row(
            [5, "merge", "-b 指定分支名; 组件 指定 merge的组件", "然后基于组件merge 分支到当前开发分支，并自动提交", "yhgit merge -b master abc"])
        table.add_row([6, "release", "组件： 指定组件", "根据本地PodfileModule.yaml中配置的仓库或者手动指定仓库，合并开发分支到master，并自动打新的tag",
                       "yhgit release abc"])
