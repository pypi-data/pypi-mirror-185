...
class NLPEmbeddingGenerator(BaseEmbeddingGenerator):
    ...
    def tokenize(
        self, batch: Dict[str, List[str]], text_col_name: str
...
class CVEmbeddingGenerator(BaseEmbeddingGenerator):
    ...
    def extract_image_features(
        self, batch: Dict[str, List[str]], local_image_path_col_name: str
    ):
        return self.feature_extractor(
            [
                self.open_image(image_path)
                for image_path in batch[local_image_path_col_name]
            ],
            return_tensors="pt",
        ).to(self.device)
