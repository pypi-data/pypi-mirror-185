...
class EmbeddingGeneratorForCVImageClassification(CVEmbeddingGenerator):
    ...
    def generate_embeddings(
        self, df: pd.DataFrame, local_image_path_col_name: str
    ) -> pd.Series:
        if not isinstance(df, pd.DataFrame):
            raise TypeError("df must be a pandas DataFrame")
        if not isinstance(local_image_path_col_name, str):
            raise TypeError("local_image_path_col_name must be a string")
        if local_image_path_col_name not in df.columns:
            raise ValueError(
                "local_image_path_col_name must be a column of the dataframe"
            )

        # Validate that there are no null image paths
        if df[local_image_path_col_name].isnull().any():
            raise ValueError(
                f"There can't be any null values in the column: df[{local_image_path_col_name}]"
            )

        ds = Dataset.from_pandas(df[[local_image_path_col_name]])
        ds.set_transform(
            partial(
                self.extract_image_features,
                local_image_path_col_name=local_image_path_col_name,
            )
        )
