...
from iacer.config import TestConfig, IAC_NAME
from iacer.exceptions import IacerException
from iacer.plugin.ros import StackPlugin
from iacer.util import generate_client_token_ex
...
class Stacker:
    ...
    async def create_stacks(self):
        if self.stacks:
            raise IacerException('Stacker already initialised with stack objects')
...
class Stack:
    ...
    def __init__(self, region: str, stack_id: str, test_name: str = None,
        ...
        self.region = region
        self.plugin: StackPlugin = StackPlugin(region_id=region)
    ...
    @classmethod
    def from_stack_response(cls, stack: dict):
        return cls(
    ...
    async def create(cls, test: TestConfig, tags: dict = None, uuid: UUID = None) -> 'Stack':
        ...
        region = test.region
        plugin = StackPlugin(region_id=test.region, credential=test.auth.credential)
        ...
        try:
            ...
            stack = cls(region, stack_id, name, uuid, status_reason=ex.message,
                        stack_name=stack_name, parameters=parameters)
            stack.status = ex.code
            stack._launch_succeeded = False
            stack.timer.cancel()
            return stack
        stack = cls(region, stack_id, name, uuid, stack_name=stack_name, parameters=parameters)
        await stack.refresh()
        return stack
    ...
    async def set_stack_properties(self, stack_properties: Optional[dict] = None) -> None:
        ...
        if not props:
            if self.id:
                props = await self.plugin.get_stack(self.id)
