...
class DataFrameImpl(metaclass=DataFrameMeta):
                                ...
                                if element_as_series.dtype != polars_dtype:
                                    if _polars_dtype_contains_struct(polars_dtype):
                                        ...
                                        try:
                                            ...
                                            raise TypeError(
                                                f"Feature `{k_str}` at index {i} with value '{element}' could not be converted to dtype `{polars_dtype.string_repr()}`"
                                            ) from e
    ...
    def read_parquet(
        ...
        if isinstance(columns, dict):
            ...
            column_map = None
        data = pl.read_parquet(path, columns_to_read, storage_options=DataFrame._get_storage_options())
    ...
    def read_csv(
        ...
        if isinstance(columns, dict):
            ...
            columns_to_read = columns
            column_map = None
        data = pl.read_csv(path, has_header, columns_to_read, storage_options=DataFrame._get_storage_options())
        if column_map is not None:
            data = data.rename(column_map)
        return cls(data)
