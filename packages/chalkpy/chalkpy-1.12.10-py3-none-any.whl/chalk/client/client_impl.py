...
class ChalkAPIClientImpl(ChalkAPIClientProtocol):
    ...
    def query(
        ...
        for feature in output:
            if isinstance(feature, str):
                # Treat a root feature as a root fqn
                outputs.append(feature)
            else:
                outputs.append(ensure_feature(feature).root_fqn)
    ...
    def _get_training_dataframe(
        ...
        output_features: Sequence[Union[str, Feature, FeatureWrapper, Any]],
        output_id: bool,
        output_ts: bool,
        ...
        query_output = [str(f) for f in output_features]

        try:
            response = self._create_and_await_offline_query_job(
                output_root_fqns=query_output,
                output_id=output_id,
                output_ts=output_ts,
                input=query_input,
    ...
    def sample(
        ...
        output_root_fqns = [f if isinstance(f, str) else ensure_feature(f).root_fqn for f in output]

        try:
            response = self._create_and_await_offline_query_job(
                input=None,
                output_root_fqns=output_root_fqns,
                max_samples=max_samples,
                output_id=output_id,
                output_ts=output_ts,
                dataset_name=dataset,
    ...
    def _create_and_await_offline_query_job(
        self,
        output_root_fqns: List[str],
        output_id: bool,
        output_ts: bool,
        input: Optional[_OfflineQueryInput],
        max_samples: Optional[int],
        ...
        return load_dataset(
            ...
            output_features=output_root_fqns,
            output_id=output_id,
            output_ts=output_ts,
        )
