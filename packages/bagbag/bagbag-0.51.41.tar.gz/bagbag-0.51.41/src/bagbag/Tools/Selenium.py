...
class ChromeWire(seleniumBase):
    def __init__(self, maxRequests:int=100, urlFilterRegex:list[str]=[], PACFileURL:str=None, httpProxy:str=None, sessionID=None, randomUA:bool=True):
        options = webdriver.ChromeOptions()
    ...
    def Flows(self) -> typing.Iterable[seleniumFlow]:
        while True:
            for req in self.driver.requests:
                if req.date.timestamp() <= self.flowtime or req.response == None:
                    continue 
...
if __name__ == "__main__":
    ...
    with ChromeWire(randomUA=False) as se:
        ...
        for request in se.Flows():
            # if 'content-encoding' in request.Response.Headers:
            #     if not Os.Path.Exists("log/" + request.Response.Headers['content-encoding']):
            #         Lg.Trace(request)
            #         File("log/" + request.Response.Headers['content-encoding']).Write(request.Response.BodyBytes)
            # else:
            #     if not Os.Path.Exists("log/plain"):
            #         Lg.Trace(request)
            #         File("log/plain").Write(request.Response.BodyBytes)
            # Lg.Trace(request)
