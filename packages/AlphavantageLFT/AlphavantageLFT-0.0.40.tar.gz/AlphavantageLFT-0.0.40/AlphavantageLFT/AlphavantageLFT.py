...
class Alphavantage:
    ...
    def __init__(self,key="", keys=[]): 
        ...
        self.urls = {
            ...
            'IntraDay': "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol={}&interval=5min&outputsize=full" # Add customizable intervals!!
        }
    ...
    def IntraDayPriceData(self,symbol): #TODO: Have the option to return the data as a pandas dataframe or a SQLite3 command/string command to a cusor
        lookup = self.SymbolLookUp(symbol,internal=True)
        if lookup==True: return self._request(self.urls['IntraDay'].format(symbol))
        else: return lookup
